...
module parallel_nonlin
  ...
  use fourier, only: initialize_fourier,finalize_fourier,&
       &fft_kx_to_x, fft_ky_to_y,fft_x_to_kx,fft_y_to_ky,&
       &fft_ff_to_xy,fft_xy_to_ff,to_fourier_y,to_real_y
  use prefactors
...
contains
  ...
  Real function mem_est_parallel_nonlin(mem_req_in)
    ...
    mem_est_parallel_nonlin = mem_est_parallel_nonlin + &
         & pi0*pj0*lk0*ln0*SIZE_OF_REAL_MB
...
!    if (.not.nonlinear.and.nky0.gt.1) CALL initialize_fourier(li0da,ly0da)
    ...
    allocate(pdphidz(pi1:pi2,pj1:pj2,lk1:lk2,ln1:ln2))
    do n=ln1,ln2
       do k=lk1,lk2
          do j=pj1,pj2
             pdphidz(:,j,k,n) = rho_lref * spec(n)%charge/spec(n)%mass/sqrt(2*spec(n)%temp/spec(n)%mass) * &
                  & C_xy(pi1:pi2)/(geom%jacobian(pi1:pi2,j,k)*geom%Bfield(pi1:pi2,j,k))
          enddo
       enddo
    enddo
...
!--------------------------------------------------------------------------------------
  ...
  subroutine initialize_parallel_nonlin_ff
    ...
    if (yx_order) stop 'yx_order not implemented in initialize_parallel_nonlin_ff'
...
!    if (.not.nonlinear.and.nky0.gt.1) call finalize_fourier
    ...
    do klmn=lb1,lb2
       ...
       Do j=lj1,lj2
          ...
          do sten=-par_sten_bound,par_sten_bound
             ...
             call axpy_ij(li0da,pdphidz(pi1,pj1,k,n)*par_sten(sten),val,dphibar_terms(:,j))
          enddo
  ...
  Subroutine calc_parallel_nonlin_df(p_f,p_bar_emfields,p_dbarchidxy,p_rhs,lb1,lb2,first)
    ...
    if (nky0.gt.1) then
       do klmn=lb1,lb2
          ...
          do j=lj1,lj2
             dphibar_terms(:,j) = dphibar_terms(:,j) + pdphidz(:,pj1,k,n)*dbarphidz(:,j,k)
          enddo
       ...
       do klmn=lb1,lb2
          ...
          !dphidx/y terms
          dphibar_terms(:,j) = pdphidx(:,j,k,l)*p_dbarchidxy(:,j,1,klmn-lb1+1)
          ...
          if (first) then
             ve_pnl_max_loc=maxval(real(dphibar_terms))
             ve_pnl_max=max(ve_pnl_max,ve_pnl_max_loc)
          endif

          PERFON_I('comp_pnl')
          pnonlin = real(dphibar_terms)*dfdv
          PERFOFF_I
