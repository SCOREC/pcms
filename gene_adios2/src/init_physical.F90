...
module phys_ini
  USE arrays, ONLY: allocate_arrays, deallocate_arrays
  USE blockindex, only: initialize_blockindex
  USE boundaries, ONLY: initialize_boundary_exchange,finalize_boundary_exchange, exchange_z_nopb
  USE box_data_module, ONLY: initialize,set_grids
  USE collisions,ONLY: mem_est_collisions,coll, initialize_collisions,finalize_collisions
  USE communications, ONLY: free_comm, split_comm, my_barrier
  USE coordinates, ONLY: simulation_box, ki, deli,kj,xval_a,dz,dv,lv,lw,kymin,kjmin,x0,lp1,ky
  USE discretization, ONLY: mype, li1,li2,ni0,nx0,pi1,pi2,lj1,lj2,nj0,pj1,pj2,&
       &lbz,lk1,lk2,ubz,nz0,nzb,ll1,ll2,nv0,nw0,ln1,ln2,&
       &evenx,yx_order,hkx,n_spec,&
       &pi1gl,pi2gl,omega0_tor,&
       &n_procs_x,n_procs_y,n_procs_z,n_procs_v,n_procs_w,n_procs_s,&
       &my_pespec,my_pez,kj0_ind,&
       &initialize_discretization
  use dzv_terms, only: arakawa_zv_order
  USE equilibrium_fields, ONLY: mem_est_equilibrium_fields, initialize_equilibrium_fields,finalize_equilibrium_fields
  USE external_contr, ONLY: mem_est_external_contr,init_external_contributions, finalize_external_contributions
  USE geometry, ONLY: mem_est_geometry, dpdx_pm,dpdx_pm_arr,geom, q_prof,minor_r, mag_prof,dqdx_prof,&
       &initialize_geometry, finalize_geometry
  USE Grid1DModule, ONLY: grid1d, set_boundaries, set_fourier_boundaries, initialize, finalize
  USE lagrange_interpolation, only: lag3interp
  USE numerical_damping, ONLY: initialize_numerical_damping,finalize_numerical_damping
  USE par_geom, ONLY: q0,shat,rhostar
  USE par_in, ONLY: arakawa_zv, collision_op, rad_bc_type,spec,beta,debye2,x_local, vparscheme,&
       &parallel_nl,parscheme,lilo,n0_global,shifted_metric
  USE par_mod, ONLY: np1coeff,np2coeff,np3coeff,np4coeff,vderivative, par_sten,par_sten_bound
  USE par_other, ONLY: print_ini_msg, xy_local, p_has_0_mode, p_has_00_mode,prec,pi,imag
  USE prefactors,ONLY: mem_est_prefactors, initialize_prefactors,finalize_prefactors
  use profiles_mod, only: set_spec_profiles, unset_spec_profiles, &
       &calc_derived_ref_vals, calc_dpdx_pm
  USE vel_space, ONLY: mem_est_vel_space,initialize_vel_space,finalize_vel_space
  USE x_derivatives, only: initialize_radscheme, finalize_radscheme,CENTERED4TH
          ...
          do i=pi1,pi2
             ...
             case('u3rd')
                if (geom%dBdz(i,lj1,k).ge.0) then
                   ...
                   vderivative(:,i,k,l)=-u3coeff(2:-2:-1)/dv
                else
  ...
  subroutine initialize_boundaries
    ...
    if (xy_local) then
       ...
       CALL initialize(simulation_box, q0, shat, mag_prof, &
            &rhostar,x0,kymin,n0_global,lp1,lilo) !mapped to box_data_initialize
