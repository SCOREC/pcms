...
double percent_error(double test, double reference)
{
  return 100 * std::abs(test - reference) / reference;
}

double f(double x, double y)
{
  return std::sin(20 * x * y) / 10 + 0.5;
}
...
TEST_CASE("evaluate omega_h_field")
{
  ...
  pcms::Rank1View<double, pcms::HostMemorySpace> eval_view{evaluation.data(),
                                                           evaluation.size()};
  pcms::Rank2View<double, pcms::HostMemorySpace> coords_view(
  ...
  for (int i = 0; i < coords.size() / 2; ++i) {
    ...
    REQUIRE(percent_error(evaluation[i], f(x, y)) < 1.0);
  }
}
