...
#endif
#ifndef PetscScalar
#define PetscScalar complex
#endif
!>Parameters for eigenvalue computations
module eigen_parameters

  use discretization, only: mype, nky0, ky0_ind
  use par_in, only: comp_type, nonlinear
  use coordinates, only: kymax, ky, kymin
...
contains
  ...
  subroutine check_eigenvalue_comp
    ...
    if ((kymin*(nky0-1+ky0_ind)).gt.5) ev_shift=cmplx(2.0*(kymin*(nky0-1+ky0_ind)),0.0)
