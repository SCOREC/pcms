...
use par_in
use par_other
use par_geom, only: Bprof_coeffs, tracer_rt_scheme, write_maps, major_Z
...
namelist /in_out/ &
     ...
     chpt_hac, chpt_read_hac, chpt_write_hac, istep_xvsp, xvsp_iz, xvsp_iy, &
     istep_nlt, istep_gav, write_flux_final, istep_GyroLES,istep_fe_time,&
     istep_fe_twoD, istep_fe_threeD, istep_fe_transfer, avg_window, iterdb_file, iterdb_time, &
     momentum_flux, istep_fsa_moments, cat_output, istep_antenna, &
     quasilin_model, istep_energy_exchange, istep_triplet
...
contains
  ...
  subroutine read_parameters(par_in_dir)
    ...
    namelist /geometry/ &
         ...
         sign_Ip_CW, sign_Bt_CW, sign_Omega_CW, & !EQDSK input: signs of plasma current, toroidal field, rotation viewed from top
         ...
         write_maps !write efit maps for interpolation
  ...
  Subroutine write_parameters(skip_info)
    ...
    if (x_local) then
       if (.not.adapt_lx) then
          ...
          Write(PAR_OUT_FILE,"(A,I0)") "nexc  = ",nexc
       elseif (nexc.ne.1) then
          Write(PAR_OUT_FILE,"(A,I0)") "nexc  = ",nexc
       endif
    ...
    if (.not.arakawa_zv) then
       ...
       if (arakawa_zv_order.ne.4) then
          write(PAR_OUT_FILE,"(A,I3)") "arakawa_zv_order = ",arakawa_zv_order
          write(PAR_OUT_FILE,"(A,L3)") "arakawa_cons_bc = ",arakawa_cons_bc
       endif
...
!          Write(PAR_OUT_FILE,"(A,G16.8)") "rho      = ",rho
          ...
          elseif(magn_geometry.eq.'tracer_efit') THEN
             if (edge_opt .gt. 0) Write(PAR_OUT_FILE,"(A,G16.8)") "edge_opt = ",edge_opt
             if (write_maps) write(PAR_OUT_FILE,"(A,L1)") "write_maps = ", write_maps
          endif
    ...
    if (sign_Ip_CW.ne.0) WRITE(PAR_OUT_FILE,"(A,I7)") "sign_Ip_CW = ", sign_Ip_CW
    if (sign_Bt_CW.ne.0) WRITE(PAR_OUT_FILE,"(A,I7)") "sign_Bt_CW = ", sign_Bt_CW
    if (sign_Omega_CW.ne.0) WRITE(PAR_OUT_FILE,"(A,I7)") "sign_Omega_CW = ", sign_Omega_CW
    ...
    !****** general namelist ********
       ...
       if (.not.arakawa_zv) then
          ...
          if (arakawa_zv_order.ne.4) then
             ...
             call attach(fidall_param_h5, "/input", "arakawa_cons_bc", arakawa_cons_bc)
          endif
...
!            &call attach(fidall_param_h5, "/input", "pressure_term"  , pressure_term)
    ...
    !****** geometry namelist ********
       ...
       if (INDEX(magn_geometry,'slab').eq.1) then
          ...
          elseif (INDEX(magn_geometry,'miller').eq.1) then
             ...
             elseif(magn_geometry.eq.'tracer_efit') then
                ...
                if (write_maps) call attach(fidall_param_h5, "/input", "write_maps" , write_maps)
             endif
       ...
       if (sign_Ip_CW.ne.0) call attach(fidall_param_h5, "/input", "sign_Ip_CW", sign_Ip_CW)
       if (sign_Bt_CW.ne.0) call attach(fidall_param_h5, "/input", "sign_Bt_CW", sign_Bt_CW)
       if (sign_Omega_CW.ne.0) call attach(fidall_param_h5, "/input", "sign_Omega_CW", sign_Omega_CW)
