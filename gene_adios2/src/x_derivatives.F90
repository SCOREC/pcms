...
MODULE x_derivatives
  ...
  IMPLICIT NONE
  PUBLIC:: initialize_radscheme, x_deriv, x_deriv_exc_transp, x_deriv_exc, &
       finalize_radscheme, CENTERED4TH, calc_gradient, calc_full_x_deriv
  ...
  Interface x_deriv_exc_transp
     Module Procedure x_deriv_exc_2d_nonlin_transp
  End Interface
...
#define USE_OLD_VERSION
 ...
 !> radial derivative 2-dimensional for Arakawa nonlinearity
  SUBROUTINE x_deriv_2d_nonlin_transp(y,dydx,li0da)
    INTEGER, INTENT(in):: li0da
    REAL, DIMENSION(0:ly0da-1,-nib:li0da-1+nib), INTENT(in):: y
    REAL, DIMENSION(0:ly0da-1,0:li0da-1), INTENT(out)::       dydx
    INTEGER :: j, sten
    ...
    Do j=0, li0da-1
       sten = -radscheme/2
       dydx(:,j) = coeff(sten)*y(:,j+sten)
       DO sten=-radscheme/2+1,radscheme/2
          dydx(:,j) = dydx(:,j)+coeff(sten)*y(:,j+sten)
       END DO
    Enddo

    PERFOFF_I

  END SUBROUTINE x_deriv_2d_nonlin_transp


 SUBROUTINE x_deriv_exc_2d_nonlin_transp(y_in,dydx,li0da)
   INTEGER,INTENT(in):: li0da
   ...
   REAL :: y_avg
    ...
    !boundary exchange in the combined x-y dimension.
    my_pexy=my_pex*n_procs_y+my_pey

    call exchange_x_nltransp(y_in,li0da)

    !inner and outer boundaries for the non-periodic cases
    if(rad_bc_type.ne.0) then
       !inner boundary
       if (my_pexy.eq.0) then
          if (rad_bc_type.eq.2) then
             do i=1,nib
                !apply average values since we are in real space
                y_avg = SUM(y_in(:,0+i))/(ly0da)
                y_in(:,0-i) = y_avg !y(0+i,j)
             end do
          else
             do i=-nib,-1
                y_in(:,i) = 0.
       ...
       !outer x boundary
       if (my_pexy.eq.n_procs_x*n_procs_y-1) then
          do i=1,nib
             y_in(:,li0da-1+i) = 0.
          end do
       end if
    ...
    CALL x_deriv_2d_nonlin_transp(y_in,dydx,li0da)

    PERFOFF_I

  END SUBROUTINE x_deriv_exc_2d_nonlin_transp
