...
Module diag_Gyro_LES_transfer
  ...
  real, dimension(:,:,:,:), allocatable :: tf_cumul_8
  ...
  real, dimension(:,:,:,:), allocatable :: spec_cumul_8
...
!!!**************************************************************************!!!
  ...
  Subroutine initialize_diag_transfer_std
    ...
    IF (t_log_kperp_h1) call calc_initialize_diag_transfer_h(n_shells_log_kperp_1,6)
    IF (t_log_kperp_h2) call calc_initialize_diag_transfer_h(n_shells_log_kperp_2,8)
  ...
  Subroutine diagnostic_transfer_std
    ...
    IF (.not.with_nvp) THEN
        ...
        IF (t_log_kperp_h1) call calc_diagnostic_transfer_h(n_shells_log_kperp_1,6)
        IF (t_log_kperp_h2) call calc_diagnostic_transfer_h(n_shells_log_kperp_2,8)
    ELSE
  ...
  Subroutine finalize_diag_transfer_std
    ...
    IF (t_log_kperp_h1) call calc_finalize_diag_transfer_h(6)
    IF (t_log_kperp_h2) call calc_finalize_diag_transfer_h(8)
  ...
  Subroutine calc_initialize_diag_transfer_h(n_shells,which_model)
    ...
    case(6)
     ...
     case(7)
       ...
       label ='log_kperp_fb_'
     case(8)
       allocate(tf_cumul_8(n_shells,n_shells,ln1:ln2,n_transfer_models))
       tf_cumul_8 = 0
       label ='log_kperp2_'
   case default
   end select
   ...
   !transfer array and flux
    ...
    do ls2=1,n_shells
      ...
      case(6) ! perpendicular shells modification
       ...
       case(8) !log kperp shells with automatic shell spacing and externally defined lowest shell
          call calc_n_shells_log_kperp_2(ls2,g_1,g_ls1)
          k_value(ls2) = lambda1*2**(real(ls2-1)/auto_exp)
       end select
       ...
       do ls1=1,n_shells
          ...
          case(6) ! log kperp_1 shells
             call calc_n_shells_log_kperp_1(ls1,g_1,g_ls1)
             if (any(antenna_type.eq.(/2,3/))) call calc_n_shells_log_kperp_1(ls1,cfgamma_ant,cfgamma_ant_ls1)
          case(7) ! log kperp shells for back scattering
             call calc_n_shells_log_kperp(k0,ls1,g_1,g_ls1)
             if (any(antenna_type.eq.(/2,3/))) call calc_n_shells_log_kperp(k0,ls1,cfgamma_ant,cfgamma_ant_ls1)
          case(8) ! log kperp shells with automatic spacing
             call calc_n_shells_log_kperp_2(ls1,g_1,g_ls1)
             if (any(antenna_type.eq.(/2,3/))) call calc_n_shells_log_kperp_2(ls1,cfgamma_ant,cfgamma_ant_ls1)
         end select
    ...
    if ( .not.( mod(itime,istep_fe_transfer*avg_window) .eq. 0) ) then
       ...
       case(7)
          ...
          !spec_cumul_7 = fe_tot + spec_cumul_7
       case(8)
          tf_cumul_8 = fe_tf_tot + tf_cumul_8
          !spec_cumul_8 = fe_tot + spec_cumul_8
       case default
       end select
    ...
    if ( itime .gt. 0 .and. mod(itime,istep_fe_transfer*avg_window) .eq. 0 ) then
       ...
       case(6)
        ...
        case(8)
          tf_cumul_8 = fe_tf_tot + tf_cumul_8
          transfer_counter(which_model) = transfer_counter(which_model) + 1
          fe_tf_tot =  tf_cumul_8/real(transfer_counter(which_model))

          !spec_cumul_8 = fe_tot + spec_cumul_8
          !fe_tot =  spec_cumul_8/real(transfer_counter(which_model))
       case default
       end select
...
Subroutine calc_spectral(g_ls2,fe_tot)
    ...
    if (collision_op.ne.'none') then
       if(coll_on_h) then
          call equ_collisions(h_,coll_rhs,.true.)
       else
    ...
    call add_dchidxy(emfields, gc_rhs,ptr2,lbg1,lbg2,.true.)
    !call add_dchidxy_1(emfields,gc_rhs,ptr2,lbg1,lbg2,.true.)

    g_rhs  = cmplx(0.0,0.0)
    call add_dchidxy(emfields, g_rhs,ptr2,lbg1,lbg2,.false.)
...
!!!************************************************************************************!!!
  ...
  Subroutine calc_finalize_diag_transfer_h(which_model)
    ...
    case(7)
       IF (.not.with_nvp) deallocate(tf_cumul_7)
    case(8)
       IF (.not.with_nvp) deallocate(tf_cumul_8)
    case default
    end select
