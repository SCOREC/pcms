...
module field_solver
  ...
  use island
#ifdef COUPLE_XGC
  use poloidal_planes_nonuni, only: xyz_to_planes_nonuni, planes_to_xyz_nonuni,dump_original_field_wb
  use par_poloidal_planes, only: istep_planes
...
contains
  ...
  subroutine check_field_solver
    ...
    if (island_contrib) n_fields = 2

    !field modifications
    if (write_pe) then
       if (delzonal) write(*,"(A)") "The zonal component of phi will be deleted."
       if (delzonal_fields) then  !\todo delzonal_fields is confusing and should maybe be renamed to delzonal_apar
       ...
       if (delzonal_bpar) then
          if (n_fields.gt.2) then
             write(*,"(A)") "The zonal component of B_par will be deleted."
          else
             delzonal_bpar = .false.
             write(*,"(A)") "n_fields < 3: set parameter delzonal_bpar= F"
          endif
       endif
  ...
  subroutine compute_emfields(g_1_in,emfields_out,stage)
    ...
    complex, dimension(li1:li2,lj1:lj2,lk1:lk2) :: field_XGC
...
#ifdef COUPLE_XGC
       if (mype.eq.0) write(*,*)'send to XGC(stage, time,itime)',stage,time,itime
       call start_timer("TO_PLANES")

       call xyz_to_planes_nonuni(moments(:,:,:,1))

       call stop_timer("TO_PLANES")
#else
...
#ifdef COUPLE_XGC
       if (mype.eq.0) write(*,*)'wait from XGC (stage, time,itime)',stage,time,itime

       call start_timer("FROM_PLANES")

       call planes_to_xyz_nonuni(field_XGC)

       emfields_out(:,:,lk1:lk2,1)=field_XGC

       call stop_timer("FROM_PLANES")
!if (mype.eq.0) write(*,*)emfields_out(li1:li2,0,0,1)
!field_XGC)

!       do i=li1,li2
!          do j=lj1,lj2
!             do k=lk1,lk2
!                emfields_out(i,j,k,1)=field_XGC(i,j,k)
!                if (isnan(real(field_XGC(i,j,k))).or.(isnan(aimag(field_XGC(i,j,k))))) then
!                   print*,'nan in field',i,j,k,real(field_XGC(i,j,k)),aimag(field_XGC(i,j,k))
!                   stop
!                end if
!             end do
!          end do
!       end do
!       call exchange_z(emfields_out(li1:li2,lj1:lj2,lbz:ubz,1))
