...
!! files; the main file contains the basic infrastructure and
!! refers to the specialized sub-modules for most diagnostics
!! (some are found here for historical reason)
Module diagnostics
  use compute_f
  use mpi
!  use geometry, only: geom, rhostar
  Use par_mod
  Use file_io
  ...
  use diagnostics_auxiliary, only: mem_est_diag_aux, initialize_mats_diag_aux, &
       &finalize_mats_diag_aux, calc_dfields_dy, &
       &mat_20, mat_01, nmat_00, nmat_20, nmat_01, mat_30, mat_11, pTj_qjB0, &
       & calc_moments_trap_pass, calc_gyrocenter_moments
  use diagnostics_df
#ifndef COUPLE_XGC
...
#endif
  use diagnostics_vsp, only: mem_est_all_diags_vsp, initialize_all_diags_vsp,&
       &exec_all_diags_vsp, finalize_all_diags_vsp, check_all_diags_vsp,&
       &istep_vsp, istep_xvsp
  use convergence_monitoring
  use spatial_averages, only: sum_3d
  use time_averages
  use checkpoint
...
#define WRITE_STDOUT
#endif
    use poloidal_planes, only: diag_planes
  Implicit None
  ...
  PUBLIC :: istep_field, istep_mom, istep_nrg, istep_g1, istep_vsp, &
#ifndef COUPLE_XGC       
       istep_neoclass, istep_neoclass2D,&
#endif
       istep_gav, &
  ...
  PUBLIC :: rescale_g_1, set_mats_diag_nrg, &
  ...
  Integer:: istep_field=-1, istep_mom=-1, istep_nrg=-1
  Integer:: istep_dfout=0, istep_antenna=0

  Character(Len=8):: filestat='replace', filepos='rewind'

  INTEGER:: NRGFILE, FIELDFILE, G1FILE, antenna_file
  integer:: nrgcols=10
  ...
  !administration
  !last_exec_diag_time stores last exec time of all/most diags
  !in order to avoid double entries if the diags are called
  !twice, e.g., upon convergence exit; currently static array
  !assuming 100 diags max; ind_diag is the index browsing through
  !the array (will be incremented in call_diag)
  REAL,dimension(0:99):: last_exec_diag_time
  INTEGER :: ind_diag
  ...
  integer :: fidnrg_h5, fidfield_h5, fidcoord_h5
  integer, allocatable, dimension(:) :: fidmom_h5
  integer :: isnap_field = 0, isnap_mom = 0
  ...
  integer :: isnap_planes_phi=0, isnap_planes_mom=0, isnap_flux_XGC
  integer,dimension(:), allocatable :: fidflux_XGC_h5
  integer ::fidmom_XGC_h5
#else
#ifdef ADIOS
  integer :: isnap_field = 0, isnap_mom = 0
#endif
...
CONTAINS
  ...
  SUBROUTINE check_diag
    ...
    if (istep_nlt.gt.0.and.nonlin_h) stop &
         'diag_nlt diagnostic is currently not implemented for "h" distribution nonlinearity!'
    ...
    if (comp_type.eq.'EV') then
       if (istep_field.gt.0) istep_field=1
       if (istep_mom.gt.0) istep_mom=1
       if (istep_nrg.gt.0) istep_nrg=1
       if (istep_dfout.gt.0) istep_dfout=1
       if (istep_antenna.gt.0) istep_antenna=1
    endif
...
!!!******************************************************************!!!
  ...
  Real Function mem_est_diag(mem_req_in)
    ...
    !(large) arrays shared by several diagnostics
    mem_loc=mem_est_diag_aux(mem_loc)
    ...
    !velocity space diags
    mem_loc = mem_est_all_diags_vsp(mem_loc)
...
!!!******************************************************************!!!
  ...
  SUBROUTINE initialize_all_diags
    ...
    if (istep_mom.gt.0) then
       call initialize_diag_mom
       call initialize_diag_flux_XGC
    endif
    call initialize_all_diags_vsp
...
#ifndef COUPLE_XGC
    IF (istep_triplet.GT.0) CALL initialize_diag_triplet
#endif
...
!!!******************************************************************!!!
  SUBROUTINE exec_all_diags(itime,time,overflow_exit,underflow_exit,reset)
    ...
    PERFON('diag_exec')
#ifndef COUPLE_XGC
    IF (call_diag(itime,istep_neoclass,no_aux_yet)) &
         &CALL exec_all_diags_neoclass

    IF (call_diag(itime,istep_neoclass2D,no_aux_yet)) &
         &CALL exec_all_diags_neoclass2D

    IF (call_diag(itime,istep_triplet,no_aux_yet)) &
      & CALL diag_triplet
#endif
    ...
    IF (call_diag(itime,istep_omega,no_aux_yet).and.&
         &(.not.convergence_exit)) THEN
       IF (itime.GT.0) then
    ...
    IF (call_diag(itime,istep_field,no_aux_yet)) then
       call start_timer("DIAG_FIELD")
       CALL diag_field
       call stop_timer("DIAG_FIELD")
    end if

    IF (call_diag(itime,istep_mom,no_aux_yet)) then
       call start_timer("DIAG_MOM")
       CALL diag_mom
       CALL diag_mom_XGC
       call stop_timer("DIAG_MOM")
    end if
    ...
    IF (call_diag(itime,istep_energy_exchange)) &
         &call diag_energy_exchange

    IF (call_diag(itime,istep_nlt,no_aux_yet)) &
         &call exec_all_diags_nlt(itime, istep_nlt)
#endif
    IF (call_diag(itime,istep_vsp,no_aux_yet).OR.&
         call_diag(itime,istep_xvsp,no_aux_yet)) &
         &CALL exec_all_diags_vsp(itime,time)

    IF (write_fielddiff) THEN
       IF (call_diag(itime,1)) &
            &CALL exec_all_diags_finit(itime,time)
    ENDIF

    IF (.not.xy_local) THEN
       IF (call_diag(itime,1)) &
            &call exec_all_diags_df(itime,time,reset)
!       IF (call_diag(itime,1)) &
!            &call exec_all_diags_fsa_moments(itime,time)
    ENDIF
    ...
    !For gyroLES routine
    IF (GyroLES) THEN
       IF (call_diag(itime,istep_GyroLES)) THEN
          IF (no_aux_yet) THEN
    ...
    IF (diag_GyroLES.AND.(call_diag(itime,1))) &
         & call exec_all_diags_GyroLES(itime,time)
#endif
...
#ifdef COUPLE_XGC
  subroutine exec_s_chpt(itime,time,overflow_exit,underflow_exit,reset)
    integer, intent(in) :: itime
    real, intent(in) :: time
    logical, intent(inout) :: overflow_exit, underflow_exit, reset
    logical:: no_aux_yet
  ...
  end subroutine exec_s_chpt
#endif
!!!******************************************************************!!!
  !>Finalizes all GENE internal diagnostics
  SUBROUTINE finalize_all_diags

    if ((comp_type.eq.'IV').and.write_checkpoint) call checkpoint_write(g_1, 'checkpoint', finalstep=.true.)
    ...
    IF (istep_mom.GT.0) then
       call finalize_diag_mom
       call finalize_diag_flux_XGC
    ENDIF
    call finalize_all_diags_vsp
...
#ifndef COUPLE_XGC
    IF (istep_triplet.GT.0) CALL finalize_diag_triplet
#endif
    call finalize_mats_diag_aux
...
#ifdef WRITE_STDOUT
    If (mype == 0.and.write_nrg) Write(*, "(F13.6)") time
#endif
...
!!!******************************************************************!!!
  ...
  SUBROUTINE diag_field
    ...
    complex, dimension(li1:li2,lj1:lj2,lk1:lk2,ln1:ln2) :: bar_emfield
#endif
...
#ifdef ADIOS
      if (write_hac) then
         call adios_field_write(isnap_field, mpi_comm_xyz)
         added = .true.

#ifdef ADIOS_KY0
         call adios_field_write_ky0(isnap_field, mpi_comm_xyz)
#endif
      end if
#endif

      if (added) isnap_field = isnap_field+1
...
#if 0
#ifdef WITHFUTILS
   write(dset_name_field, "(A, '/', i10.10)") "/planes/phi", isnap_planes_phi
   tmp_field = emfields(li1:li2,lj1:lj2,lk1:lk2,1)*rhostar*minor_r*Tref*1e3
   call diag_planes(tmp_field,  dset_name_field)
   isnap_planes_phi= isnap_planes_phi+1
#endif
#endif
    !PERFOFF
...
!!!******************************************************************!!!
  ...
  Real Function mem_est_diag_mom(mem_req_in)
    ...
    END IF
    IF (all_moms) mem_loc = mem_loc + 3.0 * lijk0 * ln0 * SIZE_OF_COMPLEX_MB ! additional moments
...
!WARNING: probably we need to modify the trapped/passing boundary for each x,y position
  ...
  SUBROUTINE diag_mom
    ...
    complex, dimension(li1:li2,lj1:lj2,lk1:lk2):: data_out
    ...
    complex, dimension(:,:,:,:), allocatable :: dummy_mom, tmp_mom
    character(len=50) :: info

    if (write_hac.and.((my_pev+my_pew) .eq. 0)) then
       allocate(dummy_mom(li1:li2,lj1:lj2,lk1:lk2,1:3))
       allocate(tmp_mom(li1:li2,lj1:lj2,lk1:lk2,1:3))
       dummy_mom = 0.0
       trpl = diag_trap_levels - 1

       call adios_mom_open(isnap_mom, mpi_comm_xyz,adios_handle)
    end if
#endif
    ...
    do n=ln1,ln2
       ...
       call calc_moments_trap_pass(6+n_moms_Bpar,f_(:,:,:,:,:,n),vmom,n, &
          ...
          if (x_local) then
             ...
             do i=li1,li2
                ...
                vmom_corr(i,:,:,3) = (corrections(i,:,:,n,3)-&
                     &spec(n)%temp_prof(i)*corrections(i,:,:,n,1))/spec(n)%dens_prof(i)
                !upar, profile included here
                vmom(i,:,:,6,:) = vmom(i,:,:,6,:)/spec(n)%dens_prof(i)
...
#if 0
       write(dset_name_mom, "(A, '/', i10.10)") "/planes/n", isnap_planes_mom
       do k=lk1,lk2
          do j=lj1,lj2
             do i=li1,li2
                data_out(i,j,k) = vmom(i,j,k,1,0) * &
                     nref * 1.0e19 * rhostar * minor_r
             end do
          end do
       end do

       call diag_planes(data_out,  dset_name_mom)
!       call diag_planes(vmom(li1:li2,lj1:lj2,lk1:lk2,1,0),  dset_name_mom)

       tmp_buf(:,:,:,1,0)=1/3.0*vmom(:,:,:,2,0)+2/3*vmom(:,:,:,3,0)
       write(dset_name_mom, "(A, '/', i10.10)") "/planes/T", isnap_planes_mom
!       call diag_planes(tmp_buf(li1:li2,lj1:lj2,lk1:lk2,1,0), dset_name_mom)
       isnap_planes_mom=isnap_planes_mom+1
#endif
#endif
...
#include "gwrite_mom.fh"
    ...
    if (write_hac) then
       added = .true.
       if ((my_pev+my_pew) .eq. 0) then
          call adios_mom_close(mpi_comm_xyz,adios_handle)
          deallocate(tmp_mom)
          deallocate(dummy_mom)
       end if
    end if
#endif
  ...
  SUBROUTINE initialize_diag_flux_XGC
    Integer :: n, ierr
    Real :: maxB_loc
#ifdef WITHFUTILS
    integer, dimension(2) :: dims
...
#ifdef WITHFUTILS
    isnap_flux_XGC = 0
    if ((write_h5).and.(my_pey+my_pez+my_pev+my_pew+my_pespec).eq.0) then
      allocate(fidflux_XGC_h5(0:n_spec-1))

       do n=0,n_spec-1
          call creatf(trim(diagdir)//'/flux_XGC_'//trim(spec(n)%name)//trim(file_extension)//'.h5', &
               fidflux_XGC_h5(n), "gyrocenter flux", 'd', mpi_comm_x)
          call creatg(fidflux_XGC_h5(n), '/flux_'//trim(spec(n)%name))
          rank = 0
          call creatd(fidflux_XGC_h5(n), rank, dims, "/flux_"//trim(spec(n)%name)//"/time", "time")
          call creatg(fidflux_XGC_h5(n), '/flux_'//trim(spec(n)%name)//'/'//"heat_flux")
          call creatg(fidflux_XGC_h5(n), '/flux_'//trim(spec(n)%name)//'/'//"heat_flux_2")
          call creatg(fidflux_XGC_h5(n), '/flux_'//trim(spec(n)%name)//'/'//"heat_flux_3")
          call creatg(fidflux_XGC_h5(n), '/flux_'//trim(spec(n)%name)//'/'//"heat_flux_4")
          call creatg(fidflux_XGC_h5(n), '/flux_'//trim(spec(n)%name)//'/'//"particle_flux")
       end do
    endif
    isnap_flux_XGC = 0
    if ((write_h5).and.(my_pev+my_pew+my_pespec).eq.0) then
       ...
       do n=0,n_spec-1
          call creatf(trim(diagdir)//'/mom_XGC_'//trim(spec(n)%name)//trim(file_extension)//'.h5', &
               fidmom_XGC_h5, "gyrocenter moments", 'd', mpi_comm_xyz)
          call creatg(fidmom_XGC_h5, '/mom_XGC_'//trim(spec(n)%name))
          rank = 0
          call creatd(fidmom_XGC_h5, rank, dims, "/mom_XGC_"//trim(spec(n)%name)//"/time", "time")
          call creatg(fidmom_XGC_h5, '/mom_XGC_'//trim(spec(n)%name)//'/'//"density")
       end do
    endif

#endif

  END SUBROUTINE initialize_diag_flux_XGC
  ...
  SUBROUTINE diag_mom_XGC
    ...
    complex, dimension(li1:li2,lj1:lj2,lk1:lk2,lm1:lm2) :: dfields_dy, dfields_dy2
    real, dimension(li1:li2,lj1:lj2,lk1:lk2,5) :: mom
    complex, dimension(li1:li2,lj1:lj2,lk1:lk2) :: data_out


    INTEGER :: k, l, m, n
    COMPLEX, DIMENSION(li1:li2,lj1:lj2) :: f_av
    complex, dimension(li1:li2,lj1:lj2,lk1:lk2):: var_in
    INTEGER :: i, j, o, n_corrs, write_pe, trap_level
    real, dimension(li1:li2) :: fnorm
    real, dimension(li1:li2,5) :: flux
    real, dimension(li1:li2,1) :: tmp_set


#ifdef WITHFUTILS
    character(len=FILENAME_MAX) :: dset_name_flux_XGC, dset_name_mom
    integer:: ng, pes, ierr
#endif
    ...
    do n=ln1,ln2
       call calc_gyrocenter_moments(6+n_moms_Bpar,f_(:,:,:,:,:,n),vmom,n)

       !get ExB velocity
       do m=lm1,lm2
          tmp_field = emfields(li1:li2,lj1:lj2,lk1:lk2,1)
          call gyro_average_df(tmp_field,bar_field(:,:,:),m,n,transA_in='C')
          do k=lk1,lk2
             do j=lj1,lj2
                do i=li1,li2
                   dfields_dy(i,j,k,m)=imag*ky(j)*bar_field(i,j,k)*flux_geomfac(i,pj1,k)
                   dfields_dy2(i,j,k,m)=imag*ky(j)*bar_field(i,j,k)*flux_geomfac(i,pj1,k)*sqrt(abs(geom%gii(i,pj1,k)))
                enddo
             enddo
          enddo
       enddo

       !compute particle and heat flux
       mom=0.0
       data_out=cmplx(0.0,0.0)
       do m=lm1,lm2
          !particle
!          mom(:,:,:,1) = mom(:,:,:,1)+vmom(:,:,:,m,1)!-REAL(CONJG(vmom(:,:,:,m,1)) * dfields_dy(:,:,:,m))
          data_out=data_out(:,:,:)+ vmom(:,:,:,m,1)
          !heat
          do k=lk1,lk2
             do j=lj1,lj2
                var_in(:,j,k) = (0.5*vmom(:,j,k,m,2)+vmom(:,j,k,m,3))*spec(0)%dens_prof+&
                     1.5*vmom(:,j,k,m,1)*spec(0)%temp_prof
             end do
          end do

          mom(:,:,:,2) = mom(:,:,:,2)-Real(Conjg(var_in) * dfields_dy(:,:,:,m))
          mom(:,:,:,3) = mom(:,:,:,3)-Real(Conjg(var_in) * dfields_dy2(:,:,:,m))

          !heat
          do k=lk1,lk2
             do j=lj1,lj2
                var_in(:,j,k) = (vmom(:,j,k,m,2)+vmom(:,j,k,m,3))
             end do
          end do
          
          mom(:,:,:,4) = mom(:,:,:,4)-Real(Conjg(var_in) * dfields_dy(:,:,:,m))
          mom(:,:,:,5) = mom(:,:,:,5)-Real(Conjg(var_in) * dfields_dy2(:,:,:,m))
       end do
#if 0
       !integrate in vpar
       call my_complex_sum_vw(data_out(:,:,:),li0*lj0*lk0)
       write(dset_name_mom, "(A, '/', i10.10)") "/planes/n_GC", isnap_planes_mom
       do k=lk1,lk2
          do j=lj1,lj2
             do i=li1,li2
                data_out(i,j,k) = data_out(i,j,k) * &
                     nref * 1.0e19 * rhostar *minor_r
             end do
          end do
       end do
       ...
       call my_real_sum_w(mom(:,:,:,1), size(mom(:,:,:,1)))
       call my_real_sum_w(mom(:,:,:,2), size(mom(:,:,:,2)))
       call my_real_sum_w(mom(:,:,:,3), size(mom(:,:,:,3)))
       call my_real_sum_w(mom(:,:,:,4), size(mom(:,:,:,4)))
       call my_real_sum_w(mom(:,:,:,5), size(mom(:,:,:,5)))

       !fsavg
       flux=0.0
       do k=lk1,lk2
          do j=lj1,lj2
             flux(:,1) = flux(:,1) + mom(:,j,k,1)*geom%jacobian(li1:li2,pj1,k)
             flux(:,2) = flux(:,2) + mom(:,j,k,2)*geom%jacobian(li1:li2,pj1,k)             
             flux(:,3) = flux(:,3) + mom(:,j,k,3)*geom%jacobian(li1:li2,pj1,k)
             flux(:,4) = flux(:,4) + mom(:,j,k,4)*geom%jacobian(li1:li2,pj1,k)
             flux(:,5) = flux(:,5) + mom(:,j,k,5)*geom%jacobian(li1:li2,pj1,k)
          enddo
       enddo

       Call my_sum_to_0_real(flux(:,1), li0, mpi_comm_z)
       Call my_sum_to_0_real(flux(:,2), li0, mpi_comm_z)
       Call my_sum_to_0_real(flux(:,3), li0, mpi_comm_z)
       Call my_sum_to_0_real(flux(:,4), li0, mpi_comm_z)
       Call my_sum_to_0_real(flux(:,5), li0, mpi_comm_z)

       flux(li1:li2,1) = flux(li1:li2,1)*spec(n)%dens*fnorm
       flux(li1:li2,2) = flux(li1:li2,2)*spec(n)%dens*spec(n)%temp*fnorm
       flux(li1:li2,3) = flux(li1:li2,3)*spec(n)%dens*spec(n)%temp*fnorm
       flux(li1:li2,4) = flux(li1:li2,4)*spec(n)%dens*spec(n)%temp*fnorm
       flux(li1:li2,5) = flux(li1:li2,5)*spec(n)%dens*spec(n)%temp*fnorm


#ifdef WITHFUTILS
       if (write_h5.and.((my_pey+my_pez+my_pev+my_pew) .eq. 0))  then
          if(my_pespec .eq. 0) then
             do pes = 0, n_procs_s-1
                ng = pes*ln0 + n-ln1
                call append(fidflux_XGC_h5(ng), "/flux_"//trim(spec(ng)%name)//"/time", real(time,8))
                call attach(fidflux_XGC_h5(ng), "/flux_"//trim(spec(ng)%name)//"/time", "n_steps", isnap_flux_XGC+1)

                write(dset_name_flux_XGC, "(A, '/', i10.10)") "/flux_"&
                   &//trim(spec(ng)%name)//"/particle_flux", isnap_flux_XGC
                tmp_set(li1:li2,1)=flux(:,1)
                call putarrnd(fidflux_XGC_h5(ng), trim(dset_name_flux_XGC),tmp_set, (/1/))

                write(dset_name_flux_XGC, "(A, '/', i10.10)") "/flux_"&
                   &//trim(spec(ng)%name)//"/heat_flux", isnap_flux_XGC
                tmp_set(li1:li2,1)=flux(:,2)
                call putarrnd(fidflux_XGC_h5(ng), trim(dset_name_flux_XGC),tmp_set, (/1/))

                write(dset_name_flux_XGC, "(A, '/', i10.10)") "/flux_"&
                   &//trim(spec(ng)%name)//"/heat_flux_2", isnap_flux_XGC
                tmp_set(li1:li2,1)=flux(:,3)
                call putarrnd(fidflux_XGC_h5(ng), trim(dset_name_flux_XGC),tmp_set, (/1/))

                write(dset_name_flux_XGC, "(A, '/', i10.10)") "/flux_"&
                   &//trim(spec(ng)%name)//"/heat_flux_3", isnap_flux_XGC
                tmp_set(li1:li2,1)=flux(:,4)
                call putarrnd(fidflux_XGC_h5(ng), trim(dset_name_flux_XGC),tmp_set, (/1/))

                write(dset_name_flux_XGC, "(A, '/', i10.10)") "/flux_"&
                   &//trim(spec(ng)%name)//"/heat_flux_4", isnap_flux_XGC
                tmp_set(li1:li2,1)=flux(:,5)
                call putarrnd(fidflux_XGC_h5(ng), trim(dset_name_flux_XGC),tmp_set, (/1/))

                call flushh5(fidflux_XGC_h5(ng))
             end do
          endif
       endif
       ...
       if (write_h5.and.((my_pev+my_pew) .eq. 0))  then
          if(my_pespec .eq. 0) then
             ng=0
             call append(fidmom_XGC_h5, "/mom_XGC_"//trim(spec(ng)%name)//"/time", real(time,8))               
             call attach(fidmom_XGC_h5, "/mom_XGC_"//trim(spec(ng)%name)//"/time", "n_steps", isnap_flux_XGC+1)
             write(dset_name_flux_XGC, "(A, '/', i10.10)") "/mom_XGC_"&
                   &//trim(spec(ng)%name)//"/density", isnap_flux_XGC
             call putarrnd(fidmom_XGC_h5, trim(dset_name_flux_XGC),mom(:,:,:,1), (/3,2,1/))
             call flushh5(fidmom_XGC_h5)
          endif
       endif
...
#ifdef WITHFUTILS
    if (write_h5) then
       isnap_flux_XGC = isnap_flux_XGC+1
    end if
#endif

#endif
  END SUBROUTINE diag_mom_XGC

  Subroutine finalize_diag_flux_XGC
    integer :: n

#ifdef WITHFUTILS
    if (write_h5.and.(my_pev+my_pew+my_pespec).eq.0) call closef(fidmom_XGC_h5)
    if (write_h5.and.(my_pey+my_pez+my_pev+my_pew+my_pespec).eq.0) then
       do n=0, n_spec-1
          call closef(fidflux_XGC_h5(n))
       end do
       deallocate(fidflux_XGC_h5)
    end if
#endif
  End Subroutine finalize_diag_flux_XGC
...
!!!******************************************************************!!!
  ...
  subroutine rescale_g_1(overflow_exit)
    ...
    locmax=maxval(abs(g_1))

    Call mpi_allreduce(locmax,globmax,1,MPI_REAL_TYPE,&
         MPI_MAX, mpi_comm_vwspec, ierr)
    locmax=globmax
    Call mpi_allreduce(locmax,globmax,1,MPI_REAL_TYPE,&
         MPI_MAX, mpi_comm_z, ierr)
    locmax=globmax
    Call mpi_allreduce(locmax,globmax,1,MPI_REAL_TYPE,&
         MPI_MAX, mpi_comm_x, ierr)

    overflow_exit = (globmax.NE.globmax) ! check for NaN, NaNQ

    IF (.NOT.overflow_exit) THEN
       if (underflow_limit.le.0.) then
          g_1=g_1/globmax*sqrt(1e-6)
       else
          g_1=g_1/globmax
       endif
       if (istep_omega.ne.0) call rescale_phi_old(globmax)
    ...
    do i=1,num_ky_modes
       ...
       if((which_ky(i).ge.lj1).and.(which_ky(i).le.lj2)) then
         ...
         ! write(*,*) "nkx_keep_array(i)",nkx_keep_array(i)
          ...
          DO j = 1, nkx_keep_array(i)
            IF (kx_indices(j,i).GE.0) THEN
              g_loc = g_1(kx_indices(j,i),which_ky(i),:,:,:,:)
            ELSE
              g_loc = g_1(nx0+kx_indices(j,i),which_ky(i),:,:,:,:)
            ENDIF
...
!*************************************************************************************
  ...
  Logical function call_diag(itime,istep,no_aux_yet)
    ...
    IF (istep.gt.0) THEN
       call_diag = (MODULO(itime,istep).eq.0).AND.&
            &(time.NE.last_exec_diag_time(ind_diag))
    ELSE
       call_diag = .false.
    ENDIF

    IF (call_diag) THEN
       last_exec_diag_time(ind_diag) = time
