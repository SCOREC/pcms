PRO set_output, diag, species, suffix=suffix, ps=ps, reset=reset, dat=dat, $
  ...
  commentlines=commentlines, append=append, write_frame=write_frame, $
  mpgfile=mpgfile, get_filename=get_filename
...
; get_filename : returns filename (without extension)
  ...
  framenr_str = ''
  IF (N_ELEMENTS(write_frame) GT 0) THEN BEGIN ;AND (series.step_count GT 1)
     framenr = write_frame
     fmt = "(I0"+STRING((FIX(ALOG10(series.step_count))+1)>4,FORMAT="(I1)")+")"
     framenr_str = STRING(framenr,FORMAT=fmt)
  ...
  IF KEYWORD_SET(get_filename) THEN get_filename = filename
  ...
  IF N_ELEMENTS(dat) GT 0 THEN BEGIN
    IF gui.out.out_format[0] THEN BEGIN ; ASCII output
      ...
      IF KEYWORD_SET(header) THEN BEGIN
        ...
        PRINTF, data_lun, headerstr, FORMAT='('+n_col_str+'A-12)'
      ENDIF
  ...
  ENDIF
    IF gui.out.out_format[1] THEN BEGIN ; HDF5 output
      ...
      IF NOT append THEN BEGIN
        ...
        info_type = ['created by','creation time','diagnostic name',$
          'data path','run(s)',$
          'time window','step count','species','normalization L_ref',$
          'Tref/eV', 'nref/m^-3', 'mref/kg', 'Lref/m', 'Bref/T','omegatorref/(rad/s)']
        info_content = ['GENE Diagnostics Tool',SYSTIME(),name,$
          gui.out.data_path,run_str,$
        ...
        IF N_ELEMENTS(species) EQ 1 THEN BEGIN
           info_type = [[info_type],'n'+spec_str+' [nref]',$
                           'T'+spec_str+' [Tref]','m'+spec_str+' [mref]',$
                           'q'+spec_str+' [e]']
           info_content = [[info_content],rm0es(spec[species].dens),$
                           rm0es(spec[species].temp),rm0es(spec[species].mass),$
                           rm0es(spec[species].charge)]
        ENDIF
      ...
      IF NOT KEYWORD_SET(header) THEN BEGIN
        IF ((NOT append) OR (framenr GT 0)) THEN BEGIN
          ...
          dataset_id = H5D_CREATE(file_id,'data'+'_'+framenr_str,$
      ...
      ENDIF ELSE BEGIN
        IF N_ELEMENTS(header) EQ 1 THEN BEGIN
          datatype_id = H5T_IDL_CREATE(dat)
          dataspace_id = H5S_CREATE_SIMPLE(SIZE(dat,/DIMENSIONS))
          IF (framenr_str NE '') THEN BEGIN
            IF (append) THEN h5d_create_id = H5G_OPEN(file_id,str_h5proof(header)) $
            ELSE h5d_create_id = H5G_CREATE(file_id,str_h5proof(header))
            dataset_id = H5D_CREATE(h5d_create_id,str_h5proof(framenr_str),$
                       datatype_id,dataspace_id)
          ENDIF ELSE BEGIN
            h5d_create_id = file_id
            dataset_id = H5D_CREATE(h5d_create_id,str_h5proof(header),$
                                    datatype_id,dataspace_id)
          ENDELSE
          H5D_WRITE, dataset_id, dat
          H5D_CLOSE, dataset_id
          H5S_CLOSE, dataspace_id
          H5T_CLOSE, datatype_id
          IF (framenr_str NE '') THEN H5G_CLOSE, h5d_create_id
        ENDIF ELSE BEGIN
...
PRO write_to_z_device, filename, resolution, framenr, framenr_str, out_format,$
  ...
  IF (out_format NE 'mpg') THEN BEGIN
     frame = TVRD()
     myfilename = filename + '_'+framenr_str+'.'+out_format
  ENDIF ELSE BEGIN
