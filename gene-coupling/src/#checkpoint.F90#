...
MODULE checkpoint
  ...
  use perf_monitor
#ifdef ADIOS
  use adios_io, only: adios_checkpoint_write, adios_checkpoint_dims, adios_g_read, adios_checkpoint_read_init
#endif
  ...
  Public:: &
       ...
       & read_checkpoint, write_checkpoint, many_chpts, checkpoint_in, &
       & restart_file, &
...
#ifdef ADIOS
  character(len=FILENAME_MAX) :: restart_file = ''
#endif
...
CONTAINS
  ...
  subroutine check_par_checkpoint
    ...
    if (comp_type.eq.'EV') then
       if (istep_schpt.gt.0) istep_schpt = 1
       if (istep_g1.gt.0) istep_g1 = 1
       if (istep_gav.gt.0) istep_gav = 1
    endif
...
#ifdef ADIOS
    call adios_checkpoint_read_init(mpi_comm_xyzvwspec)
#endif
...
#ifdef ADIOS
       call adios_checkpoint_dims(restart_file, mpi_comm_xyzvwspec, adios_handle, resolution, time, dt)

       if (sum(abs(resolution-arr_of_sizes)).eq.0) then
          call adios_g_read(g_, lbound(g_,kind=8), mpi_comm_xyzvwspec, adios_handle, infile=restart_file)
       else
          call set_chpt_indices(resolution, lind, uind, npoints, arr_of_starts)
          allocate(check_in(lind(1):uind(1),lind(2):uind(2),lind(3):uind(3),lind(4):uind(4),lind(5):uind(5),lind(6):uind(6)))
          call adios_g_read(check_in, lbound(check_in,kind=8), mpi_comm_xyzvwspec, adios_handle, infile=restart_file)
          call copy_chpt_to_g1(resolution, check_in, lind, uind, g_)
          deallocate(check_in)
       endif
#endif
  ...
  subroutine checkpoint_write (g_, filename, finalstep)
    logical, intent(in), optional :: finalstep
    ...
    call start_timer('CHECKPOINT_WRITE')
    PERFON('chpt_out')
    ...
    PERFOFF
    call stop_timer('CHECKPOINT_WRITE')
