    ...
    for(LO i=0; i<p1pp3d.li0;i++){
      for(LO k=0;k<p1pp3d.lk0;k++){
        for(LO j=0;j<p1pp3d.lj0;j++){
          dp3d.densintmp[j]=dp3d.densin[i][j][k];
	}      
        ExecuteCmplxToReal(dp3d,p1pp3d);
        for(LO l=0;l<p1pp3d.lj0*2;l++){
          dp3d.densout[i][l][k]=dp3d.densouttmp[l];
        }
     }
   }
 }
...
void PotentRealdataToCmplxdata3D(DatasProc3D& dp3d, Part1ParalPar3D& p1pp3d,Part3Mesh3D& p3m3d)
{
  ...
  if(p1pp3d.npy==1){
	...
	for(LO j=0;j<p3m3d.lj0;j++){
	  dp3d.potentintmp[j]=dp3d.potentinterpo[i][j][k];  
        }  
        ExecuteRealToCmplx(dp3d,p1pp3d); 
        for(LO l=0;l<p3m3d.lj0/2;l++){
          dp3d.potentpart1[i][l][k]=dp3d.potentouttmp[l];
        }
      }      
    }
  }
