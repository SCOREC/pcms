...
class TestSpatialGrid(unittest.TestCase):
    ...
    def test_kxgrid_xlocal_ylocal(self):
        ...
        np.testing.assert_almost_equal(self.grid_xlyl.kx, np.array(
                [-0.43196899, -0.39269908, -0.35342917, -0.31415927, -0.27488936, -0.23561945,
                 ...
                 0.35342917, 0.39269908, 0.43196899, 0.4712389]))
    ...
    def test_kx_grid_slice_upopen(self):
        sliced_kx = self.grid_xlyl.kx_grid_slice(slice(8, None))
        np.testing.assert_almost_equal(sliced_kx, np.array(
                [0.3141593, 0.3534292, 0.3926991, 0.431969, 0.4712389, -0.431969, -0.3926991,
                 -0.3534292, -0.3141593]))

    def test_kx_grid_slice_lowopen(self):
        sliced_kx = self.grid_xlyl.kx_grid_slice(slice(None, 4))
        np.testing.assert_almost_equal(sliced_kx, np.array(
                [0., 0.03926991, 0.07853982, 0.11780972, -0.11780972, -0.07853982, -0.03926991]))

    def test_kx_grid_slice_closed(self):
        sliced_kx = self.grid_xlyl.kx_grid_slice(slice(4, 10))
        np.testing.assert_almost_equal(sliced_kx, np.array(
                [0.1570796, 0.1963495, 0.2356194, 0.2748894, 0.3141593, 0.3534292, -0.3534292,
                 -0.3141593, -0.2748894, -0.2356194, -0.1963495, -0.1570796]))

    def test_kx_grid_slice_full(self):
        sliced_kx = self.grid_xlyl.kx_grid_slice(slice(None, None))
        np.testing.assert_almost_equal(sliced_kx, np.array(
                [0., 0.03926991, 0.07853982, 0.11780972, 0.15707963, 0.19634954, 0.23561945,
                 0.27488936, 0.31415927, 0.35342917, 0.39269908, 0.43196899, 0.4712389, -0.43196899,
                 -0.39269908, -0.35342917, -0.31415927, -0.27488936, -0.23561945, -0.19634954,
...
class TestDiagSpace(unittest.TestCase):
    ...
    def test_natural_fullspace(self):
        diagspace_natural = DiagSpace(self.grid, False, True, False, (0, None), (0, None), (0, None))
        np.testing.assert_array_equal(self.grid.x[diagspace_natural.xslice], self.grid.x[slice(24)])
        np.testing.assert_array_equal(self.grid.ky[diagspace_natural.yslice],
                                      self.grid.ky[slice(8)])
        np.testing.assert_array_equal(self.grid.z[diagspace_natural.zslice], self.grid.z[slice(16)])

    def test_natural_fullspace2(self):
        diagspace_natural = DiagSpace(self.grid, False, True, False, (None, None), (None, None),
                                      (None, None))
        np.testing.assert_array_equal(self.grid.x[diagspace_natural.xslice], self.grid.x[slice(24)])
        np.testing.assert_array_equal(self.grid.ky[diagspace_natural.yslice],
                                      self.grid.ky[slice(8)])
    ...
    def test_natural_limitedspace(self):
        diagspace_natural = DiagSpace(self.grid, False, True, False, (5, -4), (2, -4), (5, -4))
        np.testing.assert_array_equal(self.grid.x[diagspace_natural.xslice],
                                      self.grid.x[slice(5, -4)])
        np.testing.assert_array_equal(self.grid.ky[diagspace_natural.yslice],
                                      self.grid.ky[slice(2, -4)])
        np.testing.assert_array_equal(self.grid.z[diagspace_natural.zslice],
                                      self.grid.z[slice(5, -4)])

    def test_natural_invalidspace(self):
        with np.testing.assert_raises(IndexError):
            DiagSpace(self.grid, False, True, False, (25, -4), (2, -4), (5, -4))
        with np.testing.assert_raises(IndexError):
            DiagSpace(self.grid, False, True, False, (1, -4), (-1, -4), (5, -4))
        with np.testing.assert_raises(IndexError):
            DiagSpace(self.grid, False, True, False, (1, -4), (2, -4), (4, 4))

    def test_xfourier_fullspace(self):
        diagspace = DiagSpace(self.grid, True, True, False, (0, None), (0, None), (0, None))
    ...
    def test_xfourier_limitedspace(self):
        diagspace = DiagSpace(self.grid, True, True, False, (5, -4), (2, -4), (5, -4))
    ...
    def test_xfourier_invalidspace(self):
        with np.testing.assert_raises(IndexError):
            DiagSpace(self.grid, True, True, False, (25, -4), (2, -4), (5, -4))
        with np.testing.assert_raises(IndexError):
            DiagSpace(self.grid, True, True, False, (1, -4), (-1, -4), (5, -4))
        with np.testing.assert_raises(IndexError):
            DiagSpace(self.grid, True, True, False, (1, -4), (2, -4), (4, 4))
