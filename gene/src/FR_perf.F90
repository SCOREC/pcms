...
Module FR_perf_mod
  ...
  integer, parameter :: MAX_NAME_LEN=50               ! Max length of timer names
  integer :: timer_id(1:MAX_TIMERS)                   ! Array to store timer handles
  character (LEN=MAX_NAME_LEN) :: timer_name(1:MAX_TIMERS)      ! Array to store timer names
...
SUBROUTINE perfout_all
  ...
  99 format(a16,1x,i10,1x,f8.3,1x,f5.1)
