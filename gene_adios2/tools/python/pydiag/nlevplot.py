...
def showhelp():
   ...
   print "  -dt             : plot timetrace of dt_nlev"
   print "  -ev             : plot timetrace of nlev eigenvalues"
   print "  -RK4            : show stability contour of the RK4 scheme in ev plot"
   print "                    (also works with -RK2 -RK4M -RKC1 -RKC2 -RKC3 and/or -RKC4)"
   print "  -res            : (default 100) resolution for RK-stability contour matrix"
   ...
   print "  -courant <nr>   : set courant factor (default 1.25)"
...
while iarg < (len(sys.argv)-1):
   ...
   if str(arg)=='-res':
      iarg+=1  #read following arg (should be number)
      res = int(sys.argv[iarg])
   if str(arg)=='-courant':
      iarg+=1  #read following arg (should be number)
      courant = float(sys.argv[iarg])
   ...
   for n in range(nfiles):
      ...
      if (dtevplot==1):
         ...
         if (dt2ev==-1):
             ...
             if (courant != 1.0):
                 legpm[2]=r'estimate: $\lambda_L+\lambda_{\chi,\mathrm{max}}/c_{\mathrm{cfl}}$'
             else:
         ...
         if (dt2ev==1):
             ...
             if (courant != 1.0):
                legpm[2]=r'estimate: $C_{RK}/(\lambda_L+\lambda_{\chi,\mathrm{max}}/c_{\mathrm{cfl}})$'
             else:
         ...
         if (nfiles==1):
            ...
            time[itmin:itmax],((1./(1./courant*dt_ExB[itmin:itmax]+1./dt_L))/dt_L)**(dt2ev),'r--s',\
