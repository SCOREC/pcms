...
TEST_CASE("mls_interp_test")
{
      ...
      Write<Real> radii2(ntargets, cutoffDistance, "populate initial square of cutoffdistance to all target points");
    ...
    SECTION("test interpo degree 1 poly degree 0")
    {
      ...
      SupportResults support = searchNeighbors(mesh, cutoffDistance, min_num_supports);

      auto approx_target_values = mls_interpolation(
	  source_values,source_coordinates, target_coordinates, support, dim, degree, support.radii2);
	 ...
	 CHECK_THAT(host_exact_target_values[i], Catch::Matchers::WithinAbs(host_approx_target_values[i], tolerance)); 
      }
