...
!   December 2007
!   Fortran 2003, ISO_C_BINDING by T. Dannert, RZG
!   September 2011
!
  USE hdf5
...
!
  ...
  END INTERFACE
  INTERFACE attach
     MODULE PROCEDURE attach_i, attach_i8, attach_r, attach_f, attach_l, attach_s
  END INTERFACE
  INTERFACE getatt
     MODULE PROCEDURE getatt_i, getatt_i8, getatt_r, getatt_f, getatt_l, getatt_s
  END INTERFACE
!
CONTAINS
!===========================================================================
...
!
    IF( .NOT. isinit) THEN
       CALL h5open_f(ierr)
       IF (H5T_NATIVE_INTEGER.EQ.H5T_NATIVE_REAL) THEN
          stop 'ERROR: HDF5 variables are ill defined; check linkage of cutils'
       ENDIF
       isinit = .TRUE.
    END IF
!
    fid = next_fid()
!
! Parallel/serial access to file
!
...
!!$          CALL h5pset_fapl_mpiposix_f(plist_id, mpicomm, .FALSE., ierr)
       ...
       END IF
       if (fid .eq. 0)  print *, "2. h5fcreate_f - ", file
       CALL h5fcreate_f(file, H5F_ACC_TRUNC_F, id, ierr, access_prp=plist_id)
       CALL h5pclose_f(plist_id, ierr)
    ELSE
       if (fid .eq. 0)  print *, "3. h5fcreate_f - ", file
...
! Precision of REALs for this file
!
    CALL getatt(fid, '/', 'prec', real_prec, ierr)
    IF( ierr .NE. -2) THEN
       IF( real_prec .EQ. 'd' ) THEN
          prec(fid) = H5T_NATIVE_DOUBLE
       ELSE
          prec(fid) = H5T_NATIVE_REAL
       END IF
...
!
    IMPLICIT NONE
    CHARACTER(len=1), PARAMETER :: ctype = 'C'
#ifdef HDF5_FORTRAN2003
    COMPLEX, DIMENSION(:,:), INTENT(in),target :: array
    REAL :: temp(SIZE(array,1),SIZE(array,2))

    INCLUDE 'cputarrnd_F03.tpl'
#else
    COMPLEX, DIMENSION(:,:), INTENT(in) :: array
    REAL :: temp(SIZE(array,1),SIZE(array,2))
...
!
    ...
    CHARACTER(len=1), PARAMETER :: ctype = 'Z'
#ifdef HDF5_FORTRAN2003
    DOUBLE COMPLEX, DIMENSION(:,:), INTENT(in),target :: array
    DOUBLE PRECISION :: temp(SIZE(array,1),SIZE(array,2))
    INCLUDE 'cputarrnd_F03.tpl'
#else
    DOUBLE COMPLEX, DIMENSION(:,:), INTENT(in) :: array
...
! and send it to output
    IF( PRESENT(path) ) THEN
       cstr = path(1:LEN_TRIM(path)) // CHAR(0)
       CALL fu_stof(cstr, sz, stream)
    ELSE
       CALL fu_stostdout(sz, stream)
    END IF
!
    DEALLOCATE(stream)
...
!
    ...
    INTEGER, INTENT(in)             :: fid
    CHARACTER(len=*), INTENT(in)    :: name, attr
    INTEGER(i8), INTENT(in), TARGET :: ival
    TYPE(C_PTR) :: f_ptr
    INTEGER :: ierr
    INTEGER(HID_T) :: h5_kind_type_i8, id, oid, attr_space, attr_id
    INTEGER(HSIZE_T) :: one(1) = (/1_HSIZE_T/)
!
!    HDF type corresponding to kind i8
!
    h5_kind_type_i8 = h5kind_to_type(i8, H5_INTEGER_KIND)
!
    id = file_id(fid)                   ! file id
    CALL getoid(fid, name, oid)
!
    ...
    IF( ierr .EQ. -1 ) THEN
       CALL h5screate_f(H5S_SCALAR_F, attr_space, ierr)
       CALL h5acreate_f(oid, attr, h5_kind_type_i8, attr_space, attr_id, ierr)
       CALL h5sclose_f(attr_space, ierr)
    END IF
    CALL h5eset_auto_f(1, ierr)         ! Turn error print on
!
    f_ptr = C_LOC(ival)
    CALL h5awrite_f(attr_id, h5_kind_type_i8, f_ptr, ierr)
!
    CALL h5aclose_f(attr_id, ierr)
    CALL closeid(oid)
...
!
!  Get a 8 Bytes integer attribute from group/dataset
!
    IMPLICIT NONE
    INTEGER, INTENT(in)              :: fid
    CHARACTER(len=*), INTENT(in)     :: name, attr
    INTEGER(i8), INTENT(out), TARGET :: ival
    TYPE(C_PTR) :: f_ptr
    INTEGER, INTENT(out), OPTIONAL :: err
    INTEGER :: ierr
    INTEGER(HID_T) :: h5_kind_type_i8, id, oid, attr_id
    INTEGER(HSIZE_T) :: one(1) = (/1_HSIZE_T/)
!
!    HDF type corresponding to kind i8
!
...
!
    ...
    ELSE
       f_ptr = C_LOC(ival)
       CALL h5aread_f(attr_id, h5_kind_type_i8, f_ptr, err)
       IF( ierr .EQ. -1 ) THEN ! wrong attr type
          IF(PRESENT(err)) err = -2
       END IF
    END IF
...
!===========================================================================
  ! part1d gets from the local extent and the number of processes
  ! along a Cartesian communicator the global extent of a dimension.
  ! Also the local offsets of the local data in the global array
  ! is calculated.
...
!       CALL h5aopen_idx_f(oid, i-1, attr_id, ierr)
       CALL h5aopen_by_idx_f(oid, '.', H5_INDEX_CRT_ORDER_F, &
              & H5_ITER_INC_F,INT(i-1,HSIZE_T), attr_id, ierr)
