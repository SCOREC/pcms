...
PRO zonal_init, diag
  ...
  IF par.y_local THEN fft_format, kxky=vars $
  ELSE fft_format, kxsy=vars
...
PRO zonal_loop, diag
  ...
  nkx = par.y_local?par.nkx0:N_ELEMENTS((*mom[0,0].kxsy)[*,0,0])
  ...
  FOR isp = 0, gui.out.n_spec_sel - 1 DO BEGIN
    ...
    IF par.y_local THEN BEGIN ; phi_zonal(kx) = <phi(kx,0,z)>_z
      ...
      ELSE phi_zfl_temp[0,isp] = TOTAL(REFORM((*mom[isp,(*i).var].kxky)[*,0,*],$
         [nkx,par.nz0])*REBIN(REFORM((*series.geom).jac_norm,$
         [1,par.nz0]),[nkx,par.nz0]),2)/par.nz0
    ENDIF ELSE BEGIN  ;y-global, phi_zonal(kx) = <phi(kx,y,z)>_{y,z}
       IF ((*i).zind[0] GT -1) THEN phi_zfl_temp[0,isp] = $
               TOTAL(REFORM((*mom[isp,(*i).var].kxsy)[*,*,(*i).zind[0]])*$
               REBIN(REFORM((*series.geom).jac_norm[*,(*i).zind[0]],$
               [1,par.ny0,1]),[nkx,par.ny0,1]),2)/par.ny0 $
       ELSE phi_zfl_temp[0,isp] = TOTAL(TOTAL((*mom[isp,(*i).var].kxsy)[*,*,*]*$
               REBIN(REFORM((*series.geom).jac_norm,[1,par.ny0,par.nz0]),$
               [nkx,par.ny0,par.nz0]),3),2)/(par.ny0*par.nz0)
       ;compute negative kx modes
       phi_zfl_temp[par.nkx0/2+1:par.nkx0-1,isp] = CONJ(REVERSE(phi_zfl_temp[1:par.nkx0/2-1,isp]))
    ENDELSE
  ...
  IF (par.n_fields GE 2) AND ((*i).var EQ 0) THEN BEGIN
    IF par.y_local THEN BEGIN
      ...
      ELSE apar_zfl = TOTAL(REFORM((*mom[0,1].kxky)[*,0,*],[nkx,par.nz0])*$
       REBIN(REFORM((*series.geom).jac_norm,[1,par.nz0]),$
             [nkx,par.nz0]),2)/par.nz0
    ENDIF ELSE BEGIN  ;y-global
      IF ((*i).zind[0] GT -1) THEN apar_zfl = $
         TOTAL(REFORM((*mom[0,1].kxsy)[*,*,(*i).zind[0]])*$
         REBIN(REFORM((*series.geom).jac_norm[*,(*i).zind[0]],$
             [1,par.ny0,1]),[nkx,par.ny0,1]),2)/par.ny0 $
      ELSE apar_zfl = TOTAL(TOTAL((*mom[0,1].kxsy)*$
           REBIN(REFORM((*series.geom).jac_norm,[1,par.ny0,par.nz0]),$
             [nkx,par.ny0,par.nz0]),3),2)/(par.ny0*par.nz0)
      ;compute negative kx modes
      apar_zfl[par.nkx0/2+1:par.nkx0-1] = CONJ(REVERSE(apar_zfl[1:par.nkx0/2-1])) 
    ENDELSE
