...
module df_nonlinear_term_mod
  ...
  use discretization
  use fourier, only: initialize_fourier, finalize_fourier, fft_ky_to_y, &
       &to_fourier_y
  !use prefactors
  use mpi
#ifdef WITH_LIKWID
#include "likwid_f90.h"
  ...
  public :: transpose_cmplx, transpose_and_extend_cmplx, gto_real_nonlinearity_df
...
contains
  ...
  subroutine add_nonlinearity(this,g_block,p_dgdxy,p_emfields,p_barchi,p_dbarchidxy,p_rhs,lb1,lb2,stage)
    ...
    complex, dimension(li1:li2, lj1:lj2, lb1:*), intent(inout), target:: p_rhs
    ...
    complex, dimension(lbi:ubi, lj1:lj2, lb1:*), intent(in), target:: g_block
...
#undef DEBUGGING
  ...
  subroutine gto_real_nonlinearity_df(this,inarr,outarr,howmany)
    ...
    Real, Dimension(0:ly0da-1, 0:li0da/n_procs_y-1,1:howmany), Intent(out) :: outarr
    ...
    do ind=1,howmany
       ! Transpose x-y
       Call transpose_and_extend_cmplx(li0da, nj0, inarr(:,:,ind),&
            &extended_tarray , ly0da/2+1)
       ! Fourier transfrom in y (include dealiasing step)
       Call fft_ky_to_y(extended_tarray,outarr(:,:,ind))
    end do
