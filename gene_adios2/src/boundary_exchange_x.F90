...
MODULE boundary_exchange_x
  ...
  public :: exchange_x_general_real,exchange_x_general
  PRIVATE

  INTERFACE exchange_x
     MODULE PROCEDURE exchange_x_1D_real,exchange_x_1D,exchange_x_2D,exchange_x_2D_real,exchange_x_3D,&
...
CONTAINS
  ...
  SUBROUTINE exchange_x_1D_real(bdesc,u)
    ...
    IF ( (bdesc%n_points.NE.SIZE(u,1))&
       ...
       PRINT*,"bdesc%n_points,size(u,1) = ",bdesc%n_points,SIZE(u,1),&
            &", bdesc%count, 1 = ", bdesc%count
       STOP "Wrong BoundaryDescription in call to exchange_x_1D_real!"
    END IF

    CALL exchange_x_general_real(bdesc,u,1)
  END SUBROUTINE exchange_x_1D_real
  ...
  SUBROUTINE exchange_x_1D(bdesc,u)
    ...
    IF ( (bdesc%n_points.NE.SIZE(u,1))&
         &.OR.bdesc%count.NE.1 ) THEN
       PRINT*, "Wrong BoundaryDescription in call to exchange_x_1D!"
       STOP
    END IF
