...
module coordinates
  use discretization
  use GaussQuadrature
  use par_in, only: arakawa_zv, sign_Ip_CW, sign_Bt_CW
  use par_geom, only: magn_geometry
  USE box_data_module, only: box_data_type
  !use Grid1DModule
  implicit none
  ...
  subroutine set_y_coordinate_vars(n0_global, q0, &
    ...
    IF (rho_Lref.gt.0) THEN
       ...
       IF (n0_global.EQ.-1111) THEN
          ...
          n0_global = nint((kymin*abs(C_y0)/rho_Lref))
       ENDIF

       !for negative q0 (CHEASE), we invert the sign of n0_global
       !note that this differs from what we do in the tracer_efit interface at the moment,
       !where we keep n0_global positive
       !!\todo match sign conventions for all geometries
       if (q0.lt.0.and.n0_global.gt.0.and.magn_geometry.eq.'chease') n0_global=-n0_global
    ...
    IF (adapt_ly) kymin = n0_global/abs(C_y0)*rho_Lref
...
!    mu_weight = gl_mu_weights(lm1:lm2)
  ...
  SUBROUTINE  set_radial_box_size_with_shear(n_pol,shat,kymin,&
    ...
    lxmin = 1.0/(n_pol*abs(shat)*kymin*abs(Cyq0_x0))
    ...
    if (sign_Ip_CW*sign_Bt_CW.ne.0) nexc=nexc*sign_Ip_CW*sign_Bt_CW
