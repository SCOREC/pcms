...
!!!******************************************************************!!!
  ...
  Real Function mem_est_diag_GyroLES(mem_req_in)
    real, parameter:: mem_cn=SIZE_OF_COMPLEX_MB,&
         & mem_rn=SIZE_OF_REAL_MB
    real:: mem_req_in
    real:: mem_loc=0.
  ...
  Subroutine initialize_all_diags_GyroLES
    ...
    IF (istep_fe_threeD.gt.0) call initialize_diag_spectral_threeD
  ...
  Subroutine exec_all_diags_GyroLES(itime,time)
    ...
    IF (istep_fe_threeD.gt.0.and.itime.eq.0) THEN
       if (modulo(itime,istep_fe_threeD) .eq. 0) then
          if(arakawa_zv) then
             call calc_aux_fields(g_1,emfields,f_,.true.,h_)  !Calculate f and h
          else
             call calc_aux_fields(g_1,emfields,f_,.false.)    !Calculate f
  ...
  Subroutine finalize_all_diags_GyroLES
    ...
    IF (istep_fe_threeD.gt.0) call finalize_diag_spectral_threeD
  ...
  Subroutine initialize_diag_transfer_general
    ...
    if (any((/tky,tkx,tkperp,t_log_kperp,t_log_ky,t_log_kperp_1,t_log_kperp_fb,t_log_kperp_h,t_log_kperp_h1,t_log_kperp_h2/))) &
         call initialize_diag_transfer_std
    if (any((/tk3_ky,tk3_kx,tk3_log_kperp,tk3_log_kperp_1,tk3_log_kperp_h,tk3_log_kperp_h1,tk3_log_kperp_h2/))) &
