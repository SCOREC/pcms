...
PRO nrgtstep_output, diag
  ...
  FOR isp = 0, ((*i).var NE -1) * (gui.out.n_spec_sel - 1) DO BEGIN
    ...
    IF (*i).var NE -1 THEN set_output, diag, sp, /eps, multi=[0,1,2] $
    ELSE set_output, diag, /eps
    ...
    IF (*i).var NE -1 THEN BEGIN
      ...
      set_output, diag, sp, header=['t','dt'],$
        dat=[[time_nrg],[var_nrg]], commentline=['dt_max = '+ $
        rm0es(dt_max)]
      
      set_output, diag, sp, /reset, /eps
    ENDIF ELSE BEGIN
       set_output, diag, header=['t','dt'],$
          dat=[[time_nrg],[dt_arr]], commentline=['dt_max = '+ $
          rm0es(dt_max)]
       set_output, diag, /reset, /eps
    ENDELSE
  ENDFOR
