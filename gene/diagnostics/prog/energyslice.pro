...
PRO energyslice_loop, diag
  ...
  data = REFORM(DBLARR(xavg?1:xdim,yavg?1:ydim,zavg?1:zdim,n_vars),[xavg?1:xdim,yavg?1:ydim,zavg?1:zdim,n_vars],/OVERWRITE)
  ...
  FOR ivar = 0, (*i).n_energies - 1 DO BEGIN
     ...
     if not (min(abs((*i).var -  ivar))) then begin
        data[*,*,*,data_ind] = data_var[*,*,*]
        data_ind +=1
     endif
  ENDFOR  ; --- ivar loop
     ;if ivar=6 is requested: fill it in!
     if not (min(abs((*i).var -  6))) then $
          data[*,*,*,data_ind] = cancel_check[*,*,*]

  (*i).data_id = time_avg((*i).data_id,data,energy_time,$
...
;              my_label[j] = 'q'
  ...
  rdims = [nvars,n_res_steps]

  IF (nzind GT 1) THEN rdims = [nzind,[rdims]]
  IF (nyind GT 1) THEN rdims = [nyind,[rdims]]
  IF (nxind GT 1) THEN rdims = [nxind,[rdims]]
  ...
  FOR n = 0, n_res_steps - 1 DO BEGIN
    ...
    FOR ivar= 0 , nvars - 1 DO BEGIN
      ...
      CASE ndims OF
         0 : BEGIN
            IF (ivar) EQ 0 THEN PRINT, timestr
            namestr = rm_idl_fmt(mytitle)
            PRINT, STRING(namestr,format='(A-16)') + rm0es(data[ivar,n])
         END
         1 : BEGIN
            IF (ivar EQ 0) THEN BEGIN
               collect_dat = REFORM(data[*,ivar,n])
               collect_names = mytitle ;rm_idl_fmt(mytitle)
            ENDIF ELSE BEGIN
               collect_dat = [[collect_dat], [REFORM(data[*,ivar,n])]]
            ...
            IF (ivar EQ nvars-1) THEN BEGIN
               ...
               set_output, diag, commentlines=[timestr+', '+$
	  ...
	  END
      2 : BEGIN
            energyslice_plot, REFORM(data[*,*,ivar,n]), *my_axis[0], $
            ...
            set_output, diag, commentlines=[timestr,$
              ...
              append=(first EQ 0), dat=[[REFORM(data[*,*,ivar,n])]]
            plot_info_str, diag, time=(gui.out.res_steps ? time[n] : undef)
	  END
      3 : BEGIN
            energyslice_plot, REFORM(data[*,*,*,ivar,n]), *my_axis[0], $
...
PRO energyslice_plot, dat, xaxis, yaxis, zaxis, xtitle=xtitle, ytitle=ytitle,$
  ...
  ENDIF ELSE IF N_ELEMENTS(zaxis) LT 1 THEN BEGIN
    ...
    ylog = 1
    IF (ylog) THEN BEGIN
       datmin = MIN(dat[*,1:*],max=datmax)
       yrange= [yaxis[1],yaxis[N_ELEMENTS(yaxis)-1]]
    ENDIF ELSE BEGIN
       yrange = [yaxis[0],yaxis[N_ELEMENTS(yaxis)-1]]
    ENDELSE
