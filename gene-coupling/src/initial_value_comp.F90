...
module initial_value_comp
  ...
  use poloidal_planes, only: initialize_planes, finalize_planes
...
contains
  ...
  subroutine initial_value
    ...
    logical :: reset=.false., nltdt_off_sav=.false.
    ...
    call start_timer("INITIAL_VALUE_INIT")
    ...
    call stop_timer("INITIAL_VALUE_INIT")
    call flush_perf_monitor(-1)
    ...
    Do itime = 1, ntimesteps
...
#else
       call exec_s_chpt(itime,time,overflow_exit,underflow_exit,reset)
#endif
       ...
       IF (.not.nonlinear.and..not.parallel_nl.and.&
            (overflow_exit).and.(rescale)) &
