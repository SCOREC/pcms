...
!  if(sml_mype.eq.0) print *,'[initial_perturbation.F90]',rho,tta,eps,chi,qs,dN
  ...
  if( 0.5>(filter_ij(i,j)+filter_ij(i+1,j)+filter_ij(i,j+1)+filter_ij(i+1,j+1)))then
    qs  = wj*(wi* qs_ij(i,j)+(1D0-wi)* qs_ij(i+1,j))+(1D0-wj)*(wi* qs_ij(i,j)+(1D0-wi)* qs_ij(i+1,j+1));
    chi = wj*(wi*chi_ij(i,j)+(1D0-wi)*chi_ij(i+1,j))+(1D0-wj)*(wi*chi_ij(i,j)+(1D0-wi)*chi_ij(i+1,j+1));
    rho = wj*(wi*rho_ij(i,j)+(1D0-wi)*rho_ij(i+1,j))+(1D0-wj)*(wi*rho_ij(i,j)+(1D0-wi)*rho_ij(i+1,j+1));
