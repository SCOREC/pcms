...
class SrcmomFile(BinaryFile):
    ...
    def readvar(self, var, subvar):
        ...
        if not (self.tind, var, subvar) == self.prev_data:
            self.file.seek(self.offset(var, subvar))
            self.srcdata = np.fromfile(self.file, count=self.cm.pnt.nx0, dtype=self.nprt)
...
class SrcmomSeries(TimeSeries):
    ...
    def __init__(self, common, momtype, srctype):
    ...
    def generate_timeseries(self):
        srcmomfiles = [SrcmomFile("srcmom_"+spec+self.cm.fileextension, self.cm)
        ...
        for ispec, smf in enumerate(srcmomfiles):
            for pos in tinds:
                smf.set_time(self.timearray[pos])
