...
MODULE circular_mod
  use discretization
  use coordinates
  use par_geom, only: geomtype, Bprof_coeffs, Lref
  ...
  INTEGER:: SIGN_Ip, SIGN_Bt    ! sign (+1 clockwise, -1 counter-clockwise)
...
CONTAINS
  ...
  SUBROUTINE init_circ(pn_pol,pq,peps,pdqdxa,pmaj_R,pmaj_Z,pminor_r,sign_Ip_CW, sign_Bt_CW)
    INTEGER :: pn_pol
    REAL    :: pmaj_R, pmaj_Z, pminor_r
    REAL, dimension(pi1gl:) :: pq,peps,pdqdxa       ! use assumed-shape arrays
    INTEGER, INTENT(IN) :: sign_Ip_CW, sign_Bt_CW
    INTEGER :: i
    ...
    do i=pi1gl,pi2gl
       q(i)=sign_Ip*sign_Bt*abs(pq(i)); eps(i)=peps(i)
       r(i)=eps(i)*maj_R; dqdxa(i)=sign_Ip*sign_Bt*abs(pdqdxa(i))
    end do
  ...
  SUBROUTINE get_circ(geom,Cy,C_xy,q0)
    ...
    do i=pi1gl,pi2gl
       qbar=abs(q(i))*sqrt(1-eps(i)**2)
       dxdr=1.0                ! x=r
       dpsidr=r(i)/qbar                ! 1/(Bref*Lref)*dpsi/dr
       dqdr=dqdxa(i)*dxdr/minor_r
       dqbardr=sqrt(1-eps(i)**2)*(abs(dqdr)-abs(q(i))*eps(i)/maj_R/(1-eps(i)**2))
       Cy(i) = abs(r0/q0)*sign_Ip !r(i)/q(i)
       dCydr_Cy = 0.0 !1./r(i)+dqdr
       C_xy(i)=dpsidr/(dxdr*abs(Cy(i)))
       ...
       do k=lk1,lk2
          ...
          sint = sqrt(1.0-eps(i)**2)*sin(sign_Ip*zval(k))/(1.0-eps(i)*cos(zval(k)))
          ! derivatives with respect to r,theta
          dchidr =  -1.0/maj_R*sin(zval(k))/(1-eps(i)**2)
          dchidt = sign_Ip * sqrt(1-eps(i)**2)/(1+eps(i)*cost)

          B = 1./(1.+eps(i)*cost)*sqrt(1.+(eps(i)/qbar)**2)
          if (.not. use_Bprof_corr) then
            dBdr = B/maj_R*(- cost / (1.+eps(i)*cost) + &
              eps(i)/(eps(i)**2+qbar**2) * &
          ...
          dBdt = eps(i)*sint*B / (1+eps(i)*cost)
          ...
          Do j=pj1,pj2
             ...
             geom%gij(i,j,k)=dxdr*Cy(i)*sign_Ip*q(i)*(fac2*zval(k)*g11+g12)
             gxz(i,j,k)=dxdr*g12
             gyz(i,j,k)=Cy(i)*q(i)*sign_Ip*(fac2*zval(k)*g12+g22)
             geom%gzz(i,j,k)=g22

             ! jacobian
             geom%jacobian(i,j,k)=C_xy(i)*abs(q(i))*maj_R*(1+eps(i)*cost)**2
          ...
          geom%dxdR(i,k)=cost
          geom%dxdZ(i,k)=sint
          geom%R_hat(i,k)=maj_R*(1.+eps(i)*cost)
          geom%Z_hat(i,k)=maj_Z+maj_R*eps(i)*sint
          if (Lref.ne.0.) then
            geom%R(i,k)=geom%R_hat(i,k)*Lref
            geom%Z(i,k)=geom%Z_hat(i,k)*Lref
          else
            geom%R(i,k)=geom%R_hat(i,k)
            geom%Z(i,k)=geom%Z_hat(i,k)
          endif
       end do
    end do
