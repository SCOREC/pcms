...
module discretization
  USE par_in, ONLY: y_local, x_local, &
       &parscheme, nonlinear, collision_op, arakawa, nblocks, Omega0_tor, coll_order
...
#else
  Integer :: lg0, lh0, li0, lj0, lk0, ll0, lm0, ln0
#endif
...
#else
  integer :: li1da, li2da, li0da, ly0da , lbida, ubida, lbida2, ubida2
#endif
  integer:: nibda
...
contains
  ...
  subroutine initialize_discretization(print_ini_msg)
    ...
    if (collision_op .eq. 'sugama') then
       ...
       !endif
#endif
    elseif ((collision_op.ne.'none').and.(n_procs_w.gt.1)) then
       nwb = 1
    else
       nwb = 0
    endif
