FUNCTION fluxgrad_info
  ...
  RETURN, {$
    type      : 'mom global',$
    title     : 'Chi vs. gradient (nonlocal)',$
    help_text : ['Plots heat diffusivity vs. gradient'],$
    ext_vars  : [['xrange','0','lower/upper radial range for averaging'+$
                 '(real[2] between 0.0 and 1.0; default: 0.4,0.6)'],$
...
PRO fluxgrad_init, diag
  ...
  IF N_ELEMENTS(*e.xrange) NE 2 THEN $
    xrange = [0.4,0.6] ELSE xrange = *e.xrange
  xval_a = par.x0+(-0.5*series.lx+INDGEN(par.nx0)*series.dx)*par.rhostar
  xind = WHERE((xval_a GE xrange[0]) AND (xval_a LE xrange[1]))
  nx = N_ELEMENTS(xind)

  IF NOT KEYWORD_SET(*e.norm_projection) THEN *e.norm_projection = 0
  IF (*e.norm_projection) THEN BEGIN
    proj_fac = par.x_local?1.0/REBIN(REFORM(sqrt((*series.geom).gxx[xind]),$
         [1,1,par.nz0]),[nx,par.nky0,par.nz0]):$
          1.0/REBIN(REFORM(sqrt((*series.geom).gxx[xind,*]),$
  ...
  IF (*e.norm_projection) THEN BEGIN
     geo_fac = TOTAL(sqrt((*series.geom).gxx)*jac_fac,2)
  ENDIF ELSE BEGIN
     geo_fac = TOTAL((*series.geom).gxx*jac_fac,2)
  ENDELSE
...
PRO fluxgrad_loop, diag
  ...
  avflux = DBLARR(3,(*i).n_fluxes,gui.out.n_spec_sel,/NOZERO)
  ...
  FOR isp = 0, gui.out.n_spec_sel - 1 DO BEGIN
    ...
    FOR n = 0, (*i).n_fluxes - 1 DO BEGIN
      avflux[0,n,isp] = TOTAL((*i).flux[*,n],1,/DOUBLE) / nx ; Gamma, Q
      avflux[1,n,isp] = $ ; <Q(t) / -n(t) dT(t)/dx >_r
         TOTAL((*i).flux[*,n]/(spec[sp].dens*total_dens[(*i).xind]*$
               spec[sp].temp*total_temp[(*i).xind]*omt[(*i).xind]*(*i).geo_fac),$
               1,/DOUBLE) / nx
      avflux[2,n,isp] = TOTAL((*i).flux[*,n],1,/DOUBLE)/TOTAL(spec[sp].dens*total_dens[(*i).xind]*$
               spec[sp].temp*total_temp[(*i).xind]*omt[(*i).xind]*(*i).geo_fac,$
               1,/DOUBLE) ; <Q(t)>_r / < -n(t) dT(t)/dx >_r
   ENDFOR
  ENDFOR ; species loop
...
PRO fluxgrad_output, diag
  ...
  avflux = REFORM(store_step((*i).flux_id,/get,/reg_array),$
    [3,(*i).n_fluxes,gui.out.n_spec_sel,series.step_count])
  ...
  title = 'avg. over x/a = ' + $
    rm0es((*i).xvals[(*i).xind[0]],prec=3) + '-' + $
    rm0es((*i).xvals[(*i).xind[N_ELEMENTS((*i).xind)-1]],prec=3)

  geo_fac_str = (*i).norm_projection?'<sqrtgxx>_FS':'<gxx>_FS'
  chi_unit = rm_idl_fmt(get_nrg_string(12,vel_str=vel_str,/ounit))
  ...
  FOR isp = 0, gui.out.n_spec_sel - 1 DO BEGIN
    ...
    fluxgrad_plot, time, avflux[1,1,isp,*], $
      't,' , '<!7v!6!D' + spec[sp].name + '!N>', $
      timeline, titleline=title

    fluxgrad_plot, time, avgrad[1,0,isp,*], $
      't', '<R/L!IT' + spec[sp].name + '!N>', $
      timeline, titleline=title

    fluxgrad_plot, avgrad[0,isp,*], avflux[1,1,isp,*], $
      '<R/L!DT' + spec[sp].name + '!N>', $
      '<!7v!6!D' + spec[sp].name + '!N>', $
      timeline, titleline=title

    chi_xt_avg = INT_TABULATED(time, avflux[1,1,isp,*])/$
    ...
    IF ((*i).n_fluxes GT 2) THEN BEGIN
       set_output, diag, sp, header=['t','<R/LT>',$ ;'<R/Ln>',$
             '<chi_es>','<chi_em>','<chi2_es>','<chi2_em>','<Q_es>','<Q_em>'], $
             dat=[[time],[REFORM(avgrad[0,isp,*])],$ ;[REFORM(avgrad[1,isp,*])],$
                  [REFORM(avflux[1,1,isp,*])],[REFORM(avflux[1,3,isp,*])],$
                  [REFORM(avflux[2,1,isp,*])],[REFORM(avflux[2,3,isp,*])],$
                  [REFORM(avflux[0,1,isp,*])],[REFORM(avflux[0,3,isp,*])]],$
                  commentline=[[title],['<chi>=<Q(x)/[-n(x) d/dx T(x) '+geo_fac_str+']>'],$
                                ['<chi2(x)>=<Q>/<-n(x) d/dx T(x) '+geo_fac_str+'>'],$
                                ['chi_gB = '+chi_unit]]
    ENDIF ELSE BEGIN
       set_output, diag, sp, header=['t','<R/LT>',$ ;'<R/Ln>',$
             '<chi_es>','<chi2_es>','<Q_es>'], $
             dat=[[time],[REFORM(avgrad[0,isp,*])],$ ;[REFORM(avgrad[1,isp,*])],$,$
                  [REFORM(avflux[1,1,isp,*])],[REFORM(avflux[2,1,isp,*])],$
                  [REFORM(avflux[0,1,isp,*])]],$
                   commentline=[[title],['<chi>=<Q(x)/[-n(x) d/dx T(x) '+geo_fac_str+']>'],$
                                ['<chi2(x)>=<Q>/<-n(x) d/dx T(x) '+geo_fac_str+'>'],$
                                ['chi_gB = '+chi_unit]]
    ENDELSE
