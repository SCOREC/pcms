...
MODULE boundary_exchange_z
  USE BoundaryDescriptionModule, only: BoundaryDescription
  USE boundary_exchange_general, only: exchange_general
  USE box_data_module, ONLY: box_data_type, get_lilo, get_n0_global, get_q0, get_Rhostar,&
       &get_kymin, get_lx,get_shat, HasMagneticProfile, get_xposition
  USE discretization, ONLY: pi1gl, pi2gl, pi1,pi2, li1,li2, lj1,lj2, pj0,pj1,pj2, &
       & nzb,lbz,lk1,lk2,ubz,&
       & hky, ky0_ind, my_pez, n_procs_z, pmi0, nky0
  USE par_in, ONLY: shifted_metric, y_local
  use par_other, only: pi, imag, print_ini_msg
  USE communications, ONLY: mpi_comm_z
  USE fourier, ONLY: initialize_fourier_x_1d, finalize_fourier_x_1d,to_fourier_x_1d, to_real_x_1d
  use mpi
  implicit none
