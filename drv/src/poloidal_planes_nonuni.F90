...
module poloidal_planes_nonuni
  ...
  use lagrange_interpolation, only: lag3interp,lag3interp_complex !This lag3interp interface is not unique to the cce. 
  ! It is used by other routines e.g. chease, miller_geometry etc. The lag3interp_complex isnt used in the cce
  
  use geometry, only: geom, C_y, q_prof
  use par_other, only: pi, imag
  use fourier, only: initialize_fourier_arb, finalize_fourier_arb !These subroutines are unique to the coupling.
  !Their parent module is not unique to cce, so it might be easier to isolate the subroutines the cce uses.
  ...
  use coupling_core_gene, only: &
      ...
      cce_process_density, &
      write_check_file, &
      cce_receive_gene_density
#endif

#ifdef INIT_XGC
  use coupling_core_gene, only: cce_receive_density, &
      cce_send_density
...
!                        & lag_out,zval(lk1:lk2),lk0)
  ...
  subroutine import_planes_nonuni
    ...
    call cce_receive_field(data_block, block_start, block_end, block_count, n_cuts, my_mpi_comm_world)
...
!   data_block(maxplane, :) = first_plane(:)
 ...
 ! testing purpose only, read what written in existing file
    ...
    do o=0,n_cuts-1
#ifdef COUPLE_XGC
       call cce_receive_GENE_density(data_block,block_start,block_end,block_count,o,mpi_comm_x)
...
#ifdef INIT_XGC
    call cce_receive_density(data_block, block_start, block_end, block_count, n_cuts, my_mpi_comm_world)
#endif
    current_data = 0.0
...
!if ((i.eq.90).and.((my_pey+my_pez+my_pew+my_pev).eq.0)) print *,'surface 91',data_unst(i)%data(0,:),my_pex
       deallocate(data_fs,tmp)
    enddo
...
!       call creatg(fidinterp_h5, '/init_cond')
  ...
  subroutine write_planes(diagnostic,dset_name)
    ...
    if ((my_pez+my_pev+my_pew+my_pespec).eq.0) then
       data_block = data_block * norm_fact_mom
       if (.not.present(diagnostic)) call cce_send_density(data_block, o, n_cuts, block_count, block_start, block_end, mpi_comm_x)
    endif
#endif
