...
module tracer_mod
  ...
  use par_in, only: lilo, sign_Ip_CW, sign_Bt_CW
  use par_geom, only: geomtype, tracer_rt_scheme
  use communications

  implicit none

  public:: init_tracer, get_tracer, finalize_tracer, get_efit_refvals,&
       set_tracer_defaults
  private
  ...
  logical:: tracer_initialized=.false., write_pe, write_pex
contains

  subroutine set_tracer_defaults
    tracer_rt_scheme='F2D'
  ...
  subroutine init_tracer(geomdir, geomfile, x0_in, lx, rhostar, minor_r, major_R, edge_opt, trpeps)
    ...
    real, intent(inout):: major_R, minor_r, trpeps
    real:: dx, c11_int, r, p, t, zsym, hn1, err, dphi, dtau, zmin, zmax
    real:: q, last_q, dqdx, qefit, shat_efit, distance, acc
    real:: z,Br,Bp,Bz,dBrdr,dBrdp,dBrdz,dBpdr,dBpdp,&
               &dBpdz,dBzdr,dBzdp,dBzdz, rhorz, extra_var
    real:: psirz, relpsirz, psif, relpsif
    integer, parameter :: neqn=8 !Number of ODEs
    real, dimension(pi1gl:pi2gl):: q_prof, dqdx_prof
    real, dimension(pi1:pi2):: acc_loc
    ...
    write_pe = (mype.eq.0)
    write_pex = (my_pey+my_pez+my_pev+my_pew+my_pespec.eq.0)
    ...
    call initialize_efit(geomdir, geomfile, Lref, Bref, rmag, zmag)
    call get_rz_pars(xval_a, rmag, rzstart, zmin, zmax)
    ...
    do i=i1,i2
       ...
       do while (n_runs.le.n_iterations+1)
          ...
          dtau=distance/real(resolution-mod(resolution,2))
          dphi=dtau
          if (sign_Ip_CW*sign_Bt_CW.ne.0) dphi=dtau*sign_Bt_CW
          hn1=0.1*dtau !guessed step for integrator
          ...
          call get_from_efit(r,z,Br,Bp,Bz,dBrdr,dBrdp,dBrdz,dBpdr,dBpdp,&
               &dBpdz,dBzdr,dBzdp,dBzdz,extra_var)
          call calc_quantities(r,z,Br,Bp,Bz,dBrdr,dBrdp,dBrdz,dBpdr,&
               &dBpdp,dBpdz,dBzdr,dBzdp,dBzdz,rmag,zmag,extra_var)
          ...
          do kk=1,resol_b
             ...
             call odeint(y,t+dtau,t,err,hn1,0.)
...
!             dqdx_prof(i)=dqdx
          ...
          if (n_runs.gt.1.and.n_runs.ne.n_iterations+1) then
             acc_loc(i)=abs(q-last_q)/q
             if (lilo) acc_loc(pi1:pi2)=acc_loc(pi1)
             !we require an accuracy of 0.01%
             if (acc_loc(i).gt.1e-4) then
                n_runs=n_runs+1
                last_q=q
             else
       ...
       if (abs((q-qefit)/qefit).gt.0.005) then
          ...
          if (write_pex) then
             write(*,'(A,F6.3)') 'WARNING: rel. q difference larger than 0.5% at x=',&
                  &xval_a(i)
             write(*,'(2(A,F7.4,1X),A,F6.1,A)') '         q_tracer = ',&
       ...
       if (abs((dqdx*xval_a(i)/q-shat_efit)/shat_efit).gt.0.03.and.shat_efit.gt.0.1) then
          ...
          if (write_pex) then
             write(*,'(A,F6.3)') 'WARNING: rel. shat difference larger than 3.0% at x=',&
                  &xval_a(i)
             write(*,'(2(A,F7.4,1X),A,F6.1,A)') '         shat_tracer = ',&
    ...
    enddo
     ...
     if (n_procs_x.gt.1) then
        call mpi_allreduce(maxval(acc_loc),acc,1,MPI_REAL_TYPE,MPI_MAX,mpi_comm_x, ierr)
     else
        acc=maxval(acc_loc)
     endif
  ...
  subroutine get_tracer(geom,&
       ...
       &q0_out,shat_out, Lref_out, Bref_out,edge_opt,jpar_out)
    type(geomtype),intent(inout):: geom
    real, dimension(pi1gl:pi2gl),intent(out):: C_y_out,C_xy_out,q_prof_out,dqdx_prof_out,dpdx_prof_out
    real, dimension(pi1gl:pi2gl,lk1:lk2),intent(out):: jpar_out
    ...
    real, dimension(pi1gl:pi2gl,lk1:lk2) :: jpar
    real, intent(out):: q0_out,shat_out
    real:: q0, shat, Bref_out, Lref_out, edge_opt !, pref
    integer:: i,k

    if (edge_opt.ne.0.) then
       call get_from_tracer_aux(gxx,gxy,gxz,gyy,gyz,gzz,Bfield,dBdx,dBdy,dBdz,jpar,jacobian,geo_R,geo_p,geo_Z,&
    ...
    do k=lk1,lk2
        do i=pi1gl,pi2gl
           geom%THETA_pol(i,k)=atan2(geom%Z(i,k)-zmag,geom%R(i,k)-rmag)
        enddo
    enddo
    C_y_out=C_y
    C_xy_out=C_xy
...
!       write(*,'(4(ES12.5,1X))') x0, pref, 8.0E-7*pi*pref/Bref**2, -dpdx_prof/pref
  ...
  subroutine get_efit_refvals(Bref_out,Lref_out,major_R,minor_r,&
       geomdir,geomfile)
    real, intent(out):: Bref_out, Lref_out, major_R, minor_r
    character(len=100), intent(in) :: geomdir
    character(len=80), intent(in)  :: geomfile
    !real :: zmag
