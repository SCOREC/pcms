...
module initcond
  ...
  USE coordinates, ONLY: kx,kxmin,kx_center,lx,ky,kymin,ly,&
       &zval,vp,lv,mu,ki,kj,kjmax,deli,lp1,xval_a,xval
  USE discretization, ONLY: nx0,lkx,pi1gl,ni0,hkx,li1,li2,pi0,&
       & pi1,pi2,li0da,evenx,even_ni0,pmi0,&
       & lj1,lj2,pj0,nj0,nky0,ky0_ind,ly0da,n_procs_y,my_pey,lj0,pj1,&
       & lg1,lg2,lk1,lk2,&
       & ll1,ll2,nv0,lm1,lm2,ln1,ln2,pn1,pn0,n_spec,&
       & mype,yx_order
  ...
  USE geometry, ONLY: shat, geom,rhostar,minor_r
  USE gyro_average_ff_mod, only: get_jfac
  USE mtrandom, ONLY: grnd,sgrnd
  USE neo_equilibrium, only: g1_nc
  USE par_in, ONLY: spec,tau,underflow_limit,precomp_nc,nonlinear,y_local,x_local,n0_global, beta
  USE par_mod, ONLY: time,g_1
  USE par_other, ONLY: p_has_0_mode, p_has_00_mode,print_ini_msg,pi,imag,xy_local
  USE parallel_nonlin,only: nl_to_fourier_xy
  USE vel_space, only: fm
!#ifdef COUPLE_XGC
  use poloidal_planes_nonuni, only: map_XGC_init_cond
!#endif
...
contains
  ...
  Subroutine init_g_1
    ...
    Select case (init_cond)
      ...
      case('bmj')
         Call blobmodejac(init_aux_x, init_aux_y, init_aux_z, init_aux_amp)
      ...
      case('orszag_tang') ! orszag-tang vortex problem (for 2D)
         CALL orszag_tang(init_aux_amp)
      case('zero') ! everything zero (use with antenna currect)
         Call antenna_zero
      case('XGC') !for XGC comparison. constant or gaussian inx, can multiply by noise
         Call XGC_like(init_aux_x, init_aux_y, init_aux_z, init_aux_amp)
      case('XGC2') !for XGC comparison. constant or gaussian inx, can multiply by noise
         Call XGC_like2(init_aux_x, init_aux_y, init_aux_z, init_aux_amp)
      case('read_XGC')
         call read_XGC_file
      case default
  ...
  SUBROUTINE powjac(aux_x,aux_y,aux_z, aux_amp,rnd_phase,odd_z_parity)
    ...
    complex, dimension(0:ni0-1,0:nj0-1):: phasefac
...
!    kxmin=2.0*pi/lx
  ...
  !> Alternative density blob initialization
  !! Gaussian in radial direction with FWHM in units of r/a
  !! constant in y direction
  !! jacobian to the power of aux_x in parallel direction
  Subroutine blobmodejac(aux_x,aux_y,aux_z,aux_amp)
    REAL, intent(IN) :: aux_x, aux_y, aux_z, aux_amp
    COMPLEX, DIMENSION(li1:li2,lj1:lj2,lk1:lk2) :: dens
    REAL::   amplitude,sigma_x, es
    INTEGER:: i, j, k, n, ljs, lje

    if (xy_local) stop 'db1 not implemented for local code'
    ...
    If ((aux_x.ne.-100.).And.(aux_x.ne.0.0)) sigma_x=aux_x
    If (aux_y.lt.0) then
       ljs = lj1
       lje = lj2
    else
    ...
    If (write_pe) Then
       Write(*,"(A)")        "  Gaussian density blob in radial direction"
       Write(*,"(A,ES10.3)") "  with  FWHM = ", sigma_x
       If (aux_y.lt.0) Then
          Write(*,"(A)")     "  for all ky"
       else
          Write(*,"(2(A,ES10.3))") "  for ky = ",kymin*aux_y,&
               &", n = ",n0_global*aux_y
       endif
       Write(*,"(A,ES10.3,A)") "  and jacobian to the power of ",es," in z"
    End If

    !translate to FWHM
    sigma_x = sigma_x/(2.0*sqrt(ALOG(2.0)))
    ...
    Do k = lk1,lk2
       Do j = ljs,lje
          Do i = li1,li2
             dens(i,j,k) = amplitude*exp(-((xval_a(i)-0.5)/sigma_x)**2)*&
                  &geom%jacobian(i,pj1,k)**es
          End Do
       End Do
    End Do
  ...
  !! the density is given by (1-Gamma0)/Gamma0 *Jfac*amplitude and hence the electrostatic
    ...
    do n=ln1,ln2
       ...
       do i=li1,li2
          ...
          do k=lk1,lk2
             ...
             dens(i,j,k) = spec(n)%charge/spec(n)%temp*(1.0-gamma0(b2spec))/gamma0(b2spec)*amplitude*&
  ...
  SUBROUTINE current_sheets(aux_x,aux_y,aux_z,aux_amp,force_free)
    ...
    INTEGER :: i, j, k, l, m, n, x_mode, pni
                   ...
                   if (.not.force_free) then
                      ...
                      do j=lj1,lj2
                         if (j.eq.0) then
                            DO i = li1, li2
                               if (spec(n)%charge.eq.-1) then
                                  g_1(i,j,k,l,m,n) = dens(i,j,k) * fm(pi1,pj1,k,l,m,pni) * (-lv+l*dv) * xfunc(i,ffcs_width)
                               else
                                  g_1(i,j,k,l,m,n) = 0.
                               endif
                            END DO
                            ...
                            DO i = li1, li2
                               if (spec(n)%charge.eq.-1) then
                                  g_1(i,j,k,l,m,n) = dens(i,j,k) * fm(pi1,pj1,k,l,m,pni) * (-lv+l*dv) * cos(2.0*pi*i/nx0-pi/2.)
                               else
                                  g_1(i,j,k,l,m,n) = 0.
                               endif
                            END DO
                            ...
                            DO i = li1, li2
                               if (spec(n)%charge.eq.-1) then
                                  g_1(i,j,k,l,m,n) = dens(i,j,k) * fm(pi1,pj1,k,l,m,pni) * (-lv+l*dv)
                               else
                                  g_1(i,j,k,l,m,n) = 0.
                               endif
                            enddo
  ...
  contains
    real function xfunc(i,width)
      integer, intent(in):: i
      real:: x, width, dx
      !use local dx as a temporary fix to periodicity problem with even nx0
      dx=lx/nx0
...
!  !*************************************************************************!
  SUBROUTINE orszag_tang(aux_amp)

    IMPLICIT NONE

    REAL, INTENT(INOUT) :: aux_amp
    REAL, DIMENSION(:,:), ALLOCATABLE :: temp_sxsy, dens
    COMPLEX, DIMENSION(li1:li2,lj1:lj2) :: temp_kxky
    REAL :: lmod, mmod
    REAL :: fm_lmod, dv, pi_n15, c_phi, c_apar, bperp0
    REAL :: phi_max_loc, phi_max, apar_max, apar_max_loc
    INTEGER :: i, j, k, l, m, n, nlj_lo, nlj_hi, ierr

    !For noise

    IF (aux_amp.eq.-100.0) aux_amp = 1.0
    call sgrnd(1)

    ! check: code needs to be run in local mode
    IF (.NOT. xy_local) THEN
      IF (write_pe) WRITE(*,"(A)") &
        "ERROR: orszag-tang initial condition requires local code"
      STOP
    END IF
    ...
    ALLOCATE(temp_sxsy(0:ly0da-1,nlj_lo:nlj_hi),dens(0:ly0da-1,nlj_lo:nlj_hi))
    dens = 1.0
    ...
    DO n = ln1, ln2
          DO m = lm1, lm2
             DO l = ll1, ll2
                DO k = lk1, lk2
                  DO j = nlj_lo, nlj_hi    ! this is x
                      DO i = 0, ly0da - 1  ! this is  y
                         ! shifted Maxwellians (different bulk velocity sign for ions, electrons)
                         IF (spec(n)%charge .EQ. -1) THEN
                               lmod =vp(l)*(COS(2.0*pi*i/Real(ly0da)) + 2*cos(4.0*pi*j/Real(li0da)))
                               mmod = 1.0
                         ELSE
                               lmod = 1.0
                               mmod = COS(2.0*pi*i/Real(ly0da)) + cos(2.0*pi*j/Real(li0da))
                         END IF
                         ! construct fractional index (lmod) shifted Maxwellian
                         fm_lmod = lmod * mmod * pi_n15 * EXP(-(mu(m)*geom%Bfield(pi1,pj1,k)+(vp(l))**2))
                         temp_sxsy(i,j) = (dens(i,j) + aux_amp*(1-2*grnd()))*fm_lmod
                      END DO
                  ENDDO
                  CALL nl_to_fourier_xy(temp_sxsy,temp_kxky)
                  g_1(li1:li2,lj1:lj2,k,l,m,n) = temp_kxky(li1:li2,lj1:lj2)
    ...
    phi_max_loc = maxval(abs(emfields(:,:,:,1)))
    apar_max_loc = maxval(abs(emfields(:,:,:,2)))
    call mpi_allreduce(phi_max_loc,phi_max,1,MPI_REAL_TYPE,MPI_MAX,mpi_comm_y,ierr)
    call mpi_allreduce(apar_max_loc,apar_max,1,MPI_REAL_TYPE,MPI_MAX,mpi_comm_y,ierr)

    bperp0 = 0.5
    c_phi   = - bperp0*(0.5*lx/pi)*sqrt(2.0/beta)/phi_max
    c_apar  = - bperp0*(lx/(2.0*pi))/apar_max
    ...
    CALL finalize_fourier
    DEALLOCATE(temp_sxsy,dens)

  END SUBROUTINE orszag_tang

!*************************************************************************!
  subroutine read_XGC_file
    complex, dimension(li1:li2,lj1:lj2,lk1:lk2) :: data_from_XGC
    integer :: n

    cond_local_in_i = .false.

    If (write_pe) Then
       Write(*,"(A)") "  Reading XGC file for initial condition"
    End If

    call map_XGC_init_cond(data_from_XGC)

    do n=ln1,ln2
       Call add_fm(data_from_XGC,n)
    end do


  end subroutine read_XGC_file

!*************************************************************************!
  Subroutine XGC_like(aux_x,aux_y,aux_z,aux_amp)
    REAL, intent(IN) :: aux_x, aux_y, aux_z, aux_amp
    COMPLEX, DIMENSION(li1:li2,lj1:lj2,lk1:lk2) :: dens
    REAL::   amplitude,sigma_i,sigma_j,sigma_z,xvar, noise_amp
    INTEGER:: i, j, k, n
    complex, dimension(li1:li2,0:nky0-1):: noise
    logical :: gauss_env, add_noise, use_ky0, gauss_z
    ...
    If ((aux_x.ne.-100.).And.(aux_x.ne.0.0)) then
        gauss_env=.true.
        sigma_i=aux_x
        IF (aux_x.eq.-1.0) sigma_i = lp1/8.0
    endif
    If ((aux_z.ne.-100.).And.(aux_z.ne.0.0)) then
       if (aux_z.lt.0.) then
          add_noise=.true.
          noise_amp=-aux_z
       else
    ...
    If ((aux_y.ne.-100.).And.(aux_y.ne.0.0)) then
        use_ky0=.true.
    endif

    If (aux_amp.ne.-100.) amplitude = aux_amp/(rhostar*minor_r)


    If (write_pe) Then
       Write(*,"((A,ES10.3))") "  For XGC comparison, amplitude", amplitude
       if (gauss_env) Write(*,"((A,ES10.3))") "  gaussian in x, sigma_i=",sigma_i
       if (gauss_z) Write(*,"((A,ES10.3))") "  gaussian in z, sigma_z=",sigma_z
       if (add_noise) write(*,"((A,ES10.3))") "  times white noise, ampl=",noise_amp
       if (use_ky0) Write(*,"(A)") "  exciting also ky=0 mode"
    End If
    if (add_noise) then
      call sgrnd(1)
      !reality condition for noise
      do j=0,nky0-1
         do i=li1,li2
            noise(i,j)=noise_amp*(grnd()-0.5+imag*(grnd()-0.5))
         enddo
      enddo
    endif
    if (mype.eq.0) print *, zval
    Do k = lk1,lk2
       Do j = lj1,lj2
          Do i = li1,li2
             if ((n0_global*j).lt.124) then
                if ((mype.eq.0).and.(i.eq.0)) print *, 'intializing', n0_global*j
                dens(i,j,k) = amplitude
             else
                dens(i,j,k) = 0.0D0
             endif
             if (gauss_env) dens(i,j,k) = dens(i,j,k) * exp(-((xval_a(i)-0.7)/sigma_i)**2/2.0)
             if (gauss_z) dens(i,j,k)= dens(i,j,k) * exp(-(zval(k)/sigma_z)**2/2.0)
             if (add_noise) dens(i,j,k) = dens(i,j,k) + noise_amp*(grnd()-0.8+imag*(grnd()-0.5))
          End Do
       End Do
     End Do

     if ((y_local).and.(.not.use_ky0)) then
       If (p_has_0_mode.and.(abs(kjmax).gt.epsilon(kjmax))) then
          do k=lk1,lk2
            dens(:,lj1,k) = 0.0D0
          enddo
  ...
  End Subroutine XGC_like


  Subroutine XGC_like2(aux_x,aux_y,aux_z,aux_amp)
    REAL, intent(IN) :: aux_x, aux_y, aux_z, aux_amp
    COMPLEX, DIMENSION(li1:li2,lj1:lj2,lk1:lk2) :: dens
    REAL::   amplitude,sigma_i,sigma_j,sigma_z,xvar, noise_amp
    INTEGER:: i, j, k, n
    ...
    Do k = lk1,lk2
       Do j = lj1,lj2
          Do i = li1,li2
             if ((j.gt.0).and.((n0_global*j).lt.128)) then
                if ((mype.eq.0).and.(k+i.eq.0)) print *, 'intializing', n0_global*j
                dens(i,j,k) = amplitude !/(n0_global*j)
             else
                dens(i,j,k) = 0.0D0
             endif
             if (gauss_env) dens(i,j,k) = dens(i,j,k) * exp(-((xval_a(i)-0.7)/sigma_i)**2/2.0)
...
!                      g_1(i,j,k,l,m,n) = my_dens(i,j,k)*fm(pii,pji,k,l,m,pni)/(spec(n)%temp_prof(pii))**(-1.5)
                      g_1(i,j,k,l,m,n) = my_dens(i,j,k)*spec(n)%dens_prof(i)*&
                           (pi*spec(n)%temp_prof(i))**(-1.5)*&
                           exp(-(mu(m)*geom%Bfield(i,lj1,k)+(vp(l))**2)/&
                           (spec(n)%temp_prof(i)))
                   endif
                   if (J0_pow.ne.0) then
