...
;  file_par.exist = 0
  ...
  par.nlp_kyind = 0
  par.n_triplets = 0
  par.n_pod_triplets = 0
  kx_triplets = INTARR(60)
  ky_triplets = INTARR(60)
  par.n_ev = 0
  which_ev = ''
  par.lx = 0
  par.kx_center = 0.0
  par.mu_grid_type = 'gau_lag'
  nexc = 0
  par.n_conn = 0
  par.n_pol = 1
  par.ehat = 0
...
;      IF run EQ 0 THEN BEGIN
	  ...
	  '&geometry'   : par.norm08 = 1
          ...
          'kx_nlt_ind'  : kx_nlt_ind[0:par.num_nlt_modes-1] = $
             FIX(STRSPLIT(value,' ',/EXTRACT))
          'ky_nlt_ind'  : ky_nlt_ind[0:par.num_nlt_modes-1] = $
             FIX(STRSPLIT(value,' ',/EXTRACT))
          'n_triplets'  : par.n_triplets = FIX(value)
          'n_pod_triplets' : par.n_pod_triplets = FIX(value)
          'kx_triplets' : kx_triplets[*] = FIX(STRSPLIT(value,' ',/EXTRACT))
          'ky_triplets' : ky_triplets[*] = FIX(STRSPLIT(value,' ',/EXTRACT))
	  ...
	  'mag_prof'	: par.mag_prof = FIX(value)
          ...
          'sign_Ip_CW'  : par.sign_Ip_CW = FIX(value)
          'sign_Bt_CW'  : par.sign_Bt_CW = FIX(value)
          'sign_Omega_CW' : par.sign_Omega_CW = FIX(value)
          ; --- float values ---
	  ...
	  'geomfile'    : par.geomfile = $
          ...
          'GIT_BRANCH'  : par.git_branch = value
          'GIT_MASTER'  : par.git_master = value
...
;      ENDIF
    ...
    ;background rotation rescaling for testing
    omegatorrsc = 1.0
    IF (omegatorrsc NE 1.0) THEN BEGIN
       printerror, 'WARNING: OMEGATORREF ARTIFICIALLY RESCALED WITH '+$
                   rm0es(omegatorrsc)
       par.omegatorref *= omegatorrsc
    ENDIF

;   set to last SVN revision if git hash if found
    IF ((par.svn_rev EQ '') AND (par.git_branch NE '')) THEN $
       par.svn_rev = '4903'
...
;     now it is 8*pi*p/B^2
    ...
    IF run GT 0 THEN $ ; compare par and spec with previous run
      IF (oldpar.nkx0 NE par.nkx0) OR (oldpar.nky0 NE par.nky0) OR $
      (oldpar.nz0 NE par.nz0) OR (oldpar.lx NE par.lx) OR $
      (oldpar.kymin NE par.kymin) OR (oldpar.n_fields NE par.n_fields) OR $
      (oldpar.n_moms NE par.n_moms) OR $
      (TOTAL((ABS(oldspec[0:par.n_spec-1].omt) LT $
      0.999 * ABS(spec[0:par.n_spec-1].omt)) OR $
      (ABS(oldspec[0:par.n_spec-1].omt) GT $
      1.001 * ABS(spec[0:par.n_spec-1].omt))) NE 0) OR $
      (TOTAL((ABS(oldspec[0:par.n_spec-1].omn) LT $
      0.999 * ABS(spec[0:par.n_spec-1].omn)) OR $
      (ABS(oldspec[0:par.n_spec-1].omn) GT $
      1.001 * ABS(spec[0:par.n_spec-1].omn))) NE 0) OR $
  ...
  IF (par.nkx0 EQ 0) OR (par.nky0 EQ 0) OR (par.nz0 EQ 0) OR $
    ...
    printerror, $
      'no nkx0, nky0, nz0, n_spec, n_fields, or n_moms in parameters', /popup
  ENDIF
  ...
  IF PTR_VALID(par.kx_triplets) THEN PTR_FREE, par.kx_triplets
  IF PTR_VALID(par.ky_triplets) THEN PTR_FREE, par.ky_triplets
  ...
  par.kx_triplets = PTR_NEW(kx_triplets[0:(3*par.n_triplets-1)>0])
  par.ky_triplets = PTR_NEW(ky_triplets[0:(3*par.n_triplets-1)>0])
...
; Therefore, par.nky0 is not necessarily identical to the parameters value
  IF par.ky0_ind GT 0 THEN par.nky0 += par.ky0_ind
  ...
  IF par.x0 LT 0 THEN BEGIN
    IF par.x_local THEN BEGIN
      IF ((par.magn_geometry EQ 'circular') OR (par.magn_geometry EQ 's_alpha')) $
        ...
        par.x0 = par.minor_r EQ 0 ? 0.0 : par.trpeps * par.major_R / par.minor_r
    ENDIF ELSE BEGIN
      IF par.magn_geometry EQ 'circular' THEN par.x0 = 0.5 ; default for old runs
    ENDELSE
  ENDIF ELSE BEGIN
...
PRO read_tracer_ff, geo_lun, geom
  ...
  IF par.norm08 THEN BEGIN
    ...
    IF par.write_h5 THEN BEGIN
      ; determine whether metric coefficient names use _ (old) or ^ (new)
      n_metric_coeffs = H5G_GET_NMEMBERS(geo_lun,'metric/')
      found_caret = 0
      FOR j = 0, n_metric_coeffs - 1 DO $
        IF STRPOS(H5G_GET_MEMBER_NAME(geo_lun,'metric/',j),'^') GE 0 THEN $
        found_caret = 1
      h5_fields = found_caret ? $
        ['g^xx','g^xy','g^xz','g^yy','g^yz','g^zz',$
...
PRO read_tracer_df, geo_lun, geom
  ...
  IF par.write_h5 THEN BEGIN
    ...
    geom.gxx = read_datakey(geo_lun,'metric/g'+indstr+'xx')
    geom.gxy = read_datakey(geo_lun,'metric/g'+indstr+'xy')
    geom.gxz = read_datakey(geo_lun,'metric/g'+indstr+'xz')
    geom.gyy = read_datakey(geo_lun,'metric/g'+indstr+'yy')
    geom.gyz = read_datakey(geo_lun,'metric/g'+indstr+'yz')
    geom.gzz = read_datakey(geo_lun,'metric/g'+indstr+'zz')
    ...
    IF (par.magn_geometry NE 'circular') THEN BEGIN
       ; same trick for shape and derivatives
       IF (indstr EQ '_') THEN BEGIN
       ...
       ENDIF ELSE BEGIN
          geom.R = read_datakey(geo_lun,'shape/R')
          geom.Z = read_datakey(geo_lun,'shape/Z')
          geom.c1 = read_datakey(geo_lun,'shape/dxdR')
          geom.c2 = read_datakey(geo_lun,'shape/dxdZ')
       ENDELSE
    ENDIF
  ENDIF ELSE BEGIN
