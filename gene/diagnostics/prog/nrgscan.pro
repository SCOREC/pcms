...
PRO nrgscan_loop, diag
  ...
  ; set nrgindex
  IF (par.comp_type EQ 'IV') THEN nrgindex = N_ELEMENTS((*nrg).time) - 1 $
  ...
  FOR isp = 0, gui.out.n_spec_sel - 1 DO BEGIN 
    ...
    ENDIF ELSE BEGIN
      ...
      FOR isort = 0, (*scanlog).dsize[0]-1 DO BEGIN
         indsortf = (runnr-1)*(*scanlog).dsize[0]+isort
          IF (-(*scanlog).sortfield[indsortf]) EQ FIX(mom_time) THEN BEGIN
            (*i).eigen[isp].nrgfield[indsortf] $
                = (*nrg)[nrgindex].data[(*i).nrgpar,sp]/divisor
             found+=1
          ENDIF
      ENDFOR
