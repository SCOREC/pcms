...
module adiabatic_response
  ...
  implicit none
  public:: mem_est_adiabatic_response, check_for_adiabatic_species, &
       &set_adiabatic_response_defaults, adiabatic_electrons
  ...
  !>Sets default values for the public module variables
  subroutine set_adiabatic_response_defaults
    check_qn_gradients = .true.
  end subroutine set_adiabatic_response_defaults
  ...
  subroutine check_for_adiabatic_species
    ...
    IF(adiabatic_electrons.and.(tau.ne.1.0)) THEN
       STOP "tau parameter not implemented for adiabatic electrons"
    END IF
    ...
    if (any(abs(qnomn).gt.1e-6)) then
       if (mype.eq.0) print *, &
            &'Maximum violation of quasineutrality in gradients: ', &
            &maxval(abs(qnomn))
       if (check_qn_gradients) &
  ...
  subroutine initialize_adiabatic_response
    ...
    if (.not.x_local) then
       call inv_p_doubleprime_ions_m%initialize(ni0,ni0)
       call inv_p_doubleprime_ions_m%allocate()
       call inv_p_doubleprime_ions_m%set_zero()
  ...
  subroutine finalize_adiabatic_response
    ...
    if (.not.x_local) then
       call inv_p_doubleprime_ions_m%finalize()
  ...
  subroutine solve_for_fs_avg(mat, rhs, res)
    ...
    call rhsvec%attach(rhs)
    call solvec%attach(res)
    call mat%dot_multiply(rhsvec,solvec)
