...
module compute_dt
  use antenna, only: antenna_contrib
  use island, only: island_contrib
  use par_mod
  use discretization
  use coordinates
  use communications
...
contains
  ...
  subroutine compute_dt_max
    ...
    logical:: sav_print_ini_msg, sav_antenna_contrib, sav_island_contrib
    real:: dt_max_in
    ...
    if(write_pe) then
       ...
       write(*,"(a,2(a,3(i1,a)))") &
#else
       write(*,"(a,2(a,(i1,a),(i2,a),(i1,a)))") &
#endif
            'Starting time step computation with ','PETSC-',&
            PETSC_VERSION_MAJOR,'.',PETSC_VERSION_MINOR,'.',PETSC_VERSION_SUBMINOR,'/',&
            'SLEPc-',SLEPC_VERSION_MAJOR,'.',SLEPC_VERSION_MINOR,'.',SLEPC_VERSION_SUBMINOR,''
       write(*,"(a)") '*************************************************************'
    endif
    ...
    sav_island_contrib=island_contrib
    ...
    island_contrib=sav_island_contrib
    ...
    if (new_evs.eq.0) then
       ...
       do iev=0,new_evs-1
          ...
          call EPSGetEigenpair(eps_obj,iev,eigenvalues(iev+num_ev),&
               &PETSC_NULL_SCALAR,glob_g,PETSC_NULL_VEC,globerr)
#else
          call EPSGetEigenpair(eps_obj,iev,eigenvalues(iev+num_ev),&
  ...
  !!
  !!maxclimit must have been called before so that maxclimitx/maxclimity are set
  ...
  subroutine adapt_dt
    ...
    if (dt_inst.gt.0.0.and.dt_inst/dt.lt.thresh_down) then
