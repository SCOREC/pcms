...
contains
  ...
  subroutine get_entrysize_iterdb(infile, target_time, var_name, nrho_db)
    ...
    integer :: fileunit, it, ierr, ntime, nsc, tvarind, nx, ny
    integer, intent(inout) :: nrho_db
    character (10) :: varunit
    character (20) :: var
    character (1000) :: line
    logical:: found
    ...
    do while (.not. found)
       ...
       read (fileunit,fmt='(1X,I3)') nsc
       do it=1,nsc
          read (fileunit,'(A)') line
       enddo

       do it=0,1 !read X and Y description and determine time position
          read (fileunit,'(1X,A20,A10)') var, varunit
          if ((INDEX(varunit,'SECONDS').gt.0).or.(INDEX(varunit,'Seconds').gt.0).or.&
               (INDEX(varunit,'seconds').gt.0)) tvarind = it
       enddo
       !read dependent variable
       read (fileunit,'(1X,A20,A10)') var, varunit
       read (fileunit,'(A)') line

       read (fileunit,'(1X,I10)') nx
       read (fileunit,'(1X,I10)') ny

       if (tvarind.eq.0) then
          ntime = nx
          nrho_db = ny
       else
          nrho_db = nx
  ...
  !! definitions follow http://tokamak-profiledb.ccfe.ac.uk/DOCS/pr08vardoc.htm
  !! and http://w3.pppl.gov/~pshare/help/ufiles_manual.html#SEC3 for the u-file
  !! format
  ...
  !! \param opt_var declare variable as optional (no stop if not found)
  !! \param tst_varunit optional parameter to test and correct unit
  subroutine read_profiles_iterdb(infile, target_time, var_name, nrho, &
       &var_out, grad_out, rho, opt_var, tst_varunit)
    ...
    character(10), intent(in), optional:: tst_varunit
    integer, intent(inout):: nrho
    logical :: found
    integer :: fileunit, it, ierr, r, l, nrho_db, tvarind
    integer :: filedim, nsc, iproc, nx, ny !shotnum
    integer, dimension (1) :: arraymin
    character(4) :: tdev !tokamak id
    character(5) :: shotnumstr
    character (10) :: varunit, shdate
    character (14) :: fmt !11
    character (20) :: var
    character (1000) :: line
    real, dimension (:), allocatable :: datax, datay, rho_db, vartmp_db, var1d, tmp_arr
    ...
    do while (.not. found)
       ...
       if (ierr.lt.0) then
          if (present(opt_var)) then
             if (.not.opt_var) then
                ...
                if (mype==0) write (*,*) "Variable ", trim(var_name), &
                     &" not found in ", TRIM(infile)
                var_out = 0.0
                grad_out = 0.0
                exit
             endif
       ...
       read (line,fmt='(1X,A5,1X,A4,1X,I1)') shotnumstr, tdev, filedim
       if (filedim.ne.2) cycle !ignore all non-2D entries for now

       read (fileunit,fmt='(1X,A10)') shdate
       read (fileunit,fmt='(1X,I3)') nsc
       ...
       read (fileunit,'(I1)') iproc
       read (fileunit,'(1X,I10)') nx
       read (fileunit,'(1X,I10)') ny

       allocate (datax(nx))
       allocate (datay(ny))
       allocate (vartmp_db(nx*ny))

       !the iter data base uses the normalized rho_tor
       !see http://tokamak-profiledb.ccfe.ac.uk/DOCS/PR08MAN/pdbman.html
       read (fileunit,*,iostat=ierr) datax
       if (ierr.lt.0) then
          write(*,*) 'failed reading indep. var X for ',TRIM(var), 'in ',infile
          stop
       endif
       read (fileunit,*,iostat=ierr) datay
       if (ierr.lt.0) then
          write(*,*) 'failed reading indep. var Y for ',TRIM(var),' in ', infile
          stop
       endif

       do l = 1, (nx*ny)/6
          read (fileunit,'(1X,6(ES13.6))',iostat=ierr) vartmp_db(1+(l-1)*6:l*6)
          if (ierr.lt.0) then
       ...
       r = MODULO(nx*ny,6)
       if (r.NE.0) then
          write(fmt,"(a,i1,a)") '(1X,',r,'(ES13.6))'
          read (fileunit,fmt,iostat=ierr) vartmp_db(1+(l-1)*6:(l-1)*6+r)
       endif
       ...
       if (trim(adjustl(var)).EQ.trim(var_name)) then !found req. variable
          allocate (var_db(nx,ny))
          call cast2darr1d(vartmp_db,var_db,nx*ny) !type cast
          deallocate(vartmp_db)

          !assign axes depending on rho/time order in file
          if (tvarind.eq.0) then
             arraymin = minloc(abs(datax-target_time))
             nrho_db = ny
             ...
             arraymin = minloc(abs(datay-target_time))
             nrho_db = nx
             allocate(var1d(nrho_db))
             allocate(rho_db(nrho_db))
             rho_db = datax
          ...
          deallocate(datax,datay,var_db)

          if (present(tst_varunit)) then
             !account for various iterdb unit formats
             if (trim(varunit).eq.'N/CM**3') varunit='cm^-3'
             if (trim(varunit).eq.'M^3') varunit='m^3'
             if (trim(varunit).eq.'M^-3') varunit='m^-3'
             if (trim(varunit).eq.'EV') varunit='eV'
             if (trim(varunit).ne.trim(tst_varunit)) then
                if ((trim(varunit).eq.'cm^-3').and.&
                     (trim(tst_varunit).eq.'m^-3')) then
                   var1d = var1d * 1E6
                elseif ((trim(varunit).eq.'eV').and.&
                     (trim(tst_varunit).eq.'keV')) then
                   var1d = var1d * 1E-3
                elseif ((trim(varunit).eq.'keV').and.&
                     (trim(tst_varunit).eq.'eV')) then
                   var1d = var1d * 1E3
                elseif ((trim(varunit).eq.'CM/s').and.&
                     (trim(tst_varunit).eq.'m/s')) then
                   var1d = var1d * 1E-2
                else
                   write (*,*) "Unit mismatch for ", trim(var_name), &
                        &" in ", TRIM(infile)
                   stop
                endif
             endif
          ...
          deallocate(datax,datay,vartmp_db)
       endif
       read (fileunit,'(A)') line !read final line
    end do
  ...
  !>Read number of radial gridpoints from D-IIID IterDB/ONETWO file
...
100 stop 'STOP (d3d_iterdb): file i/o problem in d3d iterdb/onetwo file'
  ...
  !>Read variable defined by var_name from DIII-D ITER data base / ONETWO file
  !! \param infile string containing the file name
