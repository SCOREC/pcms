...
MODULE StoreBandedMatrixModule
  ...
  TYPE,public :: StoreBandedMatrixObject 
     ...
     COMPLEX, DIMENSION(:,:), POINTER :: localMatrix => NULL()
     !> data needed for LU factorization
     COMPLEX, DIMENSION(:,:), POINTER :: factoredBand => NULL()
     COMPLEX, DIMENSION(:),   POINTER :: auxData => NULL()
  ...
  INTERFACE initialize
     MODULE PROCEDURE sbm_initialize_matrix
  END INTERFACE

  INTERFACE finalize
     MODULE PROCEDURE sbm_finalize_matrix
  END INTERFACE
  
  INTERFACE isInitialized
     module procedure sbm_isInitialized
  END INTERFACE

  INTERFACE allocate
     module procedure sbm_allocate
  END INTERFACE

  INTERFACE set_value
     MODULE PROCEDURE sbm_set_value
  END INTERFACE

  INTERFACE add_value
     MODULE PROCEDURE sbm_add_value
  END INTERFACE

  INTERFACE commit_values
     MODULE PROCEDURE sbm_commit_values
  END INTERFACE

  INTERFACE set_zero
     MODULE PROCEDURE sbm_set_zero
  END INTERFACE

  INTERFACE get_global_matrix_locally
     module procedure sbm_get_global_matrix_locally
  END INTERFACE

  INTERFACE mat_get_value
     MODULE PROCEDURE sbm_get_value
  END INTERFACE

  interface mat_get_row_pointer
     module procedure sbm_get_row_pointer
  end interface

  INTERFACE get_local_abs_square_sum
     module procedure sbm_get_local_abs_square_sum
  END INTERFACE

  interface autotune
     module procedure sbm_autotune
  end interface

  INTERFACE dot_multiply
     MODULE PROCEDURE sbm_dot_multiply, sbm_matmat_dot_multiply
     module procedure sbm_dot_multiply_Banded_with_Full
  END INTERFACE

  INTERFACE square_matrix
     MODULE PROCEDURE sbm_square_matrix
  END INTERFACE

  INTERFACE show
     MODULE PROCEDURE sbm_show_on_screen, sbm_show_in_file
  END INTERFACE

  INTERFACE add_matrix
     MODULE PROCEDURE sbm_add_to_matrix, sbm_add_matrix
  END INTERFACE

  INTERFACE subtract_matrix
     MODULE PROCEDURE sbm_subtract_matrix, sbm_subtract_from_matrix
  END INTERFACE
  ...
  INTERFACE ASSIGNMENT(=)
     MODULE PROCEDURE sbm_assign_matrix
  END INTERFACE

  INTERFACE get_number_of_bands
     module procedure sbm_get_number_of_bands
  END INTERFACE

  INTERFACE my_sum
     MODULE PROCEDURE sbm_matrix_sum
  END INTERFACE

  INTERFACE row_axpy
     module procedure sbm_row_axpy
  END INTERFACE

  interface transpose_and_conjugate
     module procedure sbm_transpose_and_conjugate
  end interface

  interface transpose_storage
     module procedure sbm_transpose_storage
  end interface

  INTERFACE LU_factor
     module procedure sbm_LU_factor
  END INTERFACE

  INTERFACE LU_factor_ok
     module procedure sbm_LU_factor_ok
  END INTERFACE

  INTERFACE LU_solve
     module procedure sbm_LU_solve
  END INTERFACE

  interface exit_if_factored
     module procedure sbm_exit_if_factored
  end interface

  interface print_storage_details
     module procedure sbm_print_storage_details
  end interface

#ifdef ALL_ROUTINES
  INTERFACE invert
     module procedure sbm_invert_matrix
  END INTERFACE
#endif
...
CONTAINS
  ...
  subroutine sbm_get_row_pointer(mat,global_row,ptr_row,start_col,end_col)
    ...
    complex,dimension(:),pointer,intent(INOUT) :: ptr_row !originally intent(OUT) -> crash with intel/18.0
  ...
  SUBROUTINE sbm_assign_matrix(lmat,rmat)
    ...
    ! we assume that lmat is not yet initialized
    ! copy the scalar components
    lmat%PG = rmat%PG
    lmat%NRows = rmat%NRows
    lmat%NCols = rmat%NCols
    lmat%LowerBandwidth = rmat%LowerBandwidth
    lmat%UpperBandwidth = rmat%UpperBandwidth
    lmat%ColsPerBlock = rmat%ColsPerBlock
    lmat%NumberOfStoredRows = rmat%NumberOfStoredRows
    lmat%isInitialized = rmat%isInitialized
    lmat%isTransposed = rmat%isTransposed
    lmat%isFactored = .False.
    lmat%Desc = rmat%Desc
    if (.not.isEmpty(rmat%storage_list)) then
       call output(rmat%storage_list)
       write(*,"(A)") "storebandedmatrix.F90: Please commit the matrices before assigning them to a new matrix."
       stop
    else
       call initialize(lmat%storage_list)
    end if

    ! now allocate the arrays and copy the values
    if (associated(lmat%localMatrix)) then
       deallocate(lmat%localMatrix)
    end if
    allocate(lmat%localMatrix,source=rmat%localMatrix)
    
    if (associated(lmat%factoredBand)) deallocate(lmat%factoredBand)
    if (associated(rmat%factoredBand)) then
       allocate(lmat%factoredBand,source=rmat%factoredBand)
    else
       nullify(lmat%factoredBand)
    end if

    if (associated(lmat%auxData)) deallocate(lmat%auxData)
    if (associated(rmat%auxData)) then
       allocate(lmat%auxData,source=rmat%auxData)
    else
       nullify(lmat%auxData)
    end if

    if (associated(lmat%ipiv)) deallocate(lmat%ipiv)
    if (associated(rmat%ipiv)) then
       allocate(lmat%ipiv,source=rmat%ipiv)
    else
       nullify(lmat%ipiv)
    end if
  ...
  SUBROUTINE sbm_LU_factor(mat)
    ...
    INTEGER :: my_rank, n_procs, mpierr, status(mpi_status_size), np_send, np_recv
    ...
    IF(mat%isTransposed) THEN
      ...
      call mpi_sendrecv(sbuf, n, MPI_COMPLEX_TYPE, np_send, 111, &
                        ...
                        mat%PG%Communicator, status, mpierr)
