...
MODULE external_contr
  ...
  use geometry, only: rhostar, Lref, q0, C_y, minor_r, jpar, geom
  use communications, ONLY: MPI_COMPLEX_TYPE,mpi_comm_x,mpi_comm_z, mpi_comm_spec, MPI_REAL_TYPE,&
       &my_sum_to_all_real
  ...
  public:: phi0_ext, phase_phi_ext, use_self_consistent_flow, omt0_ext, phase_omt_ext, omn0_ext, phase_omn_ext, &
  ...
  public:: uprime_arr, uprime_x0, kink_on
  ...
  Complex, Dimension(:,:,:,:,:,:), Allocatable, Private :: p_omt_ext,p_omn_ext,p_phi_n_kh_ext
  ...
  !For the kink term
  real, dimension(:,:,:), allocatable :: upar_arr
  real, dimension(:,:), allocatable :: uprime_arr
  real :: uprime_x0 = 0.0
  logical :: kink_on=.false.

  logical :: use_self_consistent_flow=.false.
  !For KH drive, need k_perp
  real, dimension(:,:), allocatable :: k_perp
  real, dimension(:,:,:,:,:), allocatable :: jfactor
...
CONTAINS
  ...
  Real Function mem_est_external_contr(mem_req_in)
    ...
    if ((phi0_ext.gt.0.0).and.(kxind_phi_ext.gt.0).and.(kxind_phi_ext.lt.nx0/2).and.(use_self_consistent_flow))&
  ...
  Subroutine check_external_contr
    with_phi_ext=((phi0_ext.gt.0.0).and.(kxind_phi_ext.gt.0).and.&
         &(kxind_phi_ext.lt.nx0/2).and.(.not.use_self_consistent_flow))
    ...
    with_kh_drive_ext=((phi0_ext.gt.0.0).and.(kxind_phi_ext.gt.0).and.&
         &(kxind_phi_ext.lt.nx0/2).and.(use_self_consistent_flow))

    with_external=with_phi_ext.or.with_omt_ext.or.with_omn_ext.or.&
         &with_apar_ext.or.with_kh_drive_ext
  ...
  Subroutine init_external_contributions
    ...
    !impose sign convention on toroidal rotation
    if (abs(Omega0_tor).ne.0..and.sign_Omega_CW.ne.0) Omega0_tor=sign_Omega_CW*abs(Omega0_tor)
    if (sign_Omega_CW.ne.0) then
       if (allocated(omegator).and.allocated(omegatorprime)) then
          if (all(omegator.ge.0.).or.all(omegator.lt.0.)) then
             !remember old sign of omegator so we can redefine omegatorprime without new interpolation call
             old_sign=sign(1.,omegator(pi1))
             omegator=sign_Omega_CW*abs(omegator)
             omegatorprime=omegatorprime*sign(1.,omegator(pi1))*old_sign
          else
             stop 'Toroidal rotation profile changes sign, unable to impose sign convention. Manual check of input data required.'
          endif
       endif
    endif
    if (with_external) then
       ...
       !initialize external phi for KH drive
       if (with_kh_drive_ext) then
          ALLOCATE(p_phi_ext(li1:li2,lj1:lj2))
          allocate(p_phi_n_kh_ext(li1:li2,lj1:lj2,lk1:lk2,ll1:ll2,lm1:lm2,ln1:ln2))
          allocate(k_perp(li1:li2,lk1:lk2))
          allocate(jfactor(li1:li2,lj1:lj2,lk1:lk2,lm1:lm2,ln1:ln2))

          !compute k_perp only for the zonal mode
          do k = lk1,lk2
            k_perp(li1:li2,k) = sqrt(geom%gii(pi1,pj1,k)) * kx(li1:li2)
          end do

          if (write_pe) then
             WRITE(*,'(A)') 'Using external modifications of electrostatic potential'
             WRITE(*,'(A)') '(with self-consistent n for KH drive) with'
             WRITE(*,'(A,ES10.3,A,I3,A,ES10.3)') 'amplitude=', phi0_ext,', kxind=', &
                  & kxind_phi_ext,', phase=', phase_phi_ext
          endif
          ...
          if (x_local) then
             ...
             do j=lj1,lj2
                p_phi_ext(li1:li2,j) = kx(li1:li2)*phi_ext(li1:li2)*ky(j)
                do i=li1,li2
                  !p_phi_n_kh_ext contains the density-type prefactor
                  !  which needs to be consistent with the imposed phi_ext
                  !  (compare init_cond: gaminit)
                  p_phi_n_kh_ext(i,j,:,:,:,:) = -p_phi_ext(i,j)
                enddo
             enddo
          else
             if (write_pe) then
                WRITE(*,'(A)') 'error: self-consistent KH drive currently only in local code'
                STOP
             end if
          end if
       end if
...
!          Omega_ExB = 0.0
    ...
    if (kink_on) then
       if (mype.eq.0) print*, "initialize kink term"
       allocate(upar_arr(pi1gl:pi2gl,lk1:lk2,ln1:ln2))
       allocate(uprime_arr(pi1gl:pi2gl,ln1:ln2))
       upar_arr = 0.0
       uprime_arr = 0.0
       call set_uprime
    endif
       ...
       Elseif (fac_ind.eq.2) then !external density contributions
          Do j=lj1,lj2
             p_omn_ext(li1:li2,j,k,l,m,n) = p_omn_ext(li1:li2,j,k,l,m,n)*&
                  &vel_fac*imag*ky(j)
          End Do
       !fac_inds = 3, 4 are for kh_drive_ext
       Elseif (fac_ind.eq.3) then !pass mu(m) from prefactors.F90 for j0
         rho = sqrt(2.0*spec(n)%mass*spec(n)%temp*vel_fac / &
           &(spec(n)%charge**2*geom%Bfield(pi1,pj1,k)))
         do i=li1,li2
           jfactor(i,:,k,m,n) = j0(k_perp(i,k)*rho)
         end do
       Elseif (fac_ind.eq.4) then !pass fm from prefactors.F90 for g1_ext
         do i=li1,li2
           b2spec = k_perp(i,k)**2*spec(n)%mass*spec(n)%temp/ &
             & (spec(n)%charge*geom%Bfield(pi1,pj1,k))**2
           do j=lj1,lj2
             p_phi_n_kh_ext(i,j,k,l,m,n) = p_phi_n_kh_ext(i,j,k,l,m,n)* &
             & spec(n)%charge/spec(n)%temp*(1.0-gamma0(b2spec))/ &
             & gamma0(b2spec)*jfactor(i,j,k,m,n)*vel_fac
           end do
         end do
       Else
  ...
  Subroutine add_external_contr(localrhs, fac_ind, g1_or_chi, kxind_ext,k,l,m,n)
    ...
    If (fac_ind.eq.0) then
       if (.not. with_kh_drive_ext) then
         prefac_ext = p_phi_ext(li1:li2,lj1:lj2)
       else
         prefac_ext = p_phi_ext(li1:li2,lj1:lj2) * jfactor(:,:,k,m,n)
       end if
  ...
  Subroutine finalize_external_contributions
    if (with_external) then
       ...
       if (with_kh_drive_ext) deallocate(p_phi_ext,p_phi_n_kh_ext,k_perp,jfactor)
    ...
    if (kink_on) then
       if (mype.eq.0) print*, "finalize kink term"
      deallocate(upar_arr,uprime_arr)
    endif
  ...
  subroutine set_ExBrate
    ...
    if (ExBrate.eq.-1111.0.or.lilo_w_full_omegator_profile) then
       ...
       !this definition remains the same with new sign conventions
       ix0 = (pi1gl+pi2gl)/2
       ...
       !break the Omega_ExB definition
...
!                  &Lref/cref*1.0/(rhostar*minor_r)
  ...
  subroutine set_uprime
    !This routine calculates the flux surface average of parallel electron current in normalized units
    !Then it calculates its gradient in a global simulation
    !and it ouputs the value at the center of the simulation domain (uprime_x0)
    !which then can be used a posteriori in local/lilo simulations
    !It writes a file with flux surface average of the electron current and its gradient
    integer :: i, k, n, px0, ix0, ierr, UPRIME_FILE
    real, dimension(pi1gl:pi2gl,ln1:ln2) :: upar_arr_zavg
    real, dimension(pi1gl:pi2gl,0:n_spec-1) :: upar_arr_zavg_spec, uprime_arr_spec
    upar_arr_zavg = 0.0
    upar_arr_zavg_spec = 0.0
    uprime_arr_spec = 0.0
    ...
    if (x_local.or.lilo) then
      ...
      do n=ln1,ln2
        do k=lk1,lk2
          if (spec(n)%charge.eq.-1) then
            upar_arr(pi1gl:pi2gl,k,n)=jpar(pi1gl:pi2gl,k)*2*rhostar/(beta*spec(n)%dens_prof(pi1gl:pi2gl)*spec(n)%charge)
          endif
        enddo
      enddo

      ! Here we do a flux surface average of the upar_arr
      do n =ln1,ln2
        if (spec(n)%charge.eq.-1) then
          do k=lk1,lk2
            upar_arr_zavg(:,n) = upar_arr_zavg(:,n) + upar_arr(:,k,n)*geom%jacobian(:,pj1,k)
          enddo
          upar_arr_zavg(:,n) = upar_arr_zavg(:,n)/(geom%avg_jaco_yz(:)*nz0)
          call my_sum_to_all_real(upar_arr_zavg(:,n), pmx0, mpi_comm_z)
        endif
      enddo

      ! We calculate uprime arr
      do n = ln1,ln2
        if (spec(n)%charge.eq.-1) then
            CALL lag3deriv(upar_arr_zavg(:,n),xval_a,px0,uprime_arr(:,n),xval_a,px0)
        endif
      enddo

      ! We gather to the species communicator to write it in a file
      if ((my_pex+my_pey+my_pez+my_pev+my_pew).eq.0) then

        Call mpi_gather(upar_arr_zavg(pmi1gl,ln1), pmx0*ln0,MPI_REAL_TYPE,&
            upar_arr_zavg_spec(pmi1gl,0),pmx0*ln0,MPI_REAL_TYPE,&
               0,mpi_comm_spec,ierr)

        Call mpi_gather(uprime_arr(pmi1gl,ln1), pmx0*ln0,MPI_REAL_TYPE,&
            uprime_arr_spec(pmi1gl,0),pmx0*ln0,MPI_REAL_TYPE,&
               0,mpi_comm_spec,ierr)

      endif !my_pex, ...


      ! write the middle point to the parameters file
      do n=0,n_spec-1
        if (spec(n)%charge.eq.-1) then
          uprime_x0 = uprime_arr_spec(ix0,n)
        endif
      enddo

      if(mype.eq.0) then
        call get_unit_nr(UPRIME_FILE)
        open(UPRIME_FILE, file=trim(diagdir)//&
        &'/uprime_arr'//trim(file_extension),form = 'FORMATTED',action='write')
        WRITE(UPRIME_FILE,*) "Xval, U_par,e , U'_par_e"
        do i=pi1gl,pi2gl
           Write(UPRIME_FILE,"(3ES12.4)") xval_a(i), upar_arr_zavg_spec(i,1), uprime_arr_spec(i,1)
        enddo
        close(UPRIME_FILE)
      endif
  ...
  subroutine initialize_shear_flow
    ...
    if(ExB) then
       if (init_status_ExB==0) then
          if (x_local) allocate(kxExB(lh1:lh2))
          init_status_ExB=1
       endif
       if (x_local) kxExB=0.
    end if
