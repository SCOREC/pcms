...
int main(int argc, char **argv){
 ...
 //intialize GENE class
  ...
  std::cerr << "0.2, xgc_start= "<<xgc_numsurf->val(0)<< "\n";
 
  coupler::Array1d<double>* xgc_xcoords = coupler::receive_gene_pproc<double>(dir, xXcoord,MPI_COMM_WORLD);//x_XGC
  MPI_Barrier(MPI_COMM_WORLD);
  coupler::Array1d<double>* xgc_zcoords = coupler::receive_gene_pproc<double>(dir, xZcoord,MPI_COMM_WORLD);
...
//  coupler::Array1d<coupler::CV>* gene_moments = coupler::receive_gene_pproc<coupler::CV>(dir, gComp,MPI_COMM_WORLD);//matching gene's moments arr
