...
!!    to avoid the Ampere cancellation problem
  ...
  EXTERNAL erf
    ...
    DO n=ln1,ln2
       DO k=lk1,lk2
          DO j=lj1,lj2
            !mom00_flrcorr(j,k,n) = BandedMatrix(nCoeffs,nCoeffs,transposed)
            call mom00_flrcorr(j,k,n)%initialize(nCoeffs,nCoeffs,transposed)
             CALL mom00_flrcorr(j,k,n)%ALLOCATE()
             !mom01_flrcorr(j,k,n) = BandedMatrix(nCoeffs,nCoeffs,transposed)
             call mom01_flrcorr(j,k,n)%initialize(nCoeffs,nCoeffs,transposed)
             CALL mom01_flrcorr(j,k,n)%ALLOCATE()
             IF (n_fields.gt.1) then
               !gamma0_num_matrix(j,k,n) = BandedMatrix(nCoeffs,nCoeffs,transposed)
               call gamma0_num_matrix(j,k,n)%initialize(nCoeffs,nCoeffs,transposed)
                CALL gamma0_num_matrix(j,k,n)%ALLOCATE()
             end IF
             !mom20_flrcorr(j,k,n) = BandedMatrix(nCoeffs,nCoeffs,transposed)
             call mom20_flrcorr(j,k,n)%initialize(nCoeffs,nCoeffs,transposed)
             CALL mom20_flrcorr(j,k,n)%ALLOCATE()
          END DO
       END DO
    END DO
    ...
    !profilematrix = BandedMatrix(nCoeffs,nCoeffs,transposed)
    call profilematrix%initialize(nCoeffs,nCoeffs,transposed)
    call profilematrix%allocate()

    !profdotgyro = BandedMatrix(nCoeffs,nCoeffs,transposed)
    call profdotgyro%initialize(nCoeffs,nCoeffs,transposed)
    call profdotgyro%allocate()

    !CALL initialize(single_gyromatrix,nCoeffs,nCoeffs)
    !call allocate(single_gyromatrix)

    !gyromatrix2 = BandedMatrix(nCoeffs,nCoeffs,transposed)
    call gyromatrix2%initialize(nCoeffs,nCoeffs,transposed)
    CALL gyromatrix2%allocate()
    ...
    do k=lk1,lk2
       do j=lj1,lj2
          do n=ln1,ln2
             CALL mom00_flrcorr(j,k,n)%set_zero()
             CALL mom01_flrcorr(j,k,n)%set_zero()
             IF (n_fields.gt.1) CALL gamma0_num_matrix(j,k,n)%set_zero()
                ...
                do index=1,4
                   ...
                   if (index.ne.3) then
                      ...
                      DO i=my_pex*li0+1,(my_pex+1)*li0
                         ...
                         CALL profilematrix%set_value(i,i,tmpvar)
                         !diag_profilematrix(i) = tmpvar
                      ENDDO
                      call profilematrix%commit_values()
                      ...
                      CALL profilematrix%dot_multiply(gyromatrix(j,k,m,n),profdotgyro,'N')
                      ...
                      CALL gyromatrix_dagger(j,k,m,n)%dot_multiply(profdotgyro,gyromatrix2,'N')
                      ...
                      case (1)
                         DO i=my_pex*li0+1,(my_pex+1)*li0
                            CALL mom00_flrcorr(j,k,n)%row_axpy(i,gyromatrix2,cmplx(1.0,0.0))
                         ENDDO
                         CALL mom00_flrcorr(j,k,n)%commit_values()
                      case (2)
                         DO i=my_pex*li0+1,(my_pex+1)*li0
                            CALL mom01_flrcorr(j,k,n)%row_axpy(i,gyromatrix2,cmplx(1.0,0.0))
                         ENDDO
                         CALL mom01_flrcorr(j,k,n)%commit_values()
                      case (4)
                         DO i=my_pex*li0+1,(my_pex+1)*li0
                            CALL mom20_flrcorr(j,k,n)%row_axpy(i,gyromatrix2,cmplx(1.0,0.0))
                         ENDDO
                         CALL mom20_flrcorr(j,k,n)%commit_values()
                      end select
                   elseif (n_fields.gt.1) then
                      do l=ll1,ll2
                         ...
                         do i=my_pex*li0+1,(my_pex+1)*li0
                            CALL gamma0_num_matrix(j,k,n)%row_axpy(i,gyromatrix2,cmplx(1.0,0.0))
                         enddo
                         CALL gamma0_num_matrix(j,k,n)%commit_values()
                      enddo
                   endif
                enddo
             enddo
             ...
             DO m=lm1,lm2
                ...
                !CALL convert_Full_to_Banded(full_gyromatrix2,gyromatrix2)
                CALL gyromatrix(j,k,m,n)%dot_multiply(gyromatrix(j,k,m,n),gyromatrix2,G_op)
                ...
                DO i=my_pex*li0+1,(my_pex+1)*li0
                   ...
                   CALL mom00_flrcorr(j,k,n)%row_axpy(i,gyromatrix2,tmpvar)
                   CALL mom01_flrcorr(j,k,n)%row_axpy(i,gyromatrix2,&
                ...
                IF (n_fields.gt.1) THEN
                   ...
                   DO l=ll1,ll2
                      DO i=pi1,pi2
                         ...
                         CALL gamma0_num_matrix(j,k,n)%row_axpy(i-pi1gl+1,gyromatrix2,&
                              & tmpvar )
                      END DO
                   END DO ! l
             ...
             DO i=my_pex*li0+1,(my_pex+1)*li0
                ...
                CALL mom00_flrcorr(j,k,n)%add_value(i,i,&
                     & -spec(n)%charge * spec(n)%dens_prof(i-1+pi1gl)/&
                     & (spec(n)%temp*spec(n)%temp_prof(i-1+pi1gl)) * tmpfac)
                CALL mom01_flrcorr(j,k,n)%add_value(i,i,&
                     & -spec(n)%charge * spec(n)%dens_prof(i-1+pi1gl)/&
                     & (spec(n)%temp) * tmpfac)
#ifdef GDAGGER_G
                Call mom20_flrcorr(j,k,n)%add_value(i,i,&
                     & -0.5*spec(n)%charge * spec(n)%dens_prof(i-1+pi1gl)/&
                     & (spec(n)%temp) * tmpfac)
#else
                !in this case, mom20_flrcorr is simply Tp(x)/2*mom00_flrcorr
                DO ibase=1,nCoeffs
                   CALL mom20_flrcorr(j,k,n)%set_value(i,ibase,&
                        & 0.5*spec(n)%temp_prof(i-1) * &
                        & mom00_flrcorr(j,k,n)%mat_get_value(i,ibase) * tmpfac)
                END DO
#endif
             END DO
    ...
    !-------------------------- FINALIZATION -------------------------------
    !call profilematrix%finalize()
    !CALL profdotgyro%finalize()

    !CALL finalize(single_gyromatrix)
    !CALL finalize(full_gyromatrix2)
    !CALL gyromatrix2%finalize()
  ...
  SUBROUTINE finalize_flr_corr_moms
    ...
    DO n=ln1,ln2
       DO k=lk1,lk2
          DO j=lj1,lj2
             CALL mom00_flrcorr(j,k,n)%finalize()
             CALL mom01_flrcorr(j,k,n)%finalize()
             CALL mom20_flrcorr(j,k,n)%finalize()
             IF (n_fields.gt.1) CALL gamma0_num_matrix(j,k,n)%finalize()
          END DO
       END DO
    END DO
#endif
       ...
       DO n=ln1,ln2
          DO k=lk1,lk2
             DO j=lj1,lj2
                ...
                CALL mom00_flrcorr(j,k,n)%dot_multiply(tmpvec,resvec)

                CALL attach(resvec,corrections(li1:li2,j,k,n,2))
                CALL mom20_flrcorr(j,k,n)%dot_multiply(tmpvec,resvec)

                CALL attach(resvec,corrections(li1:li2,j,k,n,3))
                CALL mom01_flrcorr(j,k,n)%dot_multiply(tmpvec,resvec)
