...
Module fourier
  ...
  use omp_lib
#endif
  USE discretization, ONLY: ni0,nj0,li0,lj0,lij0,li1,li2,lj1,lj2,&
       xy_local,n_procs_x,n_procs_y,mype, my_pex, my_pey
  USE communications, ONLY: omp_num_threads,mpi_comm_x,mpi_comm_y,&
  ...
  Type(mkldescriptor),dimension(:),allocatable, private:: fft_ky2y_1d, fft_y2ky_1d
!#ifdef WITHOMP
...
Contains
  ...
  Subroutine initialize_fourier(a_li0da,a_ly0da)
    ...
    allocate(fft_ky2y(0:omp_num_threads-1),fft_y2ky(0:omp_num_threads-1))
    if (xy_local) allocate(fft_kx2x(0:omp_num_threads-1),fft_x2kx(0:omp_num_threads-1))
...
! note : y is the first coordinate
  ...
  Subroutine finalize_fourier
    INTEGER :: status, thrnum!,nthreads,thread_id

    !nthreads=omp_get_num_threads()
    !thread_id=omp_get_thread_num()

    !WRITE(*,"(2(A,I3),A)") "Finalizing MKL FFT on thread ",thread_id," of ",nthreads," threads."
    do thrnum=0,omp_num_threads-1
       !WRITE(*,"(2(A,I3))") "Free Descriptor for thrnum=",thrnum," of ",omp_num_threads
    ...
    If (xy_local) DEALLOCATE(fft_kx2x,fft_x2kx)
...
!----------------------------------------------------------------------
  ...
  !The one-dimensional FFT in y direction is only rarely needed
  Subroutine initialize_fourier_y_1d
    integer:: Status, thrnum

    stop 'fourier_y_1d not yet fully implemented in MKL interface'
...
! WITHOUT DEALIASING - ONLY FOR POST-PROCESSING
  Subroutine to_real_y_1d(inarr,outarr)
    ...
    ! Local variables
    COMPLEX, DIMENSION(nj0),target :: local_fullarray,local_Fourierarray
    INTEGER :: ierr, status, thrnum
    ...
    IF (n_procs_y.GT.1) THEN
       ...
       IF (my_pey.EQ.0) THEN
          ! do Fourier transform on process 0
          status=DftiComputeBackward(fft_ky2y_1d(thrnum)%desc,local_Fourierarray,local_fullarray)
          IF (Status /= 0) PRINT *, __LINE__, DftiErrorMessage(status)
       END IF
       ! now scatter the result array
       CALL MPI_Bcast(outarr(1),2*nj0,MPI_REAL_TYPE,0,mpi_comm_y,ierr)
    ELSE
       status=DftiComputeBackward(fft_ky2y_1d(thrnum)%desc,inarr, outarr)
       IF (Status /= 0) PRINT *, __LINE__, DftiErrorMessage(status)
    END IF
    PERFOFF
...
!----------------------------------------------------------------------
  !routines for y global boundary condition
  ...
  subroutine initialize_fourier_boundary
    ...
    do thrnum=0,omp_num_threads-1
       ! to real space in first dimension (not necessarily x-direction!!!)
       Status = DftiCreateDescriptor( fft_kx2x(thrnum)%desc, DFTI_SINGLE,&
            DFTI_COMPLEX, 1, ni0)
       If (Status /= 0) Print *, __LINE__, DftiErrorMessage(status)
       ...
       Status = DftiSetValue( fft_kx2x(thrnum)%desc, DFTI_NUMBER_OF_TRANSFORMS, lj0)
       If (Status /= 0) Print *, __LINE__, DftiErrorMessage(status)

       Status = DftiSetValue( fft_kx2x(thrnum)%desc, DFTI_INPUT_DISTANCE, ni0)
       If (Status /= 0) Print *, __LINE__, DftiErrorMessage(status)

       Status = DftiSetValue( fft_kx2x(thrnum)%desc, DFTI_OUTPUT_DISTANCE, ni0)
       If (Status /= 0) Print *, __LINE__, DftiErrorMessage(status)
       ...
       ! to fourier space in first dimension (not necessarily x-direction!!!)
       Status = DftiCreateDescriptor( fft_x2kx(thrnum)%desc, DFTI_SINGLE,&
            DFTI_COMPLEX, 1, ni0)
       If (Status /= 0) Print *, __LINE__, DftiErrorMessage(status)
       ...
       Status = DftiSetValue( fft_x2kx(thrnum)%desc, DFTI_NUMBER_OF_TRANSFORMS, lj0)
       If (Status /= 0) Print *, __LINE__, DftiErrorMessage(status)

       Status = DftiSetValue( fft_x2kx(thrnum)%desc, DFTI_INPUT_DISTANCE, ni0)
       If (Status /= 0) Print *, __LINE__, DftiErrorMessage(status)

       Status = DftiSetValue( fft_x2kx(thrnum)%desc, DFTI_OUTPUT_DISTANCE, ni0)
       If (Status /= 0) Print *, __LINE__, DftiErrorMessage(status)

       Status = DftiSetValue( fft_x2kx(thrnum)%desc, DFTI_FORWARD_SCALE, facbound)
       If (Status /= 0) Print *, __LINE__, DftiErrorMessage(status)
  ...
  Subroutine to_fourier_boundary(inarr,outarr)
    Complex, Dimension(0:lij0-1), intent(in) :: inarr
    Complex, Dimension(0:lij0-1), intent(out) :: outarr
  ...
  Subroutine to_real_boundary(inarr,outarr)
    Complex, Dimension(0:lij0-1) :: inarr
    Complex, Dimension(0:lij0-1), intent(out) :: outarr
