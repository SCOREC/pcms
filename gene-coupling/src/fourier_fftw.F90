...
Module fourier
  USE discretization, ONLY: ni0,nj0,li0,lj0,li1,li2,lj1,lj2,xy_local,&
       y_local,n_procs_x,n_procs_y,mype,my_pex,my_pey,yx_order
  USE communications, ONLY: mpi_comm_x,mpi_comm_y,&
       &MPI_COMPLEX_TYPE, MPI_REAL_TYPE
  USE par_in, ONLY: fourier2D
  USE par_poloidal_planes, only: res_fact

  Implicit None

  Include "fftw3.f"

  public ::  initialize_fourier,fft_kx_to_x,fft_ky_to_y,fft_ff_to_xy,&
       fft_ff_to_xy_t,fft_y_to_ky,fft_x_to_kx,fft_xy_to_ff,fft_xy_to_ff_t,&
       to_real_y,to_fourier_y,finalize_fourier,initialize_fourier_x_1d,&
       ...
       to_real_y_arb_nl,to_fourier_y_arb_nl,&
       to_real_y_arb_lin,to_fourier_y_arb_lin, &
       initialize_fourier_arb,finalize_fourier_arb
  ...
  Integer(Kind=8), save :: bplan_x_1d, fplan_x_1d, fplan_y_1d
  integer(kind=8), save :: fplan_y_arb, bplan_y_arb


  Real, save :: facnnx, facnny, facbound, facnny_arb
...
! note : y is the first coordinate
  ...
  Subroutine to_fourier_x_1d(inarr,outarr)
    ...
    IF (n_procs_x.GT.1) THEN
       ! first gather the array on one processor,
       ! as we do not have a parallelized Fourier transformation
...
! transform the 1-dimensional array inarr(kx) to outarr(x)
  ...
  SUBROUTINE initialize_fourier_y_1d
    COMPLEX, DIMENSION(nj0) :: tmpcmplx
    REAL, DIMENSION(2*nj0-2) :: tmpreal

    Call sfftw_plan_dft_c2r_1d(fplan_y_1d, 2*nj0-2, tmpcmplx, tmpreal, FFTW_BACKWARD, FFTW_PATIENT)

  End Subroutine initialize_fourier_y_1d


! transform the 1-dimensional array inarr(ky) to outarr(y)
! WITHOUT DEALIASING - ONLY FOR POST-PROCESSING
  Subroutine to_real_y_1d(inarr,outarr)
    COMPLEX, DIMENSION(lj1:lj2),intent(IN) :: inarr
    REAL, Dimension(2*nj0-2), intent(out) :: outarr

    ! Local variables
    COMPLEX, DIMENSION(nj0),target :: local_Fourierarray
    integer :: ierr

    PERFON('to_rey1d')
    IF (n_procs_y.GT.1) THEN
       CALL MPI_Gather(inarr(lj1),lj0,MPI_COMPLEX_TYPE,&
            & local_Fourierarray(1),lj0,MPI_COMPLEX_TYPE,0,mpi_comm_y,ierr)
       IF (my_pey.EQ.0) & ! do Fourier transform on process 0
            &CALL sfftw_execute_dft_c2r(fplan_y_1d,local_Fourierarray,outarr)
       ! now scatter the result array
       CALL MPI_Bcast(outarr(1),2*nj0,MPI_REAL_TYPE,0,mpi_comm_y,ierr)
    ELSE
       CALL sfftw_execute_dft_c2r(fplan_y_1d, inarr, outarr)
    END IF
    PERFOFF

  end Subroutine to_real_y_1d


  subroutine finalize_fourier_y_1d

    Call sfftw_destroy_plan(fplan_y_1d)

  end subroutine finalize_fourier_y_1d


  Subroutine initialize_fourier_arb
    Complex,dimension(:,:),allocatable:: tmpin
    Real,dimension(:,:),allocatable:: tmpout

    allocate(tmpin(res_fact*nj0+1,li0))
    allocate(tmpout(2*res_fact*nj0,li0))

    facnny_arb=1.0/Real(2*nj0*res_fact)

    !print*,"Initialize fourier with li0da and ly0da = ",li0da, ly0da
    Call sfftw_plan_many_dft_c2r(fplan_y_arb, 1, 2*nj0*res_fact, li0/n_procs_y,&
         tmpin, 0, 1, nj0*res_fact+1,&         !input
         tmpout, 0, 1, 2*nj0*res_fact,&        !output
         FFTW_PATIENT)

    Call sfftw_plan_many_dft_r2c(bplan_y_arb, 1, 2*nj0*res_fact, li0/n_procs_y,&
         tmpout, 0, 1, 2*nj0*res_fact,&         !input
         tmpin, 0, 1, nj0*res_fact+1,&        !output
         FFTW_PATIENT)

  end subroutine initialize_fourier_arb


  Subroutine to_real_y_arb_nl(inarr,outarr)
    Complex, Dimension(0:nj0-1, 0:li0/n_procs_y-1)  , intent(in) :: inarr
    Real,    Dimension(0:2*nj0*res_fact-1,0:li0/n_procs_y-1), intent(out) :: outarr
    Complex, Dimension(0:nj0*res_fact,0:li0/n_procs_y-1) :: tmparr

    tmparr(0:nj0-1,:)=inarr
    tmparr(nj0:,:)=cmplx(0.0,0.0)

    call sfftw_execute_dft_c2r(fplan_y_arb, tmparr(0,0), outarr(0,0))

  end Subroutine to_real_y_arb_nl


  Subroutine to_fourier_y_arb_nl(inarr,outarr)
    Real,    Dimension(0:2*nj0*res_fact-1,0:li0/n_procs_y-1), intent(in) :: inarr
    Complex, Dimension(0:nj0-1, 0:li0/n_procs_y-1), intent(out) :: outarr
    Complex, Dimension(0:nj0*res_fact, 0:li0/n_procs_y-1) :: tmparr

    call sfftw_execute_dft_r2c(bplan_y_arb, inarr(0,0), tmparr(0,0))
    outarr=tmparr(0:nj0-1,:)*facnny_arb

  end Subroutine to_fourier_y_arb_nl


  Subroutine to_real_y_arb_lin(inarr,outarr)
    Complex, Dimension(0:nj0-1,0:li0/n_procs_y-1)  , intent(in) :: inarr
    Real,    Dimension(0:2*nj0*res_fact-1,0:li0/n_procs_y-1), intent(out) :: outarr
    Complex, Dimension(0:nj0*res_fact,0:li0/n_procs_y-1) :: tmparr

    tmparr(0,:)=cmplx(0.0,0.0)
    tmparr(1:nj0,:)=inarr
    tmparr(nj0+1:,:)=cmplx(0.0,0.0)

    call sfftw_execute_dft_c2r(fplan_y_arb, tmparr(0,0), outarr(0,0))

  end Subroutine to_real_y_arb_lin


  Subroutine to_fourier_y_arb_lin(inarr,outarr)
    Real,    Dimension(0:2*nj0*res_fact-1,0:li0/n_procs_y-1), intent(in) :: inarr
    Complex, Dimension(0:nj0-1,0:li0/n_procs_y-1), intent(out) :: outarr
    Complex, Dimension(0:nj0*res_fact, 0:li0/n_procs_y-1) :: tmparr

    call sfftw_execute_dft_r2c(bplan_y_arb, inarr(0,0), tmparr(0,0))
    outarr=tmparr(1:nj0,:)*facnny_arb

  end Subroutine to_fourier_y_arb_lin


  subroutine finalize_fourier_arb

       Call sfftw_destroy_plan(fplan_y_arb)
       Call sfftw_destroy_plan(bplan_y_arb)

  end subroutine finalize_fourier_arb
