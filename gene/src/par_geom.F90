...
module par_geom
  ...
  public :: q0, major_R, major_Z, minor_r, trpeps, shat, amhd, rhostar
  public :: Bprof_coeffs
  public :: tracer_rt_scheme, set_par_geom_defaults
  public :: write_maps
  private
  ...
  Character(len=80)  :: x_def
  Real:: flux_pos

  ! used in tracer module
  Character(len=80)  :: tracer_rt_scheme !scheme for generation of toroidal flux grid
  !possible choices: 'q1D': 1d integration of q   (old default)
  !                  'F2D': 2d integration of F/R (more robust but slower, current default)
  logical :: write_maps=.false. !write EFIT maps to a file

  Real:: q0, major_R, major_Z, minor_r
  Real:: trpeps, rhostar
  ...
  TYPE geomtype
     ...
     real, dimension(:,:),allocatable:: R, Z, PHI, dxdR, dxdZ, THETA_pol
...
contains
  ...
  subroutine set_par_geom_defaults
    q0=-1111.0
    major_R=1.
    major_Z=0.
    minor_r=1.
    ...
    tracer_rt_scheme='F2D' !scheme for generation of toroidal flux grid
  end subroutine set_par_geom_defaults
  ...
  subroutine initialize_geomtype(geom,pi1gl,pi2gl,pg1gl,pg2gl,pi1,pi2,&
    ...
    allocate(geom%R(pi1gl:pi2gl,lk1:lk2), geom%Z(pi1gl:pi2gl,lk1:lk2), &
         ...
         & geom%THETA_pol(pi1gl:pi2gl,lk1:lk2))
