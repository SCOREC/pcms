...
Module diagnostics_svd
  ...
  use mpi
  use arrays
  use prefactors, only: set_prefactors
  use aux_fields, only: initialize_calc_aux_fields, finalize_calc_aux_fields
  use phys_ini, only: initialize_current_parall, finalize_current_parall
  use discretization_adptv_module, only: initialize_discretization_adptv, &
    finalize_discretization_adptv
  use sources_mod, only: initialize_g1_source, finalize_g1_source

  Implicit None

  PUBLIC :: svd_projection, integral_weight_serial, integral_unweight_serial
...
!!!!Arrays and Matrices
  ...
  integer :: fm_dbg,mat_00_dbg,jaco_dbg,avg_jaco_dbg
...
!!!******************************************************************!!!
  ...
  subroutine contains_invalid(df_in, abc)
    COMPLEX, INTENT(In) :: df_in(nx0*nz0*nv0*nw0*n_spec)
    LOGICAL, INTENT(out) :: abc
    COMPLEX :: ddd

    abc=.False.
    ddd=SUM(df_in)
    if (ddd/=ddd) abc=.True.
  end subroutine contains_invalid
  ...
  subroutine svd_projection
    ...
    if(svd_parallel) then
#ifdef WITHSCAL
       STOP "parallel svd projection not implemented for multiple svd"
       ...
       DO index=1,n_SVD_ind
          ...
          !call finalize_g_1
          !print*, "CHECK FOR TIME SCHEME"
          kx_center=(3.14159*2.0/lx_orig)*SVD_kx_ind(index)!check for negative ggw
          kymin=ky_min_orig*SVD_ky_ind(index)
          ...
          !Call init_g_1
          !curvature terms keep getting bigger! what's happening ggw?

          CALL svd_projection_serial(index)
       ENDDO
       kymin=ky_min_orig
       kx_center=0.0
       SVD_df_file_name=''
  ...
  subroutine svd_projection_serial(index)
    ...
    WRITE(kx_ind,'(i4.4)') ABS(SVD_kx_ind(index))
    WRITE(ky_ind,'(i4.4)') SVD_ky_ind(index)
    IF (SVD_kx_ind(index).GE.0) THEN
       PRINT*, "kx="//kx_ind
    ELSE
       PRINT*, "kx=-"//kx_ind
    ENDIF
!    PRINT*, "ky="//ky_ind
    ...
    IF((-1)**(NINT(SVD_ky_ind(index)*shat*lx*kymin)).GT.0) THEN
      SVD_pbc_phase=.TRUE.
      IF(mype==0) PRINT*, "Applying phase shift for parallel b.c."
    ELSE
      SVD_pbc_phase=.FALSE.
      IF(mype==0) PRINT*, "No phase shift for parallel b.c."
    END IF

    IF ((SVD_df_file_name.EQ.'no_input').OR.(n_svd_ind.NE.1)) THEN
      IF(SVD_kx_ind(index).GE.0) THEN
        SVD_df_file_name="df_ky"//ky_ind//"kx"//kx_ind !!GGW I think indices are redundant and unused
      ELSE
        SVD_df_file_name="df_ky"//ky_ind//"kxn"//kx_ind
      ENDIF
    ENDIF
    ...
    do rnum=1,SVD_n_restarts
       ...
       do while(i.lt.SVD_df_n_time(rnum))
          ...
          if(mod(i,SVD_sparse_factor)==0.and.i.ge.s_index(rnum).and.i0.lt.n_time) then
             i0=i0+1
             read(df_handle, IOSTAT=IOstatus) time_array(i0)
             IF (IOstatus .lt. 0) then
                GOTO 1
             endif
             read(df_handle, IOSTAT=IOstatus) df_matrix(:,i0)
             IF (IOstatus .lt. 0) then
                GOTO 1
             endif
          else
             read(df_handle, IOSTAT=IOstatus) dummy_r
             IF (IOstatus .lt. 0) then
                GOTO 1
             endif
             read(df_handle, IOSTAT=IOstatus) df2
             IF (IOstatus .lt. 0) then
                GOTO 1
             endif
          end if
...
1      close(df_handle)
    ...
    if(i0.ne.n_time) print*, "i0 does not equal n_time, probably reached EOF!"
    n_time=i0 !added in with mods
    n=i0 !added in with mods
    write(*,*) "Weighting the vectors so that scalar product = integral."
    !This gives more physical meaning to the POD procedure
    do i=1,n_time
       CALL contains_invalid(df_matrix(:,i),invalid)
       if (invalid) then
          PRINT*, "INVALID ELEMENT #", i
          df_matrix(:,i)=CMPLX(0.0)
       ELSE
