...
; ounit: returns only the unit in fancy style
  ...
  IF KEYWORD_SET(addspec) THEN BEGIN
     isp = (var_index/par.nrgcols)
     sp_str = spec[isp].name
     var_index = (var_index MOD par.nrgcols)
  ENDIF
  ...
  IF KEYWORD_SET(addspec) THEN v += ','+sp_str $
  ELSE sp_str = 'j'           ;as n, T and upar are normalized to current species!!!

  vu[0] = 'n!D'+sp_str+'0!N'+rho_str+'/'+Lref_str
  vu[1] = 'v!DT'+sp_str+'!N'+rho_str+'/'+Lref_str ;consider factor sqrt(2) below
  vu[2] = 'T!D'+sp_str+'0!N'+rho_str+'/'+Lref_str
  vu[3] = 'T!D'+sp_str+'0!N'+rho_str+'/'+Lref_str
  vu[0:3] = '('+vu[0:3]+')!U2!N'
  vu[1] += '/2' ; u_par is given in sqrt(T_0j/m_j) rho_ref/Lref
...
PRO plot_nrg_data, diag=diag, t_corr_in=t_corr_in, show_SI=show_SI, $
  show_sdev=show_sdev
  ...
  IF ((N_ELEMENTS(t_corr_in) LT 1) AND NOT par.nonlinear) THEN $
     t_corr_in = 0
  ...
  IF KEYWORD_SET(diag) THEN BEGIN
    ...
    ENDELSE
  ENDIF ELSE BEGIN
    step_start = (par.comp_type NE "'EV'") OR (par.comp_type NE 'EV')
  ...
  IF KEYWORD_SET(diag) THEN BEGIN
    ...
    sat_sdev_vals = DBLARR(n_spec*nrgcols)
    ...
    sat_sdev_vals2 = DBLARR(n_spec*4)

    request_err = t_corr_in NE 0

    ; compute correlation time self-consistently from all nrg columns
    ; unless no errors are to be computed, tcorr will have one value
    ; per nrg field and species
    ; set use_sparse_corrdata = 0 to use full nrg data for all fields
    use_sparse_corrdata = 1
    ; increase this value for more approximate but faster t_corr:
    relative_sparse_threshold = 0.05
    IF t_corr_in LT 0 THEN BEGIN
      corr_field = INDGEN(n_spec*nrgcols)
      n_corr_fields = N_ELEMENTS(corr_field)

      t_corr = FLTARR(n_corr_fields)

      nrg_time = (*nrg)[step_start:step_end].time

      FOR c = 0, n_corr_fields - 1 DO BEGIN
        nrg_data = (*nrg)[step_start:step_end].data[c]

        ; determine sparseness based on first field's t_corr
        IF use_sparse_corrdata * c GT 0 THEN BEGIN
          IF !QUIET NE 1 THEN PRINT, 'using sparse nrg data to compute t_corr'

          dt_avg = (nrg_time[step_range] - nrg_time[0]) / (step_range + 1.0)
          sparse_factor = FLOOR(relative_sparse_threshold*$
            t_corr_est_sparse/dt_avg)
          IF sparse_factor GT 1 THEN BEGIN
            n_steps_corr = (step_range + 1) / sparse_factor
            selinds = LINDGEN(n_steps_corr) * sparse_factor

            nrg_data_corr = nrg_data[selinds]
            nrg_time_corr = nrg_time[selinds]
          ENDIF ELSE BEGIN
            nrg_data_corr = nrg_data
            nrg_time_corr = nrg_time
            n_steps_corr = step_range + 1
          ENDELSE
        ENDIF ELSE BEGIN
          nrg_data_corr = nrg_data
          nrg_time_corr = nrg_time
        ...
        ; for correlation, use zero average of data
        nrg_data_corr -= REBIN([TOTAL(nrg_data_corr)/n_steps_corr],$
          n_steps_corr)

        corr = pf_arr([n_steps_corr])
        FOR t = 0L, n_steps_corr - 1L DO corr[t] = TOTAL(nrg_data_corr*$
        nrg_data_corr[(LINDGEN(n_steps_corr)+t) MOD n_steps_corr])
        corr /= REBIN([TOTAL(nrg_data_corr^2)],n_steps_corr)

        tind = (WHERE(corr[*] LE 0.36788))[0]
        IF tind GT 0 THEN BEGIN
          tind_eth = tind - (0.36788 - corr[tind]) / $
            (corr[tind-1] - corr[tind])
          dt = nrg_time_corr[tind] - nrg_time_corr[tind-1]

          IF !QUIET NE 1 THEN PRINT, 'nrg correlation time: ' + $
            rm0es(tind_eth*dt) + ' in time units for nrg/spec field ' + $
            rm0es(c)

          t_corr[c] = tind_eth * dt
        ENDIF ELSE IF !QUIET NE 1 THEN PRINT, $
          'nrg correlation time calculation failed, nrg/spec field ' + $
          rm0es(c)

        IF c EQ 0 THEN t_corr_est_sparse = t_corr[0]
      ENDFOR
    ENDIF ELSE $
      IF t_corr_in GE 0 THEN t_corr = REBIN([t_corr_in],n_spec*nrgcols)
    ...
    FOR spec_field = 0, n_spec * nrgcols - 1 DO BEGIN
      ...
      stat_avg_err, (*nrg)[fit_start:fit_end].time, $
        ...
        sat_val, sat_err_val, sat_sdev_val, $
        t_corr=(N_ELEMENTS(t_corr) GT 0 ? t_corr[spec_field] : undef)
      sat_vals[spec_field] = sat_val
      sat_err_vals[spec_field] = sat_err_val
      sat_sdev_vals[spec_field] = sat_sdev_val
    ...
    IF gr_min / gr_max GT 0.98 THEN $
      gr_str_tot = 'series growth rate: ' + rm0es(gr_vals[0]/2.0,prec=4) + $
      ' ' + get_var_string(-1,/time,/ounit) $
      ELSE gr_str_tot = 'series growth rate: not converged'
    IF (KEYWORD_SET(diag) AND (NOT par.nonlinear)) THEN print, rm_idl_fmt(gr_str_tot)
    gr_str_tot = '!6'+gr_str_tot
    ...
    FOR spec_field = 0, n_spec * nrgcols - 1 DO BEGIN
      ...
      IF (spec_field MOD nrgcols) EQ 4 THEN BEGIN ; G_es
        ...
        sat_sdev_vals2[counter] = spec[spec_nr].omn NE 0.0 ? $
          sat_sdev_vals[spec_field] * diffusion_factor : !VALUES.F_NAN
        counter += 1.0D
      ENDIF
      ...
      IF (spec_field MOD nrgcols) EQ 6 THEN BEGIN ; Q_es
        ...
        sat_err_vals2[counter] = spec[spec_nr].omt NE 0.0 ? $
          sat_err_vals[spec_field] * diffusion_factor : !VALUES.F_NAN
        sat_sdev_vals2[counter] = spec[spec_nr].omt NE 0.0 ? $
          sat_sdev_vals[spec_field] * diffusion_factor : !VALUES.F_NAN
        counter += 1.0D
      ENDIF
  ...
  IF KEYWORD_SET(diag) THEN BEGIN
    ...
    FOR page = 0, CEIL(1.0*n_spec/spp) - 1 DO BEGIN
      ...
      FOR j = 0, nrgcols - 1 DO BEGIN ; rows
        ...
        FOR isp = 0, nsp - 1 DO BEGIN ; columns
          errorout = sat_err_vals[j+nrgcols*(isp+page*spp)] NE 0 ? $
            '!9+!6' + rm0es(sat_err_vals[j+nrgcols*(isp+page*spp)],prec=2) : ''
          IF show_sdev AND sat_sdev_vals[j+nrgcols*(isp+page*spp)] NE 0 $
            THEN errorout += ' (!7r!6=' + $
            rm0es(sat_sdev_vals[j+nrgcols*(isp+page*spp)],prec=2) + ')'
      ...
      FOR j = nrgcols, nrgcols + 3 DO BEGIN ; D and chi values
        ...
        FOR isp = 0, nsp - 1 DO BEGIN
          errorout = sat_err_vals2[(j-nrgcols)+4*(isp+page*spp)] NE 0 ? $
            '!9+!6' + rm0es(sat_err_vals2[(j-nrgcols)+4*(isp+page*spp)],prec=2) : ''
          IF show_sdev AND sat_sdev_vals2[(j-nrgcols)+4*(isp+page*spp)] NE 0 $
            THEN errorout += ' (!7r!6=' + $
            rm0es(sat_sdev_vals2[(j-nrgcols)+4*(isp+page*spp)],prec=2) + ')'
    ...
    IF KEYWORD_SET(show_SI) THEN BEGIN ; --- SI fluxes and diffusivities ---
      ...
      IF par.x_local THEN BEGIN
        ...
        area_ref *= ABS((*series.geom).C_y) / par.nz0
      ENDIF ELSE BEGIN
        IF par.norm_flux_projection THEN $
         ...
         area_ref *= ABS((*series.geom).C_y[par.nx0/2]) / par.nz0
      ENDELSE
      ...
      si_units_str = ['n!Dj0!N','(T!Dj0!N/m!Dj!N)!U1/2!N','T!Dj0!N','T!Dj0!N',$
      ...
      FOR page = 0, CEIL(1.0*n_spec/spp) - 1 DO BEGIN
        ...
        FOR j = 0, 3 DO BEGIN ; rows
          ...
          FOR isp = 0, nsp - 1 DO BEGIN ; columns
            errorout = sat_err_vals[j+nrgcols*(isp+page*spp)] NE 0 ? $
              '!9+!6' + rm0es(SQRT(sat_err_vals[j+nrgcols*(isp+page*spp)])*$
              si_units_fac[j],prec=3) : ''
            IF show_sdev AND sat_sdev_vals[j+nrgcols*(isp+page*spp)] NE 0 $
              THEN errorout += ' (!7r!6=' + $
              rm0es(SQRT(sat_sdev_vals[j+nrgcols*(isp+page*spp)])*$
              si_units_fac[j],prec=3) + ')'
        ...
        FOR j = 4, nrgcols-1 DO BEGIN ; rows
          ...
          FOR isp = 0, nsp - 1 DO BEGIN ; columns
            ...
            errorout = sat_err_vals[nrg_j] NE 0 ? $
              '!9+!6' + rm0es(sat_err_vals[nrg_j]*si_units_fac[j],prec=2) : ''
            IF show_sdev AND sat_sdev_vals[nrg_j] NE 0 $
              THEN errorout += ' (!7r!6=' + $
              rm0es(sat_sdev_vals[nrg_j]*si_units_fac[j],prec=2) + ')'
        ...
        FOR j = nrgcols, nrgcols+nfluxes-1 DO BEGIN ; rows
          ...
          FOR isp = 0, nsp - 1 DO BEGIN ; columns
            ...
            errorout = sat_err_vals[nrg_j] NE 0 ? $
              '!9+!6' + rm0es(sat_err_vals[nrg_j]*si_units_fac[fl_j],prec=2) : ''
            IF show_sdev AND sat_sdev_vals[nrg_j] NE 0 $
              THEN errorout += ' (!7r!6=' + $
              rm0es(sat_sdev_vals[nrg_j]*si_units_fac[fl_j],prec=2) + ')'
        ...
        FOR j = nrgcols + nfluxes, nrgcols + nfluxes + 3 DO BEGIN
          ...
          FOR isp = 0, nsp - 1 DO BEGIN
            ...
            errorout = sat_err_vals2[nrg_j] NE 0 ? $
              '!9+!6' + rm0es(sat_err_vals2[nrg_j]*si_units_fac[diff_j],prec=2) : ''
            IF show_sdev AND sat_sdev_vals2[nrg_j] NE 0 $
              THEN errorout += ' (!7r!6=' + $
              rm0es(sat_sdev_vals2[nrg_j]*si_units_fac[diff_j],prec=2) + ')'
