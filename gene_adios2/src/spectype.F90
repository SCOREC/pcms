...
CONTAINS
  ...
  SUBROUTINE spec_to_spec(left,right)
    ...
    IF (ALLOCATED(right%temp_prof)) THEN
    ...
    ENDIF
    IF (ALLOCATED(right%dens_prof)) THEN
    ...
    ENDIF
    IF (ALLOCATED(right%omt_prof)) THEN
    ...
    ENDIF
    IF (ALLOCATED(right%omn_prof)) THEN
  ...
  SUBROUTINE PRINT(sp)
    ...
    write(*,"(A,G16.8)") "mass   = ", sp%mass
    write(*,"(A,G16.8)") "temp   = ", sp%temp
    WRITE(*,"(A,G16.8)") "dens   = ", sp%dens
    write(*,"(A,G16.8)") "omn    = ", sp%omn
    write(*,"(A,G16.8)") "omt    = ", sp%omt
    write(*,"(A,G16.8)") "omn_prof = ", sp%omn_prof
    write(*,"(A,G16.8)") "omt_prof = ", sp%omt_prof
    if ((sp%kappa_T.ne.-1.0).and.(sp%kappa_n.ne.-1.0)) Then
       Write(*,"(A,G16.8)") "kappa_T  = ", sp%kappa_T
       Write(*,"(A,G17.8)") "LT_center  = ", sp%LT_center
       Write(*,"(A,G16.8)") "LT_width  = ", sp%LT_width
       Write(*,"(A,G16.8)") "kappa_n  = ", sp%kappa_n
       Write(*,"(A,G16.8)") "Ln_center  = ", sp%Ln_center
       Write(*,"(A,G16.8)") "Ln_width  = ", sp%Ln_width
    end if
