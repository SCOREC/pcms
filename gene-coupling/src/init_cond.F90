...
!*************************************************************************!
  Subroutine XGC_like(aux_x,aux_y,aux_z,aux_amp)
    ...
    INTEGER:: i, j, k, n
    complex, dimension(li1:li2,0:nky0-1):: noise
    logical :: gauss_env, add_noise, use_ky0
    ...
    Do k = lk1,lk2
       Do j = lj1,lj2
          Do i = li1,li2
             ...
             if (gauss_env) dens(i,j,k) = dens(i,j,k) * exp(-(xvar/sigma_i)**2/2.0)
             if (add_noise) dens(i,j,k) = dens(i,j,k) + noise(i,j)
          End Do
       End Do
     End Do
