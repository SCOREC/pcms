                                           ...
                                           Omega_h::Int entity_dim)
{
  OMEGA_H_CHECK_PRINTF(
    entity_dim >= Omega_h::VERT && entity_dim <= mesh.dim(),
  ...
  if (dim == 2 && entity_dim == Omega_h::EDGE) {
    Omega_h::parallel_for(
      "entity_centroids_edge_2d", nents,
      OMEGA_H_LAMBDA(const Omega_h::LO ent) {
        const auto verts = Omega_h::gather_verts<2>(ent2verts, ent);
        const auto vert_coords = Omega_h::gather_vectors<2, 2>(mesh.coords(), verts);
    ...
    Omega_h::parallel_for(
      "entity_centroids_face_2d", nents,
      OMEGA_H_LAMBDA(const Omega_h::LO ent) {
        const auto verts = Omega_h::gather_verts<3>(ent2verts, ent);
        const auto vert_coords = Omega_h::gather_vectors<3, 2>(mesh.coords(), verts);
        const auto centroid = Omega_h::average(vert_coords);
        centroids[2 * ent + 0] = centroid[0];
        centroids[2 * ent + 1] = centroid[1];
      });
    ...
    Omega_h::parallel_for(
      "entity_centroids_edge_3d", nents,
      OMEGA_H_LAMBDA(const Omega_h::LO ent) {
        const auto verts = Omega_h::gather_verts<2>(ent2verts, ent);
        const auto vert_coords = Omega_h::gather_vectors<2, 3>(mesh.coords(), verts);
        const auto centroid = Omega_h::average(vert_coords);
    ...
    Omega_h::parallel_for(
      "entity_centroids_face_3d", nents,
      OMEGA_H_LAMBDA(const Omega_h::LO ent) {
        const auto verts = Omega_h::gather_verts<3>(ent2verts, ent);
        const auto vert_coords = Omega_h::gather_vectors<3, 3>(mesh.coords(), verts);
        const auto centroid = Omega_h::average(vert_coords);
        centroids[3 * ent + 0] = centroid[0];
        centroids[3 * ent + 1] = centroid[1];
        centroids[3 * ent + 2] = centroid[2];
    ...
    Omega_h::parallel_for(
      ...
      OMEGA_H_LAMBDA(const Omega_h::LO ent) {
        ...
        const auto vert_coords = Omega_h::gather_vectors<4, 3>(mesh.coords(), verts);
        const auto centroid = Omega_h::average(vert_coords);
        centroids[3 * ent + 0] = centroid[0];
        centroids[3 * ent + 1] = centroid[1];
        centroids[3 * ent + 2] = centroid[2];
