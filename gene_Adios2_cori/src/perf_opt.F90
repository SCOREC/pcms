...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine initialize_optimize_perf
    ...
    if ((collision_op.ne.'none').and.(collision_op.ne.'sugama')) then
       num_opts(4)=3
    else
       num_opts(4)=1
    endif
  ...
  subroutine optimize_parall_perf
    ...
    IF (auto_parall) THEN
       ...
       if (n_procs_s.gt.0) then
          ...
          do while ((nps2.gt.1).and.(mod(n_spec,nps2).ne.0))
             nps2=nps2-1
          enddo
       endif
       if (n_procs_v.gt.0) then
          ...
          do while ((npv2.gt.1).and.(mod(nv0,npv2).ne.0))
             npv2=npv2-1
          enddo
       endif
       if (n_procs_w.gt.0) then
          ...
          do while ((npw2.gt.1).and.(mod(nw0,npw2).ne.0))
             npw2=npw2-1
          enddo
       endif
       if (n_procs_x.gt.0) then
          ...
          if (x_local) then
             ...
             do while ((npx2.gt.1).and.(mod(nx0,npx2).ne.0))
                npx2=npx2-1
             enddo
          endif
       endif
       if (n_procs_y.gt.0) then
          ...
          if (nonlinear) then
             ...
             do while ((npy2.gt.1).and.(mod(nx0,2*npy2).ne.0).or.&
                  &(mod(nky0,npy2).ne.0))
                npy2=npy2-1
             enddo
          else
       ...
       if (n_procs_z.gt.0) then
          ...
          do while ((npz2.gt.1).and.(mod(nz0,npz2).ne.0))
             npz2=npz2-1
          enddo
       endif
       ...
       IF (count.eq.0) THEN
          ...
          IF (mype.eq.0) THEN
             ...
             WRITE(*,"(A)") "   s   v   w   x   y   z   total"
             WRITE(*,"(6(I4),I7)") nps2, npv2, npw2, npx2, npy2, npz2, &
                  nps2*npv2*npw2*npx2*npy2*npz2
          ENDIF
...
!         & optimal_nblocks,mintime
  ...
  function optimize_perf(time,fastest_first_perfvec_time,MFloprate_min, MFloprate_max, &
    ...
    !check whether perf_vec is fixed and valid for given parallelization/grid
    !need this to be treated separately since we still want that perf_vec to
    !be checked while do_optimization below would skip it
    perf_vec_fixed = ALL(perf_vec.gt.0)
    DO i_opt=1,dimopts
       perf_vec_fixed = perf_vec_fixed.AND.(perf_vec(i_opt).LE.num_opts(i_opt))
    ENDDO
    first_perfvec=.true.
    ...
    ALLOCATE(do_optimization(loc_dimopts))
    do_optimization = perf_vec_fixed

    IF (.NOT.perf_vec_fixed) THEN
       DO i_opt=1,loc_dimopts
    ...
    DO i_opt=1,loc_dimopts
       ...
       fastest_value = 1
       start_val = 1
       IF (count.GT.0) start_val = 2 !start_val=1 has been treated in prev. perf_vec
       IF (perf_vec_fixed) start_val = perf_vec(i_opt)

       DO i_val=start_val,num_opts(i_opt)
          perf_vec(i_opt) = i_val
