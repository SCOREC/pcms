...
Module calc_rhs
  ...
  use communications, only: reduce_per_thread, threadlocal_mpi_comm_y, my_secnds,debug_output
...
Contains
  ...
  Subroutine CalFullRhs(p_g_,rhs,stage)
    ...
    DEBUG_OUTPUT("p_g_ = ",p_g_)
    !if (OUTPUT) then
    !  call calculate_test_sum(p_g_,local_sum, global_sum)
    !   IF (mype.EQ.0) WRITE(*,"(A,ES20.12)") "p_g_ = ",global_sum
    ! end if
    ...
    if (rhs_only_coll) then
       ...
       call calc_aux_fields(p_g_,emfields,f_,comp_h=(arakawa_zv.or.coll_on_h),h_out=h_)
       call calc_rhs_only(f_, p_g_, emfields, rhs, stage, h_)
    endif

    DEBUG_OUTPUT("rhs = ",rhs)
    DEBUG_OUTPUT("phi = ",emfields(:,:,lk1:lk2,1))
    DEBUG_OUTPUT("f_ = ",f_(:,:,lk1:lk2,ll1:ll2,lm1:lm2,ln1:ln2))

    !if (OUTPUT) then
    !   call calculate_test_sum(rhs,local_sum, global_sum)
    !   IF (mype.EQ.0) WRITE(*,"(A,ES20.12)") "rhs = ",global_sum
    !   call calculate_test_sum(emfields(:,:,lk1:lk2,1),local_sum, global_sum)
  ...
  Subroutine calc_rhs_only (p_f_, p_g_, p_emfields, a_rhs, stage, p_h_)
    ...
    do iblock=1,nblocks
       ...
       if (with_dfdperp) then
          ...
          DEBUG_OUTPUT("after dfdxy: rhs_block = ",rhs_block)
       endif
       ...
       PERFOFF
       DEBUG_OUTPUT("after dchidxy: rhs_block = ",rhs_block)

       if (.not.arakawa_zv) then
         PERFON('dchidz')
         call add_dchidz(p_emfields, bar_emfields, rhs_block, lbg1, lbg2)
         PERFOFF
         DEBUG_OUTPUT("after dchidz: rhs_block = ",rhs_block)
       end if
       ...
       PERFOFF
       DEBUG_OUTPUT("after dgdxy: rhs_block = ",rhs_block)
       ...
       if (rhs_nl) then
          if (nonlinear) then !perpendicular nonlinearity
            PERFON('add_nl')
             ...
             PERFOFF
             DEBUG_OUTPUT("after nonlin: rhs_block = ",rhs_block)
          endif
          if (parallel_nl) then
             PERFON('paral_nl')
             ...
             PERFOFF
             DEBUG_OUTPUT("after parallel nonlin: rhs_block = ",rhs_block)
          end if
       endif
       ...
       if (with_sources.and.(.not.precond_approx)) then
          ...
          PERFOFF
          DEBUG_OUTPUT("after sources: rhs_block = ",rhs_block)
       end if
