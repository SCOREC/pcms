...
PRO phases_output, diag
  ...
  sumhist = REFORM(time_avg((*i).sumhist_id,/avg,fwd=gui.out.res_steps,$
    tarr=time),[(*i).nbins+1,nkyind,n_corrs,gui.out.n_spec_sel,$
    gui.out.res_steps*(series.step_count-1)+1])
    ...
    FOR n = 0, gui.out.res_steps * (series.step_count - 1) DO BEGIN
      FOR page = 0, n_corrs / cpp + ((n_corrs MOD cpp) GT 0) - 1 DO BEGIN
        FOR cv = page * cpp, ((page + 1) * cpp - 1) < (n_corrs - 1) DO BEGIN
          var1 = (*i).corr[0,cv]
          var2 = (*i).corr[1,cv]
          maxz = MAX(sumhist[*,*,cv,isp,n])

          IF nkyind GT 1 THEN BEGIN
            SURFACE, sumhist[*,*,cv,isp,n], winkind, (*series.ky)[(*i).kyind], $
          ...
          ENDIF ELSE BEGIN
            PLOT, winkind, sumhist[*,0,cv,isp,n], XRANGE=[-!PI,!PI], /XSTYLE, $
