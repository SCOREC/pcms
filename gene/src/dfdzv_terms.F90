...
!***********************************************************************************************
  ...
  subroutine initialize_dfdzv_1
    integer:: i,j,k,l,m,n,sten,ijklmn_f,klmn, pii, stensign=1
    ...
    do n=ln1,ln2
       do m=lm1,lm2 
          ...
          enddo
          do l=ll1,ll2
             if (parscheme(1:1).eq.'u') stensign = sign(1.0,vp(l))
             do k=lk1,lk2
...
#endif
                            do sten=-par_sten_bound,par_sten_bound
                               dfdz_pref3(ijklmn_f,sten)=pdf1dz_1(pii)*stensign*par_sten(stensign*sten)
                            end do
#ifndef oldparbc
                         endif
#endif
                         ...
                         if (shifted_metric) then
                            ...
                            do sten=-par_sten_bound,par_sten_bound
                               dfdz_pref3_shm(ijklmn_f,sten)=pdf1dz_1(pii)*&
                                    &stensign*par_sten(stensign*sten)*geom%phasefac(pii,j,k,sten)
                            enddo
                            do sten=-hyp_z_order/2,hyp_z_order/2
                            ...
                            do sten=-par_sten_bound,par_sten_bound
                               dfdz_pref3(ijklmn_f,sten)=pdf1dz_1(pii)*&
                                    &stensign*par_sten(stensign*sten)
                            enddo
                            do sten=-hyp_z_order/2,hyp_z_order/2
                               dfdz_pref3(ijklmn_f,sten)=dfdz_pref3(ijklmn_f,&
...
!***********************************************************************************************
  ...
  subroutine initialize_dfdzv_2
    integer:: i,j,k,l,m,n,sten,klmn,stensign=1
    real, dimension(pi1:pi2) :: pdf1dz_1
    real, dimension(pi1:pi2,pj1:pj2,lk1:lk2) :: trp
    real:: Erad_term 
                ...
                do k=lk1,lk2
                   ...
                   do j=lj1,lj2
                      do i=pi1,pi2
                         ...
                         do sten=-par_sten_bound,par_sten_bound
                            df_pref_shm(i,j,sten,klmn)=df_pref_shm(i,j,sten,klmn)+&
                                 &pdf1dz_1(i)*stensign*par_sten(stensign*sten)*geom%phasefac(i,j,k,sten)
                         enddo
                         !add parallel damping
                         do sten=-hyp_z_order/2,hyp_z_order/2
                            df_pref_shm(i,j,sten,klmn)=df_pref_shm(i,j,sten,klmn)+&
                      ...
                      do i=pi1,pi2
                         ...
                         do sten=-par_sten_bound,par_sten_bound
                            df_pref(i,j,sten,klmn)=df_pref(i,j,sten,klmn)+&
                                 & pdf1dz_1(i)*stensign*par_sten(stensign*sten)
                         enddo
                         !add parallel damping
                         do sten=-hyp_z_order/2,hyp_z_order/2
                      ...
                      do i=pi1,pi2
                         ...
                         do sten=-par_sten_bound,par_sten_bound
                            df_pref_shm(i,j,klmn,sten)= & 
                                 ...
                                 & stensign*par_sten(stensign*sten)*geom%phasefac(i,j,k,sten)
                         enddo
                         !add parallel damping
                         do sten=-hyp_z_order/2,hyp_z_order/2
                            df_pref_shm(i,j,klmn,sten)=df_pref_shm(i,j,klmn,sten)+&
                ...
                do k=lk1,lk2                    
                   ...
                   do j=pj1,pj2
                      do i=pi1,pi2
                         ...
                         !parallel dynamics
                         do sten=-par_sten_bound,par_sten_bound
                            df_pref(i,j,klmn,sten)=df_pref(i,j,klmn,sten)+pdf1dz_1(i)*stensign*par_sten(stensign*sten)
                         enddo
                         !add parallel damping
                         do sten=-hyp_z_order/2,hyp_z_order/2
