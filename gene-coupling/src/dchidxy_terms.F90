...
module dchidxy_terms
  ...
  use x_derivatives, only: x_deriv_exc, x_deriv
  ...
  interface add_dchidxy
    module procedure add_dchidxy_df,add_dchidxy_ff,add_dchidxy_ff_6D
    module procedure add_dchidxy_df_1_orig
  end interface add_dchidxy
...
contains
  ...
  subroutine add_dchidxy_df(chi_block,p_rhs,p_dbarchidxy,lb1,lb2,with_curv)
    ...
    if (xy_local) then
      print*,"add_dchidxy_df is only implemented for non-local runs"
      stop
    else
       call add_dchidxy_df_1(chi_block,p_rhs,p_dbarchidxy,lb1,lb2,with_curv)
    endif

  end subroutine add_dchidxy_df

  subroutine add_dchidxy_ff_6D(p_emfields,p_rhs,p_barchi,lb1,lb2,with_curv)
    integer,intent(in) :: lb1,lb2
    complex, dimension(li1:li2, lj1:lj2, lbz:ubz, 1:n_fields), intent(in):: p_emfields
    complex, dimension(li1:li2, lj1:lj2, lk1:lk2, ll1:ll2, lm1:lm2, ln1:ln2), intent(inout):: p_rhs
    complex, dimension(:,:,:), pointer:: p_barchi
    logical, intent(in) :: with_curv

    call add_dchidxy_ff(p_emfields,p_rhs,p_barchi,lb1,lb2,with_curv)
  end subroutine add_dchidxy_ff_6D

  subroutine add_dchidxy_ff(p_emfields,p_rhs,p_barchi,lb1,lb2,with_curv)
    integer,intent(in) :: lb1,lb2
    complex, dimension(li1:li2, lj1:lj2, lbz:ubz, 1:n_fields), intent(in):: p_emfields
    ...
    if (xy_local) then
      ...
      print*,"add_dchidxy_ff is only implemented for local runs."
      stop
    endif

  end subroutine add_dchidxy_ff
  ...
  subroutine add_dchidxy_1(p_emfields,p_rhs,p_barchi,lb1,lb2,with_curv)
    ...
    do klmn=lb1,lb2
       ...
       if (.not.associated(p_barchi)) then
          ...
          if ((with_omt_ext).or.(with_omn_ext).or.(with_kh_drive_ext)) then
            if (with_omt_ext) call add_external_contr(p_rhs(:,:,klmn),1,&
              &p_bar_chi_3d(:,:,sk(klmn)),kxind_omt_ext,sk(klmn),&
              &sl(klmn),sm(klmn),sn(klmn))
            if (with_omn_ext) call add_external_contr(p_rhs(:,:,klmn),2,&
              &p_bar_chi_3d(:,:,sk(klmn)),kxind_omn_ext,sk(klmn),&
              &sl(klmn),sm(klmn),sn(klmn))
            if (with_kh_drive_ext) call add_external_contr(p_rhs(:,:,klmn),4,&
              &p_bar_chi_3d(:,:,sk(klmn)),kxind_phi_ext,sk(klmn),&
              &sl(klmn),sm(klmn),sn(klmn))
          endif
       else
          klmn1 = klmn-lb1+1
          ...
          if ((with_omt_ext).or.(with_omn_ext).or.(with_kh_drive_ext)) then
            if (with_omt_ext) call add_external_contr(p_rhs(:,:,klmn),1,&
              &p_barchi(:,:,klmn1),kxind_omt_ext,sk(klmn),&
              &sl(klmn),sm(klmn),sn(klmn))
            if (with_omn_ext) call add_external_contr(p_rhs(:,:,klmn),2,&
              &p_barchi(:,:,klmn1),kxind_omn_ext,sk(klmn),&
              &sl(klmn),sm(klmn),sn(klmn))
            if (with_kh_drive_ext) call add_external_contr(p_rhs(:,:,klmn),4,&
              &p_barchi(:,:,klmn1),kxind_phi_ext,sk(klmn),&
              &sl(klmn),sm(klmn),sn(klmn))
          endif
       endif
    enddo
  ...
  subroutine add_dchidxy_2(p_emfields,p_rhs,lb1,lb2,p_chi)
    ...
    if ((with_omt_ext).or.(with_omn_ext).or.(with_kh_drive_ext)) then
       do klmn=lb1,lb2
          ...
          if (with_kh_drive_ext) call add_external_contr(&
               &p_rhs(li1:li2,lj1:lj2,klmn),4,tmparr,&
               &kxind_phi_ext,sk(klmn),sl(klmn),sm(klmn),sn(klmn))
       enddo
    endif
...
!            &) then
  ...
  subroutine add_dchidxy_df_1_orig(p_bar_emfields,p_rhs,p_barchi,p_dbarchidxy,lb1,lb2,with_curv)
    integer, intent(in) :: lb1,lb2
    complex, dimension(li1:li2,lj1:lj2,lbz:ubz,lm1:lm2,ln1:ln2,1:n_fields), intent(IN) :: p_bar_emfields
    complex, dimension(li1:li2, lj1:lj2, lb1:lb2), intent(inout):: p_rhs
    complex, dimension(:,:,:), pointer :: p_barchi
    complex, dimension(:,:,:,:), pointer :: p_dbarchidxy
    logical, intent(in) :: with_curv
