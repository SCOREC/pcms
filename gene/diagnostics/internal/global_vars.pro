PRO global_vars
  ...
  COMMON global_vars, cfg, gui, series, par, spec, nrg, mom, mom_time, $
    ...
    nlp_gdt, nlp_kxind, nlp_kyind, triplet_time, triplet_data, cl_mode
  ...
  show_global = TOTAL(GETENV('USER') EQ ['global','dtold','flm','fsj','lapillon',$
    'mjpuesch','sbrunner','tbg','tisd','xal','merlo','hkd','tbird','pax','davidhat',$
    'mobp']) GT 0 ? 1 : 0
...
;      diags        : 0L,$
  ...
  par = {$ ; these variables contain values from the parameter file
    ...
    sign_Ip_CW     : 1,$
    sign_Bt_CW     : 1,$
    sign_Omega_CW  : 0,$ ;only used if -1/1
    diag_trap_levels : 0,$
  ...
  triplet_time = PTR_NEW()

  cdiag0 = PTR_NEW()

  vm_sav = 0 ; by default, no virtual machine sav support
  cl_mode = 0 ; switch for command line mode
