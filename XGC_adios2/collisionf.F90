...
#if defined(_OPENMP) && !defined(COL_MIRA_ION)
...
!$omp end critical
  ...
  deltayi = f0_dsmu*vth
  deltaxi = f0_dvp*vth                  ! parallel spacing 
  ...
  deltaye = f0_dsmu*vth
  deltaxe = f0_dvp*vth                  ! parallel spacing 
...
#if defined(_OPENMP) && !defined(COL_MIRA_ELEC)
  isize = min(inode2-inode1+1,size(col_f_mat_list),nthreads)
  if (f0_grid_load_balance) then
     ! use wrap map instead of block map to improve intra-node load balance
     do ith=1,isize
...
subroutine col_f_lambda_gamma(den,ti_ev,te_ev, massi, masse, gammac)
  ...
  if( sml_electron_on ) then
     ...
     if(ti_ev * masse/massi .lt. te_ev) then
        ...
        lambda(2) = 3D1 - log(sqrt(den*1D-6/(ti_ev**3))*(sml_prot_mass/massi))
     endif
...
#include <petsc/finclude/petscpc.h>
  ...
  integer :: index_I, index_J, iter_inter
...
!$  ithread = omp_get_thread_num()
    ...
    ! M_ie --> 1: M_rr, 2:M_rz, 3:M_zz, 4:M_ra, 5:M_za
    ! M_i -->1: M_rr, 2:M_rz, 3:M_zz, 4:M_ra
