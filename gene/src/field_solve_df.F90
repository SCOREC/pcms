...
MODULE fieldsolver_df
  ...
  use par_in, only: trap_pass, debye2, beta, tau, rad_bc_type, &
       &spec, del_phi, only_Er, delzonal, add_zonal_phi,no_electron_response
  ...
  USE flr_corr, ONLY: initialize_flr_corr_moms, finalize_flr_corr_moms, get_mom00_flrcorr,&
       &get_gamma0_num_matrix
...
CONTAINS
  ...
  SUBROUTINE field_solve_df(moments,a_emfields)
    ...
    If (island_contrib) call add_Apar_island_nonlocal(a_emfields)
  ...
  SUBROUTINE solve_fieldequations(mat, rhs, sol)
    ...
    DO k=lk1,lk2
       DO j=lj1,lj2
          CALL rhsvec%attach(rhs(:,j,k))
          CALL mat(j,k)%LU_solve(rhsvec,solvec,1)
       END DO
    END DO
    DO k=lk1,lk2
       DO j=lj1,lj2
          CALL mat(j,k)%LU_solve(rhsvec,solvec,2)
       END DO
    END DO
    DO k=lk1,lk2
       DO j=lj1,lj2
          CALL solvec%attach(sol(li1:li2,j,k))
          CALL mat(j,k)%LU_solve(rhsvec,solvec,3)
       END DO
    END DO
  ...
  SUBROUTINE initialize_field_solve_df
    ...
    CALL rhsvec%initialize(ni0)
    CALL solvec%initialize(ni0)
    ...
    DO k=lk1,lk2
       DO j=lj1,lj2
          CALL inv_qn_matrix(j,k)%initialize(nCoeffs,nCoeffs)
          IF (n_fields.GT.1) &
             & CALL inv_amp_matrix(j,k)%initialize(nCoeffs,nCoeffs)
       END DO
    END DO
    ...
    DO iDeriv=1,nDeriv
      !derivmat(iDeriv) = DerivativeMatrix(xgrid,derivative_order,transposed)
      call derivmat(iDeriv)%initialize(xgrid,derivative_order,transposed)
       IF (rad_bc_type.EQ.2) THEN
          CALL derivmat(iDeriv)%calculate(iDeriv,1)
          !derivmat_vN(iDeriv) = DerivativeMatrix(xgrid,derivative_order,transposed)
          call derivmat_vN(iDeriv)%initialize(xgrid,derivative_order,transposed)
          CALL derivmat_vN(iDeriv)%calculate(iDeriv,rad_bc_type)
       ELSE
          CALL derivmat(iDeriv)%calculate(iDeriv,rad_bc_type)
       ENDIF
    END DO

    CALL calculate_premats

    IF (island_contrib) call initialize_island
...
!    INTEGER :: j,k,ideriv
    ...
    !call rhsvec%finalize()
    !call solvec%finalize()
    ...
    DO k=lk1,lk2
       DO j=lj1,lj2
          CALL inv_qn_matrix(j,k)%finalize()
          IF (n_fields.GT.1) CALL inv_amp_matrix(j,k)%finalize()
       END DO
    END DO
#endif
    ...
    IF (island_contrib)  call finalize_island
...
!    Integer:: ierr
    ...
    CALL temparr%initialize(nCoeffs,nCoeffs)
    call temparr%allocate()


    IF ((n_fields.GT.1).OR.(debye2.GT.0)) THEN  !build laplacian_perp matrix
       CALL laplacian_perp%initialize(nCoeffs,nCoeffs)
       CALL laplacian_perp%ALLOCATE()
    END IF


    CALL p_doubleprime%initialize(nCoeffs,nCoeffs)
    CALL p_doubleprime%ALLOCATE()
    ...
    DO k=lk1,lk2
       DO j=lj1,lj2
          ...
          CALL p_doubleprime%set_zero()

          !build laplacian_perp matrix, if necessary
          IF ((n_fields.GT.1).OR.(debye2.GT.0)) THEN
             CALL laplacian_perp%set_zero()
             DO i=my_pex*li0+1,(my_pex+1)*li0
                DO ibase=1,nCoeffs
                   ...
                   IF ((rad_bc_type.EQ.2).AND.p_has_0_mode.AND.(j==lj1)) THEN
                      CALL laplacian_perp%add_value(i,ibase,&
                           &gii*derivmat_vN(2)%mat_get_value(i,ibase) &
                           & + 2.0*gij&
                           &   * derivmat_vN(1)%mat_get_value(i,ibase)*imag*kj(j))
                   ELSE
                      CALL laplacian_perp%add_value(i,ibase,&
                           &gii*derivmat(2)%mat_get_value(i,ibase) &
                           & + 2.0*gij &
                           &   * derivmat(1)%mat_get_value(i,ibase)*imag*kj(j))
                   ENDIF
#else
                   ...
                   IF ((rad_bc_type.EQ.2).AND.p_has_0_mode.AND.(j==lj1)) THEN
                      CALL laplacian_perp%add_value(i,ibase,&
                           &geom%gii(i-1+pi1gl,pj1,k)*derivmat_vN(2)%mat_get_value(i,ibase) &
                           & + 2.0*geom%gij(i-1+pi1gl,pj1,k) &
                           &   * derivmat_vN(1)%mat_get_value(i,ibase)*imag*kj(j))
                   ELSE
                      CALL laplacian_perp%add_value(i,ibase,&
                           &geom%gii(i-1+pi1gl,pj1,k)*derivmat(2)%mat_get_value(i,ibase) &
                           & + 2.0*geom%gij(i-1+pi1gl,pj1,k) &
                           &   * derivmat(1)%mat_get_value(i,ibase)*imag*kj(j))
                   ENDIF
#endif
                ...
                END DO
                call laplacian_perp%commit_values()
                CALL laplacian_perp%add_value(i,i,&
          ...
          DO n=ln1,ln2
             ...
             DO i=my_pex*li0+1,(my_pex+1)*li0
                DO ibase=1,nCoeffs
                   CALL p_doubleprime%add_value(i,ibase,&
                        & -spec(n)%charge * spec(n)%dens * &
                        & mom00_flrcorr(j,k,n)%mat_get_value(i,ibase))
                END DO
             ENDDO
             call p_doubleprime%commit_values()
          END DO
          ! parallel sum over the species
          CALL my_sum(p_doubleprime,mpi_comm_spec)

          IF (debye2.GT.0) THEN
             CALL laplacian_perp%multiply_matrix_with_scalar(debye2,temparr)
             CALL p_doubleprime%subtract_matrix(temparr)
          END IF
                ...
                IF (p_has_0_mode.AND.(j.EQ.lj1)) THEN
                   ...
                   DO icol=1,nCoeffs
                      DO irow=my_pex*li0+1,(my_pex+1)*li0
                         ...
                         !! average)
#ifdef FLMSLV
                         !Alternative flux surface approximation as suggested by F. Merz in svn rev2820/2829
                         CALL inv_p_doubleprime_ions_m%add_value(irow,icol,&
                              &p_doubleprime%mat_get_value(irow,icol) &
...
#else
                         !Old approximation (i.e., <LHS of Poisson Eq.>_FS ~ <(1-Gamma_0)>_FS <phi>_FS)
                         !again in use since b95f514d5632 since it agrees much better with the exact
                         !solution being implemented in the local version in rev4356.
                         !The differences are small/negligible in case of
                         !circular geometries and presumably for stellarators.
                         CALL inv_p_doubleprime_ions_m%add_value(irow,icol,&
                              &p_doubleprime%mat_get_value(irow,icol)*geom%jacobian(irow-1+pi1gl,pj1,k)&
                              /(nz0*geom%avg_jaco_yz(irow-1+pi1gl)))
#endif
                      END DO
                   END DO
                   call inv_p_doubleprime_ions_m%commit_values()
                END IF
             ELSE
                DO icol=1,nCoeffs
                   DO irow=my_pex*li0+1,(my_pex+1)*li0
          ...
          CALL inv_qn_matrix(j,k)%allocate()
          inv_qn_matrix(j,k) = p_doubleprime

          IF (adiabatic_electrons) THEN
             DO i=1,nCoeffs
                CALL inv_qn_matrix(j,k)%add_value(i,i,&
          ...
          ELSE IF (trap_pass) THEN
             ...
             END DO
          ELSE IF (no_electron_response) THEN
             !add nothing ...
          ELSE
             ...
             ENDIF
          END IF
          call inv_qn_matrix(j,k)%commit_values()
          ...
          IF (p_has_0_mode.AND.periodic_boundary.AND.(j.EQ.lj1)) THEN
             DO ibase=1,nCoeffs
                CALL inv_qn_matrix(lj1,k)%set_value(1,ibase,1.0)
             END DO
             CALL inv_qn_matrix(j,k)%commit_values()
          END IF

          call inv_qn_matrix(j,k)%LU_factor()

          ! the same procedure for the matrix for Ampere's law
          IF (n_fields.GT.1) THEN
             CALL inv_amp_matrix(j,k)%allocate()
             CALL inv_amp_matrix(j,k)%set_zero()

             DO n=ln1,ln2
                DO i=my_pex*li0+1,(my_pex+1)*li0
                   DO ibase=1,nCoeffs
                      CALL inv_amp_matrix(j,k)%add_value(i,ibase,&
                           &gamma0_num_matrix(j,k,n)%mat_get_value(i,ibase))
                   END DO
                END DO
                CALL inv_amp_matrix(j,k)%commit_values()
             END DO

             CALL inv_amp_matrix(j,k)%multiply_matrix_with_scalar(beta)
             CALL my_sum(inv_amp_matrix(j,k),mpi_comm_spec)

             CALL inv_amp_matrix(j,k)%subtract_matrix(laplacian_perp)

             IF ((periodic_boundary).AND.(p_has_0_mode).AND.(j.EQ.lj1)) THEN
                DO ibase=1,nCoeffs
                   CALL inv_amp_matrix(lj1,k)%set_value(1,ibase,1.0)
                END DO
             END IF

             CALL inv_amp_matrix(j,k)%LU_factor()
    ...
    IF ((n_fields.GT.1).OR.(debye2.GT.0)) THEN
       CALL laplacian_perp%finalize()
    END IF

    call temparr%finalize()
    call p_doubleprime%finalize()
       ...
       IF (.NOT.x_local) THEN
          IF (p_has_0_mode) THEN
             ...
             IF (periodic_boundary) THEN
                DO ibase=1,nCoeffs
                   CALL inv_p_doubleprime_ions_m%set_value(1,ibase,1.0)
                END DO
             END IF
             call inv_p_doubleprime_ions_m%invert()
          END IF
       ELSE IF (.not.y_local) THEN
...
!------------------------------------------------------------------------------------
  ...
  SUBROUTINE output_local_sum_jkn_banded(bmat,label)
    ...
    DO n=ln1,ln2
       DO k=lk1,lk2
          DO j=lj1,lj2
             local_sum = local_sum + bmat(j,k,n)%get_local_abs_square_sum()
          END DO
       END DO
    END DO
  ...
  SUBROUTINE output_local_sum_jkn_full(mat,label)
    ...
    DO n=ln1,ln2
       DO k=lk1,lk2
          DO j=lj1,lj2
             local_sum = local_sum + mat(j,k,n)%get_local_abs_square_sum()
          END DO
       END DO
    END DO
    WRITE(*,"(I3,3A,ES20.8)") mype,": ",label," = ",local_sum
  ...
  SUBROUTINE output_local_sum_jk(mat,label)
    ...
    DO k=lk1,lk2
       DO j=lj1,lj2
          local_sum = local_sum + mat(j,k)%get_local_abs_square_sum()
       END DO
    END DO
    WRITE(*,"(I3,3A,ES20.8)") mype,": ",label," = ",local_sum
