...
PRO corr_loop, diag
  ...
  FOR isp = 0, gui.out.n_spec_sel - 1 DO BEGIN
    ...
    FOR ivar = 0, nvar2 - 1 DO BEGIN
      IF (*i).var2[ivar] NE (*i).var1 THEN BEGIN
        ...
        ENDCASE
      ENDIF ELSE data2[0,0,0] = data1

      IF NOT (*i).tcorr THEN BEGIN ; spatial correlation
        IF (*i).var2[ivar] NE (*i).var1 THEN BEGIN
          corr_norm_inv = 1.0 / $
            SQRT(TOTAL(data1^2*jac_norm)*TOTAL(data2^2*jac_norm))
          nx = par.nx0
          ny = par.ny0
        ENDIF ELSE BEGIN
          corr_norm_inv = 1.0 / TOTAL(data1^2*jac_norm)
          nx = par.nx0 / 2 + 1 ; for autocorr, need only 1st x-y quadrant
          ny = par.ny0 / 2 + 1
        ENDELSE
        ...
        IF nz GT 1 THEN BEGIN
          ...
          FOR y = 0, ny - 1 DO FOR x = 0, nx - 1 DO corr_xy[x,y] = $
        ...
        IF (*i).var2[ivar] EQ (*i).var1 THEN BEGIN ; add redundant autocorr
          corr_xy[nx:*,0:ny-1] = REVERSE(corr_xy[1:nx-2,0:ny-1],1)
          corr_xy[*,ny:*] = REVERSE(corr_xy[*,1:ny-2],2)
        ENDIF
  ...
  IF NOT (*i).tcorr THEN BEGIN ; --- spatial correlation
    ...
    FOR isp = 0, gui.out.n_spec_sel - 1 DO BEGIN
      ...
      PLOT, xaxis, corr_xy[*,par.ny0/2,0,isp], COLOR=1, CHARSIZE=1.41, $
        ...
        YTITLE='!8C!6(!7D!6x,0)', YRANGE=[miny,maxy], /NODATA
      ...
      PLOT, yaxis, corr_xy[par.nx0/2,*,0,isp], COLOR=1, CHARSIZE=1.41, $
        ...
        YTITLE='!8C!6(0,!7D!6y)', YRANGE=[miny,maxy], /NODATA
      PLOTS, !X.CRANGE, [0,0], COLOR=1
      PLOTS, [0,0], !Y.CRANGE, COLOR=1
      ...
      FOR ivar = 0, nvar2 - 1 DO BEGIN
        ...
        IF (*i).var2[ivar] EQ (*i).var1 THEN BEGIN
          corr_xy_cont = corr_xy[0:par.nx0/2,0:par.ny0/2,ivar,isp]
          xaxis_cont = xaxis[0:par.nx0/2]
          yaxis_cont = yaxis[0:par.ny0/2]
        ENDIF ELSE BEGIN
          corr_xy_cont = corr_xy[*,*,ivar,isp]
          xaxis_cont = xaxis
          yaxis_cont = yaxis
        ENDELSE

        lev_col = contour_levels(corr_xy_cont,20)
        CONTOUR, corr_xy_cont, xaxis_cont, yaxis_cont, LEVELS=lev_col[*,0], $
