...
public:
  ...
  // excluding the entry
  ArrayMask(ScalarArrayView<int8_t, MemorySpace> mask)
  {
             ...
             ScalarArrayView<const wdmcpl::LO, MemorySpace> permutation = {}) const -> void
  {
    if (empty()) {
      ...
      WDMCPL_ALWAYS_ASSERT(output_array.size() == mask_.size());
      WDMCPL_ALWAYS_ASSERT(filtered_data.size() == num_active_entries_);
      ...
      Kokkos::parallel_for(
        mask_.size(), KOKKOS_LAMBDA(LO i) {
          if (mask[i]) {
            const auto idx = mask[i] - 1;
            output_array[i] = (permutation.size()>0)?filtered_data[permutation[idx]]:filtered_data[idx];
          }
        });
    }
  }
  [[nodiscard]]
  bool empty() const noexcept { return mask_.size() == 0; }
  // mask is true if the mask exists
  operator bool() const noexcept { !empty(); }

  [[nodiscard]]
  LO Size() const noexcept {return num_active_entries_;}
