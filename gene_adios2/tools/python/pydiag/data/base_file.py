...
class TimeSeries(object):
    ...
    def generate_timeaverage(self):
        if len(self.dataarray):
            ...
            raise RuntimeError("Cannot time average: Data array is empty")
    ...
    def calc_positions(self):
        ...
        timearray = np.array(self.timearray)
        if self.starttime == self.endtime:
            pos = np.array([bisect_left(timearray, self.starttime)])
        else:
            pos = np.where((timearray >= float(self.starttime))&(timearray <= self.endtime))[0]
        return pos
