...
; removes IDL format substring
  ...
  FOR j = 0, N_ELEMENTS(out_str) - 1 DO BEGIN
    ...
    out_str[j] = STRJOIN(STRSPLIT(out_str[j],'!9!!',/REGEX,/EXTRACT),'|')
    out_str[j] = STRJOIN(STRSPLIT(out_str[j],'!7C',/REGEX,/EXTRACT),'Gamma')
    out_str[j] = STRJOIN(STRSPLIT(out_str[j],'!7q',/REGEX,/EXTRACT),'rho')
    out_str[j] = STRJOIN(STRSPLIT(out_str[j],'!7l',/REGEX,/EXTRACT),'mu')
    out_str[j] = STRJOIN(STRSPLIT(out_str[j],'!7v',/REGEX,/EXTRACT),'Chi')
    out_str[j] = STRJOIN(STRSPLIT(out_str[j],'!7U',/REGEX,/EXTRACT),'phi')
    out_str[j] = STRJOIN(STRSPLIT(out_str[j],'!U',/REGEX,/EXTRACT),'^')
    ...
    ENDIF ELSE BEGIN
      ...
      IF KEYWORD_SET(intrp) THEN BEGIN
         ...
         ENDIF ELSE BEGIN
            ...
            oldsize = SIZE(oldres)
            rdim = LONG64(oldsize[oldsize[0]+2]/tdim)
            ...
            IF (2 EQ 1) THEN BEGIN ;saves memory but is awfully slow
               ...
               ENDFOR
            ENDIF ELSE BEGIN
               res = MAKE_ARRAY(rdim, N_ELEMENTS(tarr),TYPE=SIZE(oldres,/TYPE),/NOZERO)
               FOR ir = 0L, rdim - 1L DO res[ir,*] = $
...
; ounit: returns only the unit in fancy style
  ...
  IF Tref_is_Ti THEN T_ref_str = '!6T!Di0!N' ELSE T_ref_str = ''
...
; returns file path/exist structure
  ...
  ENDIF ELSE BEGIN
    ...
    IF (WHERE(['fie','mom','vsp','che','pro','s_c',gfile,mgfile] EQ $
...
; charthick: char thickness of color bar axis labels
  ...
  IF NOT orientation THEN BEGIN ; vertical
    ...
    IF TOTAL(pos) NE 0 THEN BEGIN
      CONTOUR, color_bar, [0,1], lev_col[*,0], $
        ...
        XMARGIN=xmargin, YMARGIN=ymargin, CHARTHICK=charthick
      ...
      PLOT, [0,1], lev_col[*,0], COLOR=1, YLOG=log, $
        ...
        POSITION=pos, CHARTHICK=charthick
      AXIS, YAXIS=0, YTICKFORMAT='(A1)', COLOR=1, /YSTYLE
      AXIS, YAXIS=1, COLOR=1, /YSTYLE, CHARSIZE=charsize, CHARTHICK=charthick
    ENDIF ELSE BEGIN
      CONTOUR, color_bar, [0,1], lev_col[*,0], $
        LEVELS=lev_col[*,0], CHARSIZE=charsize, $
        C_COLORS=lev_col[*,1], YLOG=log, $
      ...
      PLOT, [0,1], lev_col[*,0], COLOR=1, YLOG=log, $
        ...
        /NODATA, /NOERASE, XTICKLEN=0, XTICKNAME=[' ',' '], $
        YTICKLEN=0, XTICKS=1, XMINOR=1, YTICKS=0, YMINOR=1, CHARTHICK=charthick
    ENDELSE
  ENDIF ELSE BEGIN ; horizontal
    ...
    ENDIF ELSE BEGIN
      ...
      PLOT, lev_col[*,0], [0,1], COLOR=1, XLOG=log, $
        ...
        YMARGIN=ymargin, /XSTYLE, /YSTYLE, CHARSIZE=charsize, $
        /NODATA, /NOERASE, XTICKLEN=0, YTICKNAME=[' ',' '], $
        YTICKLEN=0, XTICKS=0, XMINOR=1, YTICKS=1, YMINOR=1, CHARTHICK=charthick
    ENDELSE
  ENDELSE
...
PRO printerror, errormsg, popup=popup
  ...
  IF N_ELEMENTS(errormsg) LT 1 THEN $
    errormsg = 'an error occured, please contact support@genecode.org'
  IF errormsg NE '' THEN PRINT, errormsg
  IF NOT cl_mode THEN BEGIN
    IF KEYWORD_SET(popup) THEN message = $
      DIALOG_MESSAGE(errormsg,/ERROR,DIALOG_PARENT=gui.window.main) ELSE $
  ...
  ENDIF ELSE BEGIN
    IF ((par.magn_geometry NE 's_alpha') AND $
      (par.magn_geometry NE 'circular')) THEN $
       ...
       plot_geom_coeff, theta_pol, '!7h!6!Ipol,est!N'
    ENDIF
  ENDELSE
...
;; standing wave
;           (*datptr[f].kxky)[0,2,*] = 1.0
;        ENDIF
        ; Doppler correction
        IF request_dat[f,0] AND (series.doppler_corr_mom NE 0.0) THEN BEGIN
;          IF mom_time GT par.ExB_stime THEN BEGIN
;          APPLY PHASE FACTOR WHICH CORRESPONDS TO TRANSLATION IN
;          CONFIGURATION SPACE: x -> x - v * t:  f^LAB_k = f^CO_k * EXP(-i k v t)
;          here: y -> y + C_y Omega t:  f^LAB_k = f^CO_k * EXP(+i k C_y Omega t)
;          NOTE: Before June 2018 the sign was opposite
...
; input: geom_type (0: s-alpha, 1: circular, 2: tracer/chease etc.)
  ...
  IF bigM LE 0 THEN BEGIN
    ...
    if (par.magn_geometry EQ 'miller') then $
       bigM = FLOOR(2.0*!PI/(rho_o_Lref*par.ly)*ABS((*series.geom).C_y)) $
  ...
  IF par.x_local THEN BEGIN
     if (par.magn_geometry EQ 'miller') then $
        rho=par.kymin*(*series.geom).C_y*par.q0/(bigM*ABS(round_q0)) $
     else rho = par.kymin * r0 / (bigM * ABS(round_q0))
...
PRO stat_avg_err, time, data, avg, err, sdev, t_corr=t_corr
...
; err: window-based error of avg (return value)
; sdev: non-window-based standard deviation of avg (return value)
; t_corr: correlation time for time windows in error computation (input)
  ...
  IF (N_ELEMENTS(t_corr) NE 1) OR (n_steps LE 2) THEN BEGIN
    t_corr = 0
  ...
  ; --- non-windowed standard deviation ---

  sdev = SQRT(TOTAL((data-avg)^2)/(n_steps-1))

  err = 0
  IF t_corr EQ 0 THEN RETURN

  ; --- use window statistics for errors ---

  ; start with window width = 5 t_corr; if fewer than 8 windows,
  ; reduce width while keeping 8 windows, down to minimum width 2
  max_width = 5
  min_width = 2
  min_win = 8
  IF dt_full GE min_win * max_width * t_corr THEN BEGIN
    n_win = FLOOR(dt_full/(max_width*t_corr))
    wwidth = dt_full / n_win
  ENDIF ELSE BEGIN
    IF dt_full GE min_win * min_width * t_corr THEN BEGIN
      n_win = min_win
      wwidth = dt_full / n_win
    ENDIF ELSE BEGIN
      IF !QUIET NE 1 THEN PRINT, $
        'stat_avg_err: insufficient time range, skipping error computation'
      RETURN
    ENDELSE
  ENDELSE
  ...
  IF (WHERE((win_inds[1:n_win]-win_inds[0:n_win-1]) LE 2))[0] GE 0 THEN BEGIN
    PRINT, $
      'stat_avg_err warning: windows with <= 2 steps, skipping error computation'
    RETURN
  ENDIF

  win_avg = DBLARR(n_win)
  ...
  err = SQRT(TOTAL((win_avg-avg)^2)/(n_win*(n_win-1L)))
...
;required for old IDL version w/o intrinsic SIGNUM function

  zeroelem = WHERE(X EQ 0)

  IF (zeroelem GT 0) THEN X[zeroelem] = 1
  tmparr = FIX(X/ABS(X))
  IF (zeroelem GT 0) THEN tmparr[zeroelem] = 0
