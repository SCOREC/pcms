FUNCTION checkpoint_info
  ...
  RETURN, {$
    ...
    help_text : ['Prints scalar value or draws 1D or 2D plots of the '+$
                 ...
                 'WARNING for *y-global* analysis: in contrast to '+$
                 'other diagnostics, the checkpoint format will '+ $
                 'remain in the original order for now, i.e.'+$
                 '1st dim. = y, 2nd dim. = kx'],$
...
PRO checkpoint_init, diag
  ...
  ni0 = par.y_local?par.nx0:par.ny0
  nj0 = par.y_local?par.nky0:par.nkx0/2

  IF par.adapt_lx THEN printerror, 'Warning: kx labels may be ' + $
    'incorrect if adapt_lx is set'

  IF (gui.out.n_spec_sel NE par.n_spec) AND KEYWORD_SET(*e.g2f) THEN BEGIN
     printerror, 'Error: g2f in '+(*diag).name+$
                 ' only possible with all species selected'
     (*diag).selected = 0
     RETURN
  ENDIF
  ...
  IF N_ELEMENTS(*e.xind) LT 1 THEN xind = -1 ELSE $
    IF (*e.xind)[0] LE -2 THEN BEGIN
     ...
     IF (par.x_local AND par.y_local AND (xfft EQ 0)) THEN $
  ...
  i = set_internal_vars(diag,{$
    ...
    vavg       : vind[0] EQ -1,$
    wavg       : wind[0] EQ -1,$
    xind       : xind[0] EQ -1 ? INDGEN(ni0) : xind,$
    yind       : yind[0] EQ -1 ? INDGEN(nj0) : yind,$
...
FUNCTION compute_f, diag, g1, calc_apar=calc_apar
  ...
  k_perp = SQRT(REBIN(REFORM((*series.geom).gxx,[1,1,nz]),[nkx,nky,nz])*REBIN(kx^2,[nkx,nky,nz])+$
    2.0*REBIN(REFORM((*series.geom).gxy,[1,1,nz]),[nkx,nky,nz])*$
    REBIN(kx,[nkx,nky,nz])*REBIN(REFORM(ky,[1,nky]),[nkx,nky,nz])+$
    REBIN(REFORM((*series.geom).gyy,[1,1,nz]),[nkx,nky,nz])*REBIN(REFORM(ky^2,[1,nky]),[nkx,nky,nz]))
...
PRO checkpoint_output, diag
  ...
  CASE dims[0] OF
    1 : BEGIN
         ...
         ENDIF ELSE BEGIN
            xtitle = (*i).xfft EQ 1 ? $
              '!6k!Dy!N!7q!6!Dref!N' : '!6y/!7q!6!Dref!N'
            xaxis = (*i).xfft EQ 1 ? $
              (*par.ky)[(*i).yind,0] : $
              ((*i).xind / FLOAT(par.ny0-1) - 0.5) * par.ly
         ENDELSE
        END
    2 : BEGIN
         IF (par.y_local) THEN BEGIN
         ...
         ENDIF ELSE BEGIN
            xtitle = (*i).yfft ? '!6x/!7q!6!Dref!N' : '!6k!Dx!N!7q!6!Dref!N'
            xaxis = (*i).yfft EQ 1 ? $
              (*par.kx)[(*i).yind,0] : $
                    ((*i).yind / FLOAT(par.nx0-1) - 0.5) * par.lx
         ENDELSE
        END
    3 : BEGIN
          xtitle = '!6z/L!Dref!N'
      ...
      2 : BEGIN
           IF (par.y_local) THEN BEGIN
              ...
              yaxis = (*i).yfft ? FINDGEN(par.ny0) / par.ny0 * $
                      par.ly - par.ly / 2 : (*par.ky)[(*i).yind+par.ky0_ind]
           ENDIF ELSE BEGIN
              ytitle = (*i).yfft ? '!6x/!7q!6!Dref!N' : '!6k!Dx!N!7q!6!Dref!N'
              yaxis = (*i).yfft ? FINDGEN(par.nx0) / (par.nx0-1) * $
                      par.lx - par.lx / 2 : 2.0*!PI/par.lx*(*i).yind
           ENDELSE
          END
      3 : BEGIN
            ytitle = '!6z/L!Dref!N'
  ...
  IF (*i).xavg THEN title += ', x avg.' ELSE IF (*i).nxind EQ 1 THEN $
    title += par.y_local?((par.x_local + (*i).xfft EQ 1) ? $
  ...
  IF (*i).yavg THEN title += ', y avg.' ELSE IF (*i).nyind EQ 1 THEN $
    title += par.y_local?((*i).yfft ? rm0es(FINDGEN(par.ny0) / par.ny0 * $
