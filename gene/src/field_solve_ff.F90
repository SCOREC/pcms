...
Module fieldsolver_ff
  ...
  public:: initialize_field_solve_ff, field_solve_ff, field_solve_ff_f, &
       ...
       antenna_contrib, a11det_inv_antenna, a11det_inv_island
  ...
  real, dimension(:,:,:), allocatable:: a11det_inv_island
...
contains
  ...
  Real Function mem_est_field_solve_ff(mem_req_in)
    ...
    mem_loc=2*SIZE_OF_REAL_MB*lijk0
    !a11det_inv_antenna
  ...
  Subroutine field_solve_ff(moments,p_emfields)
    ...
    if (island_contrib) &
         call add_Apar_island(p_emfields,a11det_inv_island)
    ...
    End If
    !\todo delzonal_fields is confusing and should maybe be renamed to delzonal_apar
    If ((delzonal_fields) .and. (n_fields.ge.2)) Then
       Call del_zonal_phi(p_emfields(:,:,:,2))
    End If
    If ((delzonal_bpar) .and. (n_fields.ge.3)) Then
       Call del_zonal_phi(p_emfields(:,:,:,3))
    End If
    if (only_Er) then
  ...
  EXTERNAL erf
    ...
    END IF
     if (island_contrib) then
       call initialize_island
       allocate(a11det_inv_island(li1:li2,lj1:lj2,lk1:lk2))
    end if
    IF (n_fields .GT. 2) THEN
    ...
    Do k=lk1,lk2
       ...
       Do i = li1,li2
          Do j = lj1,lj2
             ...
             if (.not.(adiabatic_electrons.or.trap_pass.or.no_electron_response)) then
                ...
                If ((ki(i).Ne.0.).Or.(kj(j).Ne.0.)) Then
                   ...
                   IF (island_contrib) then
                      a11det_inv_island(i,j,k) = k_perp2 / (k_perp2 + 2.0 * sum1)
                   ENDIF
                ...
                ELSE
                   ...
                   IF (island_contrib) a11det_inv_island(i,j,k) = 0.0
                End If
  ...
  Subroutine finalize_field_solve_ff
    ...
    if (island_contrib) then
       call finalize_island
       DEALLOCATE(a11det_inv_island)
    endif
    IF (n_fields .GT. 2) THEN
