...
#ifdef WITHFUTILS
  integer :: isnap_srcmom = 0
  integer, dimension(:), allocatable :: fidsrcmom_h5
  character(len=7), dimension(1:3) :: src_label =(/'ck_heat', 'ck_part', 'f0_term'/)
  character(len=3), dimension(1:3) :: mom_label = (/'M00','M10','M22'/)
#endif
...
Contains
  ...
  Subroutine check_diag_df
    ...
    if (comp_type.eq.'EV') then
       if (istep_prof.gt.0) istep_prof = 1
       if (istep_srcmom.gt.0) istep_srcmom = 1
    endif
...
!*****************************************************************
  ...
  Subroutine initialize_diag_srcmom
    ...
    if (write_std.and.write_srcmom_pe) then
       ! Moment files for each species in a different file
       DO n=ln1,ln2
    ...
    if (write_h5.and.(my_pey+my_pez+my_pev+my_pew).eq.0) then
       ...
       allocate(fidsrcmom_h5(ln1:ln2))

       DO n=ln1,ln2
          isnap_srcmom = 0
          call creatf(trim(diagdir)//'/srcmom_'//trim(spec(n)%name)//trim(file_extension)//'.h5', &
               fidsrcmom_h5(n), "source diagnostics", 'd', mpi_comm_x)
          call creatg(fidsrcmom_h5(n), '/srcmom_'//trim(spec(n)%name))
          rank = 0
          call creatd(fidsrcmom_h5(n), rank, dims, "/srcmom_"//trim(spec(n)%name)//"/time", "time")
          ...
          do s = 1, 3
             ...
             do o = 1, 3
                call creatg(fidsrcmom_h5(n), '/srcmom_'//trim(spec(n)%name)//'/'//&
                     &trim(src_label(s))//'/'//trim(mom_label(o)))
             enddo
          end do
...
#ifdef WITHFUTILS
    character(len=FILENAME_MAX) :: dset_name_srcmom
#endif
...
!       if (write_srcmom_pe) write(SRCMOMFILE(n), "(F13.6)") time
       if (write_std.and.write_srcmom_pe) write(SRCMOMFILE(n)) time
       do s=1,3
          ...
          if (write_h5.and.((my_pey+my_pez+my_pev+my_pew).eq.0))  then
             do o=1,3
                write(dset_name_srcmom, "(A, '/', i10.10)") "/srcmom_"&
                    & //trim(spec(n)%name)//"/"//trim(src_label(s))//&
                    & "/"//trim(mom_label(o)), isnap_srcmom

                call putarr(fidsrcmom_h5(n), trim(dset_name_srcmom),&
                        & out_arr(:,o))
             enddo
       endif
#endif
       enddo !loop on source

       if (write_std.and.write_srcmom_pe) call flush(SRCMOMFILE(n))

#ifdef WITHFUTILS
       if (write_h5.and.((my_pey+my_pez+my_pev+my_pew).eq.0)) then
           call append(fidsrcmom_h5(n), "/srcmom_"//trim(spec(n)%name)//"/time", real(time,8))
           call attach(fidsrcmom_h5(n), "/srcmom_"//trim(spec(n)%name)//"/time", "n_steps", isnap_srcmom+1)

           call flushh5(fidsrcmom_h5(n))
       end if
#endif
  ...
  Subroutine finalize_diag_srcmom
   ...
   if (write_h5.and.(my_pey+my_pez+my_pev+my_pew).eq.0) then
      do n=ln1,ln2
          call closef(fidsrcmom_h5(n))
      end do
      deallocate(fidsrcmom_h5)
   endif
#endif
