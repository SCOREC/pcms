...
PRO spectra_y_loop, diag
  ...
  ; Computes 'y-spectra'.  Note that the flux surface average performed here
  ; is not correct as the jacobian is not included.  Still useful as a
  ...
  FOR isp = 0, gui.out.n_spec_sel - 1 DO BEGIN
    FOR ivar = 0, nvars - 1 DO BEGIN
      FOR ikx = 0, nkxind - 1 DO BEGIN
           ...
           yfftdata[0:nkxpos-1,*,0:par.nz0-1] = $
              FFT((*mom[isp,(*i).vars[ivar]].kxsy),DIMENSION=2)
