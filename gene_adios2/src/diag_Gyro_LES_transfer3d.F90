...
Module diag_Gyro_LES_transfer3d
  ...
  real, dimension(:,:,:,:,:), allocatable :: tf_cumul_3d_8
...
contains
  ...
  Subroutine initialize_diag_transfer_3d
    ...
    IF (tk3_log_kperp_h1) call calc_initialize_diag_transfer_3d_h(n_shells_log_kperp_1,6)
    IF (tk3_log_kperp_h2) call calc_initialize_diag_transfer_3d_h(n_shells_log_kperp_2,8)
#endif
  ...
  Subroutine diagnostic_transfer_3d
    ...
    IF (tk3_log_kperp_h1) call calc_diagnostic_transfer_3d_h(n_shells_log_kperp_1,6)
    IF (tk3_log_kperp_h2) call calc_diagnostic_transfer_3d_h(n_shells_log_kperp_2,8)
#endif
  ...
  Subroutine finalize_diag_transfer_3d
    ...
    IF (tk3_log_kperp_h1) call calc_finalize_diag_transfer_3d_h(6)
    IF (tk3_log_kperp_h2) call calc_finalize_diag_transfer_3d_h(8)
...
Subroutine calc_initialize_diag_transfer_3d_h(n_shells,which_model)
    ...
    case(4)
     ...
     case(6)
        ...
        Write(header,*) '#', lambda1, kperp_max
     case(8)
        allocate(tf_cumul_3d_8(n_shells,n_shells,n_shells,ln1:ln2,1))
        tf_cumul_3d_8 = 0
        transfer_counter_3d(8) = 0
        label ='log_kperp2_'
        Write(header,*) '#', lambda1, kperp_max
     case default
        return
...
!    allocate(h_mod_ls3(li1:li2, lj1:lj2, lk1:lk2, ll1:ll2, lm1:lm2, ln1:ln2))
    ...
    do ls3=1,n_shells
        ...
        if (which_model.eq.8) call calc_n_shells_log_kperp_2(ls3,g_1,g_ls3)
...
!        h_mod_ls3 = h_ls3(li1:li2,lj1:lj2,lk1:lk2,ll1:ll2,lm1:lm2,ln1:ln2)
        !with antenna, we need another filtering step on the fields since the field solver adds the antenna
        !contributions regardless of whether g_1 is already filtered or not
        if (any(antenna_type.eq.(/2,3/))) then
           allocate(fields_tmp(li1:li2,lj1:lj2,lbz:ubz,1:n_fields))
           if (.not.any(which_model.eq.(/4,6,8/))) &
                stop 'triple-shell transfers not implemented for antenna runs with this type of spectrum'
           fields_tmp=fields_ls3
           if (which_model.eq.4) call calc_n_shells_log_kperp_fields(k0,ls3,fields_tmp,fields_ls3)
           if (which_model.eq.6) call calc_n_shells_log_kperp_1_fields(ls3,fields_tmp,fields_ls3)
           if (which_model.eq.8) call calc_n_shells_log_kperp_2_fields(ls3,fields_tmp,fields_ls3)
           deallocate(fields_tmp)
        endif
        do ls2=1,n_shells
            if (which_model.eq.1) call calc_n_shells_ky(ls2,g_1,g_ls2)
            if (which_model.eq.2) call calc_n_shells_kx(ls2,g_1,g_ls2)
            if (which_model.eq.4) call calc_n_shells_log_kperp(k0,ls2,g_1,g_ls2)
            if (which_model.eq.6) call calc_n_shells_log_kperp_1(ls2,g_1,g_ls2)
            if (which_model.eq.8) call calc_n_shells_log_kperp_2(ls2,g_1,g_ls2)
            ...
            do ls1=1,n_shells
                ...
                if (which_model.eq.4) call calc_n_shells_log_kperp(k0,ls1,g_1,g_ls1)
                if (which_model.eq.6) call calc_n_shells_log_kperp_1(ls1,g_1,g_ls1)
                if (which_model.eq.8) call calc_n_shells_log_kperp_2(ls1,g_1,g_ls1)
                ...
                if (any(antenna_type.eq.(/2,3/))) then
                   ...
                   if (which_model.eq.8) call calc_n_shells_log_kperp_2(ls1,cfgamma_ant,cfgamma_ant_ls1)
...
!    deallocate(h_mod_ls3)
    ...
    if ( .not.( mod(itime,istep_fe_transfer*avg_window) .eq. 0) ) then
       ...
       case(6)
          ...
          transfer_counter_3d(6) = transfer_counter_3d(6) + 1
       case(8)
          tf_cumul_3d_8 = fe_tf_tot_3d + tf_cumul_3d_8
          transfer_counter_3d(8) = transfer_counter_3d(8) + 1
       case default
       end select
   endif
    ...
    if ( itime .gt. 0 .and. mod(itime,istep_fe_transfer*avg_window) .eq. 0 ) then
       ...
       case(8)
          tf_cumul_3d_8 = fe_tf_tot_3d + tf_cumul_3d_8
          transfer_counter_3d(8) = transfer_counter_3d(8) + 1
          fe_tf_tot_3d =  tf_cumul_3d_8 /real(transfer_counter_3d(8))
       case default
       end select
  ...
  Subroutine calc_finalize_diag_transfer_3d_h(which_model)
    ...
    if (which_model.eq.4) deallocate(tf_cumul_3d_4)
    if (which_model.eq.6) deallocate(tf_cumul_3d_6)
    if (which_model.eq.8) deallocate(tf_cumul_3d_8)
