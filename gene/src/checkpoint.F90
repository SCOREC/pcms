...
MODULE checkpoint
  ...
  USE futils
#endif
  use GaussQuadrature, only: getmuweightsandknots
#ifdef WITHHAC
  USE hlst_adios_checkpoint, ONLY: &
       & HAC_KIND, HAC_VERBOSE, hac_info, hac_read, hac_write, hac_init, hac_exit
#endif
  use lagrange_interpolation, only: lag3interp_complex
  Use mpi
  Use par_mod, only: comp_type, time, dt, dt_max, itime, prec, rad_bc_type, &
       &lx, lv, lw, xval, zval, mu, vp, pi, n_pol, file_extension, print_ini_msg
...
!    HAC_FIELD(KIND=NT_K),DIMENSION(:,:,:,:,:,:) :: g_old
  ...
  subroutine checkpoint_write (g_,filename)
    ...
    integer,pointer:: handle
    integer(MPI_OFFSET_KIND),pointer:: offset
    integer(MPI_OFFSET_KIND) :: rkind
    integer:: count1, count2, count_rate, ierr
    ...
    case(1) !MPI-IO checkpoint 
       ...
       if (.not.frewind) then
          ...
          offset=offset+rkind*2*li0*lj0*lk0*ll0*lm0*ln0*n_procs_sim
       else
