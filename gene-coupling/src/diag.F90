...
Module diagnostics
  ...
  USE geometry, ONLY: write_geometry
#ifndef COUPLE_XGC
  use Gyro_LES
#endif
#ifdef WITHFUTILS
...
#include "gwrite_mom.fh"
  ...
  SUBROUTINE diag_mom_XGC
    ...
    real, dimension(li1:li2,lj1:lj2,lk1:lk2,2) :: mom

    INTEGER :: k, l, m, n
    COMPLEX, DIMENSION(li1:li2,lj1:lj2) :: f_av
    INTEGER :: i, j, o, n_corrs, write_pe, trap_level
    real, dimension(li1:li2) :: fnorm
    real, dimension(li1:li2,2) :: flux
    real, dimension(li1:li2,1) :: tmp_set


#ifdef WITHFUTILS
    character(len=FILENAME_MAX) :: dset_name_flux_XGC
    integer:: ng, pes, ierr
#endif
    ...
    do n=ln1,ln2
       ...
       !integrate in vpar
       call my_complex_sum_v(vmom,size(vmom))

       !get ExB velocity
       do m=lm1,lm2
          tmp_field = emfields(li1:li2,lj1:lj2,lk1:lk2,1)
          call gyro_average_df(tmp_field,bar_field(:,:,:),m,n)
          do k=lk1,lk2
             do j=lj1,lj2
                do i=li1,li2
       ...
       do m=lm1,lm2
          !particle
          mom(:,:,:,1) = mom(:,:,:,1)-REAL(CONJG(vmom(:,:,:,m,1)) * dfields_dy(:,:,:,m))
          !heat
          mom(:,:,:,2) = mom(:,:,:,2)-Real(Conjg(vmom(:,:,:,m,2)+vmom(:,:,:,m,3)) * dfields_dy(:,:,:,m))
       end do
