...
struct hash<wdmcpl::DimID>
{
  std::size_t operator()(const wdmcpl::DimID & key) const noexcept {
    ...
    return h1^(h2 <<1); // hash combine see https://en.cppreference.com/w/cpp/utility/hash
  }
};
} // namespace std
namespace wdmcpl
...
{
public:
  using DataMapType = std::unordered_map<DimID, std::unordered_set<LO>> ;
  void Insert(const DimID& key, ScalarArrayView<LO,wdmcpl::HostMemorySpace> data) {
  ...
  void Deserialize(ScalarArrayView<LO, wdmcpl::HostMemorySpace> serialized_data);
  bool operator==(const ReverseClassificationVertex& other) const {return data_ == other.data_;}
  const std::unordered_set<LO>* Query(const DimID& geometry) const noexcept {
...
ReverseClassificationVertex ReadReverseClassificationVertex(std::filesystem::path&);
ReverseClassificationVertex ReadReverseClassificationVertex(std::istream&);
ReverseClassificationVertex ReadReverseClassificationVertex(std::istream&, MPI_Comm, int root=0);
ReverseClassificationVertex ReadReverseClassificationVertex(std::filesystem::path&, MPI_Comm, int root=0);
