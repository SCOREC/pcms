...
namespace coupler {
  ...
  Array2d<T>* receive_density(const std::string cce_folder,
    ...
    return receive2d_from_ftn<T>(cce_folder,name, io, engine);
  }
  ...
  void send_density(const std::string cce_folder, const Array2d<T>* density,
      adios2::IO &io, adios2::Engine &engine, adios2::Variable<T> &send_id) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  ...
  Array2d<T>* receive_field(const std::string cce_folder,
    ...
    return receive2d_from_ftn<T>(cce_folder,name, io, eng);
  }
