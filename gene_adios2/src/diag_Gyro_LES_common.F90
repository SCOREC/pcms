...
!  use blockindex
  ...
  public:: calc_n_shells_ky, calc_n_shells_log_kperp_1, calc_n_shells_log_kperp_2, calc_n_shells_log_kperp, calc_n_shells_kx
  public:: calc_n_shells_log_kperp_fields, calc_n_shells_log_kperp_1_fields, calc_n_shells_log_kperp_2_fields
  ...
  ! Allocatable variables for the 3d real space free energy
  Real, Dimension(:,:,:,:,:,:), Allocatable :: cfgamma_real
  Real, Dimension(:,:,:,:,:,:), Allocatable :: temp_rhs_real
  Real, Dimension(:,:,:,:,:,:), Allocatable :: v6d_real
  ...
  logical :: tk3_log_kperp_h1 = .false.
  logical :: tk3_log_kperp_h2 = .false.
  ...
  integer :: n_shells_log_kperp_2 = 25 ! For security in the allocatable variable
  logical :: with_nvp = .false.
  real ::  delta_ky, delta_kx, delta_kperp, lambda1, lambda2
  real :: kperp_max, k0, k0_y, auto_exp
  integer :: n_transfer_models = 8
...
contains
  ...
  subroutine init_kperp
    ...
    if (t_log_kperp_h2.or.tk3_log_kperp_h2) then
       if (lambda1.eq.0.0) stop 'lambda1 has to be defined for automatic kperp shells!'
       auto_exp=real(n_shells_log_kperp_2-1)/log(kperp_max/lambda1)
    endif
...
!this routine is used to apply shell filtering directly to a (3d) field variable
!**********************************************************************************************
  Subroutine calc_n_shells_log_kperp_fields(k0,ls1,fields_in,fields_out)

    complex, dimension(li1:li2, lj1:lj2, lbz:ubz, 1:n_fields),intent(in) :: fields_in
    complex, dimension(li1:li2, lj1:lj2, lbz:ubz, 1:n_fields),intent(out) :: fields_out
    integer, intent(in) :: ls1
    real, intent(in) :: k0
    integer :: i,j,k
    ...
    !Calculate the shell splitting
    !delta_k of the shells
    fields_out=cmplx(0.0,0.0)
    do k=lk1,lk2
       do j=lj1,lj2
          do i=li1,li2
            if (ls1.eq.1) then
                if ((kperp(i,j,k).ge.(0)).and.(kperp(i,j,k).le.(k0*2**real((ls1-1)/5.)))) then
                    fields_out(i,j,k,:) = fields_in(i,j,k,:)
                endif
            else if ((ls1.gt.1).and.(ls1.lt.n_shells_log_kperp)) then
                if ((kperp(i,j,k).gt.(k0*2**real((ls1-2)/5.))).and.(kperp(i,j,k).le.(k0*2**real((ls1-1)/5.)))) then
                    fields_out(i,j,k,:) = fields_in(i,j,k,:)
  ...
  End Subroutine calc_n_shells_log_kperp_fields
...
!!!******************************************************************************************
  ...
  Subroutine calc_n_shells_log_kperp_1_fields(ls1,fields_in,fields_out)
       ...
       do j=lj1,lj2
          do i=li1,li2
             if (ls1.eq.1) then
                ...
                if ((kperp(i,j,k).gt.(lambda1)).and.(kperp(i,j,k).le.(lambda1*2**real((ls1-1)/5.)))) then
                   fields_out(i,j,k,:) = fields_in(i,j,k,:)
                endif
             else if (ls1.eq.3) then
                if ((kperp(i,j,k).gt.(lambda1+lambda2)).and.(kperp(i,j,k).le.((lambda1+lambda2)*2**real((ls1-2)/5.)))) then
                ...
                if ((kperp(i,j,k).gt.((lambda1+lambda2)*2**real((ls1-2)/5.))).and.&
                     (kperp(i,j,k).le.((lambda1+lambda2)*2**real((ls1-1)/5.)))) then
                   fields_out(i,j,k,:) = fields_in(i,j,k,:)
                endif
             else
                if ((kperp(i,j,k).gt.((lambda1+lambda2)*2**real((ls1-2)/5.)))) then
                   fields_out(i,j,k,:) = fields_in(i,j,k,:)
                endif
             endif
          enddo
  ...
  End Subroutine calc_n_shells_log_kperp_1_fields


!!!******************************************************************************************

!> Applies shell filter with automated spacing between shells. Requires only lambda1 as input.
Subroutine calc_n_shells_log_kperp_2(ls1,g_1,g_ls1)
  ...
  !Lambda 1 defines the size of the first shell, and the exponent separating all the other shells
  !is automatically chosen to span the available kperp range with the selected number of shells
  g_ls1=cmplx(0.0,0.0)
  do k=lk1,lk2
     do j=lj1,lj2
        do i=li1,li2
           ...
           else if ((ls1.ge.2).and.(ls1.lt.n_shells_log_kperp_1)) then
              if ((kperp(i,j,k).gt.(lambda1*exp(real(ls1-2)/auto_exp))).and.&
                   (kperp(i,j,k).le.(lambda1*exp(real(ls1-1)/auto_exp)))) then
                 g_ls1(i,j,k,:,:,:) = g_1(i,j,k,:,:,:)
              endif
           else
              if ((kperp(i,j,k).gt.(lambda1*exp(real(ls1-2)/auto_exp)))) then
                 g_ls1(i,j,k,:,:,:) = g_1(i,j,k,:,:,:)
              endif
           endif
        enddo
...
End Subroutine calc_n_shells_log_kperp_2


!!!******************************************************************************************

!> Applies shell filter with automatic spacing between shells to field arrays.
Subroutine calc_n_shells_log_kperp_2_fields(ls1,fields_in,fields_out)
...
End Subroutine calc_n_shells_log_kperp_2_fields
