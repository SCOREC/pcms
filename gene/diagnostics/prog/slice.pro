...
PRO slice_init, diag
  ...
  i = set_internal_vars(diag,{$
    ...
    kx2x     : *e.kx2x,$
    ky2y     : *e.ky2y,$
    modifier : 3,$ ; 0=abs; 1=Re; 2=Im; 3=abs^2
    data_id  : PTR_NEW()})
  ...
  omrot = 0.0 ;comment line for doppler shift

  CASE 2 * (*i).kx2x + (*i).ky2y OF
    0 : fft_format, kxky=(*i).var,doppler_freq=omrot
    1 : fft_format, kxsy=(*i).var,doppler_freq=omrot
    2 : fft_format, sxky=(*i).var,doppler_freq=omrot
    3 : fft_format, sxsy=(*i).var,doppler_freq=omrot
  ENDCASE
...
PRO slice_loop, diag
  ...
  data = DBLARR(xavg?1:xdim,yavg?1:ydim,zavg?1:zdim,n_vars,gui.out.n_spec_sel)
...
PRO slice_output, diag
  ...
  nxind = xavg ? 1 : N_ELEMENTS((*i).xind)
  nyind = yavg ? 1 : N_ELEMENTS((*i).yind)
  ...
  FOR n = 0, n_res_steps - 1 DO BEGIN
   ...
   FOR isp = 0, nsp - 1 DO BEGIN
    ...
    FOR ivar= 0 , nvars - 1 DO BEGIN
	  ...
	  END
      2 : BEGIN
            slice_plot, REFORM(data[*,*,ivar,isp,n]), *my_axis[0], $
