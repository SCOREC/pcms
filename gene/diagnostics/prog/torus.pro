...
PRO torus_init, diag
  ...
  glob_x_red  = 0 ; floor(0.8*par.nx0/2) ; omit inner/outer flux surfaces,
                  ; default: 0
  ...
  rad_pol_cuts = 0 ; plot and print radial and poloidal cuts
                   ; (e.g. for benchmarking)
  use_sxky = (contourfill EQ 0)$; use sxky data instead of sxsy; default: 0
         OR (rad_pol_cuts EQ 1) ; note: sxky is more accurate and may
  ...
  IF N_ELEMENTS(*e.phi_cut) LT 1 THEN BEGIN
     IF *e.mode EQ 2 THEN *e.phi_cut = [0.5,0.0] $
     ELSE *e.phi_cut = 0.0
  ENDIF
...
;    shmet_corr     : FLTARR(res[0],res[2]+1,/NOZERO),$
  ...
  FOR x = 0, (*i).x_res - 1 DO BEGIN
     IF par.x_local THEN BEGIN
        ...
        qprof_auxterm = !PI * ((*i).r0 + par.shat * $
                               (x - (*i).x_res / 2) * (*i).dx) * Cyq0_r0 * par.sign_Ip_CW
     ENDIF ELSE BEGIN
        phi_auxterm = REBIN(REFORM((*i).phi_cut*C_y[x],$
                                   [1,(*i).n_cuts]),[(*i).z_res,(*i).n_cuts])
        qprof_auxterm = !PI * q_prof[x] * C_y[x] * par.sign_Ip_CW

        IF use_sh_met THEN BEGIN
           IF (par.sign_Ip_CW NE 1) THEN printerror, $
              'Warning: sign_Ip_CW not yet implemented for shifted metric torus viz'
           xc = par.nx0 / 2
           nx = ABS(x-xc) + 1
           ...
           ENDIF ELSE BEGIN
              gxx_x = gxx[x:xc-1,*]
              gxz_x = gxz[x:xc-1,*]
              qprof_x = q_prof[x:xc-1]*par.sign_Ip_CW
              shmet_int = - TOTAL(REBIN(par.rhostar*par.minor_r*qprof_x,$
           ...
           (*i).y_shift[x] = round(2.0 * !PI / (*i).dy*(C_y[xc]*q0*par.sign_Ip_CW))
  ...
  IF (*i).mode NE 0 THEN BEGIN
     ...
     FOR x = 0, 1 DO BEGIN
        ...
        FOR p = 0, phi_res - 1 DO BEGIN
           ...
           ;GENE phi is in mathematically negative direction (COCOS 2/12)
  ...
  IF (par.sign_Omega_CW NE 0) THEN $
     omrot = par.sign_Omega_CW*ABS(par.omegatorref)*par.Lref/par.cref $
  ELSE omrot = par.omegatorref*par.Lref/par.cref

  fft_format, sxky=use_sxky*(*e.var), sxsy=(use_sxky EQ 0)*(*e.var),doppler_freq=omrot
...
;  coltable = 0 ; black-white (use with contourfill=0?)
  ...
  eps = (n_res_steps EQ 1) AND ((*i).n_cuts EQ 1) AND $
        ((*i).rad_pol_cuts EQ 0)
  ...
  IF (*i).mode NE 1 THEN BEGIN   ; not partial torus
     IF (*i).png_out THEN BEGIN  ; prepare direct graphics output
        ...
        ENDIF ELSE BEGIN        ; geom_type = 2 (tracer, chease, ...)
           ...
           increasing_theta = ((*i).space_theta[(*i).z_res/2,0] LE $
                               (*i).space_theta[(*i).z_res/2+1,0])

           FOR z = 0, (*i).z_res - 1 DO BEGIN
              z_plus_1 = (z + 1) MOD (*i).z_res

              pi_jump_i = ROUND((inner_border_theta[z]-$
                                 inner_border_theta[z_plus_1])/!PI)*!PI
              pi_jump_o = ROUND((outer_border_theta[z]-$
                                 outer_border_theta[z_plus_1])/!PI)*!PI

              IF increasing_theta THEN BEGIN
                 theta_range_inner = $
                    WHERE(((img_theta GE inner_border_theta[z]) AND $
                         (img_theta LT inner_border_theta[z_plus_1]+pi_jump_i)) OR $
                          ((img_theta GE inner_border_theta[z]-pi_jump_i) AND $
                           (img_theta LT inner_border_theta[z_plus_1])))
                 theta_range_outer = $
                    WHERE(((img_theta GE outer_border_theta[z]) AND $
                         (img_theta LT outer_border_theta[z_plus_1]+pi_jump_o)) OR $
                          ((img_theta GE outer_border_theta[z]-pi_jump_o) AND $
                           (img_theta LT outer_border_theta[z_plus_1])))
              ENDIF ELSE BEGIN
                 theta_range_inner = $
                    WHERE(((img_theta LE inner_border_theta[z]) AND $
                           (img_theta GT inner_border_theta[z_plus_1]+pi_jump_i)) OR $
                          ((img_theta LE inner_border_theta[z]-pi_jump_i) AND $
                           (img_theta GT inner_border_theta[z_plus_1])))
                 theta_range_outer = $
                    WHERE(((img_theta LE outer_border_theta[z]) AND $
                           (img_theta GT outer_border_theta[z_plus_1]+pi_jump_o)) OR $
                          ((img_theta LE outer_border_theta[z]-pi_jump_o) AND $
                         (img_theta GT outer_border_theta[z_plus_1])))
              ENDELSE
  ...
  FOR sp = 0, gui.out.n_spec_sel - 1 DO BEGIN
     ...
     IF NOT (*i).norm_local THEN BEGIN
        ...
        IF (*i).show_minmax THEN $
           PRINT, (*diag).name + ': global min/max for species ' + $
                  spec[sp_id].name + ': ' + rm0es((*i).data_minmax[0,sp]) + $
                  '/' + rm0es((*i).data_minmax[1,sp])
     ENDIF
     ...
     FOR n = 0, n_res_steps - 1 DO BEGIN ; reading data and plotting
        ...
        FOR c = 0, (*i).n_cuts - 1 DO BEGIN
           ...
           IF (*i).rad_pol_cuts THEN create_rad_theta_cuts, $
              diag, REFORM(cut_data[*,*,0,sp,n]), c, n, time
           ...
           IF (*i).mode NE 1 THEN BEGIN
              IF (*i).png_out THEN BEGIN
                 ...
                 IF (*i).norm_local THEN $
                    img[img_rselect] = contour_levels(img_unnorm,$
                                                      n_levels,/img,no_fix_zero=(NOT (*i).fix_color_zero)) $
                 ELSE img[img_rselect] = contour_levels(img_unnorm,$
                                                        min=(*i).data_minmax[0,sp],max=(*i).data_minmax[1,sp],$
                                                        n_levels,/img,no_fix_zero=(NOT (*i).fix_color_zero))
...
;                 charsize=0.75*csize, orientation=1
        ...
        IF (*i).png_out THEN BEGIN
           IF (*i).mode EQ 2 THEN BEGIN
              ...
              FOR x = 0, 1 DO BEGIN
                 hdat_norm = REFORM(contour_levels((*i).norm_local ? $
                     hull_data[*,*,x,sp,n] : (*i).data_minmax[*,sp],n_levels,/img,$
;                     n_levels,min=(*i).data_minmax[0,sp],max=(*i).data_minmax[1,sp],/img,$
                 ...
                 IF interpolate_to_theta THEN BEGIN
                    ...
                    FOR p = 0, (*i).phi_res - 1 DO BEGIN
                       old_theta[*,p] = $
                          [(*i).space_theta[*,x*((*i).x_res_red-1)],$
                           (*i).space_theta[0,x*((*i).x_res_red-1)]]
                       old_phi[*,p] = 2.0 * !PI * p * $
                                      (*i).phi_range / (*i).phi_res
                    ENDFOR
...
;                image[3,*,*] = 255B
                 ...
                 WRITE_PNG, png_name, image, /ORDER
              ENDFOR
              IF (framenr EQ 0) THEN torus_write_inc_file, diag, filename_run, framenr_str
           ENDIF
...
PRO torus_write_inc_file, diag, filename_run, framenr_str
  ...
  filename = gui.out.out_path+(*diag).name + filename_run + '_'+$
           framenr_str + '.inc'
  ;png_name_base = gui.out.out_path + (*diag).name + filename_run + '_'
  png_name_base = (*diag).name + filename_run + '_'
...
; (cases exist where the cuts do not perfectly fit)
  ...
  FOR icut = 0, (*i).n_cuts-1 DO BEGIN
     nturn = FIX((*i).phi_cut[icut]/(2.0*!PI))
     signstr = (((*i).phi_cut[icut]-nturn*2.0*!PI) GT !PI)?'':'-'
     PRINTF, lun, '// A plane, which gets cut '+rm0es(icut)+$
             ' at phi='+rm0es((*i).phi_cut[icut])+' projected onto.'
     PRINTF, lun, '#declare cut'+rm0es(icut)+' ='
     PRINTF, lun, 'plane { <0, 0, -1>, '+signstr+'offset1'
     PRINTF, lun, '  texture{'
     PRINTF, lun, '    pigment {'
     PRINTF, lun, '      image_map{'
  ...
  nturn = FIX((*i).phi_cut[(*i).n_cuts-1]/(2.0*!PI))
  nturn0 = FIX((*i).phi_cut[0]/(2.0*!PI))

  diff_phi = ((*i).phi_cut[(*i).n_cuts-1]-nturn*2.0*!PI-(*i).phi_cut[0]+nturn0*2.0*!PI)
  IF diff_phi LT 0.0 THEN diff_phi+=2.0*!PI

  IF diff_phi GT !PI THEN $
     PRINTF, lun, '    union {'
  FOR iicut=0,(diff_phi NE !PI) DO BEGIN
     icut = iicut*((*i).n_cuts-1)
     PRINTF, lun, '      box { <-max_r,min_z,-max_r+offset1>, <max_r,max_z,-offset1>'
     PRINTF, lun, '            translate <0.0,major_Z/Lref,0.0>'
...
;######################################################################
;compute radial and poloidal cuts, e.g. for benchmarking
;rpos : radial position for poloidal cut in meters(default:0.5);
;result will be adjusted such that the dominant 'm' mode has zero phase
;the radial cut will be a poloidal average of the squared signal
;NOTE: currently only implemented for circular model
PRO create_rad_theta_cuts, diag, data, c, n, time, rpos=rpos

  COMMON global_vars

  i = (*diag).internal_vars

  IF N_ELEMENTS(rpos) LT 1 THEN rpos = 0.5*par.minor_r*par.Lref
  ...
  commentline='phi = '+rm0es((*i).phi_cut[c]/!PI)+((*i).phi_cut[c] EQ 0 ? '' : $
              ' pi')+', t = '+(n_res_steps GT 1 ? rm0es(time[n]) : $
              rm0es(gui.out.start_t) + '-' + rm0es(gui.out.end_t))

  ;find index closest to rpos
  dmin = MIN(ABS(rpos-(*i).space_r[(*i).z_res/2,*]),ix)

  ;compute q value at that position for comparison
  xa = REFORM((*i).space_r[(*i).z_res/2,*])/(par.minor_r*par.Lref)
  xa_q = par.x0 + par.rhostar * (INDGEN(par.nx0) * par.dx - 0.5 * par.lx)
  q00 = 1.0 ;to initialize q00
  lag3interp,(*series.geom).q[*],xa_q,par.nx0,q00,xa[ix],1

; uncomment for straight field line angle representation
; theta = REFORM((*i).space_theta[*,ix])
; poldat = REFORM(data[*,ix])

  ;translate from straight field line to equidistant poloidal angle
  theta = 2.0*!PI*INDGEN((*i).z_res)/(*i).z_res-!PI
  poldat = INTERPOL(REFORM(data[*,ix]),$
                    REFORM((*i).space_theta[*,ix]),theta)

  ;fft poloidal signal, find maximum m mode and renormalize
  fftpol = FFT(REFORM(poldat))
  maxfft = MAX(ABS(fftpol),mmax)
  fftpol /= fftpol[mmax]
  ;reestablish reality condition
  FOR ind = (*i).z_res/2+1,(*i).z_res-1 DO $
     fftpol[ind] = CONJ(fftpol[(*i).z_res-ind])

  pol_info = 'x/a = '+rm0es(xa[ix],prec=4)+', adjusted to m = '+rm0es(mmax)+$
             '; q = '+rm0es(q00,prec=4)
  print, 'poloidal cut at '+pol_info

  invfftpol = FFT(fftpol,/INVERSE)
  ;normalize to maximum amplitude
  invfftpol /= MAX(ABS(invfftpol))

  PLOT, theta, invfftpol, color=1, /xstyle, xtitle='!N!7h!6!Ipoloidal!N',$
        ytitle=get_var_string((*i).var,/fancy)+'(x/a = '+rm0es(xa[ix])+$
        ') adjusted to m = '+rm0es(mmax)
  set_output, 'rad_theta_cuts_'+get_var_string((*i).var),sp, $
              header=['theta',get_var_string((*i).var)], $
              append=(c+n GT 0), commentline=[commentline,pol_info],$
              dat=[[theta],[FLOAT(invfftpol)]]

  radprof = FLTARR((*i).x_res)
  ;average over poloidal data (simple sum without Jacobian)
  FOR ix = 0, (*i).x_res-1 DO BEGIN
     poldat = INTERPOL(REFORM(data[*,ix]),$
                       REFORM((*i).space_theta[*,ix]),theta)
     radprof[ix] = TOTAL(ABS(poldat)^2)/(*i).z_res
  ENDFOR
  ;normalize to integrated area below curve
  radprof /= INT_TABULATED(xa,radprof)

  PLOT, xa, radprof, color=1, /xstyle, xtitle='x/a', $
        ytitle='!12<!9!!!6'+get_var_string((*i).var,/fancy)+$
        '!9!!!6!U2!N!12>!6'

  set_output, 'rad_theta_cuts_'+get_var_string((*i).var),sp, $
              header=['x/a','<|'+get_var_string((*i).var)+'|^2>'], $
              /append, commentline=commentline,dat=[[xa],[radprof]]
