  ...
  FOR isp = 0, gui.out.n_spec_sel - 1 DO BEGIN
    FOR ivar = 0, nvars - 1 DO BEGIN
      FOR iky = 0, nkyind - 1 DO BEGIN
        ...
        IF (nzind EQ par.nz0) THEN IF (par.x_local AND par.y_local) THEN $
           xtempdata *= REBIN(REFORM((*series.geom).jac_norm,[1,nzind]),[nkxpos,nzind]) $
        ELSE xtempdata *= REBIN(REFORM((*series.geom).jac_norm[0,*],[1,nzind]),[nkxpos,nzind])
      ...
      FOR ikx = 0, N_ELEMENTS((*i).kxind) - 1 DO BEGIN
        ...
        IF (nzind EQ par.nz0) THEN IF (par.x_local AND par.y_local) THEN $
           ytempdata *= REBIN(REFORM((*series.geom).jac_norm,[1,nzind]),[par.nky0,nzind]) $
        ELSE ytempdata *= REBIN(REFORM((*series.geom).jac_norm[0,*],[1,nzind]),[par.nky0,nzind])
...
PRO spectra_plot, xdata, ydata, log=log, position=position, $
  ...
  IF mymax GT minmax[1] THEN BEGIN
    mymax = minmax[1]
    IF warn THEN BEGIN
      PRINT, 'suppressed ' + xname + ' spectra values > ' + rm0es(mymax)
      warned = 1
    ENDIF
  ENDIF
  ...
  IF log THEN BEGIN
    ...
    IF mymin LT minmax[0] THEN BEGIN
      mymin = minmax[0]
      IF warn THEN BEGIN
        PRINT, 'suppressed ' + xname + ' spectra values < ' + rm0es(mymin)
        warned = 1
      ENDIF
    ENDIF
  ENDIF ELSE BEGIN
  ...
  IF log AND (TOTAL(fit) NE 0) THEN BEGIN
    ...
    FOR ivar = 0, n_vars - 1 DO BEGIN
      ...
      OPLOT, xdata[0.75*fit[0]*ndata:(1.25*fit[1]<1.0)*ndata<(ndata-1)],$
        10^fitres[0]*xdata[0.75*fit[0]*ndata:(1.25*fit[1]<1.0)*ndata<(ndata-1)]^fitres[1],$
  ...
  FOR isp = 0, gui.out.n_spec_sel - 1 DO BEGIN
    ...
    FOR ikx = 0, N_ELEMENTS((*i).kxind) - 1 DO BEGIN
      ...
      IF maxy GT (*i).minmax[1] THEN BEGIN
        maxy = (*i).minmax[1]
;        PRINT, 'suppressed ky spectra values > ' + rm0es(maxy)
      ENDIF
      IF miny LT (*i).minmax[0] THEN BEGIN
        logminy = (*i).minmax[0]
;        PRINT, 'suppressed ky spectra values < ' + rm0es(logminy)
      ENDIF
       ...
       FOR n = 0, n_res_steps - 1 DO BEGIN 
        IF par.nky0 GT (1 + (par.ky0_ind EQ 0)) THEN BEGIN    
          spectra_plot, ky_axes[*,ky_type], REFORM(ysumspec[*,ikx,*,isp,n]), $
            ...
            position=[0.15,0.596,0.95,0.9615], /log, warn=warn, $
        ...
        IF ky_type EQ 0 THEN BEGIN
         ...
         IF ((*i).zind NE -1) THEN commentline=[commentline,$
            '(R,Z) = ('+rm0es((*series.geom).R[(*i).zind],prec=3)+','+$
            rm0es((*series.geom).Z[(*i).zind],prec=3)+')m',$
            'k2fac_cm = '+rm0es(get_k2_fac((*i).zind)/rho_cm)+', '+'kthetafac_cm = '+$
            rm0es(get_ktheta_fac((*i).zind)/rho_cm),$
            'rhoref/rho(z) = '+rm0es((*series.geom).Bfield[(*i).zind])]
         commentline = [commentline,$
            'sqrt(sum(<|'+get_var_string((*i).vars)+'/'+get_var_string((*i).vars)+'0|^2>)) = '+$
            rm0es(SQRT(2.0*TOTAL(ysumspec[*,ikx,*,isp,n])-TOTAL(ysumspec[0,ikx,*,isp,n]))*$
                  par.rhostar*par.minor_r*100,prec=3)+'%']
    ...
    FOR iky = 0, N_ELEMENTS((*i).kyind) - 1 DO BEGIN
      ...
      FOR n = 0, gui.out.res_steps * (series.step_count - 1) DO BEGIN
        ...
        commentline = [commentline,$
            'sqrt(sum(<|'+get_var_string((*i).vars)+'/'+get_var_string((*i).vars)+'0|^2>)) = '+$
            rm0es(SQRT(2.0*TOTAL(xsumspec[*,iky,*,isp,n])-TOTAL(xsumspec[0,iky,*,isp,n]))*$
                  par.rhostar*par.minor_r*100,prec=3)+'%']

        set_output, diag, sp, header=['kx',get_var_string((*i).vars)], $
                    dat=[[kx],[REFORM(xsumspec[*,iky,*,isp,n])]], $
                    commentline=commentline, /append
        ENDFOR ; time loop
    ENDFOR ; ky loop
