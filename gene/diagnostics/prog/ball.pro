...
;  IF NOT KEYWORD_SET(*e.reim) THEN *e.reim = 0
  ...
  jbar = 0
  aux_jbar_fac = 1
  IF nxmod GT 1 THEN BEGIN      ;need to consider connections

     sign_shear = par.shat LT 0 ? -1 : 1
     nexc_sign = sign_shear*par.sign_Ip_CW*par.sign_Bt_CW

     IF (par.adapt_lx) THEN nexc = 1 $
     ELSE nexc = ROUND(par.lx*par.n_pol*ABS(par.shat)*par.kymin*ABS(Cyq0_x0))
     nexc *= nexc_sign

     ;jbar : index from -kx to kx (correcting for FFT alignment)
     jbar = INTARR(nxmod)
     jbar[0] = par.nx0 / 2 + 1 + INDGEN(nxmod/2)
     jbar[nxmod/2] = INDGEN(nxmod-nxmod/2)

     ;reverse indices if kx is coupled in negative direction
     IF nexc_sign LT 0 THEN jbar = REVERSE(jbar)
     ...
     IF reim THEN BEGIN
        ...
        ;phasefac per ky for f(kx,ky,z) = f(kx',ky,z+2pi)*phasefac
        ...
        FOR k = 0, par.nz0 -1 DO BEGIN
           FOR j = 0, nky - 1 DO BEGIN
              ;f(kx,ky,z+2pi) = f(kx',ky,z)*conjg(phasefac)
              aux_jbar_fac[nxmod/2+1:nxmod-1,j,k] = $
                   CONJ(phasefac[j])^ABS((INDGEN(nxmod-(nxmod/2+1))+1)/shift[j])

              ;f(kx,ky,z-2pi) = f(kx',ky,z)*(phasefac)
              aux_jbar_fac[0:nxmod/2-1,j,k] = $
                   (phasefac[j])^ABS((INDGEN(nxmod/2)-par.nx0/2)/shift[j])
...
;computes theta as used in the Miller definition for the Z coordinate
  ...
  IF par.sign_Ip_CW*theta[0] GT 0 THEN theta[0]-= par.sign_Ip_CW*2.0*!PI
  finiteth_ind=WHERE(FINITE(theta) EQ 0)
  IF (finiteth_ind GT 0) THEN $
     theta[finiteth_ind] = 0.5*!PI*SIGNARR(theta[finiteth_ind-1])
  B = theta
...
PRO ball_output, diag
  ...
  IF par.sign_Ip_CW*theta_pol[0] GT 0 THEN theta_pol[0]-= par.sign_Ip_CW*2.0*!PI
     ...
     FOR n = 0, (gui.out.res_steps AND NOT (*i).use_pod) * $
        ...
        FOR v = 0, n_varplots - 1 DO BEGIN
           ...
           FOR modeind = 0, nky - 1 DO BEGIN ; --- y mode loop
              bigN = FIX(ABS((*i).nexc * (*i).kyind[modeind]))

              ; plot indices: select 2/4/... pi ranges to plot
              ;pindc: number of total connections
              pindc = ((((par.nx0 - 1) / 2) / bigN) * 2 + 1)
              ; pinddim: number of points in the final curve
              pinddim = pindc * par.nz0
...
;             tppr = (2 * FINDGEN(pinddim) - pinddim) / par.nz0 * par.n_pol
              ...
              FOR p = 0, pindc-1 DO BEGIN
                 ...
                 tpol[p*par.nz0:(p+1)*par.nz0-1] = theta_pol/!PI+par.sign_Ip_CW*(-pindc/2+p)*2.0
                 IF (with_miller) THEN tpolm[p*par.nz0:(p+1)*par.nz0-1] = $
                    theta_mil/!PI+par.sign_Ip_CW*(-pindc/2+p)*2.0
              ENDFOR
...
;                plotdata_ext = (TRANSPOSE(TEMPORARY(V_matrix_tz)))[*,0:(*i).n_pod-1]
              ...
              FOR p = 0, ((*i).n_pod > 1) - 1 DO BEGIN
                 ...
                 IF (*i).reim THEN BEGIN
                    plotmin = plotmin < MIN(FLOAT(plotdata),MAX=plotmax1)$
                              < MIN(IMAGINARY(plotdata),MAX=plotmax2)
                    plotmax = plotmax > plotmax1 > plotmax2
                 ENDIF

                 fulltitle = fulltitle_base
                 IF (*i).use_pod THEN fulltitle += $
                    ': !6POD ' + rm0es(p+1) + '/' + rm0es((*i).n_pod)
                 ...
                 IF (*i).range[0] GE 0.0 THEN BEGIN ; plot zoomed region
                    ...
                    IF (*i).reim THEN BEGIN
                       plotmin = plotmin < MIN(FLOAT(plotdata[detail_range]),MAX=plotmax1)$
                                 < MIN(IMAGINARY(plotdata[detail_range]),MAX=plotmax2)
                       plotmax = plotmax > plotmax1 > plotmax2
                    ENDIF
