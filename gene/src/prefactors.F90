...
!         ((abs(Omega0_tor).ge.epsilon(Omega0_tor)).and.x_local)
             ...
             do k=lk1,lk2
                do j=pj1,pj2
                   ...
                   if ((with_omn_ext).and.(xy_local)) &
                        ...
                        &   C_xy(pi1)),k,l,m,n)
                   if ((with_kh_drive_ext).and.(xy_local)) then
                     call build_ext_contr_prefac(3,mu(m),k,l,m,n)
                     call build_ext_contr_prefac(4,fm(pi1,pj1,k,l,m,pn1),k,l,m,n)
                   end if
                   ! --- end external stuff ---
                enddo !j
             end do !k
  ...
  function edr(j,k,l,m,n)
    ...
    if (xy_local.and.pfsrate.ne.0.) then
       ...
       if (magn_geometry=='s_alpha'.or.magn_geometry=='s_alpha_B') then
          ...
          if (sign_Ip_CW.ne.0) then
             edr=edr+sign_Ip_CW*pfsrate*B_Bstar(j,k,l,n)*fm(:,j,k,l,m,pni)/&
    ...
    ! Here, it is where the kink term (uprime_arr) with its prefactors is added to the RHS of the GK equation
    if (kink_on) then
      edr=edr+uprime_arr(pi1:pi2,n)*fm(:,j,k,l,m,pni)/spec(n)%temp_prof(pi1:pi2)&
           *spec(n)%mass*vTvpar(l,n)*B_Bstar(j,k,l,n)/C_y(pi1:pi2)
    endif
  ...
  function coriolis_i(j,k,l,n)
    ...
    if (yx_order) then
       ...
       if (sign_Ip_CW.ne.0) coriolis_i = coriolis_i * sign_Ip_CW
    else
  ...
  function coriolis_j(j,k,l,n)
    ...
    if (yx_order) then
       ...
       coriolis_j = coriolis_j * C_y(pi1:pi2) * (q_prof(pi1:pi2) * gt_dot_gz + zval(k) *&
        &dqdx_prof(pi1:pi2) * geom%dxdZ(pi1:pi2,k))
       if (sign_Ip_CW.ne.0) coriolis_j = coriolis_j * sign_Ip_CW
    endif
  ...
  function centrifugal_i(j,k,l,m,n)
    ...
    do i=pi1,pi2
       ...
       if (sign_Ip_CW.ne.0) dydR(i)=dydR(i)*sign_Ip_CW
    enddo
