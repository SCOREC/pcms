...
Contains
  ...
  subroutine initialize_collisions
    ...
    integer:: i, j, k, l, m, n, sprime, ll, ul, lm, um
    logical:: active_spec
  ...
  function cfk1(vvar) result (erg)
    ...
    if (vvar.gt.25.) then !!avoid SIGFPE due to large arguments
  ...
  function cfk3(vvar) result (erg)
    ...
    case default
       if (vvar.gt.25) then !!avoid SIGFPE due to large arguments
          ...
          erg=2.0*erf(vvar)-4.0*vvar*exp(-vvar**2.0)/sqrt(pi)
       endif
    endselect
  end function cfk3
  ...
  function Hfunc(vvar) result (erg)
    real:: vvar,erg
    real:: erf
    if ((vvar/sqrt(2.0)).gt.25) then
       !erg=1./vvar**3*(1.0-sqrt(2./pi)*vvar*exp(-vvar**2/2.))
       erg=1./vvar**3
    else
