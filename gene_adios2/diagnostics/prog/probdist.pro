...
PRO probdist_output, diag
  ...
  z_values_avg = REFORM(time_avg((*i).z_values_id,/avg,$
    fwd=gui.out.res_steps,tarr=time),(*i).use_2d ? [(*i).bin_count_1,$
    (*i).bin_count_2,gui.out.n_spec_sel,1+gui.out.res_steps*$
    (series.step_count-1)] : [(*i).bin_count_1,gui.out.n_spec_sel,$
    1+gui.out.res_steps*(series.step_count-1)])
  ...
  FOR isp = 0, gui.out.n_spec_sel - 1 DO BEGIN
    ...
    FOR n = 0, gui.out.res_steps * (series.step_count - 1) DO BEGIN
      IF (*i).use_2d THEN BEGIN
        SURFACE, z_values_avg[*,*,isp,n], (*i).x_axis, (*i).y_axis, $
        ...
        set_output, diag, dat=[REFORM(z_values_avg[*,*,isp,n])], $
          append=(n GT 0)
      ENDIF ELSE BEGIN
        PLOT, (*i).x_axis, z_values_avg[*,isp,n], $
        ...
        set_output, diag, sp, header=[get_var_string((*i).var1),'count'], $
          dat=[[(*i).x_axis],[z_values_avg[*,isp,n]]], append=(n GT 0)
      ENDELSE
