...
Module diagnostics_fsa_moments
  ...
  use calc_rhs, only: calc_rhs_only, ptr_barchi, bar_emfields,&
  ...
  INTEGER,dimension(:),allocatable :: FSAMOMFILEN
  INTEGER,dimension(:),allocatable :: FSAMOMFILEP
  INTEGER,dimension(:),allocatable :: FSAMOMFILEE
  Logical :: write_prof_pe
  integer::block_nr = 0
  integer::n_fsa_mats = 3
...
!!!*******************public routines *******************************!!!
  ...
  SUBROUTINE check_diag_fsa_moments
    ...
    if (comp_type.eq.'EV') then
       if (istep_fsa_moments.gt.0) istep_fsa_moments = 1
    endif
  ...
  Real Function mem_est_diag_fsa_moments(mem_req_in)
    ...
    if (arakawa_zv.or.coll_on_h) mem_loc = mem_loc + SIZE_OF_COMPLEX_MB*lij0*lzvw0*ln0
...
!!!***********private subroutines ***********************************!!!
  subroutine initialize_diag_fsa_moments
    ...
    ALLOCATE(FSAMOMFILEE(ln1:ln2))
    ALLOCATE(FSAMOMFILEP(ln1:ln2))
    ALLOCATE(FSAMOMFILEN(ln1:ln2))
    ...
    if (write_prof_pe) then
       ...
       DO n=ln1,ln2
          call get_unit_nr(FSAMOMFILEE(n))
          OPEN(FSAMOMFILEE(n), file=trim(diagdir)//'/fsamome_'//&
               &trim(spec(n)%name)//''//trim(file_extension),&
               form='formatted', status='replace', position='rewind')
          ! write file header
          write(FSAMOMFILEE(n),"(A)") "# Energy (v**2) moments"
          write(FSAMOMFILEE(n),"(4A)")      "#   x/a             (2)tot Q        ",&
          ...
          call get_unit_nr(FSAMOMFILEP(n))
          OPEN(FSAMOMFILEP(n), file=trim(diagdir)//'/fsamomp_'//&
               &trim(spec(n)%name)//''//trim(file_extension),&
               form='formatted', status='replace', position='rewind')
          ! write file header
          write(FSAMOMFILEP(n),"(A)") "# Parallel momentum moments"
          write(FSAMOMFILEP(n),"(4A)")      "#   x/a             (2)tot Q        ",&
               "(3)tot dQ       (4)coll         (5)dchidxy      (6)f1_sources   ",&
               "(7)f1_buffer    (8)f0 term      (9)v_D f_1      (10)hyp_v       ",&
               "(11)hyp_z       (12)hyp_xy      (13)zv poisson"!  (14)nonlinearity"
          call get_unit_nr(FSAMOMFILEN(n))
          OPEN(FSAMOMFILEN(n), file=trim(diagdir)//'/fsamomn_'//&
               &trim(spec(n)%name)//''//trim(file_extension),&
               form='formatted', status='replace', position='rewind')
          ! write file header
          write(FSAMOMFILEN(n),"(A)") "# Density moments"
          write(FSAMOMFILEN(n),"(4A)")      "#   x/a             (2)tot Q        ",&
               "(3)tot dQ       (4)coll         (5)dchidxy      (6)f1_sources   ",&
               "(7)f1_buffer    (8)f0 term      (9)v_D f_1      (10)hyp_v       ",&
               "(11)hyp_z       (12)hyp_xy      (13)zv poisson"!  (14)nonlinearity"
       END DO
  ...
  Subroutine finalize_diag_fsa_moments
    ...
    if(write_std) then
       ...
       enddo
       deallocate(FSAMOMFILEE)
       deallocate(FSAMOMFILEP)
       deallocate(FSAMOMFILEN)
    end if
  ...
  subroutine get_fsamom_terms(g_1_in,fsamom_terms)
    ...
    if (arakawa_zv.or.coll_on_h) then
    ...
    ! calling calc_rhs_only will update some pointers (e.g., bar_emfields)
    ...
    do iterm=4,n_fsa_terms
       ...
       Do iblock=1,nblocks
          ...
          case(4)
             ...
             if (xy_local) then
                call add_dchidxy(fields_in,rhs_block,ptr_barchi,lbg1,lbg2,.true.)
...
if (hyp_on_h) then
                   ...
                   if (hypz_compensation) call equ_comp_hypz(fields_in,bar_emfields,rhs_block,lbg1,lbg2)
else
...
if (hyp_on_h) then
          ...
          case(12)
             ...
             if (arakawa_zv) then
                ...
                call add_dchidz(fields_in, bar_emfields, rhs_block, lbg1, lbg2)
             end if
          case(13)
             !nonlinearity
             if (rhs_nl) then
    ...
    deallocate(g_block)
    deallocate(rhs)
    if (arakawa_zv.or.coll_on_h) deallocate(h_in)
...
PERFOFF
  ...
  subroutine diag_fsa_moments
    real,dimension(li1:li2,ln1:ln2,1:n_fsa_terms,1:n_fsa_mats) :: fsamom_terms
...
PERFOFF
  ...
  !>Gathers the fsa data arrays and outputs as formatted data to file determined by filehandle.
  !!
  !> todo possibly switch to binary output instead of gnuplot compatible!!
  subroutine fsa_moments_write(fsamom_terms)
    !****
    implicit none
    real,dimension(li1:li2,ln1:ln2,1:n_fsa_terms,1:n_fsa_mats),intent(in) :: fsamom_terms
    Real, Dimension(0:nx0-1,1:n_fsa_terms,1:n_fsa_mats) :: out_arr

    integer::i, n, o, p, ierr
    ...
    do n=ln1,ln2
       if ((my_pey+my_pez+my_pev+my_pew).eq.0) then
          do o=1,n_fsa_terms
             do p=1,n_fsa_mats
                Call mpi_gather(fsamom_terms(li1,n,o,p), li0, MPI_REAL_TYPE,&
                                out_arr(0,o,p), li0, MPI_REAL_TYPE,&
                                0, mpi_comm_x, ierr)
             enddo
          enddo
          ...
          IF (write_prof_pe) THEN
             ...
             WRITE(FSAMOMFILEE(n), "(A,F14.6,1X,I5)") '#',time, block_nr
             WRITE(FSAMOMFILEP(n), "(A,F14.6,1X,I5)") '#',time, block_nr
             WRITE(FSAMOMFILEN(n), "(A,F14.6,1X,I5)") '#',time, block_nr

             do i=0,nx0-1
                write(FSAMOMFILEE(n), "(13ES16.6)") xval_a(i), out_arr(i,:,1)
                write(FSAMOMFILEP(n), "(13ES16.6)") xval_a(i), out_arr(i,:,2)
                write(FSAMOMFILEN(n), "(13ES16.6)") xval_a(i), out_arr(i,:,3)
             enddo

             WRITE(FSAMOMFILEE(n), "(A)") ''
             WRITE(FSAMOMFILEE(n), "(A)") ''
             WRITE(FSAMOMFILEP(n), "(A)") ''
             WRITE(FSAMOMFILEP(n), "(A)") ''
             WRITE(FSAMOMFILEN(n), "(A)") ''
             WRITE(FSAMOMFILEN(n), "(A)") ''

             block_nr = block_nr + 1

            call flush(FSAMOMFILEE(n))
            call flush(FSAMOMFILEP(n))
            call flush(FSAMOMFILEN(n))
          ENDIF
       endif
    enddo
