...
!  use initial_value_comp, only: check_initial_value
  use compute_dt
  use external_contr, only: ExBrate, pfsrate, lilo_w_full_omegator_profile, kink_on
  ...
  use file_io
  use profiles_mod, only: drive_buffer
...
contains
  ...
  subroutine check_par_phys
    ...
    if ((x_local).or.(lilo)) then
       ...
       !mag_prof=.t. currently breaks the boundary_exchange_z
       !initialisation for lilo
       if (mag_prof.and.lilo) &
            stop "mag_prof=.t. currently not allowed for lilo!"
    end if
    ...
    if (any((/sign_Ip_CW,sign_Bt_CW,sign_Omega_CW/).ne.0).and.magn_geometry.ne.'tracer_efit'.and.&
         magn_geometry.ne.'miller'.and.magn_geometry.ne.'circular'.and.&
         magn_geometry.ne.'s_alpha'.and.magn_geometry.ne.'s_alpha_B'.and.&
         magn_geometry.ne.'gene') &
         stop "sign_Ip_CW, sign_Bt_CW, sign_Omega_CW not implemented for this magn_geometry"
    if (magn_geometry.eq.'tracer_efit'.or.magn_geometry.eq.'miller'.or.&
         &magn_geometry.eq.'circular'.or.magn_geometry.eq.'s_alpha') then
       if (sign_Bt_CW.ne.0.and.sign_Ip_CW.ne.0) then
          if (write_pe) write (*,"(A,I2,A,I2)") &
               "Signs for magnetic field (1=clockwise, -1=counter-clockwise): B_t: ",sign_Bt_CW,&
               ", I_p: ",sign_Ip_CW
       else if (sign_Bt_CW.eq.0.and.sign_Ip_CW.eq.0) then
          if (write_pe) write (*,"(A)") &
               "Signs for magnetic field (1=clockwise, -1=counter-clockwise): B_t: 1, I_p: 1"
          if (magn_geometry.eq.'miller'.or.magn_geometry.eq.'circular'.or.&
               &magn_geometry.eq.'s_alpha'.or.magn_geometry.eq.'s_alpha_B') then
             sign_Bt_CW = 1
             sign_Ip_CW = 1
          endif !keep old tracer_efit implementation for now
       else
          stop "Must specify both signs for Bt and Ip!"
       endif
       if (sign_Omega_CW.ne.0.and.write_pe) write (*,"(A,I2)") "Sign of toroidal rotation: ",sign_Omega_CW
    endif
    if (xy_local.and.nonlinear.and.yx_order) then
       if (write_pe) Write(*,"(A)") "yx_order not implemented in nonlinear local runs--exit!!"
       stop
    endif

    if (kink_on.and.beta.eq.0) stop "kink term does not work with beta = 0"
    if (kink_on.and.magn_geometry.ne.'tracer_efit') stop 'kink term only works with tracer_efit interface'
...
!    call check_initial_value
    ...
    if (.not.only_zonal) then
       if ((.not.adapt_ly.or.n0_global.eq.-1111).and.(kymin.le.0.0)) &
            &STOP "kymin must be larger than zero"
       if (adapt_ly.and.(n0_global.le.0.and.n0_global.ne.-1111)) &
            &STOP "n0_global must be larger than zero"
    endif
    ...
    !present implementation will set gradients to zero for lilo
    if (lilo.and.drive_buffer) &
         stop "drive_buffer not implemented for lilo"
    ...
    if ((parscheme.eq.'u3rd').and.(hyp_z.gt.0.0)) then
       hyp_z = 0.0
       if (write_pe) write(*,"(A)") "WARNING: parscheme='u3rd' contains "//&
            "z hyperdiffusion implicitly; setting hyp_z=0.0 accordingly"
    endif

    if ((parscheme.eq.'u3rd').and.(arakawa_zv)) then
       if (write_pe) write(*,"(A)") "WARNING: parscheme='u3rd' only "//&
            "possible with arakawa_zv=F; please set arakawa_zv accordingly"
       stop
    endif
    ...
    !however, if SLEPc is not used, hypz_compensation is used from input (default: T)
    ...
    if ((.not.x_local).and.(rad_bc_type.eq.2)) then
       if(write_pe) then
          write(*,"(A)") "WARNING: Neumann b.c. not implemented yet for GDAGGER_G"
    ...
    if (GyroLES.and..not.nonlinear) then
       if (write_pe) write(*,"(A)") "Switch off GyroLES for linear simulation"
       GyroLES = .false.
    endif
