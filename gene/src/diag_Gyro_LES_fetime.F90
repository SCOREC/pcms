...
!!!*****************************************************************************************!!!
  Subroutine diagnostic_fe_time(call_number)
    ...
    if (call_number.eq.2) then
       ...
       ! tisd: I assume only local runs here
       call add_dchidxy(emfields, temp_rhs,ptr2,lbg1,lbg2,.true.)
       ...
       if (collision_op.ne.'none') then
          if (coll_on_h) then
             !collisions on h_
             call equ_collisions(h_,temp_rhs,replace_rhs=.true.)
          else
             !collisions on f_
       ...
       if (.not.arakawa_zv) then
         call calc_aux_fields(g_1,emfields,f_,comp_h=.true.,h_out=h_)
       endif
       ...
       do n=ln1,ln2
         ...
         do m=lm1,lm2
           do l=ll1,ll2
             do k=lk1,lk2
                 temp_fe6d(:,:,k,l,m,n)=spec(n)%dens*spec(n)%temp*&
                 (abs(h_(:,:,k,l,m,n))**2/2/fm(pi1,pj1,k,l,m,pni)-spec(n)%charge/spec(n)%temp*emfields(:,:,k,1)*&
                 jfac(:,:,k,m,n)*conjg(h_(:,:,k,l,m,n))+(spec(n)%charge/spec(n)%temp)**2*fm(pi1,pj1,k,l,m,pni)/2*&
                 abs(emfields(:,:,k,1))**2)
             enddo
           enddo
         enddo
         ...
         Call integral_vpw(temp_fe6d(:,:,:,:,:,n),e_n_temp(:,:,:,n))
         if(p_has_00_mode) e_n_temp(0,0,:,n)=cmplx(0.0,0.0)
         call sum_int_3d(e_n_temp(:,:,:,n),e_tot_n(n,2,3))
       enddo
