...
Module communications
  USE par_mod, only: my_mpi_comm_world_member
  USE discretization, ONLY: n_procs_x,n_procs_y,n_procs_z,n_procs_v,&
       & n_procs_w,n_procs_s, n_procs_sim, n_parallel_sims,&
       &min_npx,max_npx,min_npy,max_npy,min_npz,max_npz,&
       &min_npv,max_npv,min_npw,max_npw,min_nps,max_nps,&
       &mype, my_pespec, my_pew, my_pez, my_pey, my_pev, my_pex, my_sim, mype_gl
  USE par_other, ONLY: auto_parall,print_ini_msg,par_in_dir,xy_local
  USE par_in, ONLY: rad_bc_type, file_extension
  Use mpi
  USE MatrixModule
  use VectorModule
  ...
  public:: my_barrier, calculate_test_sum, get_systime, my_secnds, my_real_get, pexyzvwspec, &
       ...
       &my_real_sum_vwspec, my_complex_sum_wspec, my_complex_sum_v, my_sum_to_all_int
  public :: debug_output
  public::my_real_gather_to_0,my_complex_gather_to_0,t_n_procs
  public::my_real_sum_w_0d, my_sum_to_all_real_0d, my_real_bcast, my_realarr_bcast
  public:: mpi_comm_x, mpi_comm_y, mpi_comm_z,  mpi_comm_yz, mpi_comm_xyz, mpi_comm_v,&
       ...
       mpi_comm_xvw, mpi_comm_zvw, mpi_comm_xyzvwspec, mpi_comm_world, mpi_comm_xyzw, mpi_comm_xy, &
       mpi_comm_zvwspec
  ...
  enumerator :: COMM_VWSPEC, COMM_XYZ, COMM_XYZVW, COMM_XVW
  endenum
#else
  ...
  INTEGER, PARAMETER :: COMM_XVW=13
#endif

  Integer:: mpi_comm_x, mpi_comm_y, mpi_comm_z,&
       &mpi_comm_v, mpi_comm_w, mpi_comm_w_spec, mpi_comm_zvw, mpi_comm_xvw
  ...
  interface debug_output
     module procedure debug_output_complex_2D,debug_output_complex_3D
     module procedure debug_output_complex_4D,debug_output_complex_5D,debug_output_complex_6D
     module procedure debug_output_real_3D
     module procedure debug_output_real_5D,debug_output_real_6D
  end interface debug_output

  INTERFACE calculate_test_sum
     MODULE PROCEDURE calculate_test_sum_2D,calculate_test_sum_3D,calculate_test_sum_4D
     module procedure calculate_test_sum_5D,calculate_test_sum_6D
     module procedure calculate_test_sum_real_3D,calculate_test_sum_real_5D
     module procedure calculate_test_sum_real_6D
  END INTERFACE
...
!    endif
  ...
  Subroutine split_comm
    ...
    if (my_mpi_comm_world_member) then
       ...
       remains_dim(COMM_V)=.TRUE.
       remains_dim(COMM_W)=.TRUE.
       CALL mpi_cart_sub(comm_cart,remains_dim,mpi_comm_xvw,ierr)
       remains_dim=.FALSE.
       remains_dim(COMM_X)=.TRUE.
       remains_dim(COMM_Y)=.TRUE.
       remains_dim(COMM_Z)=.TRUE.
  ...
  Subroutine free_comm
    ...
    if (my_mpi_comm_world_member) then
       ...
       Call mpi_comm_free (mpi_comm_xvw, ierr)
       If (ierr /= 0) Stop 'mpi_comm_free_xvw failed!'
  ...
  subroutine my_real_bcast(areal,pe)
    ...
    integer:: ierr
    call mpi_bcast(areal,1,MPI_REAL_TYPE,&
         &pe,MY_MPI_COMM_WORLD,ierr)

  end subroutine my_real_bcast

  subroutine my_realarr_bcast(areal,len,pe)
    integer :: len
    real, intent(inout):: areal(len)
    integer, intent(in):: pe
    integer:: ierr
    call mpi_bcast(areal(1),len,MPI_REAL_TYPE,&
         &pe,MY_MPI_COMM_WORLD,ierr)

  end subroutine my_realarr_bcast
  ...
  Subroutine my_real_sum_vwspec(localsum,len)
    Integer, Intent(In):: len
    Real, Intent(Inout):: localsum(len)
    Real               :: totalsum(len)
    Integer:: ierr
    ...
    Call mpi_allreduce( &
         localsum(1), totalsum(1), len, &
         MPI_REAL_TYPE, MPI_SUM, mpi_comm_vwspec, ierr)
    localsum = totalsum
    PERFOFF
  End Subroutine my_real_sum_vwspec
  ...
  ! =========================================================================
  ! == debug_output routines
  ! =========================================================================
#if defined(__GFORTRAN__) || defined(__CRAY__)
#define MAKE_SUBNAME(RC,DIMENSIONALITY) debug_output_/**/RC/**/_/**/DIMENSIONALITY
#else
#define MAKE_SUBNAME(RC,DIMENSIONALITY) debug_output_ ## RC ## _ ## DIMENSIONALITY
#endif
#define REAL_COMPLEX complex
#define SUBNAME MAKE_SUBNAME(complex,2D)
#define DB_OUT_DIM :,:
#include "debug_output_template.inc"

#define SUBNAME MAKE_SUBNAME(complex,3D)
#define DB_OUT_DIM :,:,:
#include "debug_output_template.inc"

#define SUBNAME MAKE_SUBNAME(complex,4D)
#define DB_OUT_DIM :,:,:,:
#include "debug_output_template.inc"

#define SUBNAME MAKE_SUBNAME(complex,5D)
#define DB_OUT_DIM :,:,:,:,:
#include "debug_output_template.inc"

#define SUBNAME MAKE_SUBNAME(complex,6D)
#define DB_OUT_DIM :,:,:,:,:,:
#include "debug_output_template.inc"

#undef REAL_COMPLEX
#define REAL_COMPLEX real
#define SUBNAME MAKE_SUBNAME(real,3D)
#define DB_OUT_DIM :,:,:
#include "debug_output_template.inc"

#define SUBNAME MAKE_SUBNAME(real,5D)
#define DB_OUT_DIM :,:,:,:,:
#include "debug_output_template.inc"

#define SUBNAME MAKE_SUBNAME(real,6D)
#define DB_OUT_DIM :,:,:,:,:,:
#include "debug_output_template.inc"
  ...
  subroutine calculate_test_sum_real_6D(arr,local_sum,global_sum)
    real, dimension(:,:,:,:,:,:),intent(IN) :: arr
    REAL,INTENT(OUT) :: local_sum, global_sum
    ...
    local_sum = SUM(arr*arr)
    CALL mpi_reduce(local_sum,global_sum,1,MPI_REAL_TYPE,MPI_SUM,0,MY_MPI_COMM_WORLD,ierr)
  end subroutine calculate_test_sum_real_6D

  subroutine calculate_test_sum_real_3D(arr,local_sum,global_sum)
    real, DIMENSION(:,:,:),INTENT(IN) :: arr
    REAL,INTENT(OUT) :: local_sum, global_sum
    ...
    local_sum = SUM(arr*arr)
    CALL mpi_reduce(local_sum,global_sum,1,MPI_REAL_TYPE,MPI_SUM,0,MY_MPI_COMM_WORLD,ierr)
  end subroutine calculate_test_sum_real_3D
