...
Module diagnostics_auxiliary
  ...
  PUBLIC :: pTj_qjB0
  PUBLIC :: calc_moments_trap_pass, calc_gyrocenter_moments
...
!!!******************************************************************!!!
  ...
  SUBROUTINE calc_moments_trap_pass(n_mom,p_dist,p_mom,n,diag_trap_levels,diag_Blev)
    ...
    real ::energy
    INTEGER:: i,j,k,l,m,i_mom,joff,koff,loff,moff,noff,trap_level
  ...
  ! gyrocenter flux, global only
  SUBROUTINE calc_gyrocenter_moments(n_mom,p_dist,p_mom,n)
    Implicit none
    INTEGER, INTENT(in):: n_mom
    !LOGICAL, INTENT(in):: withbounds
    integer, intent(in) :: n
    COMPLEX, DIMENSION(:,:,:,:,:),INTENT(in):: p_dist
    COMPLEX, DIMENSION(li1:li2,lj1:lj2,lk1:lk2,lm1:lm2,1:n_mom),INTENT(out):: p_mom
    !INTEGER, DIMENSION(1:n_mom), optional :: p_gy_op

    ! Local variables
    INTEGER:: i,j,k,l,m,i_mom,joff,koff,loff,moff,noff,trap_level

    PERFON('calc_gyrocenter_moments')
    ...
    DO m=lm1,lm2
       DO l=ll1,ll2
          DO k=lk1,lk2
             p_mom(:,:,k,m,1) = p_mom(:,:,k,m,1) + diag_gyroc_mom(p_dist,mat_00(:,:,k,l,m),&
                                 & k,l,m,n,joff,koff,loff,moff)
             p_mom(:,:,k,m,2) = p_mom(:,:,k,m,2) + diag_gyroc_mom(p_dist,mat_20(:,:,k,l,m),&
                                 & k,l,m,n,joff,koff,loff,moff)
             p_mom(:,:,k,m,3) = p_mom(:,:,k,m,3) + diag_gyroc_mom(p_dist,mat_01(:,:,k,l,m),&
                                 & k,l,m,n,joff,koff,loff,moff)
             p_mom(:,:,k,m,4) = p_mom(:,:,k,m,4) + diag_gyroc_mom(p_dist,mat_30(:,:,k,l,m,n),&
                                 & k,l,m,n,joff,koff,loff,moff)
             p_mom(:,:,k,m,5) = p_mom(:,:,k,m,5) + diag_gyroc_mom(p_dist,mat_11(:,:,k,l,m,n),&
                                 & k,l,m,n,joff,koff,loff,moff)
             p_mom(:,:,k,m,6) = p_mom(:,:,k,m,6) + diag_gyroc_mom(p_dist,mat_10(:,:,k,l,m,n),&
                                 & k,l,m,n,joff,koff,loff,moff)
          ENDDO
       ENDDO
    ENDDO
  ...
  END SUBROUTINE calc_gyrocenter_moments

  function diag_gyroc_mom(p_dist,p_mat,k,l,m,n,joff,koff,loff,moff) result(p_mom)
    COMPLEX, DIMENSION(:,:,:,:,:),INTENT(in):: p_dist
    COMPLEX, DIMENSION(li1:li2,lj1:lj2) :: p_mom
    REAL, DIMENSION(pi1:pi2,pj1:pj2),INTENT(in):: p_mat
    integer, intent(in) :: k,l,m,n
    integer, intent(in) :: joff,koff,loff,moff
    ...
    do j=lj1,lj2
       p_mom(:,j)= p_dist(:,j+joff,k+koff,l+loff,m+moff) &
                  & * p_mat(:,pj1)
    enddo

  end function diag_gyroc_mom
