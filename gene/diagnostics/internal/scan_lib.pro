...
FUNCTION sortmethstr, meth
  CASE meth OF
     1 : RETURN, 'smallest dist in complex plane'
     2 : RETURN, 'sort by size of imaginary part'
     3 : RETURN, 'hybrid method'
     4 : RETURN, 'squared method1'
...
;chosen will be vglval=MINIMUM
  CASE meth OF
    1 : BEGIN ; smallest distance in the complex plane
        ...
        END
    2 : BEGIN ;sort imaginary part by size
          vglval=N_ELEMENTS(arr2)
          FOR j=1,N_ELEMENTS(arr2)-1 DO BEGIN
...
PRO read_scanlog
  ...
  dimarr=STRSPLIT(line,'|',COUNT=ndims,/EXTRACT)
  ndims=ndims-1
  eigenarr=STRSPLIT(dimarr[ndims],'/',COUNT=eigenvals,/EXTRACT)
  eigenvals = eigenvals -1
  dimarr[ndims]=eigenarr[0]
  FOR indm=1,ndims DO BEGIN
     splitnames=STRSPLIT(dimarr[indm],' ',/EXTRACT)
     IF (MAX(STRCMP(splitnames[0],['omn','omt','mass','charge','temp','dens'])) EQ 1) THEN BEGIN
        dimarr[indm]=splitnames[0]+'_'+spec[splitnames[1]-1].name
     ENDIF ELSE BEGIN
        dimarr[indm]=splitnames[0]
     ENDELSE
  ENDFOR
  dsize=REPLICATE(1,ndims+1)      ;resolution of dimension (number of steps)&num of eigenvalues
  deltad=MAKE_ARRAY(ndims,/FLOAT) ;stepwidth of parameters
  scanpars=MAKE_ARRAY(ndims,/FLOAT) ;save all scanpars
  ...
  scanpars=el[1:ndims]
  rd=DINDGEN(ndims)
  FOR i=1,ndims DO rd[i-1]=el[i] ;end of Interval
  ndsize = 1
  ...
  ;sanity check in case kymin = 0.0 in incomplete scans
  IF (ndsize NE PRODUCT(dsize,/INTEGER)) THEN BEGIN
     kyminind = WHERE(dimarr EQ 'kymin')
     IF (kyminind[0] NE -1) THEN BEGIN
        dsize[kyminind] = ndsize/$
          PRODUCT(dsize[WHERE(dimarr NE 'kymin')],/INTEGER)
     ENDIF
  ENDIF
  ...
  axxis=MAKE_ARRAY(ndims, MAX(dsize), /FLOAT)
  axxindex=MAKE_ARRAY(ndims, /INTEGER)

  sortmeth = gui.out.sortmeth
  printerror, 'scan sort method = '+$
     rm0es(sortmeth)+' ('+sortmethstr(sortmeth)+')'

  OPENR,scanlog_lun,gui.out.data_path+'scan.log',/GET_LUN,ERROR=er
  IF err NE 0 THEN PRINT, 'not found'
  READF,scanlog_lun,line

  ;compare last file modification time stamp
  finfo_scanlog = FILE_INFO(gui.out.data_path+'scan.log')
  finfo_scansort = FILE_INFO(gui.out.data_path+'scansort.log')

  err = -1
  IF finfo_scansort.EXISTS THEN $
     IF (finfo_scansort.MTIME LT finfo_scanlog.MTIME) THEN $
        print, 'ignoring scansort.log - older than scan.log!' $
     ELSE err = 0

  IF err EQ 0 THEN BEGIN
     i=0
     OPENR,scansort_lun,gui.out.data_path+'scansort.log',/GET_LUN
  ...
  IF (sortmeth EQ 0) AND (err EQ 0) THEN BEGIN
      ...
      WHILE NOT EOF(scanlog_lun) DO BEGIN
          READF,scanlog_lun,line
          estr=STRTRIM(STRSPLIT(line,'|',COUNT=sortentries,/EXTRACT))
          IF axxindex[0] EQ -1 THEN BEGIN
              FOR i=0, ndims-1 DO BEGIN
                  axxis[i, 0]=FLOAT(estr[i+1])
                  axxindex[*]=0
              ENDFOR
          ENDIF ELSE BEGIN
             FOR i=0, ndims-1 DO BEGIN
                diffvals = FLOAT(estr[i+1])-axxis[i, axxindex[i]]
                IF (diffvals NE 0.0) THEN IF (SIGNUM(deltad[i])*SIGNUM(diffvals) GT 0) $
                   THEN BEGIN
                   axxindex[i]+=1
                   axxis[i, axxindex[i]]=FLOAT(estr[i+1])
                ENDIF ELSE BEGIN
                   axxindex[i]=0
                   axxis[i, axxindex[i]]=FLOAT(estr[i+1])
                ENDELSE
          ...
          FOR i=0,dsize(0)-1 DO BEGIN
              IF sortfield(evnum) LT 0 THEN BEGIN
                  estrev=STRTRIM(STRSPLIT(estr[ndims-sortfield(evnum)],' ',/EXTRACT))
          ...
          ENDFOR
      ENDWHILE
  ENDIF ELSE BEGIN ;read scan.log and fill sortfield
  ...
  ;check
  IF ((WHERE(dimarr EQ 'kymin'))[0] NE -1) THEN $
     printerror, 'WARNING: GENE CHANGES OF SCAN PARAMETERS (KYMIN) ARE NOT TAKEN INTO ACCOUNT'

  ;return data
   scanlog = PTR_NEW({$
    dimarr     : dimarr,$     ; names of the scanned parameters; starting with index 1
    dsize      : dsize,$      ; [number of eigenvalues, number of parameters in every dimension]
    deltad     : deltad,$     ; size of parameter steps
    rd         : rd,$         ; largest parameter of every dimension
    n_scans    : FIX(el[0]),$ ; number of scans/files
    eigenfield : eigenfield,$ ; complex array containing the eigenvalues
    sortfield  : sortfield,$  ; integer array containing the sortinformation
                              ;    0 means (0.0,NaN) (negative eigenvalue found)
                              ;    1 means (NaN,NaN) (no eigenvalue found)
    scanpars   : scanpars,$   ; float array of input parameters
    axxis      : axxis})      ; contains scan parameters (axis for plots)
...
;      print, 'reading last time step'
     ...
     ENDIF ELSE BEGIN  ;eigen value solver
        ...
        IF (last_time EQ 0.0) THEN CONTINUE ;RETURN
     ENDELSE
    ...
    ENDIF ELSE BEGIN
       last_time = PTR_VALID(nrg)?(*nrg)[N_ELEMENTS((*nrg))-1].time : 0.0
...
PRO scan_plotprep, diag, k, xax, yax, zax, plotorderr, parindex, mytitlex,$
  ...
  FOR m = 1,N_ELEMENTS((*scanlog).dsize) - 1 DO BEGIN
     ...
     IF (*i).plotparA[parindex] EQ '*' THEN BEGIN
        (*i).plotparA[parindex]='0:'+rm0es((*scanlog).dsize[parindex+1]-1)
     ENDIF
     interval=$
...
;                    interval[0]-1)*(*scanlog).deltad[m-1])) +')'
                    rm0es((*scanlog).axxis[m-1,interval[0]])+')'
     ENDIF ELSE BEGIN
        IF interval[0] EQ interval[1] THEN BEGIN
           (*i).plotparA[parindex]=rm0es(interval[0])+':'+rm0es(interval[0])
           k=k-1
           mytitletot+=' ('+(*scanlog).dimarr[m]+'='+$
;              rm0es((*scanlog).rd[m-1]-$
...
;  i = (*diag).internal_vars
  ...
  PRINTF,scanriem_lun, '# '+(*scanlog).dimarr[dsizepos[0]]+' '+$
         (*scanlog).dimarr[dsizepos[1]]+' remod1 remod2 immod1 immod2'
  sizearr=0
...
;first dimension mode 0
  FOR ri=0, sizearr[1]-1 DO BEGIN
     ...
     FOR rj=0, sizearr[2]-1 DO BEGIN
        ...
        PRINTF, scanriem_lun,(*scanlog).axxis[dsizepos[0]-1,ri],(*scanlog).axxis[dsizepos[1]-1,rj], $
                FLOAT(riemarr[0,ri,rj]), IMAGINARY(riemarr[0,ri,rj])
        IF withiplot EQ 1 THEN BEGIN
           iplotarr[*,rj]=[(*scanlog).axxis[dsizepos[0]-1,ri],(*scanlog).axxis[dsizepos[1]-1,rj], $
                           FLOAT(riemarr[0,ri,rj]), IMAGINARY(riemarr[0,ri,rj])]
        ENDIF
     ENDFOR
     IF withiplot EQ 1 THEN BEGIN
...
;first dimension mode 1
  FOR ri=0, sizearr[1]-1 DO BEGIN
     ...
     FOR rj=0, sizearr[2]-1 DO BEGIN
        IF (rj GT 0) THEN riemarr[*,ri,rj]=riemsort(riemarr[*,ri,rj], riemarr[*,ri,rj-1])
        PRINTF, scanriem_lun,(*scanlog).axxis[dsizepos[0]-1,ri],(*scanlog).axxis[dsizepos[1]-1,rj],$
                FLOAT(riemarr[1,ri,rj]), IMAGINARY(riemarr[1,ri,rj])
        IF withiplot EQ 1 THEN BEGIN
           iplotarr[*,rj]=[(*scanlog).axxis[dsizepos[0]-1,ri],(*scanlog).axxis[dsizepos[1]-1,rj],$
                           FLOAT(riemarr[1,ri,rj]), IMAGINARY(riemarr[1,ri,rj])]
        ENDIF
  ...
  FOR rj=0, sizearr[2]-1 DO BEGIN
     ...
     FOR ri=0, sizearr[1]-1 DO BEGIN
        ...
        PRINTF, scanriem_lun,(*scanlog).axxis[dsizepos[0]-1,ri],(*scanlog).axxis[dsizepos[1]-1,rj],$
                FLOAT(riemarr[0,ri,rj]), IMAGINARY(riemarr[0,ri,rj])
        IF withiplot EQ 1 THEN BEGIN
           iplotarr[*,ri]=[(*scanlog).axxis[dsizepos[0]-1,ri],(*scanlog).axxis[dsizepos[1]-1,rj],$
                           FLOAT(riemarr[0,ri,rj]), IMAGINARY(riemarr[0,ri,rj])]
        ENDIF
     ENDFOR
     IF withiplot EQ 1 THEN BEGIN
...
;start
  ...
  WHILE(xri LE sizearr[1]-1) DO BEGIN
     PRINTF, scanriem_lun,(*scanlog).axxis[dsizepos[0]-1,xri],(*scanlog).axxis[dsizepos[1]-1,xrj],$
             FLOAT(riemarr[0,xri,xrj]), IMAGINARY(riemarr[0,xri,xrj])
     IF withiplot EQ 1 THEN BEGIN
        iplotarr=MAKE_ARRAY([4,MIN([sizearr[2]-1-xrj,sizearr[1]-1-xri])+1],/DOUBLE)
        iplotarr[*,0]=[(*scanlog).axxis[dsizepos[0]-1,xri],(*scanlog).axxis[dsizepos[1]-1,xrj],$
                       FLOAT(riemarr[0,xri,xrj]), IMAGINARY(riemarr[0,xri,xrj])]
     ENDIF
     FOR xi=1, MIN([sizearr[2]-1-xrj,sizearr[1]-1-xri]) DO BEGIN
        ...
        PRINTF, scanriem_lun,(*scanlog).axxis[dsizepos[0]-1,ri],(*scanlog).axxis[dsizepos[1]-1,rj],$
                FLOAT(riemarr[0,ri,rj]), IMAGINARY(riemarr[0,ri,rj])
        IF withiplot EQ 1 THEN BEGIN
           iplotarr[*,xi]=[(*scanlog).axxis[dsizepos[0]-1,ri],(*scanlog).axxis[dsizepos[1]-1,rj],$
                           FLOAT(riemarr[0,ri,rj]), IMAGINARY(riemarr[0,ri,rj])]
        ENDIF
     ENDFOR
     IF withiplot EQ 1 THEN BEGIN
...
;start
  ...
  WHILE(xri LE sizearr[1]-1) DO BEGIN
     PRINTF, scanriem_lun,(*scanlog).axxis[dsizepos[0]-1,xri],(*scanlog).axxis[dsizepos[1]-1,xrj],$
             FLOAT(riemarr[1,xri,xrj]), IMAGINARY(riemarr[1,xri,xrj])
     IF withiplot EQ 1 THEN BEGIN
        iplotarr=MAKE_ARRAY([4,MIN([sizearr[2]-1-xrj,sizearr[1]-1-xri])+1],/DOUBLE)
        iplotarr[*,0]=[(*scanlog).axxis[dsizepos[0]-1,xri],(*scanlog).axxis[dsizepos[1]-1,xrj],$
                       FLOAT(riemarr[1,xri,xrj]), IMAGINARY(riemarr[1,xri,xrj])]
...
PRO scan_plot, diag, totaltitle, field2plot, reimabs,$
               pos=pos,$ ;[x1,y1,x2,y2] for plot (optional)
               legend_pos=legend_pos,$ ;[x1,y1,x2,y2] for legend (optional)
               multi=multi,$ ;
               sp = sp, $    ;species index (for output file names)
               suffix=suffix, $
               outset=outset ;2 plot per call; 1/-1/0 first/last/middle call of series
  ...
  IF N_ELEMENTS(outset) LT 1 THEN outset=2
  ...
  WHILE reim GT 0 DO BEGIN
      ...
      REPEAT BEGIN
		  ...
		  plotfield=FLOAT(plotfield)
              ...
              ENDELSE
          ENDELSE
          plotfield=REFORM(plotfield) ;discard dispensable dimensions
          xax=REFORM(xax) ;discard dispensable dimensions
          yax=REFORM(yax) ;discard dispensable dimensions
          zax=REFORM(zax) ;discard dispensable dimensions
          same=1
          ...
          CASE k OF
              0: BEGIN
	              ...
	              filearr=[[plotfield]]
                      filenames=[mytitle+STRING(reim MOD (*scanlog).dsize[0]) ]
                  ENDIF ELSE BEGIN
              ...
              1: BEGIN
                  ...
                  IF outputset EQ 0 THEN BEGIN
                    IF (outset EQ 1) OR (outset EQ 2) THEN BEGIN
                       set_output, diag, sp, /ps, multi=multi, suffix=suffix
                      ENDIF
                      outputset=1
                  ENDIF
                  IF SIZE(filearr,/N_ELEMENTS) EQ 1 THEN BEGIN
                  ...
                  ENDIF ELSE BEGIN
                     IF KEYWORD_SET(pos) THEN $
                        PLOT, xax,plotfield,/NODATA,/XSTYLE,$
                              ...
                              POSITION=pos $
                     ELSE PLOT, xax,plotfield,/NODATA,/XSTYLE,$
                                XTITLE=mytitlex, YTITLE=amplitude,COLOR=1,$
                                TITLE=mytitletot,YRANGE=[plotmin,plotmax]
  ...
  IF outputset EQ 1 THEN BEGIN
     IF KEYWORD_SET(legend_pos) THEN $
        plot_legend, coltablearr, captionsarr,$
                     per_line=(*scanlog).dsize[0],$
                     x_offset=legend_pos
     IF (outset EQ -1) OR (outset EQ 2) THEN $
        set_output, diag, sp, /reset , suffix=suffix
  ENDIF

  set_output, diag, sp, header = filenames,$
              dat=[filearr], suffix=suffix,$
              commentline=[rm_idl_fmt(mytitletot)],$
              append=((outset EQ 0) OR (outset EQ -1))

END

;######################################################################

FUNCTION scan_plotnec, diag, plotpars, iscan
; diag: diag pointer
; plotpars : array specifying filter in each scan dimension (incl. EVs)
; iscan    : scanfile number
; returns 1, if data is within filters provided by plotpars
  ...
  ;non-EV dimensions
  FOR dim=1, N_ELEMENTS((*scanlog).dsize)-1 DO BEGIN
      IF NOT (((*scanlog).dsize[dim] EQ 0)OR(plotpars[dim] EQ '*')) THEN BEGIN
          IF (STRLEN(plotpars[dim]) GT 0) THEN BEGIN
              ...
              IF ((test LT border[0])OR(test GT border[1])) THEN RETURN, 0
          ENDIF
      ENDIF
      IF ((*scanlog).dsize[dim] GT 0) THEN number=FLOOR(number / (*scanlog).dsize[dim])
  ENDFOR
  return,1
