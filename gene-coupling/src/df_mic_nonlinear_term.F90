...
module df_mic_nonlinear_term_mod
  ...
  USE discretization, ONLY: li0,li1,li2, lbi,ubi,li0da,ly0da,nky0, lj0,lj1,lj2,nj0, n_procs_y
  use fourier_mic
  use mpi
...
#define MIC_ALL_IN_ONE_LOOP
  ...
  SUBROUTINE calc_mic_nonlinearity_df(this,gy_chi,g_block,vexy,dgdxy,localrhs,first,lb1,lb2)
    class(df_mic_nonlinear_term_t) :: this
    ...
    !DEC$ out(vexy_max_loc)
#endif
    !$OMP PARALLEL default(none) &
    !$OMP shared(this,lb1,lb2,vexy,dgdxy,nonlin) &
    !$OMP shared(first,ve_max) &
    !$OMP shared(local_lbg0,li0da,ly0da,n_procs_y) &
    !$OMP reduction(max:vexy_max_loc) &
    ...
    !$OMP PARALLEL default(none) shared(nonlin,localrhs,this,lb1,lb2)
    CALL this%prefactor%multiply_with(nonlin,localrhs,this%lbg0,lb1,lb2)
    !$OMP END PARALLEL

    if (first) then
       ve_max(1)=max(ve_max(1),vexy_max_loc(1))
       ve_max(2)=max(ve_max(2),vexy_max_loc(2))
    end if
  ...
  !!\todo Get rid of transposition using striding? make some arrays for arakawa allocatable
  !!\todo Check whether the nonlinearity prefactor is important for the CFL criterion
  Subroutine calc_mic_nonlinearity_df(this,gy_chi,g_block,vexy,dgdxy,localrhs,first)
    class(df_mic_nonlinear_term_t) :: this
    Complex, Dimension(li1:li2,lj1:lj2,1:*),Intent(inout) :: gy_chi
    complex, dimension(lbi:ubi,lj1:lj2,1:*),intent(in) :: g_block
    Complex, Dimension(li1:li2,lj1:lj2,2,1:*),Intent(inout) :: vexy, dgdxy
