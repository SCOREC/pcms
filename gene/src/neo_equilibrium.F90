...
contains
  ...
  subroutine initialize_g1_nc
    ...
    integer:: nx0_in, nky0_in, ky0_ind_in, istep_neoclass_in, istep_schpt_in, nc_it_final
    ...
    if(mype.eq.0) then
       ...
       write(*,"(a,(a,3(i1,a)))") &
#else
       write(*,"(a,(a,(i1,a),(i2,a),(i1,a)))") &
#endif
       'Starting computation of neoclassical equilibrium with ','PETSC-',&
...
!stop
    ...
    nc_not_converged=(kspreason.lt.0)
    if (nc_not_converged) then
       if(mype.eq.0) then
          if (kspreason.eq.KSP_DIVERGED_ITS) &
               &write(*,'(A,I9)') 'WARNING: Reached maximum iteration number nc_max_it = ',nc_max_it
          write(*,'(A)') '** PETSc DID NOT CONVERGE - no diagnostic output will be generated! **'
       endif
    else
       call KSPGetIterationNumber(ksp_obj,nc_it_final,globerr)
       if (mype.eq.0) write(*,"(a,I9)") 'Number of iterations: ', nc_it_final
    endif
!    rhs_f0=.true.
