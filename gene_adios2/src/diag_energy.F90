...
!!!******************************************************************!!!
  ...
  SUBROUTINE check_diag_energy
    if (.not.x_local .and. mype==0) then
        write(*,"(A)") "WARNING: Results of the energy diagnostic are ill-defined for x-global"
    endif
    ...
    if (comp_type.eq.'EV') then
       if (istep_energy.gt.0) istep_energy = 1
       if (istep_energy3d.gt.0) istep_energy3d = 1
       if (istep_energy_exchange.gt.0) istep_energy_exchange = 1
    endif
...
!    if (nblocks.ne.1) stop 'get_energy_terms is currently only working with nblocks=1'
    ...
    call nullify_vw_boundaries(f_in)
    if (coll_on_h) call nullify_vw_boundaries(h_in)
...
!!  the following should be faster (less memory demanding, respectively)
    ...
    if (coll_split) then
       if(coll_on_h) then
          call equ_collisions(h_in,rhs,replace_rhs=.false.)
       else
    ...
    if (collision_op.ne.'none') then
       if (coll_on_h) then
          call equ_collisions(h_in,rhs,replace_rhs=.true.)
       else
       ...
       do iterm=3,10
          ...
          case(3)
             if (.not.only_neo) then
                if (xy_local) then
                   call add_dchidxy(fields_in,rhs_block,ptr_barchi,lbg1,lbg2,.false.)
                else
          ...
          case(4)
             if (.not.xy_local) then
                ...
                if (.not.x_local.and.explicit_buffer) call add_kBuffer_explicit(f_in,rhs_block,lbg1,lbg2)
             else
...
PERFON('dia_en')
    ...
    if (write_3d) then
       !write total energy dE_terms(1), non-conservative terms except hyper-diffusion dE_terms(11),
       !drive terms dE_terms(3), collisions dE_terms(5), dissipation terms dE_diss, and
       !nonlinearity dE_terms(9)
       if(write_std) then
...
PERFOFF
  ...
  subroutine diag_energy_exchange
    ...
    call nullify_vw_boundaries(f_temp)
    if (coll_on_h) call nullify_vw_boundaries(h_temp)
    ...
    if (coll_split) then
       if (coll_on_h) then
          call equ_collisions(h_temp,rhs,replace_rhs=.false.)
       else
          call equ_collisions(f_temp,rhs,replace_rhs=.false.)
       endif
    endif
  ...
  !>This subroutines sets the ghost cells in vpar and mu to zero
  !!which is strictly speaking only needed at the *outer* boundaries
  !!in the energy diagnostics routines. The inner boundaries are
  !!initialized jsut for simplicity/load balanching
  !!\TODO Rewrite, e.g., the energy diagnostics vpar derivatives
  !!similar to the RHS implementations such that no zero-valued outer
  !!boundary ghost cells are required and this subroutine can be
  !!removed
  SUBROUTINE nullify_vw_boundaries(f_inout)
    implicit none
    complex, dimension(li1:li2,lj1:lj2,lbz:ubz,lbv:ubv,lbw:ubw,ln1:ln2), intent(inout) :: f_inout

    if (collision_op .eq. 'sugama') then
       f_inout(:,:,:,:,lbw:lm1-1,:) = (0.0,0.0)
       f_inout(:,:,:,:,lm2+1:ubw,:) = (0.0,0.0)
       if (nvb.gt.0) then
          f_inout(:,:,:,lbv:ll1-1,:,:) = (0.0,0.0)
          f_inout(:,:,:,ll2+1:ubv,:,:) = (0.0,0.0)
       endif
    endif

  END SUBROUTINE nullify_vw_boundaries
