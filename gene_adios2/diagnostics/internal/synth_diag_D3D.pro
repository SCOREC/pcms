...
PRO BES_filter, GENE_data, GENE_R, GENE_Z, R00=R00, Z00=Z00, $
  ...
  BES_rasymm = 0.46 ;inner FWHM slightly less
...
;         ((hr_z-BES_zloc[ir,iz])/BES_ltheta)^2)) ;improve: map theta to r/z
  ...
  IF KEYWORD_SET(plot) THEN BEGIN
     ...
     CONTOUR, BES_spot[*,*,0,0],hr_r,hr_z,c_colors=[1,1,1],C_LINESTYLE=[1,2,0],$
       levels=[0.1,0.5,0.9]*MAX(BES_spot[*,*,0,0]),/OVERPLOT, THICK=lthick,$
              C_THICK=[lthick,lthick,lthick]
     CONTOUR, BES_spot[*,*,0,1],hr_r,hr_z,c_colors=[1,1,1],C_LINESTYLE=[1,2,0],$
       levels=[0.1,0.5,0.9]*MAX(BES_spot[*,*,0,1]),/OVERPLOT, THICK=lthick,$
              C_THICK=[lthick,lthick,lthick]
     CONTOUR, BES_spot[*,*,NR_BES-1, NZ_BES-1],hr_r,hr_z,c_colors=[1,1,1],$
       C_LINESTYLE=[1,2,0],levels=[0.1,0.5,0.9]*$
              MAX(BES_spot[*,*,NR_BES-1,NZ_BES-1]),/OVERPLOT, THICK=lthick,$
              C_THICK=[lthick,lthick,lthick]
...
;     LOADCT, 33
     ...
     CONTOUR, hr_dat,hr_r,hr_z,$
              ...
              /FILL, ISOTROPIC=isotrp, XSTYLE=1, YSTYLE=1,XRANGE=[xmin,xmax],$
              YRANGE=[zmin,zmax], POSITION=[0.5,0.5,1.0,1.0],/NOERASE
     CONTOUR, CECE_spot[*,*,0],hr_r,hr_z,c_colors=[1,1,1],C_LINESTYLE=[1,2,0],$
              levels=[0.1,0.5,0.9]*MAX(CECE_spot[*,*,0]),/OVERPLOT, THICK=lthick,$
              C_THICK=[lthick,lthick,lthick]
     CONTOUR, CECE_spot[*,*,N_CECE-1],hr_r,hr_z,c_colors=[1,1,1],C_LINESTYLE=[1,2,0],$
       levels=[0.1,0.5,0.9]*MAX(CECE_spot[*,*,N_CECE-1]),/OVERPLOT, THICK=lthick,$
              C_THICK=[lthick,lthick,lthick]
     CONTOUR, CECE_spot[*,*,N_CECE],hr_r,hr_z,c_colors=[1,1,1],C_LINESTYLE=[1,2,0],$
       levels=[0.1,0.5,0.9]*MAX(CECE_spot[*,*,N_CECE]),/OVERPLOT, THICK=lthick,$
              C_THICK=[lthick,lthick,lthick]
     CONTOUR, CECE_spot[*,*,2*N_CECE-1],hr_r,hr_z,c_colors=[1,1,1],C_LINESTYLE=[1,2,0],$
       levels=[0.1,0.5,0.9]*MAX(CECE_spot[*,*,2*N_CECE-1]),/OVERPLOT, THICK=lthick,$
              C_THICK=[lthick,lthick,lthick]
...
;#############################################################################
;arbitrary generic filtering in R,Z with PointSpreadFunction
;(currently, only Gaussian in R/Z with FWHM lr and lz)
;GENE_data : R/Z data from GENE
;GENE_R : radial coordinate of this data
;GENE_Z : vertical coordinate of this data
;rpos : radial position of spots
;zpos : vertical position of spots
;lr : radial FWHMs of spots
;lz : vertical FWHMs of spots
;raw_data : raw (interpolated) data w/o PSF at rpos and zpos for reference
;sig_data : PSF weighed signals at rpos and zpos
;filter : filter/modify(!) the original R/Z data
;plot : switch on for contour plots of the various signals
;oplot : oplot contours of PSFs (executed before plot)
;get_actual_pos : replace rpos, zpos by the actually used positions

PRO RZ_arb_filter, GENE_data, GENE_R, GENE_Z, rpos=rpos,zpos=zpos, $
                lr=lr, lz=lz, raw_sig=raw_sig, sig_data=sig_data, $
                get_actual_pos=get_actual_pos, $
                filter=filter, plot=plot, oplot=oplot
  COMMON global_vars

  IF N_ELEMENTS(GENE_data) LE 1 THEN RETURN
  IF N_ELEMENTS(rpos) NE N_ELEMENTS(zpos) THEN BEGIN
     print, 'ERROR: R/Z spot position arrays differ in size'
     RETURN
  ENDIF

  NSIG = N_ELEMENTS(rpos)
  NR = NSIG
  NZ = NSIG

  IF N_ELEMENTS(lr) LT 1 THEN lr = 0.01+FLTARR(NSIG) ELSE $
     IF N_ELEMENTS(lr) EQ 1 THEN lr = lr+FLTARR(NSIG)
  IF N_ELEMENTS(lz) LT 1 THEN lz = 0.01+FLTARR(NSIG) ELSE $
     IF N_ELEMENTS(lz) EQ 1 THEN lz = lz+FLTARR(NSIG)

  ; proper translation of FWHM and normalization for Gaussian
  lr /= (2.0*sqrt(2.0*ALOG(2.0)))
  lz /= (2.0*sqrt(2.0*ALOG(2.0)))

  ;2D position array required for contours
;  rpos2d = FLTARR(NR,NZ)
;  zpos2d = FLTARR(NR,NZ)
;  FOR iz = 0, NZ - 1 DO rpos2d[*,iz] = rpos
;  FOR ir = 0, NR - 1 DO zpos2d[ir,*] = zpos

  ;measure signal array
  rmax = MAX(rpos,min=rmin)
  rdist = MAX(ABS(rpos[1:NSIG-1]-rpos[0:NSIG-2]))
  zmax = MAX(zpos,min=zmin)
  zdist = MAX(ABS(zpos[1:NSIG-1]-zpos[0:NSIG-2]))

  ;create local high res. rectangular grid
  nind1 = 128
  nind2 = 128

  hr_lr = rmax-rmin+2.0*(3.0*MAX(lr))>0.10 ;>rdist)>0.10
  hr_lz = zmax-zmin+2.0*(3.0*MAX(lz))>0.10 ;>zdist)>0.10
  hr_r = (rmin+rmax)/2.+REBIN((-0.5+FINDGEN(nind1)/(nind1-1))*hr_lr,[nind1,nind2])
  hr_z = (zmin+zmax)/2.+REBIN(REFORM((0.5-(FINDGEN(nind2)/(nind2-1)))*$
           hr_lz,[1,nind2]),[nind1,nind2])
  TRIANGULATE, GENE_R, GENE_Z, tria, TOLERANCE=0
  hr_dat = TRIGRID(GENE_R,GENE_Z,GENE_data,tria,NX=nind1,NY=nind2,$
                   XOUT=hr_r[*,0],YOUT=hr_z[0,*])

  ;get points on this grid being closest to the original positions
  hr_rpos = FLTARR(NSIG)
  hr_zpos = FLTARR(NSIG)
  hr_idx = LONARR(NSIG)
  psf_func = FLTARR(nind1,nind2,NSIG)
  sig_data  = FLTARR(NSIG)

  ;create signal array and apply PointSpreadFunction (PSF)
  FOR isig = 0, NSIG - 1 DO BEGIN
     mind = MIN((hr_r-rpos[isig])^2+(hr_z-zpos[isig])^2,minidx)
     hr_idx[isig] = minidx
     hr_rpos[isig] = hr_R[minidx]
     hr_zpos[isig] = hr_Z[minidx]

     FOR ind2 = 0, nind2-1 DO BEGIN
        FOR ind1 = 0, nind1-1 DO BEGIN
           rdiff = (hr_r[ind1,ind2]-rpos[isig])
;           rdiff /= 2.0*((rdiff LT 0)?BES_rasymm:(1.0-BES_rasymm))
           zdiff = (hr_z[ind1,ind2]-zpos[isig])
           psf_func[ind1,ind2,isig] = EXP(-0.5*((rdiff/lr[isig])^2+$
                 (zdiff/lz[isig])^2)) ;improve: map theta to r/z
        ENDFOR
     ENDFOR
     psf_norm = TOTAL(psf_func[*,*,isig])

     IF ABS(psf_norm) GT 0 THEN BEGIN
        psf_func[*,*,isig] /= psf_norm
        sig_data[isig] = TOTAL(hr_dat*psf_func[*,*,isig])
     ENDIF ELSE print, 'found empty RZ filter spots'
  ENDFOR

  ;just the values at the updated spot positions (w/o PSF)
  raw_sig = hr_dat[hr_idx]

  ;filter input GENE data
  IF KEYWORD_SET(filter) THEN $
     GENE_data[*] *= TOTAL(TOTAL(pfs_func,4),3)/(NR*NZ)
  ...
  IF KEYWORD_SET(oplot) THEN BEGIN
     ...
     lthick = 3.0 ;contour lines
     lev_col = contour_levels(GENE_data[*,*],64)
     FOR isig = 0, nsig-1 DO $
       CONTOUR, psf_func[*,*,isig],hr_r,hr_z,c_colors=[1,1,1],C_LINESTYLE=[1,2,0],$
         levels=[0.1,0.5,0.9]*MAX(psf_func[*,*,isig]),/OVERPLOT, THICK=lthick,$
              C_THICK=[lthick,lthick,lthick]
     OPLOT, rpos, zpos, PSYM=psym, COLOR=1, SYMSIZE=0.5
     OPLOT, hr_rpos, hr_zpos, PSYM=3, COLOR=1
  ENDIF
  ...
  IF KEYWORD_SET(plot) THEN BEGIN
     ...
     lthick = 3.0 ;contour lines
     lev_col = contour_levels(GENE_data[*,*],64)
     rmin = MIN(hr_r,max=rmax)
     zmin = MIN(hr_z,max=zmax)

     CONTOUR, GENE_data[*,*],GENE_R, GENE_Z,$
              C_COLORS=lev_col[*,1], LEVELS=lev_col[*,0], $
              /FILL, /ISOTROPIC, XSTYLE=1, YSTYLE=1,XRANGE=[rmin,rmax],$
              YRANGE=[zmin,zmax], POSITION=[0.0,0.5,0.5,1.0]
     OPLOT, GENE_R, GENE_Z, PSYM=3, COLOR=1
     ...
     IF ((NSIG GT 2) AND (1 EQ 0)) THEN BEGIN
        CONTOUR, raw_sig,rpos,zpos,/IRREGULAR,$
                 C_COLORS=lev_col[*,1], LEVELS=lev_col[*,0], $
                 /FILL, /ISOTROPIC, XSTYLE=1, YSTYLE=1,XRANGE=[rmin,rmax],$
                 YRANGE=[zmin,zmax], POSITION=[0.0,0.0,0.5,0.5],/NOERASE
        OPLOT, rpos, zpos, PSYM=psym, COLOR=1, SYMSIZE=0.5

        CONTOUR, sig_data,rpos,zpos,/IRREGULAR,$
                 C_COLORS=lev_col[*,1], LEVELS=lev_col[*,0], $
                 /FILL, /ISOTROPIC, XSTYLE=1, YSTYLE=1,XRANGE=[rmin,rmax],$
                 YRANGE=[zmin,zmax], POSITION=[0.5,0.0,1.0,0.5],/NOERASE
        OPLOT, rpos, zpos, PSYM=psym, COLOR=1, SYMSIZE=0.5
  ...
  IF KEYWORD_SET(get_actual_pos) THEN BEGIN
     rpos = hr_rpos
     zpos = hr_zpos
  ENDIF
