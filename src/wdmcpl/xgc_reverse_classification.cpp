...
std::vector<LO> ReverseClassificationVertex::Serialize() {
  ...
  for(auto& geom : data_) {
    ...
    std::copy(geom.second.begin(), geom.second.end(), std::back_inserter(serialized_data));
  }
  return serialized_data;
}
void ReverseClassificationVertex::Deserialize(ScalarArrayView<LO, wdmcpl::HostMemorySpace> serialized_data) {
  ...
  while(i<serialized_data.size()) {
    DimID geom{
      .dim = serialized_data[i],
      .id = serialized_data[i+1]
    };
...
ReverseClassificationVertex ReadReverseClassificationVertex(std::istream& in) {
  ...
  while(!in.eof() && in.good()){
    ...
    if(in.eof()) {break;}
    in >> geometry.id;
    getline(in,line); // read to the end of the cuurent line with geom_dim/geom_id
    getline(in,line);
...
ReverseClassificationVertex ReadReverseClassificationVertex(std::istream& instr, MPI_Comm comm, int root) {
  int rank = -1;
  MPI_Comm_rank(comm, &rank);
  if(rank == root) {
    auto rc =  ReadReverseClassificationVertex(instr);
    auto serialized_rc = rc.Serialize();
    static_assert(std::is_same_v<LO, int32_t>, "mpi type hardcoded, must update");
    size_t sz = serialized_rc.size();
    MPI_Bcast(&sz, 1, MPI_INT64_T,root,comm);
    MPI_Bcast(serialized_rc.data(), serialized_rc.size(), MPI_INT32_T,root,comm);
    return rc;
  }
  else {
    size_t sz =0;
    ...
    wdmcpl::ScalarArrayView<wdmcpl::LO, wdmcpl::HostMemorySpace> av{serialized_rc.data(),serialized_rc.size()};
    ReverseClassificationVertex rc;
    rc.Deserialize(av);
    return rc;
...
ReverseClassificationVertex ReadReverseClassificationVertex(std::filesystem::path& classification_file) {
...
ReverseClassificationVertex ReadReverseClassificationVertex(std::filesystem::path& classification_file, MPI_Comm comm, int root) {
  WDMCPL_ALWAYS_ASSERT(classification_file.has_filename());
  std::ifstream infile(classification_file);
