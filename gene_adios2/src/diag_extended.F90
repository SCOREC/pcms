...
Module diagnostics_extended
  ...
  use diagnostics_energy, only: get_cfgamma_h, get_cfgamma_g, get_cfgamma_f,&
      get_energy_terms,scalar_product, block_mult,nullify_vw_boundaries
#ifdef WITHFUTILS
  use futils
  use hashtable
#endif
  ...
  PUBLIC :: initialize_all_diags_nlt, exec_all_diags_nlt, &
       &finalize_all_diags_nlt, get_k_indices
...
!!!******************************************************************!!!
  ...
  SUBROUTINE initialize_all_diags_nlt
...
#else
       if (mype==0) Write(*,'(a)') 'ERROR: with_extended_diags must be set in switches.h for using diag_nlt'
       stop
#endif
    end if
    last_energy_nlt=0.0
    ...
    if (comp_type.eq.'EV') then
       if (istep_nlt.gt.0) istep_nlt=0 !check
    endif
...
!    if (nblocks.ne.1) stop 'get_energy_terms is currently only working with nblocks=1'
    ...
    call nullify_vw_boundaries(f_2_in)
    if (coll_on_h) call nullify_vw_boundaries(h_2_in)
...
!!  the following should be faster (less memory demanding, respectively)
    ...
    if(coll_split) then
       if (coll_on_h) then
          call equ_collisions(h_2_in,rhs,replace_rhs=.false.)
       else
    ...
    if (collision_op.ne.'none') then
       if (coll_on_h) then
          call equ_collisions(h_2_in,rhs,.true.)
       else
...
PERFON('en_block')
    ...
    Do iblock=1,nblocks
       ...
       do iterm=3,10
          ...
          case(3)
            if (xy_local) then
              call add_dchidxy(fields_2_in,rhs_block,ptr_barchi,lbg1,lbg2,.false.)
            else
              call add_dchidxy(bar_emfields,&
          ...
          case(7)
             ...
             if (arakawa_zv) then
                if (hyp_on_h) then
                   ...
                   if (hypz_compensation) call equ_comp_hypz(fields_2_in,bar_emfields,rhs_block,lbg1,lbg2)
                else
          ...
          case(10)
             ...
             if (arakawa_zv) then
                ...
                call add_dchidz(fields_2_in, bar_emfields, rhs_block, lbg1, lbg2)
             end if
          case default
...
if (hyp_on_h) then
    ...
    if (arakawa_zv.or.coll_on_h) deallocate(h_2_in)
