...
!including tVec and tMat indirectly through slepc_aux
!yields internal compiler errors with intel 16
...
!    character(len=10)::pc_string,ksp_string,eps_string
...
#endif
            'Starting eigenvalue computation with ','PETSC-',&
  ...
  subroutine analyze_eigenpairs(num_ev)
    ...
    do i=0,num_ev-1
       ...
       else if(which_ev.eq.'mfn') then
          eigenvalue(i)=log(eigenvalue(i))/taumfn
#endif
       end if
