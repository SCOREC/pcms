...
module vel_space
  use par_in,only: arakawa_zv, perf_vec, trap_pass, coll_f_fm_on, coll_on_h,&
       &n0_global, Omega0_tor, spec, istep_fe_time, all_moms
  use par_other,only: n_fields, pi
  USE coordinates, ONLY: set_mu_coordinate_vars, set_vp_coordinate_vars
  use coordinates, only: mu, mu_weight, vp, vp_weight
  use discretization
...
contains
  ...
  Real Function mem_est_vel_space(mem_req_in)
    ...
    fm_has_vzbounds = (arakawa_zv.or.coll_on_h.or.coll_f_fm_on.or.(istep_fe_time.gt.0))
