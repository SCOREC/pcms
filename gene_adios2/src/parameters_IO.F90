...
use par_in
use par_other
use par_geom, only: Bprof_coeffs, tracer_rt_scheme
use adiabatic_response, only: check_qn_gradients, &
     &set_adiabatic_response_defaults
...
use diagnostics
use diagnostics_df
use diagnostics_vsp, only: istep_vsp, istep_xvsp, xvsp_iy, xvsp_iz
use checkpoint
use reset_mod
...
#ifdef COUPLE_XGC
#ifdef ADIOS
use adios_comm_module, only: adios_comm_get_read_method
#endif
#endif
...
namelist /in_out/ &
     ...
     istep_srcmom, write_h5, write_hac, write_std, chpt_h5, chpt_read_h5, chpt_write_h5,&
     ...
     istep_fsa_moments, istep_energy_exchange, istep_triplet, &
#endif
     quasilin_model
...
contains
  ...
  subroutine read_parameters(par_in_dir)
    ...
    namelist /general/ &
         ...
         perf_vec, nblocks, parscheme, vparscheme, &
         precomp_nc, include_f0_contr,& !rescale
         timescheme, calc_dt, dt_max, dt_vlasov, ev_coll, courant, coll_split, coll_split_scheme, nltdt_off,& !time scheme
         n_ev, which_ev, ev_max_it, ev_shift, ev_n_test, ev_prec,& !Eigenvalue comp.
         vec_out_limit,ev_out,ev_left, ev_right, taumfn, &
         ...
         beta, debye2, &
         delzonal, delzonal_factor, delzonal_fields, delzonal_bpar, &
         ...
         coll, collision_op, coll_cons_model, coll_order, spacediff, spacediff_off, coll_f_fm_on, &
         coll_on_h, Zeff, adiabatic_mass,&
         ...
         hyp_z, hyp_z_order, hyp_v, hyp_on_h, hypz_opt, hypz_compensation,&
#ifndef COUPLE_XGC
         GyroLES, fracx,fracy, diag_GyroLES,tky,tkx,tkperp,n_shells_kperp,t_log_kperp_fb,t_log_kperp_h,& ! GLES related
         t_log_kperp_h1,t_log_kperp_h2,& ! GLES related
         tk3_log_kperp, tk3_ky, tk3_kx, t_log_kperp, t_log_ky, n_shells_log_kperp, n_shells_log_ky, & ! GLES related
         tk3_log_kperp_1, tk3_log_kperp_h,tk3_log_kperp_h1,tk3_log_kperp_h2,t_log_kperp_1, &! GLES related
         n_shells_log_kperp_1, n_shells_log_kperp_2, lambda1, lambda2, with_nvp, & ! GLES related
#endif
         ...
         &gpu_cpu_ratio,which_func_as, check_qn_gradients, enforce_qn, all_moms, &
         save_eval_tplt, save_eval_kymin, save_eval_kxmin, & !EV saving
         load_ev_kyind, load_ev_kxind !EV loading
    ...
    namelist /extended_diags/ &   !energy, nlt, svd (etc.) diagnostics
         ...
         SVD_field_mom, SVD_swap_endian, n_SVD_ind, &      !POD diagnostics
         num_nlt_modes,kx_nlt_ind,ky_nlt_ind,nlt_symmetrize,&        !istep_nlt
         nx0_nlt_pod, num_nlt_pod_modes, nlt_pod, &     !nlt pod
         nx0_nlt_pod,num_nlt_pod_modes ,nlt_pod,&     !nlt pod
         split_em_tplt,&
         n_tplts,n_dfs_tplts,kx_triplets,ky_triplets,&!nlt pod 3wave coupling
         kx_triplets1,ky_triplets1,kx_triplets2,ky_triplets2,&
         kx_triplets3,ky_triplets3,kx_triplets4,ky_triplets4,&
         kx_triplets5,ky_triplets5,kx_triplets6,ky_triplets6,&
         kx_triplets7,ky_triplets7,kx_triplets8,ky_triplets8,&
         kx_triplets9,ky_triplets9,kx_triplets10,ky_triplets10,&
         kx_triplets11,ky_triplets11,kx_triplets12,ky_triplets12,&
         kx_triplets13,ky_triplets13,kx_triplets14,ky_triplets14,&
         kx_triplets15,ky_triplets15,kx_triplets16,ky_triplets16,&
         kx_triplets17,ky_triplets17,kx_triplets18,ky_triplets18,&
         kx_triplets19,ky_triplets19,kx_triplets20,ky_triplets20,&
         kx_triplets21,ky_triplets21,kx_triplets22,ky_triplets22,&
         kx_triplets23,ky_triplets23,kx_triplets24,ky_triplets24,&
         kx_triplets25,ky_triplets25,kx_triplets26,ky_triplets26,&
         kx_triplets27,ky_triplets27,kx_triplets28,ky_triplets28,&
         kx_triplets29,ky_triplets29,kx_triplets30,ky_triplets30,&
         kx_triplets31,ky_triplets31,kx_triplets32,ky_triplets32,&
         kx_triplets33,ky_triplets33,kx_triplets34,ky_triplets34,&
         kx_triplets35,ky_triplets35,kx_triplets36,ky_triplets36,&
         kx_triplets37,ky_triplets37,kx_triplets38,ky_triplets38,&
         n_ev_triplets
#endif
    namelist /nonlocal_x/ &
    ...
    namelist /external_contr/ &
         phi0_ext, kxind_phi_ext, phase_phi_ext, use_self_consistent_flow, &
         ...
         lilo_w_full_omegator_profile,kink_on,uprime_x0
    ...
    namelist /geometry/ &
         ...
         rhostar, mag_prof, q_coeffs, n_pol, edge_opt, tracer_rt_scheme,&
    ...
    ! static magnetic island namelist
    namelist /island/ &
         & island_contrib, & !switchs on the static island
         & j_island,&   !island mode number
         & omega_island, & !island rotation frequency
         & width_island, & !island full-width
         & damping_island, & !damping terms so the field is smooth at the radial boundaries
         & xdamp,delta_xdamp,& !damping terms so the field is smooth at the radial boundaries
         & amp_island, & !island asymmetry parameters
         & island_zGauss, & ! use Gaussian-in-z island instead of standard
         & island_ky, & ! k_y rho_ref where z Gaussian is to be imposed
         & width_zGauss ! FWHM of Gaussian in z

    ! generate field on poloidal planes
    namelist /poloidal_planes/ &
         & nz_out, &    !poloidal points per plane
         & n_cuts, &    !number of planes
         & istep_planes, & !how often (active if >0)
         & BC_in_fourier, &
         & res_fact, &
         & XGC_file, &
         & pre_proc, &
    ...
    !default values
    call set_adiabatic_response_defaults
    ...
    call set_island_defaults
    call set_poloidal_planes_defaults
    ...
    rewind(PARFILE); ierr = 0
    read(PARFILE, nml=in_out,iostat=ierr)
    if (ierr.ne.0) stop 'on i/o error: incorrect in_out namelist'

#if defined(COUPLE_XGC) && defined(ADIOS)
    call get_unit_nr(ADIOS_IN_UNIT)
    call adios_comm_get_read_method(rank=mype, fileunit=ADIOS_IN_UNIT)
#endif
    ...
    rewind(PARFILE); ierr = 0
    read(PARFILE, nml=island,iostat=ierr)
    if (ierr.gt.0) stop 'on i/o error: incorrect island namelist'

    rewind(PARFILE); ierr = 0
    read(PARFILE, nml=poloidal_planes,iostat=ierr)
    if (ierr.gt.0) stop 'on i/o error: incorrect poloidal planes namelist'
  ...
  subroutine check_extended
   ...
   IF(istep_triplet.GT.0) THEN
    IF(n_procs_x.gt.1) STOP "triplet diagnostic cannot be used with kx parallelization."
    IF(n_procs_x.lT.1) THEN
     n_procs_x=1
     if(mype==0) print*, "triplet_diag only works with n_procs_x=1, setting as such"
    ENDIF
    IF(.NOT.arakawa_zv) THEN
      IF(mype==0) PRINT*, "Must use arakawa_zv=T with triplet diagnostic.(for free energy conservation)"
      arakawa_zv = .TRUE.
    ENDIF
    IF(.NOT.arakawa_cons_bc) THEN
      IF(mype==0) PRINT*, "Switching arakawa_cons_bc on"
      arakawa_cons_bc = .TRUE.
    END IF

    !IF(.NOT.xy_local) STOP "Triplet diagnostic only works with xylocal"

    IF(n_tplts.LE.0) STOP "minimum 1 triplet in triplet diagnostic"
    IF(n_tplts.GT.2400) STOP "maximum 2400 triplets in triplet diagnostic"


    IF (.NOT.((kx_triplets1(1).EQ.0).AND.(ky_triplets1(1).EQ.0))) THEN
      kx_triplets=(/kx_triplets1,kx_triplets2,kx_triplets3,kx_triplets4,kx_triplets5,kx_triplets6,kx_triplets7,&
         kx_triplets8,kx_triplets9,kx_triplets10,kx_triplets11,kx_triplets12,kx_triplets13,kx_triplets14,kx_triplets15,&
         kx_triplets16,kx_triplets17,kx_triplets18,kx_triplets19,kx_triplets20,kx_triplets21,kx_triplets22,kx_triplets23,&
         kx_triplets24,kx_triplets25,kx_triplets26,kx_triplets27,kx_triplets28,kx_triplets29,kx_triplets30,kx_triplets31,&
         kx_triplets32,kx_triplets33,kx_triplets34,kx_triplets35,kx_triplets36,kx_triplets37,kx_triplets38/)
      ky_triplets=(/ky_triplets1,ky_triplets2,ky_triplets3,ky_triplets4,ky_triplets5,ky_triplets6,ky_triplets7,&
         ky_triplets8,ky_triplets9,ky_triplets10,ky_triplets11,ky_triplets12,ky_triplets13,ky_triplets14,ky_triplets15,&
         ky_triplets16,ky_triplets17,ky_triplets18,ky_triplets19,ky_triplets20,ky_triplets21,ky_triplets22,ky_triplets23,&
         ky_triplets24,ky_triplets25,ky_triplets26,ky_triplets27,ky_triplets28,ky_triplets29,ky_triplets30,ky_triplets31,&
         ky_triplets32,ky_triplets33,ky_triplets34,ky_triplets35,ky_triplets36,ky_triplets37,ky_triplets38/)
    ENDIF

    DO index=1,n_tplts
      IF((kx_triplets(3*(index)-2)-kx_triplets(3*(index)-1).NE.kx_triplets(3*index))&
         .OR.((ky_triplets(3*(index)-2)-ky_triplets(3*(index)-1).NE.ky_triplets(3*index)))) THEN
        IF(mype==0) PRINT*, "k,k',k'', triplet index:", kx_triplets(3*index-2), ky_triplets(3*index-2),&
           kx_triplets(3*index-1), ky_triplets(3*index-1), kx_triplets(3*index), ky_triplets(3*index), index
        STOP "for triplet diag k-k'=k'', check indices"
      ENDIF
      IF((ABS(kx_triplets(3*index)).GT.nx0-1).OR.(ky_triplets(3*index).GT.nky0-1).OR.&
         (ky_triplets(3*index).LT.0)) &
         STOP "Selected wavenumbers out of range for diag_triplet!"
      IF((ABS(kx_triplets(3*index+1)).GT.nx0-1).OR.(ky_triplets(3*index+1).GT.nky0-1).OR.&
         (ky_triplets(3*index+1).LT.0)) &
         STOP "Selected wavenumbers out of range for diag_triplet!"
      IF((ABS(kx_triplets(3*index+2)).GT.nx0-1).OR.(ky_triplets(3*index+2).GT.nky0-1).OR.&
         (ky_triplets(3*index+2).LT.0)) &
         STOP "Selected wavenumbers out of range for diag_triplet!"
  ...
  ENDIF
   ...
   if(SVD_proj) then
      ...
      DO index=1,n_svd_ind
         IF(SVD_nkx0==-100.OR.SVD_kx_ind(index)==-100.OR.SVD_ky_ind(index)==-100) &
              STOP "Must enter SVD_kx_ind, SVD_ky_ind, and SVD_nkx0 for each index!"

         IF(SVD_df_n_time(1)==-100) &
              STOP "Must enter SVD_df_n_time"
         IF(SVD_n_restarts.GT.1.AND.TRIM(SVD_df_file_suffix(2))=='.dat') THEN
         ...
         IF(SVD_df_file_path=='no_input') THEN
            SVD_df_file_path=diagdir
         END IF
         IF((SVD_df_file_name.NE.'no_input').AND.(n_svd_ind.NE.1)) THEN
           IF(mype==0) WRITE(*,*) "Warning, when calculating svd for multiple indices inputted filenames are ignored"
         END IF
      ENDDO
    end if
    ...
    if(istep_nlt.gt.0) then
      ...
      if (num_nlt_modes.gt.60.or.num_nlt_modes.lt.0) stop "num_nlt_modes must be between 1 and 60 inclusive."
  ...
  subroutine check_params(par_in_dir)
    ...
    only_zonal = (nky0.eq.1).and.((ky0_ind.eq.0).or.&
         &(.not.adapt_ly.and.abs(kymin).lt.1e-5).or.&
         &(adapt_ly.and.((n0_global.eq.0).or.&
         &(n0_global.eq.-1111.and.abs(kymin).lt.1e-5))))
...
!       if (.not.nonlinear) stop "parallel nonlinearity requires ExB nonlinearity to be switched on"
       if (rhostar.eq.0) stop "finite rhostar required for parallel nonlinearity"
  ...
  Subroutine write_parameters(skip_info)
    LOGICAL, INTENT(IN), OPTIONAL :: skip_info
    INTEGER :: n
    CHARACTER(len=128) :: tmpstr
    LOGICAL :: skip_info_nml
    CHARACTER(len=10) :: format_string
    CHARACTER(len=11) :: format_string2
    CHARACTER(len=13) :: tplt_format
    CHARACTER(len=3) :: string_num
    CHARACTER(len=4) :: string_num2
    CHARACTER(len=6) :: tplt_string
    ...
    Write(PAR_OUT_FILE,"(A,G16.8)") "kymin = ",kymin
    Write(PAR_OUT_FILE,"(A,G13.6)") "lv    = ",lv
    Write(PAR_OUT_FILE,"(A,G13.6)") "lw    = ",lw
    ...
    if (x_local) then
       ...
       if (magn_geometry .eq. 'tracer_efit'.or.&
            ...
            &Write(PAR_OUT_FILE,"(A,G16.8)") "x0    = ",x0
    else
    ...
    if (kx_center.ne.0.) Write(PAR_OUT_FILE,"(A,G16.8)") "kx_center = ",kx_center
    IF (mu_grid_type.NE.'gau_lag') WRITE(PAR_OUT_FILE,"(3A)")  "mu_grid_type = '", TRIM(mu_grid_type),"'"
    ...
    IF (istep_xvsp.GT.0) THEN
       WRITE(PAR_OUT_FILE,"(A,I7)") "istep_xvsp   = ",istep_xvsp
       WRITE(PAR_OUT_FILE,"(A,I7)") "xvsp_iy      = ",xvsp_iy
       WRITE(PAR_OUT_FILE,"(A,I7)") "xvsp_iz      = ",xvsp_iz
    ENDIF
    ...
    IF (istep_triplet.GT.0) WRITE(PAR_OUT_FILE,"(A,I7)") "istep_triplet    = ",istep_triplet
#endif
...
!    IF (istep_neoclass2D.gt.0) Write(PAR_OUT_FILE,"(A,I7)") "istep_neoclass2D = ",istep_neoclass2D
    ...
    IF (istep_fe_threeD.gt.0) Write(PAR_OUT_FILE,"(A,I7)") "istep_fe_threeD = ", istep_fe_threeD
    ...
    IF (write_hac) WRITE(PAR_OUT_FILE,"(A,L1)") "write_hac  = ", write_hac
    IF (chpt_h5) THEN
    ...
    if (.not.arakawa_zv) then
       ...
       if (hyp_on_h) write(PAR_OUT_FILE,"(A,L3)") "hypz_compensation = ",hypz_compensation
    endif
    Write(PAR_OUT_FILE,"(A)") ""
    if (.not.enforce_qn)  WRITE(PAR_OUT_FILE,"(A,L1)") "enforce_qn  = ", enforce_qn
    if (all_moms)  WRITE(PAR_OUT_FILE,"(A,L1)") "all_moms  = ", all_moms
    ...
    case('EV')
       ...
       if (trim(which_ev).eq.'mfn') WRITE(PAR_OUT_FILE,"(2(A,G12.4),A)") &
            &"taumfn    = (",real(taumfn),",",aimag(taumfn),")"
    case('IV')
       ...
       if (skip_info_nml) write(PAR_OUT_FILE,"(A,L1)") "calc_dt    = ",calc_dt
       if (nltdt_off) write(PAR_OUT_FILE,"(A,L1)") "nltdt_off    = ",nltdt_off
       if (coll_split) then
       ...
       if(include_f0_contr) Write(PAR_OUT_FILE,"(A,L1)")  "include_f0_contr = ", include_f0_contr
    case('NC')
...
!         &Write(PAR_OUT_FILE,"(A,L3)") "pressure_term   = ",pressure_term
    ...
    if (delzonal_bpar) Write(PAR_OUT_FILE,"(A,L1)") "delzonal_bpar   = ",delzonal_bpar
    ...
    if (abs(add_zonal_phi).gt.epsilon(0.0)) &
         &Write(PAR_OUT_FILE,"(A,G16.8)") "add_zonal_phi = ", add_zonal_phi
    ...
    if (collision_op.ne.'none') then
       ...
       Write(PAR_OUT_FILE,"(3A)") "coll_order  = '",trim(coll_order),"'"
       ...
       if (coll_on_h) Write(PAR_OUT_FILE,"(A,L1)") "coll_on_h  = ", coll_on_h
    ...
    IF (no_electron_response) Write(PAR_OUT_FILE,"(A,L1)") &
         &"no_electron_response = ", no_electron_response

    IF (.not.(check_qn_gradients)) Write(PAR_OUT_FILE,"(A,L1)") &
         &"check_qn_gradients = ", check_qn_gradients

    WRITE(PAR_OUT_FILE,"(A)")   "/"
    Write(PAR_OUT_FILE,"(A)")    ""
#ifndef COUPLE_XGC
    !****** extended_diags namelist ********
    if(istep_nlt.gt.0.or.SVD_proj.or.istep_dfout.gt.0.or.istep_triplet.gt.0) then

     Write(PAR_OUT_FILE,"(A)")    "&extended_diags"
     if (istep_nlt.gt.0) then
       write(PAR_OUT_FILE,"(A,1I2)")  'num_nlt_modes = ', num_nlt_modes
       if(nlt_pod) then
          write(PAR_OUT_FILE,"(A,1I2)")  'num_nlt_pod_modes = ', num_nlt_pod_modes
          write(PAR_OUT_FILE, "(A)") 'nlt_pod=.t.'
          WRITE(string_num,'(I4)') num_nlt_modes
          format_string='(A,'//string_num//'1I3))'
          write(PAR_OUT_FILE, format_string) 'nx0_nlt_pod  = ', (nx0_nlt_pod(j),j=1,num_nlt_modes)
       end if
       WRITE(string_num,'(I3)') num_nlt_modes
       format_string='(A,'//string_num//'1I4))'
       write(PAR_OUT_FILE,format_string) 'ky_nlt_ind    = ',(ky_nlt_ind(j),j=1,num_nlt_modes)
       write(PAR_OUT_FILE,format_string) 'kx_nlt_ind    = ',(kx_nlt_ind(j),j=1,num_nlt_modes)
       write(PAR_OUT_FILE,"(A)")    ""
     endif

     IF (istep_triplet.GT.0) THEN
        WRITE(PAR_OUT_FILE,*)  'n_tplts = ', n_tplts
        WRITE(PAR_OUT_FILE,*)  'n_dfs_tplts = ', n_dfs_tplts

        WRITE(tplt_string,"(I6.6)") 3*n_tplts


        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets1 = ',(kx_triplets(j),j=1,1950)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets1 = ',(ky_triplets(j),j=1,1950)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets2 = ',(kx_triplets(j),j=1951,3900)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets2 = ',(ky_triplets(j),j=1951,3900)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets3 = ',(kx_triplets(j),j=3901,5850)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets3 = ',(ky_triplets(j),j=3901,5850)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets4 = ',(kx_triplets(j),j=5851,7800)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets4 = ',(ky_triplets(j),j=5851,7800)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets5 = ',(kx_triplets(j),j=7801,9750)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets5 = ',(ky_triplets(j),j=7801,9750)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets6 = ',(kx_triplets(j),j=9751,11700)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets6 = ',(ky_triplets(j),j=9751,11700)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets7 = ',(kx_triplets(j),j=11701,13650)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets7 = ',(ky_triplets(j),j=11701,13650)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets8 = ',(kx_triplets(j),j=13651,15600)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets8 = ',(ky_triplets(j),j=13651,15600)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets9 = ',(kx_triplets(j),j=15601,17550)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets9 = ',(ky_triplets(j),j=15601,17550)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets10 = ',(kx_triplets(j),j=17551,19500)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets10 = ',(ky_triplets(j),j=17551,19500)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets11 = ',(kx_triplets(j),j=19501,21450)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets11 = ',(ky_triplets(j),j=19501,21450)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets12 = ',(kx_triplets(j),j=21451,23400)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets12 = ',(ky_triplets(j),j=21451,23400)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets13 = ',(kx_triplets(j),j=23401,25350)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets13 = ',(ky_triplets(j),j=23401,25350)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets14 = ',(kx_triplets(j),j=25351,27300)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets14 = ',(ky_triplets(j),j=25351,27300)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets15 = ',(kx_triplets(j),j=27301,29250)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets15 = ',(ky_triplets(j),j=27301,29250)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets16 = ',(kx_triplets(j),j=29251,31200)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets16 = ',(ky_triplets(j),j=29251,31200)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets17 = ',(kx_triplets(j),j=31201,33150)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets17 = ',(ky_triplets(j),j=31201,33150)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets18 = ',(kx_triplets(j),j=33151,35100)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets18 = ',(ky_triplets(j),j=33151,35100)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets19 = ',(kx_triplets(j),j=35101,37050)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets19 = ',(ky_triplets(j),j=35101,37050)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets20 = ',(kx_triplets(j),j=37051,39000)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets20 = ',(ky_triplets(j),j=37051,39000)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets21 = ',(kx_triplets(j),j=39001,40950)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets21 = ',(ky_triplets(j),j=39001,40950)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets22 = ',(kx_triplets(j),j=40951,42900)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets22 = ',(ky_triplets(j),j=40951,42900)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets23 = ',(kx_triplets(j),j=42901,44850)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets23 = ',(ky_triplets(j),j=42901,44850)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets24 = ',(kx_triplets(j),j=44851,46800)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets24 = ',(ky_triplets(j),j=44851,46800)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets25 = ',(kx_triplets(j),j=46801,48750)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets25 = ',(ky_triplets(j),j=46801,48750)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets26 = ',(kx_triplets(j),j=48751,50700)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets26 = ',(ky_triplets(j),j=48751,50700)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets27 = ',(kx_triplets(j),j=50701,52650)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets27 = ',(ky_triplets(j),j=50701,52650)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets28 = ',(kx_triplets(j),j=52651,54600)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets28 = ',(ky_triplets(j),j=52651,54600)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets29 = ',(kx_triplets(j),j=54601,56550)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets29 = ',(ky_triplets(j),j=54601,56550)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets30 = ',(kx_triplets(j),j=56551,58500)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets30 = ',(ky_triplets(j),j=56551,58500)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets31 = ',(kx_triplets(j),j=58501,60450)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets31 = ',(ky_triplets(j),j=58501,60450)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets32 = ',(kx_triplets(j),j=60451,62400)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets32 = ',(ky_triplets(j),j=60451,62400)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets33 = ',(kx_triplets(j),j=62401,64350)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets33 = ',(ky_triplets(j),j=62401,64350)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets34 = ',(kx_triplets(j),j=64351,66300)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets34 = ',(ky_triplets(j),j=64351,66300)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets35 = ',(kx_triplets(j),j=66301,68250)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets35 = ',(ky_triplets(j),j=66301,68250)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets36 = ',(kx_triplets(j),j=68251,70200)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets36 = ',(ky_triplets(j),j=68251,70200)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets37 = ',(kx_triplets(j),j=70201,72150)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets37 = ',(ky_triplets(j),j=70201,72150)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'kx_triplets38 = ',(kx_triplets(j),j=72151,74100)
        WRITE(PAR_OUT_FILE,'(A,19501I4)')  'ky_triplets38 = ',(ky_triplets(j),j=72151,74100)
        WRITE(PAR_OUT_FILE,"(A)")    ""
    ENDIF

    IF(SVD_proj.AND..NOT.svd_field_mom) THEN
       WRITE(PAR_OUT_FILE,"(A,L3)") "SVD_proj   = ",SVD_proj
       WRITE(string_num,'(I3)') n_svd_ind
       format_string='(A,'//string_num//'I4)'
       WRITE(PAR_OUT_FILE,format_string) "SVD_kx_ind = ",(SVD_kx_ind(j),j=1,n_svd_ind)
       WRITE(PAR_OUT_FILE,format_string) "SVD_ky_ind = ",(SVD_ky_ind(j),j=1,n_svd_ind)
       WRITE(PAR_OUT_FILE,"(A,I3)") "SVD_nkx0   = ",SVD_nkx0
       WRITE(PAR_OUT_FILE,"(A,I3)") "SVD_n_restarts = ", SVD_n_restarts
       WRITE(string_num,'(I3.3)') SVD_n_restarts
       format_string='(A,'//string_num//'1I6)'
       WRITE(PAR_OUT_FILE,format_string) "SVD_df_n_time = ",(SVD_df_n_time(j),j=1,SVD_n_restarts)
       ...
       WRITE(PAR_OUT_FILE,"(A,I3)") "n_svd_ind = ",n_svd_ind
     end if
     ...
     if(istep_dfout.gt.0) then
      ...
      Write(PAR_OUT_FILE,"(A,I3)") "num_ky_modes = ",num_ky_modes
      Write(PAR_OUT_FILE,"(A,I3)") "num_kx_modes = ",num_kx_modes
      WRITE(string_num,'(I3.3)') num_ky_modes
      format_string='(A,'//string_num//'1I4)'
      WRITE(PAR_OUT_FILE,format_string) "which_ky = ",(which_ky(j),j=1,num_ky_modes)
      WRITE(PAR_OUT_FILE,format_string) "which_kx_center= ",(which_kx_center(j),j=1,num_ky_modes)
     end if
    ...
    if (with_external.or.(ExB).or.(pfsrate.ne.0.0).or.(Omega0_tor.ne.0.0).or.(kink_on)) then
       Write(PAR_OUT_FILE,"(A)")    "&external_contr"
       if ((with_phi_ext) .or. (with_kh_drive_ext)) then
          ...
          Write(PAR_OUT_FILE,"(A,L1)")    "use_self_consistent_flow = ",use_self_consistent_flow
       endif
       ...
       if (Omega0_tor.ne.0.0) then
          write(PAR_OUT_FILE,"(A,G16.8)") "Omega0_tor = ", Omega0_tor
          write(PAR_OUT_FILE,"(A,G16.8)") "R0_tor     = ", R0_tor
          write(PAR_OUT_FILE,"(A,G16.8)") "dR0_tor_dx = ", dR0_tor_dx
          Write(PAR_OUT_FILE,"(A)") ""
       ...
       if (ExB) write(PAR_OUT_FILE,"(A,G16.8)") "ExBrate    = ", ExBrate
       ...
       if (pfsrate.ne.0.0) &
            write(PAR_OUT_FILE,"(A,G16.8)") "pfsrate    = ", pfsrate
       if (kink_on) &
            write(PAR_OUT_FILE,"(A,L1)") "kink_on    = ", kink_on
       if (uprime_x0.ne.0.0) &
            write(PAR_OUT_FILE,"(A,G16.8)") "uprime_x0    = ", uprime_x0

       WRITE(PAR_OUT_FILE,"(A)") "/"
       Write(PAR_OUT_FILE,"(A)") ""
    endif

    !****** poloidal planes namelist ********
    if (istep_planes.gt.0) then
       WRITE(PAR_OUT_FILE,"(A)")  "&poloidal_planes"
       WRITE(PAR_OUT_FILE,"(A,I3)") "istep_planes   = ", istep_planes
       WRITE(PAR_OUT_FILE,"(A,G16.8)") "sign_phi       = ", sign_phi
       WRITE(PAR_OUT_FILE,"(A,I3)") "n_cuts         = ", n_cuts
       WRITE(PAR_OUT_FILE,"(A,I3)") "nz_out         = ", nz_out
       WRITE(PAR_OUT_FILE,"(A,I3)") "res_fact       = ", res_fact
       write(PAR_OUT_FILE,"(A,L1)") "BC_in_fourier  = ", BC_in_fourier
       WRITE(PAR_OUT_FILE,"(A)") "/"
       Write(PAR_OUT_FILE,"(A)") ""
    endif
    ...
    if (INDEX(magn_geometry,'slab').eq.1) then
       ...
       Write(PAR_OUT_FILE,"(A,G16.8)")  "shat     = ",shat
...
!          Write(PAR_OUT_FILE,"(A,G16.8)") "rho      = ",rho
    ...
    !****** island namelist ********
    if (island_contrib) then
       WRITE(PAR_OUT_FILE,"(A)")  "&island"
       WRITE(PAR_OUT_FILE,"(A,L1)") "island_contrib = ", island_contrib
       WRITE(PAR_OUT_FILE,"(A,G16.8)") "amp_island     = ", amp_island
       WRITE(PAR_OUT_FILE,"(A,L1)") "island_zGauss  = ", island_zGauss
       if (.not. island_zGauss) then
         WRITE(PAR_OUT_FILE,"(A,I3)") "j_island       = ", j_island
         WRITE(PAR_OUT_FILE,"(A,G16.8)") "omega_island   = ", omega_island
         WRITE(PAR_OUT_FILE,"(A,G16.8)") "width_island   = ", width_island
         WRITE(PAR_OUT_FILE,"(A,G16.8)") "damping_island = ", damping_island
         WRITE(PAR_OUT_FILE,"(A,G16.8)") "amp_island     = ", amp_island
         WRITE(PAR_OUT_FILE,"(A,G16.8)") "xdamp          = ", xdamp
         WRITE(PAR_OUT_FILE,"(A,G16.8)") "delta_xdamp    = ", delta_xdamp
         WRITE(PAR_OUT_FILE,"(A,G16.8)") "omega_island   = ", omega_island
       else
         WRITE(PAR_OUT_FILE,"(A,G16.8)") "width_zGauss   = ", width_zGauss
         WRITE(PAR_OUT_FILE,"(A,G16.8)") "island_ky      = ", island_ky
       end if
       Write(PAR_OUT_FILE,"(A)") "/"
       Write(PAR_OUT_FILE,"(A)") ""
       CALL FLUSH(PAR_OUT_FILE)
    ...
    if (.not.skip_info_nml) then
       ...
       case('IV')
          ...
          write(PAR_OUT_FILE,"(A,L1)") "nltdt_off = ",nltdt_off
       ...
       if (git_branch.ne.'')  Write(PAR_OUT_FILE,"(A,A)") &
            "GIT_BRANCH = ", git_branch
       if (git_master.ne.'')  Write(PAR_OUT_FILE,"(A,A)") &
    ...
    !****** in_out namelist ********
       ...
       if (istep_xvsp.gt.0) then
          call attach(fidall_param_h5, "/input", "istep_xvsp"   , istep_xvsp)
          call attach(fidall_param_h5, "/input", "xvsp_iy"   , xvsp_iy)
          call attach(fidall_param_h5, "/input", "xvsp_iz"   , xvsp_iz)
       endif
...
!       if (istep_neoclass2D.gt.0) call attach(fidall_param_h5, "/input", "istep_neoclass2D" , istep_neoclass2D)
       ...
       IF (istep_fe_threeD.gt.0) call attach(fidall_param_h5, "/input","istep_fe_threeD", istep_fe_threeD)
    ...
    !****** general namelist ********
       ...
       if (.not.arakawa_zv) then
          ...
          if (hyp_on_h.and..not.hypz_opt) call attach(fidall_param_h5, "/input", "hypz_opt", hypz_opt)
          if (hyp_on_h) call attach(fidall_param_h5, "/input", "hypz_compensation", hypz_compensation)
       endif
       ...
       case('EV')
          ...
          if (trim(which_ev).eq.'mfn') then
                call attach(fidall_param_h5, "/input", "taumfn_real", real(taumfn))
                call attach(fidall_param_h5, "/input", "taumfn_real", aimag(taumfn))
          endif
       case('IV')
...
!            &call attach(fidall_param_h5, "/input", "pressure_term"  , pressure_term)
       ...
       if (delzonal_bpar) call attach(fidall_param_h5, "/input", "delzonal_bpar", delzonal_bpar)
       ...
       if (collision_op.ne.'none') then
          ...
          call attach(fidall_param_h5, "/input", "coll_order",trim(coll_order))
          ...
          if (coll_on_h) call attach(fidall_param_h5, "/input", "coll_on_h" , coll_on_h)
       ...
       if (nltdt_off) call attach(fidall_param_h5, "/input", "nltdt_off", nltdt_off)
       if (all_moms) call attach(fidall_param_h5, "/input", "all_moms" , all_moms)
#ifndef COUPLE_XGC
    !****** extended_diags namelist ********
       if (istep_nlt.gt.0) then
       ...
       if (SVD_proj.and..not.svd_field_mom) then
          ...
          call putarr(fidall_param_h5, "/input/SVD_kx_ind", SVD_kx_ind, "SVD_kx_ind")
          call putarr(fidall_param_h5, "/input/SVD_ky_ind", SVD_ky_ind, "SVD_ky_ind")
    ...
    !****** external contributions namelist ********
       if (with_external.or.(ExB).or.(pfsrate.ne.0.0)) then
          if ((with_phi_ext) .or. (with_kh_drive_ext)) then
             ...
             call attach(fidall_param_h5, "/input", "use_self_consistent_flow" , use_self_consistent_flow)
          end if
          if (with_apar_ext) then
          ...
          if (kink_on) call attach(fidall_param_h5, "/input", "kink_on"   , kink_on)
          if (uprime_x0.ne.0.0) call attach(fidall_param_h5, "/input", "uprime_x0"   , uprime_x0)
       end if

      !****** poloidal planes namelist ********
      if (istep_planes.gt.0) then
         call attach(fidall_param_h5, "/input","istep_planes", istep_planes)
         call attach(fidall_param_h5, "/input","n_cuts", n_cuts)
         call attach(fidall_param_h5, "/input","nz_out", nz_out)
         call attach(fidall_param_h5, "/input","res_fact", res_fact)
         call attach(fidall_param_h5, "/input","BC_in_fourier", BC_in_fourier)
      endif
    ...
    !****** information namelist ********
       ...
       if (git_branch.ne.'')  call attach(fidall_param_h5, "/input", "GIT_BRANCH" , git_branch)
       if (git_master.ne.'') call attach(fidall_param_h5, "/input", "GIT_MASTER" , git_master)
       if (release.ne.'') call attach(fidall_param_h5, "/input", "RELEASE" , release)
    ...
    !****** island namelist ********
       if (island_contrib) then
         call attach(fidall_param_h5, "/input", "island_contrib", island_contrib)
         call attach(fidall_param_h5, "/input", "amp_island", amp_island)
         call attach(fidall_param_h5, "/input", "island_zGauss", island_zGauss)
         if (.not. island_zGauss) then
           call attach(fidall_param_h5, "/input", "j_island", j_island)
           call attach(fidall_param_h5, "/input", "omega_island", omega_island)
           call attach(fidall_param_h5, "/input", "width_island", width_island)
           call attach(fidall_param_h5, "/input", "damping_island", damping_island)
           call attach(fidall_param_h5, "/input", "xdamp", xdamp)
           call attach(fidall_param_h5, "/input", "delta_xdamp", delta_xdamp)
         else
           call attach(fidall_param_h5, "/input", "width_zGauss", width_zGauss)
           call attach(fidall_param_h5, "/input", "island_ky", island_ky)
         endif
       endif
