...
!  use x_derivatives
  ...
  implicit none
  public:: geomtype, q_prof, dqdx_prof, C_y, C_xy, C_j, geom, geom_in, jpar
  ...
  ! parallel current
  Real, dimension(:,:),allocatable :: jpar
...
contains
  ...
  subroutine check_geometry
    ...
    if (((INDEX(magn_geometry,'s_alpha').ne.1).and.&
         ...
         &(magn_geometry.ne.'circular').and.(INDEX(magn_geometry,'slab').ne.1).and.&
         &(INDEX(magn_geometry,'zpinch').ne.1 )) then
    ...
    if (pressure_term.and.dpdx_pm.eq.-1111) then !set dpdx_pm defaults
       ...
       if ((INDEX(magn_geometry,'s_alpha').eq.1).or.&
            &(magn_geometry.eq.'circular').or.(INDEX(magn_geometry,'slab').eq.1).or.&
            &(magn_geometry.eq.'zpinch')) then
    ...
    if (.not.any(tracer_rt_scheme.eq.(/'q1D','F2D'/))) &
         stop 'Incorrect choice for tracer_rt_scheme.'
  ...
  subroutine initialize_geometry
    ...
    if (init_status.lt.1) then
       ...
       allocate(jpar(pi1gl:pi2gl,lk1:lk2))
       ...
       !init status indicates that arrays are allocated (needed in read_tracer_namelist e.g.)
       init_status=1
...
#endif
          elseif (magn_geometry.eq.'tracer'.or.magn_geometry.eq.'gist'.or.magn_geometry.eq.'gene') then
             unit=-1
       ...
       if ((INDEX(magn_geometry,'s_alpha').eq.1).or.&
            &(INDEX(magn_geometry,'slab').eq.1).or.(magn_geometry.eq.'circular').or.&
            &(INDEX(magn_geometry,'zpinch').eq.1)) then
          ...
          case('circular') !flux coordinate x=r
             call init_circ(n_pol,q_prof,eps_p,dqdx,major_R,major_Z,&
                  minor_r,sign_Ip_CW, sign_Bt_CW)
             call get_circ(geom,C_y,C_xy,q0)
          case('zpinch')
             call set_zpinch(geom)
          case default
  ...
  subroutine read_tracer(dpdx_pm_geom)
    real, dimension(pmi1gl:pmi2gl) :: dpdx_pm_geom

    integer :: fileunit,i,k,line,ierr
    logical:: read_nml
    real :: dummy
    ...
    if (xy_local) then
       ...
       call read_in_1d("dpdx_pm_arr",dpdx_pm_geom,px0,fileunit,ierr)
       if ((ierr.ne.0).and.(write_pe)) write(*,"(a)")  &
            &"No pressure profile found in geometry file."
    endif
  ...
  subroutine read_tracer_namelist(fileunit,assign_vars,keep_file_open)
    ...
    integer :: gridpoints, nalpha0, sign_Ip_sav, sign_Bt_sav
    real :: my_dpdx=-100., q0_sav, shat_sav, n_pol_sav, beta_sav
    real :: trpeps_sav, minor_r_sav, major_R_sav
    ...
    namelist /parameters/ &
         ...
         & s0, nalpha0, trpeps, minor_r, major_R, sign_Ip_CW, sign_Bt_CW
    ...
    sign_Ip_sav = sign_Ip_CW
    sign_Bt_sav = sign_Bt_CW
    ...
    if (.not.assign_vars) then
       ...
       minor_r = minor_r_sav
       major_R = major_R_sav
       sign_Ip_CW = sign_Ip_sav
       sign_Bt_CW = sign_Bt_sav
    else
       if (major_R.lt.0) major_R = major_R_sav
       !these can only be assigned if arrays are allocated
       ...
       if (sign_Ip_CW.eq.0) then
          sign_Ip_CW = sign_Ip_sav
          if (mype.eq.0) write (*,'(3A,I3)') "Warning: No sign_Ip_CW entry found in ",&
               trim(geomfile)," - using user defined sign_Ip_CW = ", sign_Ip_CW
       endif
       if (sign_Bt_CW.eq.0) then
          sign_Bt_CW = sign_Bt_sav
          if (mype.eq.0) write (*,'(3A,I3)') "Warning: No sign_Bt_CW entry found in ",&
               trim(geomfile)," - using user defined sign_Bt_CW = ", sign_Bt_CW
       endif
...
! #endif
  ...
  subroutine set_s_alpha(geom)
    ...
    C_y= sign_Ip_CW*trpeps*major_R/abs(q0)    ! used to compute n0_global

    ! initialise geometric coefficients etc.
    do k=lk1,lk2
       arg    = zval(k) * shat - amhd * sin(zval(k))
       gxx(:,:,k) = 1.0
       gxy(:,:,k) = sign_Ip_CW*sign_Bt_CW*arg    !Cy*sign_Ip_CW*q0/r*shat*zval = q0/abs(q0)*arg
       ...
       geom%jacobian(:,:,k) = abs(q0)/geom%Bfield(:,:,k)*major_R  !not the real jacobian for historic reasons

       geom%dxdR(:,k)=cos(zval(k))
       geom%dxdZ(:,k)=sin(sign_Ip_CW*zval(k))
       geom%R_hat(:,k)=major_R*(1.+trpeps*cos(zval(k)))
       geom%Z_hat(:,k)=major_Z+major_R*trpeps*sin(sign_Ip_CW*zval(k))
       if (Lref.ne.0.) then
          geom%R(:,k)=geom%R_hat(:,k)*Lref
          geom%Z(:,k)=geom%Z_hat(:,k)*Lref
       else
          geom%R(:,k)=geom%R_hat(:,k)
          geom%Z(:,k)=geom%Z_hat(:,k)
       endif
    end do
    ...
    dBdy= 0.
    if (trpeps.ne.0.) then
       ! from circular: gyz ~ q0/abs(q0)*r/r^2
       gyz = sign_Ip_CW*sign_Bt_CW/(trpeps*major_R)
  ...
  !>Sets geometric quantities to zpinch
  subroutine set_zpinch(geom)
    TYPE(geomtype) :: geom
    ...
    geom%giz(:,:,:) = 0.0
    geom%gjz(:,:,:) = 0.0
    geom%gzz(:,:,:) = (1.0/major_R)**2

    geom%Bfield(:,:,:) = 1.0
    geom%dBdi(:,:,:) = -1.0/major_R
    geom%dBdj(:,:,:) = 0.0
    geom%dBdz(:,:,:) = 0.0
    geom%jacobian(:,:,:) = 1.0*major_R
  ...
  Subroutine write_geometry
    ...
    if (mype.eq.0) then
       if(write_std) then
          ...
          write(fileunit,"(A,ES24.16)") "major_R= ", major_R
          ...
          write(fileunit,"(A)") ""
          write(fileunit,"(A,I3)") "sign_Ip_CW = ", sign_Ip_CW
          write(fileunit,"(A,I3)") "sign_Bt_CW = ", sign_Bt_CW
          Write(fileunit,"(A)")    "/"

          if (xy_local) then
             do k=0,nz0-1
                write(fileunit,"(17ES24.16)") ggxx(pi1,pj1,k), ggxy(pi1,pj1,k), &
             ...
             call write_out_1d("dpdx_pm_arr",dpdx_pm_arr(:),px0,fileunit)
          endif
          close(fileunit)
       endif
       ...
       if(write_h5) then
          if(xy_local) then
             ...
             write(dset_name_geomcoeff, "(A)") "/profile/dpdx_pm_arr"
             call putarr(fidgeometry_h5, dset_name_geomcoeff, dpdx_pm_arr)
          ...
          write(dset_name_geomcoeff, "(A)") "/parameters/minor_r"
          call putarr(fidgeometry_h5, dset_name_geomcoeff, (/ minor_r /))
          write(dset_name_geomcoeff, "(A)") "/parameters/major_R"
          call putarr(fidgeometry_h5, dset_name_geomcoeff, (/ major_R /))
  ...
  subroutine set_curvature
    ...
    if (trim(magn_geometry).eq.'zpinch') return !already set
  ...
  subroutine set_q_profile(xval_a)
    ...
    if (mag_prof) then
       ...
       if (x_local.or.lilo) THEN
          ...
          elseif (INDEX(magn_geometry,'slab').ne.1.and.&
          &(INDEX(magn_geometry,'zpinch').ne.1)) then
  ...
  subroutine compute_aux_geom_quantities
    ...
    dVdx = (2.0*pi)**2*abs(C_y(pg1gl:pg2gl))*n_pol * geom%avg_jaco_yz(pg1gl:pg2gl)
    !------------------------------------------------------------------
    ...
    area_FS = (2.0*pi)**2*abs(C_y(pg1gl:pg2gl))*n_pol*area_FS
    !------------------------------------------------------------------
