...
module petsc_precond
  ...
  use petsc_aux
#if (PETSC_VERSION_GE(3,8,0))
  use petscksp  !petscksp or petscmat/petscpc needed (as below)?
#else
  use petscmat
  use petscpc
#endif
...
contains
  ...
  subroutine init_pc_obj
    ...
    case('asm')
       ...
       call PetscOptionsInsertString(PETSC_NULL_OPTIONS,trim(runtime_option_string),globerr)
#else
  ...
  subroutine set_parms_local
    ...
    case('parms_arms')
       ...
       if (comp_type.eq.'NC') then
          ...
          call PetscOptionsInsertString(PETSC_NULL_OPTIONS,trim(runtime_option_string),globerr)
          call PetscOptionsInsertString(PETSC_NULL_OPTIONS,'-pc_parms_levels 0',globerr)
       else
          write(runtime_option_string,'(a,es9.2)') '-st_pc_parms_droptol_last_schur ',100.*ev_prec
          call PetscOptionsInsertString(PETSC_NULL_OPTIONS,trim(runtime_option_string),globerr)
          call PetscOptionsInsertString(PETSC_NULL_OPTIONS,'-st_pc_parms_levels 0',globerr)
       end if
#else
       if (comp_type.eq.'NC') then
          write(runtime_option_string,'(a,es9.2)') '-pc_parms_droptol_last_schur ',ksp_prec
...
#if (PETSC_VERSION_GE(3,7,0))
          if(which_ev.eq.'EV') call PetscOptionsInsertString(PETSC_NULL_OPTIONS,'-mat_umfpack_droptol 1.e-7',globerr)
#else
  ...
  subroutine initialize_L_g
    ...
    integer:: col, n_entries, pair, nbands, dummy
    ...
    memory=memory*vlen*nbands/1024.**2
    ...
    if (mype.eq.0) then
       ...
       write(*,"(A,F8.2,A)") 'Memory requirements: ',memory,' MB per core'
    endif
    ...
    dummy=0 !PETSC_NULL_INTEGER
    call MatMPIAIJSetPreallocation(L_g_mat,dummy,d_nnz,&
         dummy,o_nnz,globerr)
#else
...
!       if(mype.eq.0) print*,rescoord(:,ind),val
  ...
  subroutine set_g(dep_ind,loc_g,rescoord,n_entries)
    ...
    integer,dimension(2,0:vlen-1),intent(out):: rescoord
