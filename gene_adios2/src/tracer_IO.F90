...
module tracer_IO
  ...
  use par_geom, only: tracer_rt_scheme
...
contains
  ...
  subroutine setup_psi_phi_grids(rmag,zmag,Lref,psiaxis,psisep,Bref,rhotor)
    ...
    !the following two variables are just dummies, to be removed once the rest of the
    !dtold/signconventions-pending branch has been included in master
    integer:: sign_Ip_CCW=0, sign_Bt_CCW=0
...
!    character(len=2):: it_str !this string is used for debug output
    ...
    signs=sign_Ip_CCW*sign_Bt_CCW.ne.0
...
!             endif
    ...
    contains
      function in_plasma(psi,psia,psie)
        ...
        if (signs) then
           in_plasma=psi*(-sign_Ip_CCW).ge.psia*(-sign_Ip_CCW).and.psi*(-sign_Ip_CCW).le.psie*(-sign_Ip_CCW)
        else
