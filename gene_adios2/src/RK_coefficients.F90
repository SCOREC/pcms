...
module RK_coefficients
  use par_mod, only: timescheme, direct_rhs, print_ini_msg, coll_split_scheme, coll_split, collision_op
  ...
  subroutine initialize_RK_coefficients_std
    ...
    if (init_status_std.eq.0) then
       ...
       case('RK2')
          ...
          RK_Taylor=(/1., 1., 0.5/)
          low_mem=.false.
          if ((mype.eq.0).and.print_ini_msg) write(*,"(A)") "2nd order Runge-Kutta"
       case('RK4','RK4f')
          rkstages=4
          allocate(a_rk(2:rkstages),b_rk(rkstages),RK_Taylor(rkstages+1))
          a_rk=(/0.5, 0.5, 1.0/)
       ...
       if (init_status_coll.ne.rkstages_coll) then
          ...
          case('RKCa')
             ...
             if (collision_op .eq. 'sugama') then
                max_rkstages_coll = 2 !only the first and second stages currently work with the sugama operator
             else
