...
Contains
  ...
  subroutine initialize_timescheme
    ...
    if (init_status.eq.0) then
       if (nonlinear.or.parallel_nl) then
    ...
    !set nonlinearity
    rhs_nl=(nonlinear.or.parallel_nl)
    !set rhs options
  ...
  subroutine finalize_timescheme
    ...
    if(nonlinear.or.parallel_nl) call finalize_adapt_dt
  ...
  Subroutine RK_standard
    ...
    character(len=12) :: stagestr
#endif
    DEBUG_LOCAL_OFF

    DEBUG_OUTPUT("start g_1 = ",g_1)
    ...
    DEBUG_OUTPUT("stage1 k1 = ",k1)

    ! Adapt the timestep
    if (nonlinear.or.parallel_nl) call adapt_dt
...
#ifdef WITH_DEBUG_OUTPUT
       IF (OUTPUT) WRITE(stagestr,"(A,I1,A)") "stage",stage," tg = "
#endif
       DEBUG_OUTPUT(stagestr,tg)
  ...
  Subroutine timestep_coll
    ...
    DEBUG_OUTPUT("coll stage1 k1 = ",k1)
...
#ifdef WITH_DEBUG_OUTPUT
       IF (OUTPUT) WRITE(stagestr,"(A,I1,A)") "coll stage",stage," tg = "
#endif
       DEBUG_OUTPUT(stagestr,tg)
    ...
    !low_mem schemes seem to need k1(a_1) = g_1 for their first RK stage
    if(low_mem) k1=g_1

    PERFOFF

    DEBUG_OUTPUT("+COLL g_1",g_1)
