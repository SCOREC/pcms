...
PRO frequency_loop, diag
  ...
  IF ((*i).zind EQ -1) THEN data = $
     par.x_local ? (*mom[0,0].kxky)[(*i).kxind,(*i).kyind,*] : $
    (*mom[0,0].sxky)[(*i).kxind,(*i).kyind,*] $
  ELSE data = par.x_local ? (*mom[0,0].kxky)[(*i).kxind,(*i).kyind,(*i).zind] : $
    (*mom[0,0].sxky)[(*i).kxind,(*i).kyind,(*i).zind]

  data = DCOMPLEX(TEMPORARY(data))
...
PRO frequency_output, diag
  ...
  nzind = ((*i).zind NE -1)?1:par.nz0

  data = store_step((*i).data_id,/get,/reg_array)
  data = REFORM(data,[nkxind,nkyind,nzind,series.step_count],/OVERWRITE)
  ...
  IF (*i).connect THEN BEGIN
    IF par.n_conn GT 0 THEN bigN = par.n_conn ELSE $
      ...
      par.lx*par.kymin*(par.x0 EQ 0 ? 1.0 : (*series.geom).C_y*par.q0/par.x0)) $
      ELSE bigN = 1
  ENDIF
  ...
  IF NOT (*i).fftmode THEN BEGIN ; --- phase mode ---
    ...
    FOR y = 0, nkyind - 1 DO BEGIN
      ...
      data_ky = REFORM(data[kxind,y,*,*],[nkxind,nzind,n_steps+1])
      ...
      ENDIF ELSE BEGIN
        IF par.x_local THEN BEGIN
          dphase_dt[0,0] = (*i).resolve_kx ? TOTAL(ALOG($
            ...
            (((*i).zind EQ -1)?REBIN(REFORM((*series.geom).jac_norm/par.nz0,$
            [1,par.nz0,1]),[nkxind,par.nz0,n_steps]):1.0),2,/NAN) / $
            REBIN(REFORM((time[1:*]-time[0:n_steps-1]),$
            [1,n_steps]),[nkxind,n_steps]) : $
            TOTAL(TOTAL(ALOG(data_ky[*,*,1:*]/data_ky[*,*,0:n_steps-1]),1)*$
            (((*i).zind EQ -1)?REBIN((*series.geom).jac_norm/par.nz0,$
            [par.nz0,n_steps]):1.0),1,/NAN) / ((time[1:*] - time[0:n_steps-1]) * nkxind)
        ENDIF ELSE BEGIN
          dphase_dt[0,0] = (*i).resolve_kx ? TOTAL(ALOG($
            data_ky[*,*,1:*]/data_ky[*,*,0:n_steps-1])*$
            (((*i).zind EQ -1)?REBIN((*series.geom).jac_norm/par.nz0,$
            [nkxind,par.nz0,n_steps]):1.0),$
            2,/NAN) / REBIN(REFORM((time[1:*]-time[0:n_steps-1]),$
            [1,n_steps]),[nkxind,n_steps]) : $
            TOTAL(TOTAL(ALOG(data_ky[*,*,1:*]/data_ky[*,*,0:n_steps-1])*$
            (((*i).zind EQ -1)?REBIN((*series.geom).jac_norm[kxind,*]/par.nz0,$
            [nkxind,par.nz0,n_steps]):1.0),1),1,/NAN) / $
      ...
      ENDIF ELSE BEGIN
        ...
        IF error_bars THEN BEGIN
          omega_err[y] = STDDEV(omega[*,y]) / SQRT(nzind*nkxind)
          gamma_err[y] = STDDEV(gamma[*,y],/NAN) / SQRT(nzind*nkxind)
        ENDIF
      ENDELSE
    ENDFOR
    ...
    IF (*i).resolve_kx THEN BEGIN
      ...
      IF nkyind EQ 1 THEN PLOT, xaxis, omega_avg, COLOR=1, $
        /XSTYLE, XTITLE=xtitle, YTITLE=ytitle ELSE BEGIN
    ...
    IF NOT (*i).resolve_kx THEN BEGIN
      PRINT, (*diag).name + ': median (gamma,omega):'
    ...
    ENDIF ELSE BEGIN
      set_output, diag, sp, header=['ky','gamma','omega'], $
        commentline='kx average; '+commentline, $
        dat=[[(*series.ky)[(*i).kyind]],[TOTAL(gamma_avg,1,/NAN)],$
        [TOTAL(omega_avg,1,/NAN)]]
      FOR j = 0, nkyind - 1 DO set_output, diag, sp, $
        /append, header=['kx','gamma','omega'], $
        commentline='kx resolved for ky='+rm0es((*series.ky)[(*i).kyind[j]]), $
        dat=[[(*series.kx)[(*i).kxind,0]],[gamma_avg[*,j]],[omega_avg[*,j]]]
    ENDELSE
...
;    nkxind = N_ELEMENTS((*i).kxind)
    nkx_mod = (*i).resolve_kx ? 1 : nkxind
    fftdata_win = pf_arr([nkxind,nkyind,n_steps_win],/zero)
    omega_max = pf_arr([2,n_freqs,nkxind,nkyind],/zero)
    omega_moms = pf_arr([nkxind,nkyind],/zero)
    ...
    FOR iky = 0, nkyind - 1 DO BEGIN
      IF (*i).connect THEN BEGIN
        ...
        data_ip = INTERPOLATE(REFORM(data[kxind,iky,*,*],$
          [nkxind,nzind,series.step_count]),$
          INDGEN(nkxind),INDGEN(nzind),equid_time,/GRID)
      ENDIF

      FOR ikx = 0, (*i).resolve_kx * (nkxind - 1) DO BEGIN
        IF NOT (*i).connect THEN BEGIN
          data_ip = INTERPOLATE(REFORM((*i).resolve_kx ? data[ikx,iky,*,*] : $
          data[*,iky,*,*],[nkx_mod,nzind,series.step_count]),$
            INDGEN(nkx_mod),INDGEN(nzind),equid_time,/GRID)
        ENDIF

        fftdata_temp = par.x_local ? $
          TOTAL(ABS(FFT(data_ip,DIMENSION=3))^2,1) * $
          (((*i).zind EQ -1)?REBIN((*series.geom).jac_norm,[par.nz0,n_steps_new]):1.0) : $
          TOTAL(ABS(FFT(data_ip,DIMENSION=3))^2*$
          (((*i).zind EQ -1)?REBIN((*series.geom).jac_norm[(*i).resolve_kx ? ikx : (*i).kxind,*],$
          [nkx_mod,par.nz0,n_steps_new]):1.0),1)

        fftdata = SHIFT(TOTAL(TEMPORARY(fftdata_temp),1),n_steps_new/2-1)
        fftdata /= nkx_mod * nzind
        ...
        fftdata_win[ikx,iky,*] /= nkx_mod * nzind * n_windows

        title = '!6k!Dy!N ' + get_var_string(/rhostr) + '=' + $
          rm0es((*series.ky)[(*i).kyind[iky]],prec=3)
        IF (*i).resolve_kx THEN title += ', k!Dx!N ' + $
          get_var_string(/rhostr) + '=' + $
          rm0es((*series.kx)[(*i).kxind[ikx],0],prec=3)

        IF with_splines THEN BEGIN
          SPLINE_P, omega_win, REFORM(fftdata_win[ikx,iky,*]), $
            omega_spline, fftdata_spline
          ...
          omega_max[0,0:n_max-1,ikx,iky] = omega_spline_max[0:n_max-1]
          omega_max[1,0:n_max-1,ikx,iky] = fftdata_spline_max[0:n_max-1]

          IF !QUIET NE 1 THEN FOR j = 0, n_freqs - 1 DO PRINT, $
            '   ' + rm0es(j+1) + '. frequency (windowed FFT, ky=' + $
            rm0es((*series.ky)[(*i).kyind[iky]],prec=3) + ((*i).resolve_kx ? $
            ', kx=' + rm0es((*series.kx)[(*i).kxind[ikx],0],prec=3) : '') + $
            '): ' + rm0es(omega_max[0,j,ikx,iky])

          ; calculate first moment
          omega_moms[ikx,iky] = INT_TABULATED(omega_spline,omega_spline*$
            fftdata_spline) / INT_TABULATED(omega_spline,fftdata_spline)
          xrange = 3.0 * MAX(ABS(omega_max[0,*,ikx,iky])) * [-1,1]
        ENDIF ELSE BEGIN
          xr_max = 3
          xrange = xr_max * [-1,1]
        ENDELSE
        ...
        OPLOT, omega_win, fftdata_win[ikx,iky,*], COLOR=3
        ...
        IF with_splines THEN BEGIN
          ...
          FOR imax = 0, n_freqs - 1 DO OPLOT, $
            [omega_max[0,imax,ikx,iky],omega_max[0,imax,ikx,iky]], !Y.CRANGE, $
            COLOR=4, LINESTYLE=2
          ; first moment
          OPLOT, [omega_moms[ikx,iky],omega_moms[ikx,iky]], !Y.CRANGE, COLOR=5
        ENDIF
        ...
        IF (*i).resolve_kx THEN BEGIN
          set_output, diag, header=['omega','amplitude'], commentline='kxind='+$
            rm0es((*i).kxind[ikx])+', kyind='+rm0es((*i).kyind[iky])+$
             ', kx='+rm0es((*series.kx)[(*i).kxind[ikx],0],prec=3)+$
            ', ky='+rm0es((*series.ky)[(*i).kyind[iky]],prec=3), $
            dat=[[omega],[fftdata]], append=(1L*(ikx+1)*(iky+1) GT 1)
        ENDIF
      ENDFOR ; --- kx loop (if resolve_kx)
    ENDFOR ; --- ky loop
    ...
    IF nkyind GT 1 THEN BEGIN
      ...
      FOR ikx = 0, (*i).resolve_kx * (nkxind - 1) DO BEGIN
        title = (*i).resolve_kx ? '!6k!Dx!N=' + $
          rm0es((*series.kx)[(*i).kxind[ikx],0],prec=3) : '!6'

        omax = MAX(omega_max[0,*,ikx,*],MIN=omin)
        PLOT, (*series.ky)[(*i).kyind], omega_max[0,0,ikx,*], COLOR=1, $
          YTITLE=ytitle, XTITLE=xtitle, TITLE=title, /XSTYLE, /NODATA, $
          YRANGE=[omin,omax], POSITION=[0.15,0.4,0.9,0.9]
        OPLOT, !X.CRANGE, [0,0], COLOR=1

        FOR imax = 0, n_freqs - 1 DO OPLOT, $
          (*series.ky)[(*i).kyind], omega_max[0,imax,ikx,*], COLOR=imax+2
        OPLOT, (*series.ky)[(*i).kyind], omega_moms[ikx,*], COLOR=n_freqs+2
        ...
        yrange = 3.0 * MAX(ABS(omega_max[0,*,ikx,*])) * [-1,1]

        LOADCT, 33, FILE='internal/colortable.tbl'
  ;      lev_col = contour_levels([MIN(fftdata_win),MAX(fftdata_win)],20)
        FOR iky = 0, nkyind - 1 DO BEGIN
          win_max = MAX(fftdata_win[ikx,iky,*])
          IF win_max GT 0 THEN fftdata_win[ikx,iky,*] = $
            fftdata_win[ikx,iky,*] / win_max
        ENDFOR
        lev_col = contour_levels([0,1],20)
        CONTOUR, fftdata_win[ikx,*,*], (*series.ky)[(*i).kyind], omega_win, $
          LEVELS=lev_col[*,0], C_COLORS=lev_col[*,1], /FILL,$
    ...
    IF NOT (*i).resolve_kx THEN BEGIN
      set_output, diag, header=['ky',rm0es(INDGEN(n_freqs)+1)+$
        ...
        [TRANSPOSE(REFORM(omega_max[0,*,0,*]))],[REFORM(omega_moms[0,*])]]
    ENDIF
  ENDELSE ; --- FFT mode
