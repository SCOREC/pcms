...
MODULE miller_mod
  ...
  use par_geom
  use fourier
  use par_in, only: fourier2D, beta, sign_Ip_CW, sign_Bt_CW
  ...
  REAL :: B0, R_0, rho, kappa, delta, s_kappa, s_delta, dPsidrho, drR, drZ, PI, V_p
  REAL :: zeta, s_zeta
...
CONTAINS
  ...
  subroutine get_miller_b(geom, C_y_out,C_xy_out,my_dpdx_pm)
    ...
    if ((sign_Ip_CW.ne.1.0).or.(sign_Bt_CW.ne.1.0)) stop 'Ip and Bt signs <> 1 not supported by miller_b'
...
!         &                 Z_zeta*R_theta_int))
  ...
  !> Miller with d/dr input and explicit metric tensor, following [J. Candy, PPCF 51 (2009) 105009]
  subroutine get_miller_a(geom, C_y_out,C_xy_out,my_dpdx_pm,q_prof,dqdx_prof,Cyq0_x0,edge_opt)
    ...
    real, dimension(np):: theta, J_r, Bp, D0, D1, D2, D3, nu, chi, theta_s, chi_s
    real, dimension(np):: tmp_reverse, theta_reverse, theta_s_reverse
    real, dimension(np):: R_s, Z_s, R_theta_s, Z_theta_s, Rc_s, cosu_s, sinu_s, Bphi_s, B, B_s, Bp_s, dlp_s, dlp_s2
    real, dimension(np):: psi1, psi1_s, nu1, nu1_s, dchidx_s, dB_drho_s, dB_dl_s, dnu_drho_s, dnu_dl_s, grad_nu
    ...
    !first and second theta derivatives
    R_theta=-rho*(d_inv*cos(theta)+1)*sin(d_inv*sin(theta)+theta)
    Z_theta=rho*kappa*(2*zeta*cos(2*theta)+1)*cos(zeta*sin(2*theta)+theta)
    R_theta_theta=rho*d_inv*sin(theta)*sin(d_inv*sin(theta)+theta)-&
         &rho*(d_inv*cos(theta)+1)**2*cos(d_inv*sin(theta)+theta)
    ...
    J_r=R*(-R_theta*(kappa*s_kappa*sin(zeta*sin(2*theta)+theta)&
         +kappa*sin(zeta*sin(2*theta)+theta)+kappa*s_zeta*sin(2*theta)*cos(zeta*sin(2*theta)+theta)+drZ)+&
         Z_theta*(-s_delta*sin(theta)*sin(d_inv*sin(theta)+theta)+cos(d_inv*sin(theta)+theta)+drR))
    ...
    tmp_arr=J_r/R**2
    drPsi=sign_Ip_CW*F/(2.*pi*q0)*sum(tmp_arr)*2*pi/(np-1) !dlp_int(tmp_arr,1.0)
...
!    Psi0=B0*Lnorm**2
    ...
    dxPsi=drPsi/dx_drho
    C_y_out=dxPsi*sign_Ip_CW
    Cyq0_x0=abs(C_y_out(pi1gl)*q0/rho)
    C_xy_out=abs(B0*dxPsi/C_y_out)
    ...
    ffprime=-(sign_Ip_CW*dq_dpsi*2.*pi+D0_full+D2_full*pprime)/D1_full
...
!         write(*,'(A,3(F12.6,1X))') 'nu1 check:   ', nu1(np)-nu1(1),&
!         -2.0*pi*sign_Ip_CW*dq_dpsi*psi1(np), nu1(np)-nu1(1)+2.0*pi*sign_Ip_CW*dq_dpsi*psi1(np)
    ...
    chi=chi*2.0*pi/(maxval(chi)-minval(chi))

    chi_s = theta !could skip chi_s as before but it's better for readability
    ...
    if (sign_Ip_CW.lt.0.0) then !make chi increasing function to not confuse lag3interp
       ...
       call lag3interp(theta_reverse,tmp_reverse,np,theta_s,chi_s,np)
       theta_s_reverse = theta_s(np:1:-1)
    else
       !lag3interp(y_in,x_in,n_in,y_out,x_out,n_out)
       call lag3interp(theta,chi,np,theta_s,theta,np)
    endif
    dtheta_dchi_s=deriv_fd(theta_s,chi_s)
    ...
    if (sign_Ip_CW.lt.0.0) then
       call lag3interp(nu1,theta,np,tmp_arr,theta_s_reverse,np)
       nu1_s = tmp_arr(np:1:-1)
       call lag3interp(Bp,theta,np,tmp_arr,theta_s_reverse,np)
       Bp_s = tmp_arr(np:1:-1)
       call lag3interp(dlp,theta,np,tmp_arr,theta_s_reverse,np)
       dlp_s = tmp_arr(np:1:-1)
       call lag3interp(Rc,theta,np,tmp_arr,theta_s_reverse,np)
       Rc_s = tmp_arr(np:1:-1)
    else
...
!if (mype==0) then
!   write(*,'(8(A12,1X))') '1-theta', '2-chi', '3-Bp', '4-nu', '5-theta_s', '6-chi_s', '7-Bp_s', '8-nu1_s'
!   do i=1,np
!      write(*,'(8(F12.6,1X))') theta(i), chi(i), Bp(i), nu1(i), theta_s(i), chi_s(i), Bp_s(i), nu1_s(i)
!   enddo
!endif

    psi1_s = R_s*Bp_s*sign_Ip_CW

    dBp_dchi_s=deriv_fd(Bp_s,chi_s)
    Bphi_s=F/R_s
    ...
    gxx=(psi1_s/dxPsi)**2
    gxy=-psi1_s/dxPsi*C_y_out(pi1gl)*sign_Ip_CW*nu1_s
    gxz=-psi1_s/dxPsi*(nu1_s+psi1_s*dq_dpsi*chi_s)/q0
    gyy=C_y_out(pi1gl)**2*(grad_nu**2+1/R_s**2)
    gyz=sign_Ip_CW*C_y_out(pi1gl)/q0*(grad_nu**2+dq_dpsi*nu1_s*psi1_s*chi_s)
    gzz=1./q0**2*(grad_nu**2+2.*dq_dpsi*nu1_s*psi1_s*chi_s+(dq_dpsi*psi1_s*chi_s)**2)
    ...
    dBdx=jacobian*C_y_out(pi1gl)/(q0*R_s)*(F/(R_s*psi1_s)*dB_drho_s+(nu1_s+dq_dpsi*theta*psi1_s)*dB_dl_s)
