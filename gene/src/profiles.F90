...
MODULE profiles_mod
  ...
  integer::init_status=0,ionindex
...
CONTAINS
  ...
  subroutine set_spec_profiles(spec, rad_out, n_spec, pi1gl, pi2gl, write_pe)
    ...
    ionindex=0
    do n=0, n_spec-1
       if (spec(n)%charge.gt.0) ionindex=ionindex+1
    ...
    if (spec(0)%prof_type.lt.0.and.enforce_qn) call enforce_quasineutrality(spec,write_pe)
  ...
  SUBROUTINE set_spec_profiles_n(sp, minor_r,write_pe)
    ...
    !do not use for lilo, this will set gradient profiles to zero!
  ...
  SUBROUTINE profile_buffer(in_profile)
    ...
    !write(*,"(2(A,I5))") "pi2gl=",pi2gl,", u_nxbuf=",u_nxbuf
    !write(*,"(A,4I5)") "size(in_profile,nu_K)=",lbound(in_profile),ubound(in_profile),lbound(nu_K),ubound(nu_K)
    !print*,"in_profile=",in_profile
    !print*,"nu_K=",nu_K
  ...
  subroutine read_numerical_profiles(filename,px0,sp,write_pe)
    ...
    real, dimension(:),allocatable :: temp_file, dens_file, xaxis_file, &
         &tmp_arr, tmp_arr2, omegator_file
    ...
    case(-2)
       ...
       if (sp%charge.lt.0.0) then !electrons
          call read_profiles_iterdb(iterdb_file, iterdb_time, 'TE', &
               &px0, sp%temp_prof, sp%omt_prof, xval_a,tst_varunit='eV        ')
          call read_profiles_iterdb(iterdb_file, iterdb_time, 'NE', &
               &px0, sp%dens_prof, sp%omn_prof, xval_a,tst_varunit='m^-3      ')
       else
          write(varname,"(a,i1)") 'TI',ionindex
          call read_profiles_iterdb(iterdb_file, iterdb_time,TRIM(varname), &
               &px0, sp%temp_prof, sp%omt_prof, xval_a,opt_var=.true.,&
               &tst_varunit='eV        ')
          IF (sum(sp%temp_prof).eq.0.0) call read_profiles_iterdb(iterdb_file, &
               iterdb_time,'TI', px0, sp%temp_prof, sp%omt_prof, xval_a,&
               &tst_varunit='eV        ')
          write(varname,"(a,i1)") 'NM',ionindex
          call read_profiles_iterdb(iterdb_file, iterdb_time,TRIM(varname), &
               &px0, sp%dens_prof, sp%omn_prof, xval_a,&
               &tst_varunit='m^-3      ',opt_var=(ionindex.eq.n_spec-1))
          write(varname,"(a,i1)") 'ZI',ionindex
          allocate(tmp_arr(px0),tmp_arr2(px0))
          call read_profiles_iterdb(iterdb_file, iterdb_time,TRIM(varname), &
               &px0,tmp_arr,tmp_arr2, xval_a,opt_var=.true.)
          if ((sum(tmp_arr).gt.0).and.(px0.eq.1)) sp%charge = tmp_arr(1)
       ...
       if (.not.allocated(omegator)) then
          ...
          call read_profiles_iterdb(iterdb_file, iterdb_time,'VROT',&
               &px0,omegator,omegatorprime,xval_a,opt_var=.true.)
          if (sum(abs(omegator)).eq.0.0) then
             call read_profiles_iterdb(iterdb_file, iterdb_time,'Vphi',&
                  &px0,omegator,omegatorprime,xval_a,opt_var=.true.,&
                  &tst_varunit='m/s       ')
             if (sum(abs(omegator)).ne.0.0) then
                if (mype==0) Write(*,"(A,1ES13.4,A)") " found Vphi instead of VROT; dividing by R0 = ", &
                     &Lref*major_R, " to obtain omega_tor"
                omegator = omegator / (major_R*Lref)
             endif
          endif
    ...
    case(-4)
       ...
       !interpolate file data to GENE grid
       xaxis_file= xaxis_file/xaxis_file(nx0_file)
       call lag3interp(temp_file,xaxis_file,nx0_file,&
            sp%temp_prof,xval_a,px0)
       tmp_arr = -log(temp_file)
       call lag3deriv(tmp_arr,xaxis_file,nx0_file,&
    ...
    if (allocated(omegator).and.(omegatorref.eq.-1).or.&
       ...
       if (x_local) then
          omegatorref = omegator(pi1)
       else
          call lag3interp(omegator,xval_a,nx0,omegatorref,x0)
       endif
  ...
  SUBROUTINE set_ref_vals(rho,nrho,spec,nspec)
    ...
    if (spec(0)%prof_type > 0 .and. (x_local.or.lilo)) then
  ...
  subroutine enforce_quasineutrality(spec,write_pe)
    ...
    if (abs(sum(qn)).gt.epsilon(qn(pi1gl))) then
       if (write_pe) then
          ...
          write (*,'(3(A))') 'enforcing quasineutrality on last active ion species (',&
               &trim(spec(iind)%name),')'
       endif
       ...
       if (x_local.or.lilo) then
          ...
          spec(iind)%omn_prof = spec(iind)%omn
       else
