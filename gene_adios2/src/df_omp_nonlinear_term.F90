...
module df_omp_nonlinear_term_mod
  ...
  use discretization
  use fourier, only: to_fourier_y, to_real_y, fft_ky_to_y
  use mpi
#ifdef WITHOMP
  use omp_lib
#endif
...
contains
  ...
  SUBROUTINE calc_omp_nonlinearity_df(this,gy_chi,g_block,vexy,dgdxy,localrhs,first,lb1,lb2)
    class(df_omp_nonlinear_term_t) :: this
    ...
    !$OMP shared(lj1,lj2,li0da,li1da,li2da,nj0,lb1,lb2) &
    !$OMP shared(this,first,vexy_re, dgdxy_re,nonlin) &
    ...
    if (number_of_lbg0_blocks.eq.0) then
       ...
       do i_block=1,number_of_lbg0_blocks
             ...
             do klmn=1,howmany
                ...
                ! Fourier transfrom in y (include dealiasing step)
                p_out => outarr(:,:,klmn)
                Call fft_ky_to_y(extended_tarray,p_out)
             end do
             !$OMP END DO
#if 0
       end do
