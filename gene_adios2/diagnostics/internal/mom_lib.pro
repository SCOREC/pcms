      ...
      IF last_time LT mom_time THEN BEGIN
        ...
        ENDIF ELSE IF mom_time GE gui.out.start_t * (1.0D - 1e-7) THEN BEGIN
          ...
          IF NOT cl_mode THEN BEGIN
            XYOUTS, 0.75, 0.95, '!6t = ' + rm0es(last_time), COLOR=0, $
              /NORMAL, CHARSIZE=1.3
            XYOUTS, 0.75, 0.95, '!6t = ' + rm0es(mom_time), COLOR=1, $
              /NORMAL, CHARSIZE=1.3
      ...
      CASE mom_nr OF
        ...
        150 : BEGIN
                ...
                *mom[j,mom_nr].(momtag) = - COMPLEX(0,1) * $
                  ...
                  (spec[j].dens) ; / REBIN(REFORM(SQRT((*series.geom).gxx),[1,1,par.nz0]),$
;                  [par.nkx0,par.nky0,par.nz0])
        ...
        151 : BEGIN
                ...
                *mom[j,mom_nr].(momtag) = - COMPLEX(0,1) * $
                  REBIN(REFORM(*series.ky,[1,par.nky0]),[par.nkx0,par.nky0,par.nz0]) * $
                  (*mom[j,0].(momtag)) * CONJ((0.5*(*mom[j,nf+1].(momtag))+$
                  (*mom[j,nf+2].(momtag))+1.5*(*mom[j,nf].(momtag)))) * $
                  (spec[j].dens * spec[j].temp) ;/ $
...
;                  [par.nkx0,par.nky0,par.nz0])
        ...
        153 : BEGIN
                ...
                *mom[j,mom_nr].(momtag) = COMPLEX(0,1) * $
                  REBIN(REFORM(*series.ky,[1,par.nky0]),[par.nkx0,par.nky0,par.nz0]) * $
                  (*mom[j,1].(momtag)) * CONJ(*mom[j,nf+5].(momtag)) * spec[j].dens
                IF (nf GE 2) AND (par.n_moms GT 6) THEN BEGIN
                  *mom[j,mom_nr].(momtag) -= COMPLEX(0,1) * $
                  REBIN(REFORM(*series.ky,[1,par.nky0]),[par.nkx0,par.nky0,par.nz0]) * $
                  (*mom[j,2].(momtag)) * CONJ(*mom[j,nf+6].(momtag)) * $
                  (spec[j].dens * spec[j].temp / spec[j].charge) * $
                ...
                *mom[j,mom_nr].(momtag) = COMPLEX(0,1) * $
                ...
                IF (nf GE 2) AND (par.n_moms GT 6) THEN BEGIN
                  *mom[j,mom_nr].(momtag) -= COMPLEX(0,1) * $
                  ...
                  (spec[j].dens * spec[j].temp^2 / (spec[j].charge)) * $
                  REBIN(REFORM(1.0/(*series.geom).Bfield,[1,1,par.nz0]),[par.nkx0,par.nky0,par.nz0])
                ENDIF
;                *mom[j,mom_nr].(momtag) /= REBIN(REFORM(SQRT((*series.geom).gxx),$
;                  [1,1,par.nz0]),[par.nkx0,par.nky0,par.nz0])
