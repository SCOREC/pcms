...
CONTAINS
  ...
  SUBROUTINE SparseMATMUL(vec,j,k,m,n,res, transA_in)
    ...
    if (transA.eq.'N') then
       CALL sparse_gyromatrix(j,k,m,n)%dot_multiply(vec_v,res_v) !,transA)
#ifdef G_DAGGER
    elseif (transA.eq.'C') then
       CALL sparse_gyromatrix_dagger(j,k,m,n)%dot_multiply(vec_v,res_v)
#endif
    else
...
SUBROUTINE allocate_gyromatrix
  ...
  allocate(BandedMatrix :: sparse_gyromatrix(lj1:lj2,lk1:lk2,lm1:lm2,ln1:ln2))
#ifdef GDAGGER_G
  ...
  DO n=ln1,ln2
     DO m=lm1,lm2
        DO k=lk1,lk2
           DO j=lj1,lj2
             call sparse_gyromatrix(j,k,m,n)%initialize(ni0,ni0,transposed)
             !sparse_gyromatrix(j,k,m,n) = BandedMatrix(ni0,ni0,transposed)
             call sparse_gyromatrix(j,k,m,n)%allocate()
#ifdef GDAGGER_G
             call sparse_gyromatrix_dagger(j,k,m,n)%initialize(ni0,ni0,transposed)
              !sparse_gyromatrix_dagger(j,k,m,n) = BandedMatrix(ni0,ni0,transposed)
              CALL sparse_gyromatrix_dagger(j,k,m,n)%ALLOCATE()
#else
...
SUBROUTINE print_gyro_matrix_df
  ...
  DO ix1=1,ni0
     DO ix2=1,ni0
        gyro_matrix(ix1,ix2) = sparse_gyromatrix(ij,ik,im,in)%mat_get_value(ix1,ix2)
     END DO
  END DO
  thisunit=30
  DO
...
SUBROUTINE calculate_J0matrix
  ...
  REAL :: dtheta, theta, rhocostheta, rhosintheta, res_factor !,invN
...
!  TYPE(DerivativeMatrix), DIMENSION(:), allocatable :: derivmat, derivmat_vN
  ...
  DO iDeriv=1,nDeriv
    call derivmat(iDeriv)%initialize(xgrid,derivative_order)
    !derivmat(iDeriv) = DerivativeMatrix(xgrid,derivative_order)
     IF (rad_bc_type.eq.2) THEN
       call derivmat_vN(iDeriv)%initialize(xgrid,derivative_order)
       !derivmat_vN(iDeriv) = DerivativeMatrix(xgrid,derivative_order)
     ...
     ENDIF
     call integral(iDeriv)%initialize(ni0,ni0,transposed)
     !integral(iDeriv) = BandedMatrix(ni0,ni0,transposed)
     CALL integral(iDeriv)%ALLOCATE()
  END DO

  ! initialize a temporary matrix
  !tempmat = BandedMatrix(ni0, ni0, transposed)
  call tempmat%initialize(ni0, ni0, transposed)
  CALL tempmat%allocate()
#ifdef GDAGGER_G
  !temp_dagger_matrix = BandedMatrix(ni0,ni0)
  call temp_dagger_matrix%initialize(ni0,ni0)
  call temp_dagger_matrix%allocate()
#endif
  pii = pi1
  ...
  DO ispec=ln1,ln2
     DO m=lm1,lm2
        DO k=lk1,lk2
           ...
           DO j=lj1,lj2
              CALL sparse_gyromatrix(j,k,m,ispec)%set_zero()
#ifdef GDAGGER_G
              CALL sparse_gyromatrix_dagger(j,k,m,ispec)%set_zero()
#endif
              DO iDeriv=1,nDeriv
                 CALL integral(iDeriv)%set_zero()
              END DO
              ! Attention, the indices for the integral dimension are running from 1 to ni0
              ! not from li1 to li2 !!!
              DO ix=1,ni0
                 pii=ix-1+pi1gl !li1
                 res_factor=MIN(rho(pii)/deli,rho(pii)/(sqrt(2.0*spec(ispec)%mass*spec(ispec)%temp*&
                      spec(ispec)%temp_prof(pii))/spec(ispec)%charge/geom%Bfield(pii,pj1,k)))
                 IF (y_local) THEN
                    ntheta = MAX(40,CEILING(sqrtgxx(pii,k) * res_factor * 40))
                 ELSE
                 ...
                 DO ibase=startbase,endbase
                    ...
                    DO itheta = 0,ntheta-1
                       if (.not.ga_spatial_var) then
                          ...
                          !"mirror" function values at (inner) boundary for Neumann bc
                          !on kj=0 mode
...
#if 1
                    Call sparse_gyromatrix(j,k,m,ispec)%set_value(ix,ibase,cmplx(real(sum_over_theta(0)),0.0))
                    DO ideriv=1,nderiv
                       CALL integral(ideriv)%set_value(ix,ibase,cmplx(real(sum_over_theta(ideriv)),0.0))
                    ENDDO
#else
                    Call sparse_gyromatrix(j,k,m,ispec)%set_value(ix,ibase,sum_over_theta(0))
                    DO ideriv=1,nderiv
                       CALL integral(ideriv)%set_value(ix,ibase,sum_over_theta(ideriv))
                    ENDDO
#endif
                 END DO !ibase
              ...
              CALL sparse_gyromatrix(j,k,m,ispec)%commit_values()

              DO ideriv=1,nderiv
                 CALL integral(ideriv)%commit_values()
              END DO
              ...
              DO ideriv=1,nderiv !2,nderiv
                 ...
                 IF ((rad_bc_type.eq.2).and.(p_has_0_mode.and.j==lj1)) THEN
                    CALL integral(ideriv)%dot_multiply(derivmat_vN(ideriv),tempmat)
                 ELSE
                    CALL integral(ideriv)%dot_multiply(derivmat(ideriv),tempmat)
                 ENDIF

                 CALL sparse_gyromatrix(j,k,m,ispec)%add_matrix(tempmat)
              END DO

#ifdef GDAGGER_G
              call sparse_gyromatrix(j,k,m,ispec)%transpose_and_conjugate(&
                   temp_dagger_matrix)
              call temp_dagger_matrix%transpose_storage(sparse_gyromatrix_dagger(j,k,m,ispec))
#endif
...
#ifdef GDAGGER_G
  !call temp_dagger_matrix%finalize()
#endif
  !call tempmat%finalize()

  !DO iDeriv=1,nDeriv
    !call finalize(derivmat(iDeriv))
     !IF (rad_bc_type.EQ.2) call finalize(derivmat_vN(iDeriv))
    !call integral(iDeriv)%finalize()
  !END DO
...
SUBROUTINE set_dagger_matrix(mat_in,mat_dagger,bandwidth)
  ...
  call mat_in%get_global_matrix_locally(temp_fullmat)
...
!!on those machines)
  ...
  DO ix=1,ni0
     ...
     ENDIF
     DO ibase=startbase,endbase
        Call mat_dagger%set_value(ibase,ix,&
             &CONJG(temp_fullmat(ix,ibase)))
     ENDDO
  ENDDO

  CALL mat_dagger%commit_values()
  ...
  IF (ALLOCATED(sparse_gyromatrix)) THEN
     ...
     DO n=ln1,ln2
        DO m=lm1,lm2
           DO k=lk1,lk2
              DO j=lj1,lj2
                 CALL sparse_gyromatrix(j,k,m,n)%finalize()
#ifdef GDAGGER_G
                 CALL sparse_gyromatrix_dagger(j,k,m,n)%finalize()
#endif
              END DO
           END DO
        END DO
