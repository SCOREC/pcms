  ...
  subroutine initialize_gyro_average_ff
    ...
    if (evenx.eq.1) then
       ...
       if (yx_order) then
          ...
          IF (n_fields .GT. 2 .or. all_moms) ifac_cmplx(:,hkx+1,:,:,:) = CMPLX(0.0)
       else
          jfac_cmplx(hkx+1,:,:,:,:)=cmplx(0.)
          IF (n_fields .GT. 2 .or. all_moms) ifac_cmplx(hkx+1,:,:,:,:) = CMPLX(0.0)
       end if
    ...
    do n=ln1,ln2
       do m=lm1,lm2
          ...
          IF (n_fields .GT. 2 .or. all_moms) CALL exchange_z(ifac_cmplx(:,:,:,m,n))
       enddo
    enddo

    call remove_pb_phase(jfac_cmplx)
    IF (n_fields .GT. 2 .or. all_moms) call remove_pb_phase(ifac_cmplx)
...
END SUBROUTINE gyro_average_5D
  ...
  subroutine finalize_gyro_average_ff
    ...
    DEALLOCATE(jfac)
    IF (n_fields.gt.2 .or. all_moms) DEALLOCATE(I1_factor)
