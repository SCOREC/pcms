...
class ProfileData(object):
    ...
    def get_profiles(self):
        ...
        for n in range(0, self.cm.pnt.n_spec):
            filename = 'profile_{}{}'.format(self.cm.specnames[n], self.cm.fileextension)
            self.prspec.append(ProfileFile(filename, self.cm))
        self.calc_nustar()
...
def glueprofiledata(prdlist):
    ...
    for prd in prdlist:
        for n in range(result.cm.pnt.n_spec):
            ...
            try:  # If the timefield has been converted to a np array before we need to undo it
